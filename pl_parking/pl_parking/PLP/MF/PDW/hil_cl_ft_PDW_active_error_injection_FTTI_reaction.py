...
SIGNAL_DATA = "INPUT_FAILURE_PDW_ACT_SS_FTTI"
...
@teststep_definition(
    step_number=1,
    name="System input failure, PDW active",
    description=(
    ...
    doors_url="https://jazz.conti.de/rm4/resources/BI_p0ZTkOE-Ee6AQb_wOaa0iQ?oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class System_safe_state_error_injected_PDW_active_SS(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, the PDW function switched to the desired state. "
            "<br>The following condition was met: "
            f"<br>{signal_name['PDW_State']} was set to FAILURE ({constants.HilCl.PDW.States.FAILURE}) within {constants.HilCl.PDW.FTTI.SAFETY_SS}us. "
            "<br> after an error was injected into the system.".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            # Wait until REVERSE gear(PDW activated) was engaged and there is an obstacle in the rear side of the ego within PDW activation range while and ego vehicle is approaching.
            if (
                (
                ...
                and fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.REVERSE
            ):
                trigger1 = True
                t1_idx = frame
                break
        if t1_idx:
            for frame in range(t1_idx, len(time_signal)):
                ...
                if error_injection_check(frame) is False:
                    trigger2 = True
                    if (
                        pdw_state[frame] != constants.HilCl.PDW.States.FAILURE
                        and pdw_state[frame] != constants.HilCl.PDW.States.INIT
                        and pdw_state[frame] != constants.HilCl.PDW.States.OFF
                    ):
                        t2_idx = frame
                        trigger3 = True
                        break
        ...
        if trigger1 is None:
            preconditions = (
                ...
                "<br>Condition 1 failed - Gear not switched to REVERSE:"
                f"<br>{signal_name['Gear_auto']} not set to {constants.HilCl.PDW.Gear.Automatic.REVERSE}"
                f"<br>or {signal_name['Gear_auto']} not set to {constants.HilCl.PDW.Gear.Manual.REVERSE} "
                "<br>OR Condition 2 failed- No object reported by rear USS within PDW activation range:"
                f"<br>{signal_name['USS8_dist_to_object']} not between {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION} - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}"
                f"<br> or {signal_name['USS9_dist_to_object']} not between {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION} - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}"
                "<br>OR Condition 3 failed - Vehicle not moving:"
                f"<br>{signal_name['Wheel_direction_FL']} not set to {constants.HilCl.PDW.Veh_Motion.Wheel_dir.REVERSE}"
            )
            ...
            preconditions = (
                "<br>Evaluation stopped!"
                " PDW not ACTIVE before error injection: "
                f"<br>{signal_name['PDW_State']} is set to INIT ({constants.HilCl.PDW.States.INIT}). "
                f"<br> or {signal_name['PDW_State']} is set to OFF ({constants.HilCl.PDW.States.OFF}). "
                f"<br> or {signal_name['PDW_State']} is set to FAILURE ({constants.HilCl.PDW.States.FAILURE}). "
            )
        elif trigger4 is None:
            preconditions = (
                "<br>Evaluation stopped!"
                "<br> One of the following situatuions happened:"
                "<br>1) There is no error injected into the system!"
                "<br>2) Gear NOT in REVERSE!"
                f"<br>2) PDW state not correct. {signal_name['PDW_State']} is equal to: {constants.HilCl.PDW.States.INIT} , {constants.HilCl.PDW.States.OFF} or {constants.HilCl.PDW.States.FAILURE} ."
            )
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, the PDW function didn't switch to the desired state. "
                "<br>The following condition was not met: "
                f"<br>{signal_name['PDW_State']} was not set to FAILURE {constants.HilCl.PDW.States.FAILURE} within {constants.HilCl.PDW.FTTI.SAFETY_SS}us. "
                "<br> after an error was injected into the system.".split()
            )
        else:
            eval_cond = [True] * 1
        ...
        fig1.add_trace(go.Scatter(x=time_signal, y=pdw_state, mode="lines", name=signal_name["PDW_State"]))
...
@testcase_definition(
    name="System state transition (within 700 miliseconds) while PDW was active",
    description="Check if the system goes to FAILURE once there is a error injected into the system.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            System_safe_state_error_injected_PDW_active_SS,
        ]
