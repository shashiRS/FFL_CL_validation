...
@teststep_definition(
    ...
    description=(
        "This test step evaluates after PDW is deactivated by Parking gear the driving tube is not displayed."
    ...
    def process(self, **kwargs):
        ...
        if state_pdw_init_mask.any():
            if not pdw_state_mask:
                ...
                state_deact_pgear_mask = self.pdw_state == pdwState.PDW_INT_STATE_DEACT_P_GEAR
                ...
                if act_validation:
                    dt_none = FALSE
                    if state_deact_pgear_mask.any():
                        idx = -1
                        for i, element in enumerate(self.pdw_state):
                            if element == pdwState.PDW_INT_STATE_DEACT_P_GEAR:
                                idx = i
                                break
                        if (
                            self.gear_state.iat[idx] == gearPosition.GEAR_P
                            and self.pdw_state.iat[idx] == pdwState.PDW_INT_STATE_DEACT_P_GEAR
                            and self.dt_req.iat[idx - delay] != drvTubeDisplay.PDC_DRV_TUBE_NONE
                        ):  # validate if parking gear was engaged
                            deactivation_metod = "deactivated by parking gear"
                            pdwState_No_value = pdwState.PDW_INT_STATE_DEACT_P_GEAR
                            pdwState_value = "PDW_INT_STATE_DEACT_P_GEAR"
                            signal_summary["AP.odoInputPort.odoSigFcanPort.gearboxCtrlStatus.gearCur_n"] = (
                                f"Gear in parking position with value GEAR_P = {int(self.gear_state.iat[idx])}"
                            )
                            signal_summary["AP.pdcpDrivingTubePort.drvTubeDisplay_nu"] = (
...
@testcase_definition(
    name="No Driving Tube Display Park Gear",
    description=("PDW function shall set the Driving tube to None if it is deactivated by Park Gear"),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class PDWDrivingTubePGEAR(TestCase):
