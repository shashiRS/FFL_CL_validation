...
SIGNAL_DATA = "PDW_DEACTIVATED_TO_ACTIVATED_BY_BUTTON"
...
@teststep_definition(
    step_number=1,
    name="PDW state transition to 'activated by button'",
    description=(
        "This step is checking if PDW switches from 'OFF' to 'activated by button' state after AUP gets activated."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_deactivated_to_activated_by_button(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation of {signal_name['PDW_State']} is PASSED, PDW State switched to 'activated by button' state ({constants.HilCl.PDW.States.ACTIVATED_BY_BUTTON})"
            f" within {constants.HilCl.PDW.FTTI.SYSTEM}us after the reactivation condition (AUP OFF/ON) was fulfilled "
            f"({signal_name['AUP_State']} switched to 'ACTIVE' ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}).".split()
        )
            ...
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if (
                        ...
                        trigger2 = True
                        # PDW button pressed for deactivation
                    ...
                    if (
                        ap_state[frame] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING
                        and trigger3
                    ):
                        if pdw_state[frame] != constants.HilCl.PDW.States.OFF:
                            trigger4 = False
        ...
        if trigger1 is False:
            preconditions = f"Check trigger1! PDW state {signal_name['PDW_State']} was set to Failure ({constants.HilCl.PDW.States.FAILURE})."
        elif trigger2 is None:
            preconditions = f"Check trigger2! Gear not switched to REVERSE ({signal_name['Gear_auto']}-{constants.HilCl.PDW.Gear.Automatic.REVERSE} or {signal_name['Gear_man']}-{constants.HilCl.PDW.Gear.Manual.REVERSE})."
        elif trigger3 is False:
            preconditions = f"Check trigger3! PDW state {signal_name['PDW_State']} not switched to 'OFF'({constants.HilCl.PDW.States.OFF}) within {constants.HilCl.PDW.FTTI.SYSTEM}us."
        elif trigger4 is False:
            preconditions = f"Check trigger4! PDW state {signal_name['PDW_State']} not remained in 'OFF' ({constants.HilCl.PDW.States.OFF}) after deactivation until AUP activation."
        elif trigger5 is None:
            preconditions = f"Check trigger5! AUP did not get activated. ({signal_name['AUP_State']} not switched to ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING})."
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, PDW State ({signal_name['PDW_State']}) did not switch to 'activated by button' state ({constants.HilCl.PDW.States.ACTIVATED_BY_BUTTON})"
                f" within {constants.HilCl.PDW.FTTI.SYSTEM}us after reactivation condition (AUP OFF/ON ) was fulfilled "
                f"({signal_name['AUP_State']} switched to 'ACTIVE' ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}).".split()
            )
        else:
            eval_cond = [True] * 1

        signal_summary["PDW function 'activated by button'."] = evaluation1
...
@testcase_definition(
    name="PDW function 'deactivated' to 'activated by button'.",
    description="Check if PDW state transition to another active state triggered by another function activation",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            Pdw_deactivated_to_activated_by_button,
        ]
