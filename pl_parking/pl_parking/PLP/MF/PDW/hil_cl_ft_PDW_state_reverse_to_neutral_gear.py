...
SIGNAL_DATA = "PDW_REVERSE_TO_NEUTRAL"
...
@teststep_definition(
    ...
    description=(
        "This step is checking if PDW remains in the same state after switching from REVERSE to NEUTRAL gear."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_state_reverse_to_neutral_gear(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, PDW State {signal_name['PDW_State']} remained into 'activated by R-GEAR' state({constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR})"
            f" from previous state - 'activated by R-Gear' {signal_name['PDW_State']} - {constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR} "
            "after switching from REVERSE to NEUTRAL gear".split()
        )
        ...
        if trigger1 is False:
            ...
            preconditions = f"Check trigger3! PDW state {signal_name['PDW_State']} not switched to 'activated by R-Gear'({constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR}) within {constants.HilCl.PDW.FTTI.SYSTEM}us."
        elif trigger4 is False or trigger4 is None:
            preconditions = f"Check trigger4! Gear not switched to NEUTRAL ({signal_name['Gear_auto']}-{constants.HilCl.PDW.Gear.Automatic.NEUTRAL} or {signal_name['Gear_man']}-{constants.HilCl.PDW.Gear.Manual.NEUTRAL})."
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, PDW State {signal_name['PDW_State']} did not remain into 'activated by R-GEAR' state({constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR})"
                f" from previous state - 'activated by R-Gear' {signal_name['PDW_State']} - {constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR} "
                "after switching from REVERSE to NEUTRAL gear".split()
            )
        else:
...
@testcase_definition(
    name="PDW state active",
    description="Check if PDW remains active after switching between different transmission gears.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_state_reverse_to_neutral_gear,
        ]
