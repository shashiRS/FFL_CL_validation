...
@teststep_definition(
    step_number=1,
    name="Reduce to mute continous zone",  # this would be shown as a test step name in html report
    description=(
...
class ReduceToMute(TestStep):
    ...
    def process(self, **kwargs):
        ...
        if act_rgear and small_dist < pdw_sector_length.SLICE_LENGTH:
            ...
            if (
                reduce >= (reduceToMute.CONTINUOUS_TIME - delay)
...
@testcase_definition(
    name="Continuous zone reduce to mute ego standstill",
    description=(
...
class ReduceToMuteContinuous(TestCase):
