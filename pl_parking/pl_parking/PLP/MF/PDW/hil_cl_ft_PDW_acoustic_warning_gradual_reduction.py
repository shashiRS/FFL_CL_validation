...
SIGNAL_DATA = "PDW_WARNING_REDUCTION_GRAD_INTERM"
...
@teststep_definition(
    step_number=1,
    name="PDW front acoustic warning reduction in intermittent tone",
    description=(
        "This step is checking if PDW is reducing the acoustic driver warning gradually once the ego vehicle and the traffic participant are in standstill mode."
    ),
    expected_result=BooleanResult(TRUE),
    doors_url="https://jazz.conti.de/rm4/resources/BI_j0MqW9IAEe2iKqc0KPO99Q?oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099",
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_acoustic_warning_reduction_front_obj_det_interm(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, the warning ({signal_name['Front_speaker_volume']}) for the front obstacle placed within PDW distance range started the reduction in 3s "
            f" and it was done as expected(5 4 2 0 - once every 0.5s) after the warning reduction condition was fulfilled: ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) "
            " while both vehicles were in standstill.".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    trigger1 = True
                    # Check if there are relevant objects around the vehicle (in PDW distance range).
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Vehicle approaching to TP and PDW gets automatically activated
                        if (
                            fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD
                            and rr_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD
                        ) and trigger3 is None:

                            if not FTTI_time:  # Wait FTTI time for functionality to change its own state as it should
                                start_waiting = 1
                            else:
                                # Check if PDW switched to 'automatically activated'
                                if pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED:
                                    ...
                                    trigger3 = False
                                    break
                        # Vehicle and TP standstill and TP in intermittent tone range
                        elif (
                            fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT
                            and rr_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT
                            and tp_velocity[frame] < constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS
                            and (
                                (
                                    uss1[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss1[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                                or (
                                    uss2[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss2[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                                or (
                                    uss3[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss3[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                                or (
                                    uss4[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss4[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                            )
                        ) and trigger3:
                            trigger4 = True
                            reduction_time += time_signal[frame] - time_signal[frame - 1]
                            if reduction_time >= 1 and val1_ok is None:
                                reduction_time = 0
                                if front_volume[frame] == 5:
                                    val1_ok = True
                                    trigger4 = False
                                else:
                                    ...
                                    break
                            if (
                                reduction_time >= constants.HilCl.PDW.FTTI.REDUCTION_INTERM
                                and val1_ok
                                and val2_ok is None
                            ):  # Wait reduction time
                                reduction_time = 0
                                if front_volume[frame] == 4 and val2_ok is None:
                                    val2_ok = True
                                    trigger4 = False
                                else:
                                    eval_status_ok = False
                                    break
                            if (
                                reduction_time >= constants.HilCl.PDW.FTTI.REDUCTION_STEP
                                and val2_ok
                                and val3_ok is None
                            ):  # Wait reduction time
                                reduction_time = 0
                                if front_volume[frame] == 2 and val3_ok is None:
                                    val3_ok = True
                                    trigger4 = False
                                else:
                                    eval_status_ok = False
                            ...
                            if (
                                ...
                                reduction_time = 0
                                if front_volume[frame] == 0 and val4_ok is None:
                                    val4_ok = True
                                else:
                                    eval_status_ok = False
                                    break
                                if val1_ok and val2_ok and val3_ok and val4_ok:
                                    eval_status_ok = True
                                    break
                        else:
                            trigger4 = None
        ...
        if trigger1 is False:
            ...
            preconditions = f"Check trigger3! PDW state {signal_name['PDW_State']} not switched to 'automatically activated' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) before standstill."
        elif trigger4 is False or trigger4 is None:
            preconditions = (
                f"Check trigger4! Vehicle not standstill ({signal_name['Wheel_direction_FL']} not  {constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT})"
                " or traffic participant not in intermittent tone range (at least one of the front sensors should have an object detection "
                f" placed between {constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE}m and {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION})"
            )
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, the warning ({signal_name['Front_speaker_volume']}) for the front obstacle placed within PDW distance range wasn't reduced to 0 as expected(5 4 2 0 - once every 0.5s)"
                f" within 3s after the warning reduction condition was fulfilled: ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
                " while both vehicles were in standstill.".split()
            )
        else:
            eval_cond = [True] * 1

        signal_summary["PDW acoustic warnings."] = evaluation1
        ...
        fig.update_layout(title="PDW function related input signals")
        ...
        fig2.add_trace(go.Scatter(x=time_signal, y=pdw_state, mode="lines", name=signal_name["PDW_State"]))
        fig2.add_trace(
        ...
        fig2.update_layout(title="PDW function related output signals")
...
@teststep_definition(
    step_number=2,
    name="PDW rear acoustic warning reduction in intermittent tone",
    description=(
        "This step is checking if PDW is reducing the acoustic driver warning gradually once the ego vehicle and the traffic participant are in standstill mode."
    ),
    expected_result=BooleanResult(TRUE),
...
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_acoustic_warning_reduction_rear_obj_det_interm(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, the warning ({signal_name['Rear_speaker_volume']}) for the rear obstacle placed within PDW distance range was reduced to 0 as expected(5 4 2 0 - once every 0.5s)"
            f" after the warning reduction condition was fulfilled: ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR}) "
            " while both vehicles were in standstill.".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Vehicle approaching to TP and PDW is activated by RGear
                        if (
                            ...
                            if not FTTI_time:  # Wait FTTI time for functionality to change its own state as it should
                                ...
                                # Check if PDW switched to 'activated by RGear'
                                if pdw_state[frame] == constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR:
                                    trigger3 = True
                                    FTTI_time = 0
                                else:
                                    trigger3 = False
                        ...
                        # Vehicle and TP standstill and TP in intermittent tone range
                            ...
                            and (
                                (
                                    uss7[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss7[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                                or (
                                    uss8[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss8[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                                or (
                                    uss9[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss9[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                                or (
                                    uss10[frame] > constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss10[frame] < constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION
                                )
                            )
                        ) and trigger3:
        ...
        if trigger1 is False:
            ...
            preconditions = f"Check trigger3! PDW state {signal_name['PDW_State']} not switched to 'activated by RGear' ({constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR}) before standstill."
        elif trigger4 is False or trigger4 is None:
            preconditions = (
                f"Check trigger4! Vehicle not standstill ({signal_name['Wheel_direction_FL']} not  {constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT})"
                " or traffic participant not in intermittent tone range (at least one of the rear sensors should have an object detection "
                f" placed between {constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE}m and {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION})"
            )
        else:
            preconditions = "Preconditions were fulfilled. Check test result!"
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, the warning ({signal_name['Rear_speaker_volume']}) for the rear obstacle placed within PDW distance range wasn't reduced to 0 as expected(5 4 2 0 - once every 0.5s)"
                f" within 3s after the warning reduction condition was fulfilled: ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR})"
                " while both vehicles were in standstill.".split()
            )
        else:
            eval_cond = [True] * 1
...
@teststep_definition(
    step_number=3,
    name="PDW front acoustic warning reduction in continuous tone",
    description=(
        "This step is checking if PDW is reducing the acoustic driver warning gradually once the ego vehicle and the traffic participant are in standstill mode."
    ),
    expected_result=BooleanResult(TRUE),
...
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_acoustic_warning_reduction_front_obj_det_cont(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, the warning ({signal_name['Front_speaker_volume']}) for the front obstacle placed within PDW distance range was reduced to 0 as expected(5 4 2 0 - once every 0.5s)"
            f" after the warning reduction condition was fulfilled: ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) "
            " while both vehicles were in standstill.".split()
        )
                        ...
                        if (
                            ...
                            if not FTTI_time:  # Wait FTTI time for functionality to change its own state as it should
                                ...
                                if pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED:
                                    ...
                                    trigger3 = False
                                    break
                        # Vehicle and TP standstill and TP in continuous tone range
                        elif (
                            fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT
                            and rr_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT
                            and tp_velocity[frame] < constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS
                            and (
                                (
                                    uss1[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss1[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                                or (
                                    uss2[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss2[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                                or (
                                    uss3[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss3[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                                or (
                                    uss4[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss4[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                            )
                        ) and trigger3:
                            trigger4 = True
                            ...
                            if reduction_time >= 1 and val1_ok is None:
                                ...
                                if front_volume[frame] == 5:
                                    ...
                                    break
                            if (
                                reduction_time >= constants.HilCl.PDW.FTTI.REDUCTION_CONT
                                and val1_ok
                                and val2_ok is None
                            ):  # Wait reduction time
                                reduction_time = 0
        ...
        if trigger1 is False:
            ...
            preconditions = (
                f"Check trigger4! Vehicle not standstill ({signal_name['Wheel_direction_FL']} not  {constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT})"
                " or traffic participant not in continuous tone range (at least one of the front sensors should have an object detection "
                f" placed between {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m and {constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE})"
            )
        else:
            preconditions = "Preconditions were fulfilled. Check test result!"
        ...
        if eval_status_ok is None:
            ...
            evaluation1 = " ".join(
                f"The evaluation is FAILED, the warning ({signal_name['Front_speaker_volume']}) for the front obstacle placed within PDW distance range wasn't reduced to 0 as expected(5 4 2 0 - once every 0.5s)"
                f" within 5s after the warning reduction condition was fulfilled: ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
                " while both vehicles were in standstill.".split()
            )
        else:
            eval_cond = [True] * 1
...
@teststep_definition(
    step_number=4,
    name="PDW rear acoustic warning reduction in continuous tone",
    description=(
        "This step is checking if PDW is reducing the acoustic driver warning gradually once the ego vehicle and the traffic participant are in standstill mode."
    ),
    expected_result=BooleanResult(TRUE),
...
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_acoustic_warning_reduction_rear_obj_det_cont(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
                    ...
                    if ObjectDetection(frame):
                        ...
                        # Vehicle and TP standstill and TP in continuous tone range
                            ...
                            and (
                                (
                                    uss7[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss7[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                                or (
                                    uss8[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss8[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                                or (
                                    uss9[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss9[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                                or (
                                    uss10[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                    and uss10[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                )
                            )
                        ) and trigger3:
                            trigger4 = True
        ...
        if trigger1 is False:
            ...
            preconditions = (
                f"Check trigger4! Vehicle not standstill ({signal_name['Wheel_direction_RR']} not  {constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT})"
                " or traffic participant not in continuous tone range (at least one of the rear sensors should have an object detection "
                f" placed between {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m and {constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE})"
            )
        else:
            preconditions = "Preconditions were fulfilled. Check test result!"
...
@testcase_definition(
    name="PDW acoustic warning reduction",
    description="Check if PDW is behaving accordingly once the TP in intermittent/continuous tone range and the ego become standstill.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_acoustic_warning_reduction_front_obj_det_interm,
            PDW_acoustic_warning_reduction_rear_obj_det_interm,
            PDW_acoustic_warning_reduction_front_obj_det_cont,
            PDW_acoustic_warning_reduction_rear_obj_det_cont,
        ]
