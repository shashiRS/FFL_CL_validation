...
@teststep_definition(
    step_number=1,
    name="WHP state transition - activation",
    description=("This step is checking if WHP is switching from INACTIVE to ACTIVE state."),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class WHP_activation_by_button(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, WHP State {signal_name['WHP_State']} switched to 'ACTIVE' state({constants.HilCl.WHP.State.ACTIVE})"
            f" from previous state - 'INACTIVE' {signal_name['WHP_State']} - {constants.HilCl.WHP.State.INACTIVE} "
            ...
            if not start_waiting:
                ...
                if whp_state[frame] != constants.HilCl.WHP.State.FAILURE:
                    ...
                    if pdw_button[frame] != constants.HilCl.PDW.Button.WHP_TAP_ON and trigger2 is None:
                        # Check if WHP is INACTIVE before WHP button press
        ...
        if trigger1 is False:
            ...
            preconditions = (
                f" Check trigger2! WHP ({signal_name['WHP_State']}) state different than INACTIVE-{constants.HilCl.WHP.State.INACTIVE} before WHP button press"
                f" ({signal_name['PDW_Button']} - {constants.HilCl.PDW.Button.WHP_TAP_ON}."
            )
        elif trigger3 is False or trigger3 is None:
            preconditions = f" Check trigger3! WHP button ({signal_name['PDW_Button']}) not pressed for function activation ({constants.HilCl.PDW.Button.WHP_TAP_ON})."
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, WHP State {signal_name['WHP_State']} did not switch to 'ACTIVE' state({constants.HilCl.WHP.State.ACTIVE})"
                f" from previous state - 'INACTIVE' {signal_name['WHP_State']} - {constants.HilCl.WHP.State.INACTIVE} "
                f" after WHP button was pressed ({signal_name['PDW_Button']} - ({constants.HilCl.PDW.Button.WHP_TAP_ON})".split()
            )
        else:
...
@testcase_definition(
    name="WHP state active",
    description="Check if WHP(wheel protection)function is activated by button.",
)
