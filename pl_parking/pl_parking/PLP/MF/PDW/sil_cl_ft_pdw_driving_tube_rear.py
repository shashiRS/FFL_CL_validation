...
@teststep_definition(
    step_number=1,
    name="Driving tube display Rear",  # this would be shown as a test step name in html report
    description=(
        "This test step evaluates if PDW displays the Driving tube in rear direction when engaged gear is Reverse."
    ...
    def process(self, **kwargs):
        ...
        for j, element in enumerate(self.dt_req):
            if element == drvTubeDisplay.PDC_DRV_TUBE_REAR:
                idx_drt = j
                break
        if self.dt_req.iat[idx] != drvTubeDisplay.PDC_DRV_TUBE_REAR:
            signal_summary["AP.odoInputPort.odoSigFcanPort.gearboxCtrlStatus.gearCur_n"] = (
                f" Gearbox position is in Rear with value {int(self.gear_state.iat[idx])} (GEAR_R)."
            )
        signal_summary["AP.pdcpDrivingTubePort.drvTubeDisplay_nu"] = (
            f"Driving tube display PDC_DRV_TUBE_REAR = {int(self.dt_req.iat[idx_drt])}"
        )
        ...
        self.result.measured_result = (
            ...
            if (
                ...
                and self.dt_req.iat[idx_drt] == drvTubeDisplay.PDC_DRV_TUBE_REAR
            )
            else FALSE
        )
...
@testcase_definition(
    name="Driving Tube Display Rear",
    description=(
        "PDW function shall display the Driving Tube in Rear direction when the current engaged gear is Reverse."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class PDWDrivingTubeRear(TestCase):
