...
@teststep_definition(
    step_number=1,
    name="PDW Fail State Machine two triggers",  # this would be shown as a test step name in html report
    description=(
...
class TestStepPDW(TestStep):
    ...
    def process(self, **kwargs):
        ...
        pdw_state_trigger1 = pdw_SysState_trigger1 = pdw_state_trigger2 = pdw_SysState_trigger2 = (
                    ...
                    if int(self.pdw_state.iat[idx1]) == pdwState.PDW_INT_STATE_FAILURE:
                        for i, element in enumerate(self.pdwFailureState):
                            if i > (idx1 + delay) and element == ap_PDWFailure.PDW_FAILURE_TRUE:
                                ...
                                break
                        if (
                            self.pdwFailureState.iat[idx1 + delay] == ap_PDWFailure.PDW_FAILURE_TRUE
        ...
        if activate_method1 == "automatically activated":
            fig.add_trace(auto_obj_dist)
        if activate_method1 == "activated by button":
            fig.add_trace(button_press)
        if activate_method1 == "activated by automated parking":
            fig.add_trace(apState_figure)
        if activate_method1 == "activated by reverse gear":
            fig.add_trace(rg_trace)
        if activate_method1 == "deactivated by failure" or activate_method2 == "deactivated by failure":
