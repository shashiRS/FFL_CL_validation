...
@teststep_definition(
    step_number=1,
    name="PDW State Machine two triggers",  # this would be shown as a test step name in html report
    description=(
...
class TestStepPDW(TestStep):
    ...
    def process(self, **kwargs):
        ...
        activate_method1 = activate_method2 = "not activated"
        deactivation_method1 = "PDW error"
        pdwState_No_value1 = pdwState_No_value2 = pdwState.PDW_INT_STATE_INIT
        pdwState_value1 = pdwState_value2 = "PDW_INT_STATE_INIT"
        pdwSysState_No_value1 = pdwSysState_No_value2 = pdwSysState.PDW_INIT
        pdwSysState_value1 = pdwSysState_value2 = "PDW_INIT"
        pdw_state_trigger1 = pdw_SysState_trigger1 = pdw_SysState_trigger2 = pdw_state_trigger2 = (
            ...
            if not pdw_state_mask and not pdw_sys_state_mask:
                ...
                if verdict == TRUE:
                    ...
                    if (
                        ...
                        for i, element in enumerate(self.gear_state):
                            if i > (idx + delay) and element == gearPosition.GEAR_R and int(self.epb_switch.iat[i]!= epbSwitch.EPB_OFF):
                                idx1 = i
                                break
                        if (
        ...
        if activate_method1 == "automatically activated" or activate_method2 == "automatically activated":
            fig.add_trace(auto_obj_dist)
        if activate_method1 == "activated by button" or activate_method2 == "deactivated by button":
            fig.add_trace(button_press)
        if activate_method1 == "activated by automated parking" or activate_method2 == "activated by automated parking":
            fig.add_trace(apState_figure)
        if activate_method1 == "activated by reverse gear" or activate_method2 == "activated by reverse gear":
