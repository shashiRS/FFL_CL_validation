...
SIGNAL_DATA = "PDW_REACTIVATION_BY_SPEED_THRESHOLD"
...
class PDW_reactivation_by_Speed_Threshold(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" after reactivation condition was fulfilled (PDW speed range threshold exceeded/not exceeded).".split()
        )
                ...
                if veh_ignition[frame] and pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            # Monitor and find the moment when the button was pressed
                        ...
                        # Find the moment when vehicle's speed is greater than the max threshold for PDW
                            ...
                            trigger4 = True
                        # Find the moment when vehicle's speed is within PDW speed range again
                        elif (
                            veh_velocity[frame] > constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS
                            and veh_velocity[frame] < constants.HilCl.PDW.Thresholds.MAX_SPEED_THRESHOLD_MPS
                            and fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD
        ...
        if trigger1 is False:
            ...
            preconditions = "Check trigger2! PDW state not switched to 'Automatically activation' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})."
        elif trigger3 is False:
            preconditions = f"Check trigger3! PDW state not switched to 'OFF' ({constants.HilCl.PDW.States.OFF})."
        elif trigger4 is None:
            preconditions = "Check trigger4! PDW speed range - max threshold not exceeded."
        elif trigger5 is None:
            preconditions = (
                "Check trigger5! None of the preconditions were fulfilled, input (CM scenario) not correct!."
            )
        else:
        ...
        if eval_status_ok is None:
            ...
            evaluation1 = " ".join(
                ...
                f" after the following reactivation condition was fulfilled (PDW speed range threshold exceeded/not exceeded).".split()
            )
        else:
            eval_cond = [True] * 1
...
@testcase_definition(
    name="PDW-reactivation by speed threshold.",
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_reactivation_by_Speed_Threshold,
        ]
