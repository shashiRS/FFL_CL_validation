...
@teststep_definition(
    step_number=1,
    name="Reduce to mute intermittent zone",  # this would be shown as a test step name in html report
    description=(
...
class ReduceToMute(TestStep):
    ...
    def process(self, **kwargs):
        ...
        if (
            act_rgear
            and small_dist < pdw_sector_length.PDW_SECTOR_LENGTH
            and small_dist > pdw_sector_length.SLICE_LENGTH
        ):
            ...
            if (
                reduce >= (reduceToMute.INTERMITTENT_TIME - delay)
                and reduce <= (reduceToMute.INTERMITTENT_TIME + delay)
                and volume_lvl == reduceToMute.PDW_REDUCE_NONE
            ):
                ...
                signal_summary["TIME"] = "".join(
                    f"Standstill time with obstacle in intermittent zone: {round(self.ap_time.iat[reduce], 3)}s. "
                ...
                signal_summary["Time"] = f"Time in standstill {reduce/100}s"
                verdict = False
        if not act_rgear:
        ...
        elif small_dist < pdw_sector_length.SLICE_LENGTH or small_dist > pdw_sector_length.PDW_SECTOR_LENGTH:
            self.result.measured_result = FALSE
            signal_summary["AP.testEvaluation.shortestDistanceCM[1]"] = (
                f"Obstacle not in intermittent tone zone {small_dist}m"
            )
        else:
...
@testcase_definition(
    name="Intermittent zone reduce to mute ego standstill",
    description=(
        "After the time in standstill of the ego vehicle has passed when an obstacle is in intermittent zone to reduce the tone to muted linearly."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class ReduceToMuteIntermittent(TestCase):
