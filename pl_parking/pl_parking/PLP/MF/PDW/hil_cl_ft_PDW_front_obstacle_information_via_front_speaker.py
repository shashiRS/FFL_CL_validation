...
SIGNAL_DATA = "PDW_INFORMATION_VIA_FRONT_SPEAKERS"
...
@teststep_definition(
    step_number=1,
    name="PDW activation - front warnings sent",
    description=(
        "This step is checking if PDW is sending only front warnings"
        "when only the front sector group is detecting an obstacle within PDW distance range."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_front_acoustic_warnings_sent(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, PDW State {signal_name['PDW_State']} switched to 'automatically activated' state ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) within {constants.HilCl.PDW.FTTI.SYSTEM}us "
            f"and {signal_name['Front_speaker_volume']} and {signal_name['Front_speaker_pitch']} were greater than 0 "
            f"and {signal_name['Rear_speaker_volume']} and {signal_name['Rear_speaker_pitch']} were equal to 0 "
            ...
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Find the moment when vehicle started moving forward within PDW speed range
                        if (
                            ...
                            if (
                                ...
                                # Check if PDW switched to 'automatically activated'. Acoustic output based only on front side object detection.
                                if (
        ...
        if trigger1 is False:
            ...
            preconditions = (
                f"Check trigger3! Vehicle is not moving forward ({signal_name['Vehicle_velocity']} < {constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS})"
                f"or {signal_name['Wheel_direction_FL']} not {constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD}."
            )
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, PDW State {signal_name['PDW_State']} did not switch to 'automatically activated' state ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) within {constants.HilCl.PDW.FTTI.SYSTEM}us "
                f"or {signal_name['Front_speaker_volume']} or {signal_name['Front_speaker_pitch']} not greater than 0 "
                f"or {signal_name['Rear_speaker_volume']} or {signal_name['Rear_speaker_pitch']} not equal to 0. "
                " Throughout the entire evaluation there were reasons for activating the function "
                f"(front object detection within PDW distance range: {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}m).".split()
            )
        else:
...
@testcase_definition(
    name="PDW function front autoactivation.",
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            Pdw_front_acoustic_warnings_sent,
        ]
