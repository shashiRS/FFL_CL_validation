...
SIGNAL_DATA = "PDW_REACTIVATION_BY_P-gear_ON_OFF_STANDSTILL"
...
class PDW_reactivation_by_Pgear_standstill(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for frame in range(0, len(time_signal)):
            ...
            if not start_waiting:
                ...
                if veh_ignition[frame] and pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    # Check standstill autoactivation is False
                        ...
                        if ObjectDetection(frame):
                            ...
                            if veh_velocity[frame] < abs(constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS):
                                if (
                                    ...
                                    gear_manual[frame]
                                    != (
                                        constants.HilCl.PDW.Gear.Manual.PARK
                                        or gear_automatic[frame] != constants.HilCl.PDW.Gear.Automatic.PARK
                                    )
                                    and (
                                        constants.HilCl.PDW.Gear.Manual.REVERSE
                                        or gear_automatic[frame] != constants.HilCl.PDW.Gear.Automatic.REVERSE
                                    )
                                    and (
                                        gear_manual[frame] != constants.HilCl.PDW.Gear.Manual.INVALID
        ...
        if trigger1 is False:
            ...
            preconditions = f"Check trigger5! Parking gear (automatic-{constants.HilCl.PDW.Gear.Automatic.PARK} , manual-{constants.HilCl.PDW.Gear.Manual.PARK}) not engaged."
...
@testcase_definition(
    name="PDW-reactivation by Pgear(parking gear) ON-OFF ,standstill situation.",
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_reactivation_by_Pgear_standstill,
        ]
