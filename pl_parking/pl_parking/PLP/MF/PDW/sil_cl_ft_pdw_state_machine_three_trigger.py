...
reset_condition = constants.SilCl.PDWConstants.PDWResetCondition
...
class Signals(SignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.PDW_AUTO_RESET: "AP.drvWarnDebugPort.autoActivResetCond",
...
@teststep_definition(
    step_number=1,
    name="PDW State Machine three triggers",  # this would be shown as a test step name in html report
    description=(
        "This test step evaluates PDW state changes and if the necessary conditions are met PDW changes to an other state."
...
class TestStepPDW(TestStep):
    ...
    def process(self, **kwargs):
        ...
        self.pdw_auto_reset = signals[Signals.Columns.PDW_AUTO_RESET]
        ...
        if state_pdw_init_mask.any():
            if not pdw_state_mask and not pdw_sys_state_mask:
                ...
                if verdict == FALSE:
                    if state_deact_fail_mask.any():
                        ...
                        if (
                            ...
                            deactivation_method1 = "deactivated by failure"
                ...
                if verdict == TRUE:
                    ...
                    for i, element in enumerate(self.pdw_state):
                        if i > (idx + delay) and element != self.pdw_state.iat[idx + delay]:
                            idx1 = i
                            break
                        # only for TATA deactivation by speed over 13 km/h
                    if (
                ...
                if verdict == TRUE:
                    ...
                    # only for TATA deactivation by speed over 13 km/h
                    if (
                        int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_DEACT_SPEED
                        and self.velocity.iat[idx2] >= (tata_overspeed_deact / speed_converter)
                        and self.velocity.iat[idx2] <= speed_threshold.SPEED_THRESHOLD_HI
                    ):
                        for i, element in enumerate(self.velocity):
                            if i > (idx1 + delay) and element >= (tata_overspeed_deact / speed_converter):
                                idx2 = i
                                break
                        if (
                            (self.velocity.iat[idx2]) >= overspeed_deact / speed_converter
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_DEACT_SPEED
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_OFF
                        ):  # validate if vehicle speed is greater than speed threshold
                            activate_method3 = "deactivated by speed"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_DEACT_SPEED
                            pdwState_value3 = "PDW_INT_STATE_DEACT_SPEED"
                            ...
                            verdict = TRUE
                            signal_summary["Car.v"] = f"Ego vehicle sped {round(self.velocity.iat[idx2], 3)} m/s"
                        # in case of deactivation by speed over 20 km/h
                    elif (
                        int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_DEACT_SPEED
                        and self.velocity.iat[idx2] >= speed_threshold.SPEED_THRESHOLD_HI
                        ...
                        for i, element in enumerate(self.velocity):
                            if i > (idx1 + delay) and element >= speed_threshold.SPEED_THRESHOLD_HI:
                                idx2 = i
                                break
                        if (
                            (self.velocity.iat[idx2]) >= overspeed_deact / speed_converter
                    ...
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_DEACT_P_GEAR:
                        for i, element in enumerate(self.gear_state):
                            if i > (idx1 + delay) and element == gearPosition.GEAR_P:
                                idx2 = i
                                break
                        if (
                            self.gear_state.iat[idx2] == gearPosition.GEAR_P
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_DEACT_P_GEAR
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_OFF
                        ):  # validate if parking gear was engaged
                            activate_method3 = "deactivated by parking gear"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_DEACT_P_GEAR
                            pdwState_value3 = "PDW_INT_STATE_DEACT_P_GEAR"
                            pdwSysState_No_value3 = pdwSysState.PDW_OFF
                            pdwSysState_value3 = "PDW_OFF"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["AP.odoInputPort.odoSigFcanPort.gearboxCtrlStatus.gearCur_n"] = (
                                f"Gear in reverse with value GEAR_P = {int(self.gear_state.iat[idx2])}"
                            )
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_DEACT_EPB:
                        for i, element in enumerate(self.epb_switch):
                            if i > (idx1 + delay) and element == epbSwitch.EPB_ON:
                                idx2 = i
                                break
                        if (
                            self.epb_switch.iat[idx2] == epbSwitch.EPB_ON
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_DEACT_EPB
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_OFF
                        ):  # validate if electronic parking brake was engaged
                            activate_method3 = "deactivated by EPB"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_DEACT_EPB
                            pdwState_value3 = "PDW_INT_STATE_DEACT_EPB"
                            pdwSysState_No_value3 = pdwSysState.PDW_OFF
                            pdwSysState_value3 = "PDW_OFF"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["DM.BrakePark"] = (
                                f"Electornic parking brake active with value EPB_ON = {int(self.epb_switch.iat[idx2])}"
                            )
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_DEACT_AP_FIN:
                        for i, element in enumerate(self.apStatus):
                            if i > (idx1 + delay) and element == apStateVal.AP_AVG_FINISHED:
                                idx2 = i
                                break
                        if (
                            self.apStatus.iat[idx2] == apStateVal.AP_AVG_FINISHED
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_DEACT_AP_FIN
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_OFF
                        ):  # validate if automated parking finished
                            activate_method3 = "deactivated by automated parking finish"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_DEACT_AP_FIN
                            pdwState_value3 = "PDW_INT_STATE_DEACT_AP_FIN"
                            pdwSysState_No_value3 = pdwSysState.PDW_OFF
                            pdwSysState_value3 = "PDW_OFF"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["AP.planningCtrlPort.apStates"] = (
                                f"Automated parking finished apStates = {int(self.apStatus.iat[idx2])} (AP_AVG_FINISHED)"
                            )
                            apState_figure = go.Scatter(
                                x=self.ap_time,
                                y=self.apStatus,
                    ...
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_DEACT_BTN:
                        for i, element in enumerate(self.hmi_interract):
                            if i > (idx1 + delay) and element == pdw_hmi_interact.TAP_ON_PDC:
                                idx2 = i
                                break
                        if (
                            self.hmi_interract.iat[idx2] == pdw_hmi_interact.TAP_ON_PDC
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_DEACT_BTN
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_OFF
                        ):  # validate if pdw button is pressed
                            activate_method3 = "deactivated by button"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_DEACT_BTN
                            pdwState_value3 = "PDW_INT_STATE_DEACT_BTN"
                            pdwSysState_No_value3 = pdwSysState.PDW_OFF
                            pdwSysState_value3 = "PDW_OFF"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            ...
                            signal_summary["AP.hmiOutputPort.userActionHeadUnit_nu"] = (
                                f"PDW button pressed with value TAP_ON_PDC = {int(self.hmi_interract.iat[idx2])}"
                            )
                            button_press = go.Scatter(
                                x=self.ap_time,
                                y=self.hmi_interract,
                    ...
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_ACT_BTN:
                        for i, element in enumerate(self.hmi_interract):
                            if i > (idx1 + delay) and element == pdw_hmi_interact.TAP_ON_PDC:
                                idx2 = i
                                break
                        if (
                            self.hmi_interract.iat[idx2] == pdw_hmi_interact.TAP_ON_PDC
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_ACT_BTN
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_ACTIVATED_BY_BUTTON
                        ):  # validate if pdw button is pressed
                            activate_method3 = "activated by button"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_ACT_BTN
                            pdwState_value3 = "PDW_INT_STATE_ACT_BTN"
                            pdwSysState_No_value3 = pdwSysState.PDW_ACTIVATED_BY_BUTTON
                            pdwSysState_value3 = "PDW_ACTIVATED_BY_BUTTON"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["First trigger for signal: AP.hmiOutputPort.userActionHeadUnit_nu"] = (
                    ...
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_ACT_AUTO:
                        for i, element in enumerate(self.pdw_state):
                            if i > (idx1 + delay) and element == pdwState.PDW_INT_STATE_ACT_AUTO:
                                idx2 = i
                                break
                        # check for reset condition
                        condition_met = False
                        if (
                            self.pdw_auto_reset.iat[idx2 - delay] == reset_condition.RESET_CONDITION_TRUE
                            or self.pdw_auto_reset.iat[idx2] == reset_condition.RESET_CONDITION_TRUE
                        ) and self.pdw_state.iat[idx1 + delay] == pdwState.PDW_INT_STATE_DEACT_BTN:
                            condition_met = True
                            if self.pdw_auto_reset.iat[idx2 - delay] == reset_condition.RESET_CONDITION_TRUE:
                                signal_summary["AP.drvWarnDebugPort.autoActivResetCond"] = (
                                    f"Reset conditions for auto activarion have met with value = {int(self.pdw_auto_reset.iat[idx2 - delay])}"
                                )
                            elif self.pdw_auto_reset.iat[idx2] == reset_condition.RESET_CONDITION_TRUE:
                                signal_summary["AP.drvWarnDebugPort.autoActivResetCond"] = (
                                    f"Reset conditions for auto activarion have met with value = {int(self.pdw_auto_reset.iat[idx2])}"
                                )
                            else:
                                condition_met = False
                            reset_condition_epb = go.Scatter(
                                x=self.ap_time,
                                y=self.epb_switch,
                                mode="lines",
                                name=signals_obj._properties[Signals.Columns.EPB_SWITCH],
                            )
                            reset_condition_trace = go.Scatter(
                                x=self.ap_time,
                                y=self.pdw_auto_reset,
                                mode="lines",
                                name=signals_obj._properties[Signals.Columns.PDW_AUTO_RESET],
                            )
                        else:
                            signal_summary["AP.drvWarnDebugPort.autoActivResetCond"] = (
                                f" NO Reset conditions for auto activarion with value = {int(self.pdw_auto_reset.iat[idx2 - delay])}"
                            )
                        for i in range(
                            len(obj_dist_list)
                        ):  # check if an obstacle is present in any of the sectors around the car
                            if (
                                obj_dist_list[i].iat[idx2] < pdw_sector_length.PDW_SECTOR_LENGTH
                                and self.pdw_state.iat[idx2] == pdwState.PDW_INT_STATE_ACT_AUTO
                                and self.pdw_sys_state.iat[idx2] == pdwSysState.PDW_AUTOMATICALLY_ACTIVATED
                            ):
                                activate_method3 = "automatically activated"
                                pdwState_No_value3 = pdwState.PDW_INT_STATE_ACT_AUTO
                                pdwState_value3 = "PDW_INT_STATE_ACT_AUTO"
                                pdwSysState_No_value3 = pdwSysState.PDW_AUTOMATICALLY_ACTIVATED
                                pdwSysState_value3 = "PDW_AUTOMATICALLY_ACTIVATED"
                                pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                                pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                                verdict = TRUE
                                signal_summary[f"{obj_dist_signal_list[i]}"] = (
                                    f"Smallest distance detected towards object = {round(obj_dist_list[i].iat[idx2], 3)} m"
                                )
                                auto_obj_dist = go.Scatter(
                                    x=self.ap_time, y=obj_dist_list[i], mode="lines", name=obj_dist_signal_list[i]
                                )
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_ACT_R_GEAR:
                        for i, element in enumerate(self.gear_state):
                            if (
                                i > (idx1 + delay)
                                and element == gearPosition.GEAR_R
                                and int(self.epb_switch.iat[i] != epbSwitch.EPB_ON)
                            ):
                                idx2 = i
                                break
                        if (
                            self.gear_state.iat[idx2] == gearPosition.GEAR_R
                            and self.pdw_state.iat[idx2 + delay] == pdwState.PDW_INT_STATE_ACT_R_GEAR
                            and self.pdw_sys_state.iat[idx2 + delay] == pdwSysState.PDW_ACTIVATED_BY_R_GEAR
                        ):  # validate if Reverse gear was engaged
                            activate_method3 = "activated by reverse gear"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_ACT_R_GEAR
                            pdwState_value3 = "PDW_INT_STATE_ACT_R_GEAR"
                            pdwSysState_No_value3 = pdwSysState.PDW_ACTIVATED_BY_R_GEAR
                            pdwSysState_value3 = "PDW_ACTIVATED_BY_R_GEAR"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["AP.odoInputPort.odoSigFcanPort.gearboxCtrlStatus.gearCur_n"] = (
                                f"Gear in reverse with value GEAR_R = {int(self.gear_state.iat[idx2])}"
                            )
                            rg_trace = go.Scatter(
                                x=self.ap_time,
                                y=self.gear_state,
                    ...
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_ACT_AP:
                        for i, element in enumerate(self.apStatus):
                            if i > (idx1 + delay) and element == apStateVal.AP_AVG_ACTIVE_IN:
                                idx2 = i
                                break
                        if (
                            self.pdw_state.iat[idx2] == pdwState.PDW_INT_STATE_ACT_AP
                            and self.apStatus.iat[idx2] == apStateVal.AP_AVG_ACTIVE_IN
                            and self.pdw_sys_state.iat[idx2] == pdwSysState.PDW_ACTIVATED_BY_BUTTON
                            and self.hmi_interract.iat[idx2] == pdw_hmi_interact.TAP_ON_START_PARKING
                        ):  # validate if automated parking button was pressed
                            activate_method3 = "activated by automated parking"
                            pdwState_value3 = "PDW_INT_STATE_ACT_AP"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_ACT_AP
                            pdwSysState_No_value3 = pdwSysState.PDW_ACTIVATED_BY_BUTTON
                            pdwSysState_value3 = "PDW_ACTIVATED_BY_BUTTON"
                            pdw_state_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "Third trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["AP.hmiOutputPort.userActionHeadUnit_nu"] = (
                                f"Automated parking started with value TAP_ON_SART_PARKING = {int(self.hmi_interract.iat[idx2])}"
                            )
                            signal_summary["AP.planningCtrlPort.apStates"] = (
                                f"Automated parking stade apStates = {int(self.apStatus.iat[idx2])} (AP_AVG_ACTIVE_IN)"
                            )
                            apState_figure = go.Scatter(
                                x=self.ap_time,
                                y=self.apStatus,
                    ...
                    elif int(self.pdw_state.iat[idx2]) == pdwState.PDW_INT_STATE_ACT_ROLLBACK:
                        for i, element in enumerate(self.pdw_state):
                            if i > (idx1 + delay) and element == pdwState.PDW_INT_STATE_ACT_ROLLBACK:
                                idx2 = i
                                break
                        # check for reset condition
                        if (
                        ...
                        if (
                            self.dt_req.iat[idx2] == drvTubeDisplay.PDC_DRV_TUBE_REAR
                            and self.pdw_state.iat[idx2] == pdwState.PDW_INT_STATE_ACT_ROLLBACK
                            and self.pdw_sys_state.iat[idx2] == pdwSysState.PDW_AUTOMATICALLY_ACTIVATED
                            and self.velocity.iat[idx2] >= speed_threshold.SPEED_THRESHOLD_LO
                            and self.wheelfl.iat[idx2]
                            == self.wheelfr.iat[idx2]
                            == self.wheelrl.iat[idx2]
                            == self.wheelrr.iat[idx2]
                            == wheel_direction.WHEEL_DIRECTION_REVERSE
                        ):  # validate if vehicle rolls backward with driving tube display in rear
                            activate_method3 = "activated by rollback"
                            pdwState_No_value3 = pdwState.PDW_INT_STATE_ACT_ROLLBACK
                            pdwState_value3 = "PDW_INT_STATE_ACT_ROLLBACK"
                            pdwSysState_No_value3 = pdwSysState.PDW_AUTOMATICALLY_ACTIVATED
                            pdwSysState_value3 = "PDW_AUTOMATICALLY_ACTIVATED"
                            pdw_state_trigger3 = "First trigger for signal: AP.drvWarnStatus.pdwState_nu"
                            pdw_SysState_trigger3 = "First trigger for signal: AP.drvWarnStatus.pdwSystemState_nu"
                            verdict = TRUE
                            signal_summary["Car.v"] = (
                                f"Ego vehicle speed while rolling backward {round(self.velocity.iat[idx2], 3)} m/s"
                            )
                            signal_summary["AP.pdcpDrivingTubePort.drvTubeDisplay_nu"] = (
                                f"Driving tube display PDC_DRV_TUBE_REAR = {int(self.dt_req.iat[idx2])}"
                            )
        ...
        if deactivation_method1 == "deactivated by failure":
            fig.add_trace(fail_deact)
        if condition_met:
            fig.add_trace(reset_condition_epb)
            fig.add_trace(reset_condition_trace)
...
@testcase_definition(
    name="PDW State Machine Three",
    description=("PDW shall react to any state change if the correct conditions are met."),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class PDWStateMachineThree(TestCase):
