...
@teststep_definition(
    ...
    description=(
        "This test step evaluates if PDW displays the Driving tube in the rolling direction."
...
class PDWDrivingTubeReq(TestStep):
    ...
    def process(self, **kwargs):
            ...
            if not pdw_state_mask:
                ...
                if state_act_ap_mask1:
                    ...
                    if (
                        ...
                        signal_summary["AP.odoInputPort.odoSigFcanPort.gearboxCtrlStatus.gearCur_n"] = (
                            f"Gear in driving position with value GEAR_D = {int(self.gear_state.iat[idx])}"
                        )
                ...
                if act_validation and activate_metod == "activated by reverse gear":
                    dt_none = FALSE
                    idx = -1
                    for i, element in enumerate(self.dt_req):
                        if element == drvTubeDisplay.PDC_DRV_TUBE_FRONT:
                            idx = i
                            break
                    if (
                        self.wheelfl.iat[idx]
                        == self.wheelfr.iat[idx]
                        == self.wheelrl.iat[idx]
                        == self.wheelrr.iat[idx]
                        == wheel_direction.WHEEL_DIRECTION_FORWARD
                        and self.gear_state.iat[idx] == gearPosition.GEAR_R
                    ):
                        signal_summary["AP.pdcpDrivingTubePort.drvTubeDisplay_nu"] = (
                            f"Driving tube display PDC_DRV_TUBE_FRONT = {int(self.dt_req.iat[idx])} with gear position {int(self.gear_state.iat[idx])} and wheels rolling direction: front left = {int(self.wheelfl.iat[idx])} front right = {int(self.wheelfr.iat[idx])} rear left = {int(self.wheelrl.iat[idx])} rear right = {int(self.wheelrl.iat[idx])}"
                        )
                        signal_summary["Car.v"] = (
                            f"Ego vehicle speed while rolling forward {round(self.velocity.iat[idx], 3)} m/s"
                        )
                    ...
                    if (
                        ...
                        == self.wheelrr.iat[idx]
                        == wheel_direction.WHEEL_DIRECTION_REVERSE
                        and self.gear_state.iat[idx] != gearPosition.GEAR_P
                    ):
                        signal_summary["AP.pdcpDrivingTubePort.drvTubeDisplay_nu"] = (
                            f"Driving tube display PDC_DRV_TUBE_REAR = {int(self.dt_req.iat[idx])} with Gear position {int(self.gear_state.iat[idx])} and Wheels rolling direction: front left = {int(self.wheelfl.iat[idx])} front right = {int(self.wheelfr.iat[idx])} rear left = {int(self.wheelrl.iat[idx])} rear right = {int(self.wheelrl.iat[idx])}"
                        )
        ...
        remark = " ".join(
            "Gear position: None = 0; Driving = 11; Reverse = 13 | Wheel direction: None = 0; Front Direction = 1; Rear Direction = -1".split(),
        )
        ...
        if activate_metod == "automatically activated":
            fig.add_trace(auto_obj_dist)
        if activate_metod == "activated by button":
            fig.add_trace(button_press)
        if activate_metod == "activated by automated parking":
            fig.add_trace(apState_figure)
        if activate_metod == "activated by reverse gear":
...
@testcase_definition(
    name="Driving Tube Display moving direction",
    description=(
        "PDW function shall display the Driving Tube in the driving direction when the rolling direction is contrary to the sector lever position."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class PDWDrivingTubeDir(TestCase):
