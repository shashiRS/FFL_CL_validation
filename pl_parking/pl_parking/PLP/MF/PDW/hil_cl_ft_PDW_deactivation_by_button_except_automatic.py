...
SIGNAL_DATA = "PDWButtonDeactivation"
...
@teststep_definition(
    ...
    name="Step 1",
    description=(
        "Test is checking if PDW function deactivates after the PDW button is pressed when PDW is active for reasons other than automatic activation."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDWButtonDeactivation(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        # Evaluation loop
        for frame in range(len(time_signal)):
            if not start_waiting:
                if veh_ignition[frame] and pdw_state[frame] not in [
                    constants.HilCl.PDW.States.FAILURE,
                ]:
                    # Detect PDW button press
                    if pdw_button[frame] == constants.HilCl.PDW.Button.PDW_TAP_ON:
                        if not button_pressed:
                            # First press: Activate
                            if pdw_state[frame] not in [
                                constants.HilCl.PDW.States.OFF,
                                constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED,
                            ]:
                                button_pressed = True
                                start_waiting = True
                                ...
                                exit_reason = "PDW state was OFF or AUTOMATICALLY_ACTIVATED on activation attempt"
                                break
                        elif activation_detected:
                            # Second press: Deactivate
                            start_waiting = True  # Begin FTTI timing to check for OFF state
                else:
                    exit_reason = "Preconditions not met"
                    break
            else:
                # Handle FTTI time delay (600ms)
                FTTI_time += time_signal[frame] - time_signal[frame - 1]
                if FTTI_time >= 600:
                    start_waiting = False
                    FTTI_time = 0

                    # Check that after the delay, the state is OFF
                    if pdw_state[frame] == constants.HilCl.PDW.States.OFF:
                        eval_status_ok = True  # Successful deactivation and transition to OFF
                    else:
                        eval_status_ok = False
                        exit_reason = "PDW state did not switch to OFF after deactivation"
                    break
        ...
        elif not eval_status_ok:
            eval_cond = [False]
            evaluation1 = f"The evaluation of {signal_name['PDW_State']} is FAILED, {exit_reason}"
        else:
            eval_cond = [True]
        ...
        remarks.append(f"Exit reason: {exit_reason}")
...
@testcase_definition(
    name="PDWButtonDeactivation",
    description="Check if PDW state switches to OFF when the PDW button is pressed while PDW is active for reasons other than automatic activation",
)
