#!/usr/bin/env python3
"""This is a PDW TP rear test case."""
import logging
import os
import sys
    ...
    def __init__(self):
        ...
        front_sectors_critical = {
            self.Columns.SECTOR_CRITICALLY.format(
                x
            ): f".MF_PDWARNPROC_DATA.pdcpSectorsPort.sectorsRear[{x}].criticalityLevel_nu"
            for x in range(constants.PDWConstants.NUMBER_SECTORS)
        }
        front_sectors_id = {
            self.Columns.SECTOR_ID.format(x): f".MF_PDWARNPROC_DATA.pdcpSectorsPort.sectorsRear[{x}].sectorID_nu"
            for x in range(constants.PDWConstants.NUMBER_SECTORS)
        }
        front_sectors_distance = {
            self.Columns.SECTOR_DISTANCE.format(
                x
            ): f".MF_PDWARNPROC_DATA.pdcpSectorsPort.sectorsRear[{x}].smallestDistance_m"
            for x in range(constants.PDWConstants.NUMBER_SECTORS)
        }
...
@teststep_definition(
    step_number=1,
    name="Rear Sectors",
    description="Check TP of PDW component(rear sector).",
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if total_standstill_events == 0 or initial_detection_timestamp is None:
                ...
                eval_text = "No standstill events were detected, making it impossible to evaluate the PDW function; therefore, the verdict is FAILED."
                ...
                fig_pdw = go.Figure()
                # Plot all rear points first
                # fig_pdw.add_trace(go.Scatter(
                #                 x=x_coords_rear,  # X-coordinates for all points
                #                 y=y_coords_rear,  # Y-coordinates for all points
                #                 mode='lines',  # Display points as lines
                #                 name='All Rear Points',
                #                 marker=dict(color='blue', size=5),  # Marker properties
                #                 showlegend=False
                # ))
                for sector_id, initial_detection_timestamp in sector_dict.items():
                    initial_critical_level = df[Signals.Columns.SECTOR_CRITICALLY.format(sector_id - 8)].iat[
                        initial_detection_timestamp
                    ]
                    initial_distance = df[Signals.Columns.SECTOR_DISTANCE.format(sector_id - 8)].iat[
                        initial_detection_timestamp
                    ]
                    ...
                    if initial_critical_level != 0:
                        ...
                        # fig_pdw = plot_multiple_sectors(sector, initial_critical_level, sector_dict_rear[sector][initial_critical_level]["name"])
                    if measured_critical_level != 0:
                        ...
                        # fig_pdw = plot_multiple_sectors(sector, measured_critical_level,sector_dict_rear[sector][measured_critical_level]["name"])
                fig_pdw = plot_multiple_sectors(pdw_list)
                #  if initial_critical_level != 0:
                #         region_data_init = sector_dict_rear[sector][initial_critical_level]
                #         fig_pdw.add_trace(go.Scatter(
                #             x=region_data_init['x'],  # X-coordinates for the polygon
                #             y=region_data_init['y'],  # Y-coordinates for the polygon
                #             mode='lines',  # Draw lines to connect the points
                #             fill='toself',  # Fill the area enclosed by the points
                #             fillcolor=region_data_init['fill'],  # Color with transparency
                #             name=f"PDW critical level({initial_critical_level})  sector-{sector_id}",  # Use the provided legend message
                #             line=dict(color='black', width=1)  # Outline of the polygon
                #         ))
                #     if measured_critical_level != 0:
                #         region_data_meas = sector_dict_rear[sector][measured_critical_level]
                #         fig_pdw.add_trace(go.Scatter(
                #             x=region_data_meas['x'],  # X-coordinates for the polygon
                #             y=region_data_meas['y'],  # Y-coordinates for the polygon
                #             mode='lines',  # Draw lines to connect the points
                #             fill='toself',  # Fill the area enclosed by the points
                #             fillcolor=region_data_meas['fill'],  # Color with transparency
                #             name=f"Real critical level({measured_critical_level}) sector-{sector_id}",  # Use the provided legend message
                #             line=dict(color='black', width=1)  # Outline of the polygon
                #         ))
                # fig_pdw.layout = go.Layout(
                #         xaxis=dict(
                ...
                #             showticklabels=False
                #         ),
                #         title='Sectors and Critical Levels Visualization',
                #     )
                # fig_pdw.add_annotation(
                #         dict(
                #             font=dict(color="black", size=12),
                #             x=0,
                #             y=-0.12,
                #             showarrow=False,
                #             text="PDC detection sectors",
                #             textangle=0,
                #             xanchor="left",
                #             xref="paper",
                #             yref="paper",
            ...
            fig.layout = go.Layout(
                ...
                xaxis_title="Time [s]",
                title="Graphical Overview of PDW Rear",
            )
...
@testcase_definition(
    name="TP PDW Rear",
    description="PDW shall set the criticality level accordingly based on the distance between the car contour and the relevant obstacle. The reference criticality is to be measured when at standstill for rear sector.",
...
class FtPdwTPRear(TestCase):
    """Test case for PDW TP rear."""
...
# Rear coordinates provided earlier
x_coords_front = [
    -79,
    -261,
    -478,
