...
SIGNAL_DATA = "PDW_PARKING_TO_NEUTRAL"
...
@teststep_definition(
    ...
    description=(
        "This step is checking if PDW remains in the same state after switching from PARKING to NEUTRAL gearb"
        "if 'Deactivation by P-Gear feature is True. "
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_state_parking_to_neutral_gear(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            "after switching from PARKING to NEUTRAL gear".split()
        )
        ...
        if trigger1 is False:
            ...
            preconditions = f"Check trigger2! 'Deactivation by P-Gear ' feature not activated ({signal_name['Deactv_by_Pgear']} - 1."
        elif trigger3 is False or trigger3 is None:
            preconditions = f"Check trigger3! Gear not switched to PARKING ({signal_name['Gear_auto']}-{constants.HilCl.PDW.Gear.Automatic.PARK} or {signal_name['Gear_man']}-{constants.HilCl.PDW.Gear.Manual.PARK}).."
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_state_parking_to_neutral_gear,
        ]
