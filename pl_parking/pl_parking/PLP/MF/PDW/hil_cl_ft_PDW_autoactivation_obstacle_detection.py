...
SIGNAL_DATA = "PDW_AUTOACTIVATION_OBSTACLE_DETECTION"
...
@teststep_definition(
    step_number=1,
    name="PDW autoactivation - front side detection",
    description=(
        "This step is checking if PDW switches to 'automatically activated' state"
        " after there is an obstacle detection in the front side of the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_autoactivation_front_side_detection(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, PDW State ({signal_name['PDW_State']}) switched to 'autoactivation' state ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
            f" within {constants.HilCl.PDW.FTTI.SYSTEM}us after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) "
            " meaning at least one USS sensor out of twelve detected an object "
            f" within PDW distance range: {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}m).".split()
        )
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            # Check if PDW switched to 'automatically activated' based on the front sector group detection
                            if (
                                pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED
                                and (
                                    front1[frame] != 0 or front2[frame] != 0 or front3[frame] != 0 or front4[frame] != 0
                                )
                                and left1[frame] == 0
                                and left2[frame] == 0
                                and left3[frame] == 0
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation of {signal_name['PDW_State']} is FAILED, PDW State did not switch to 'automatically activation' state ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
                f" within {constants.HilCl.PDW.FTTI.SYSTEM}us after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) "
                " meaning at least one USS sensor out of twelve detected an object "
                f" within PDW distance range: {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}m).".split()
            )
...
@teststep_definition(
    step_number=2,
    name="PDW autoactivation - left side detection",
    description=(
        "This step is checking if PDW switches to 'automatically activated' state"
        " after there is an obstacle detection in the left side of the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_autoactivation_left_side_detection(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, PDW State ({signal_name['PDW_State']}) switched to 'automatically activated' state ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
            f" within {constants.HilCl.PDW.FTTI.SYSTEM}us after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) "
            " meaning at least one USS sensor out of twelve detected an object "
            f" within PDW distance range: {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}m).".split()
        )
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            # Check if PDW switched to 'automatically activated' based on the left sector group detection
                            if (
                                pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED
                                and (left1[frame] != 0 or left2[frame] != 0 or left3[frame] != 0 or left4[frame] != 0)
                                and front1[frame] == 0
                                and front2[frame] == 0
                                and front3[frame] == 0
                                and front4[frame] == 0
...
@teststep_definition(
    step_number=3,
    name="PDW autoactivation - rear side detection",
    description=(
        "This step is checking if PDW switches to 'automatically activated' state"
        " after there is an obstacle detection in the rear side of the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_autoactivation_rear_side_detection(TestStep):
    """Example test step"""
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            # Check if PDW switched to 'automatically activated' based on the rear sector group detection
                            if (
                                pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED
                                and (rear1[frame] != 0 or rear2[frame] != 0 or rear3[frame] != 0 or rear4[frame] != 0)
                                and left1[frame] == 0
                                and left2[frame] == 0
                                and left3[frame] == 0
                                and left4[frame] == 0
...
@teststep_definition(
    step_number=4,
    name="PDW autoactivation - right side detection",
    description=(
        "This step is checking if PDW switches to 'automatically activated' state"
        " after there is an obstacle detection in the right side of the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_autoactivation_right_side_detection(TestStep):
    """Example test step"""
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            # Check if PDW switched to 'automatically activated' based on the right sector group detection
                            if (
                                pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED
                                and (
                                    right1[frame] != 0 or right2[frame] != 0 or right3[frame] != 0 or right4[frame] != 0
                                )
                                and left1[frame] == 0
                                and left2[frame] == 0
                                and left3[frame] == 0
...
@testcase_definition(
    name="PDW function autoactivation.",
    description="Check if PDW state is switching to an active state if there are object detections within PDW distance range.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            Pdw_autoactivation_front_side_detection,
            Pdw_autoactivation_left_side_detection,
            Pdw_autoactivation_rear_side_detection,
            Pdw_autoactivation_right_side_detection,
        ]
