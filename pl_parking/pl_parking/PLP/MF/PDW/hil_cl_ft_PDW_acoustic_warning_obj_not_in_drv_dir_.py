...
SIGNAL_DATA = "PDW_AUTOACTIVATION_OBSTACLE_NOT_IN_DRV_DIR"
...
@teststep_definition(
    step_number=1,
    name="PDW front weakened acoustic warning",
    description=(
        "This step is checking if PDW is warning the driver acoustically using a weak sound for the front object detection within PDW range"
        " which is not in the driving direction of the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FMD_IBta0NIAEe2iKqc0KPO99Q&artifactInModule=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_z4l_QkZlEe68PtCWeWkWbA&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_lWHOEPvsEeqIqKySVwTVNQ%2Fcomponents%2F_u4eQYMlKEe2iKqc0KPO99Q&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099",
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_acoustic_warning_front_obj_not_in_driv_dir(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation of {signal_name['Front_speaker_volume']} is PASSED, the warning for the front obstacle got weakened (value less than 5)"
            f" after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
            f" and vehicle was moving backwards ({signal_name['Wheel_direction_FL']} is {constants.HilCl.PDW.Veh_Motion.Wheel_dir.REVERSE})".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            if not start_waiting:
                # Check if our function is not in failure state and standstill autoactivation feature is activated
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE and pdw_standstill_autoact[frame]:
                    trigger1 = True
                    # Check if there are relevant objects around the vehicle (in PDW distance range).
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Vehicle standstill and TP in continuous tone range
                        if (
                            ...
                            and (
                                (
                                    uss1[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                    and uss1[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                )
                                or (
                                    uss2[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                    and uss2[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                )
                                or (
                                    uss3[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                    and uss3[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                )
                                or (
                                    uss4[frame] > constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION
                                    and uss4[frame] < constants.HilCl.PDW.Thresholds.CONT_TONE_RANGE
                                )
                            )
                        ):
                            if (
                                not FTTI_time and trigger3 is None
                            ):  # Wait FTTI time for functionality to change its own state as it should
                                start_waiting = 1
                            else:
                                # Check if PDW switched to 'automatically activated' and PDW is warning the driver about the front and rear obstacles normally before witching to REVERSE
                                if (
                                    pdw_state[frame] == constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED
                                    and front_volume[frame] >= 5
                        ...
                        if (
                            ...
                            if not FTTI_time:  # Wait FTTI time for functionality to change its own state as it should
                                ...
                                # Check if PDW is warning the driver about the front obstacle via a weak sound
                                if front_volume[frame] > 0 and front_volume[frame] < 5 and rear_volume[frame] == 0:
                                    eval_status_ok = True
                                    FTTI_time = 0
                                    break
                                else:
        ...
        if trigger1 is False:
            preconditions = (
                f"Check trigger1! PDW state set to Failure ({constants.HilCl.PDW.States.FAILURE})"
                f" or 'standstill autoactivation' feature not activated {signal_name['PDW_standstill_autoactivation']} not True."
            )
            ...
            preconditions = (
                f"Check trigger3! Acoustic warnings not correct ({signal_name['Front_speaker_volume']} not greater equal than 5 or"
                f" {signal_name['Rear_speaker_volume']}) not 0."
            )
        elif trigger4 is False or trigger4 is None:
            preconditions = f"Check trigger4! Vehicle not moving backwards ({signal_name['Wheel_direction_FL']} not  {constants.HilCl.PDW.Veh_Motion.Wheel_dir.REVERSE})"
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation of {signal_name['Front_speaker_volume']} is FAILED, the warning for the front obstacle didn't get weakened (value still greater equal than 5)"
                f" after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
                f" and vehicle was moving backwards ({signal_name['Wheel_direction_FL']} is {constants.HilCl.PDW.Veh_Motion.Wheel_dir.REVERSE})".split()
            )
        else:
...
@teststep_definition(
    step_number=2,
    name="PDW rear weakened acoustic warning",
    description=(
        "This step is checking if PDW is warning the driver acoustically using a weak sound for the rear object detection within PDW range"
        " which is not in the driving direction of the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FMD_IBta0NIAEe2iKqc0KPO99Q&artifactInModule=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_z4l_QkZlEe68PtCWeWkWbA&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_lWHOEPvsEeqIqKySVwTVNQ%2Fcomponents%2F_u4eQYMlKEe2iKqc0KPO99Q&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099",
...
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_acoustic_warning_rear_obj_not_in_driv_dir(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation of {signal_name['Rear_speaker_volume']} is PASSED, the warning for the rear obstacle got weakened (value less than 5)"
            f" after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
            f" and vehicle was moving forward ({signal_name['Wheel_direction_FL']} is {constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD})".split()
        )
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            if not FTTI_time:  # Wait FTTI time for functionality to change its own state as it should
                                ...
                                # Check if PDW is warning the driver about the front obstacle via a weak sound
                                if rear_volume[frame] > 0 and rear_volume[frame] < 5 and front_volume[frame] == 0:
                                    eval_status_ok = True
                                    FTTI_time = 0
                                    break
                                else:
        ...
        if trigger1 is False:
            ...
            preconditions = (
                f"Check trigger3! Acoustic warnings not correct ({signal_name['Front_speaker_volume']} not 0 or"
                f" {signal_name['Rear_speaker_volume']}) not greater equal than 5."
            )
        elif trigger4 is False or trigger4 is None:
            preconditions = f"Check trigger4! Vehicle not moving forward ({signal_name['Wheel_direction_FL']} not  {constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD})"
        else:
            preconditions = "Preconditions were fulfilled. Check test result!"
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation of {signal_name['Rear_speaker_volume']} is FAILED, the warning for the rear obstacle didn't get weakened (value still greater equal than 5)"
                f" after autoactivation condition was fulfilled ({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})"
                f" and vehicle was moving forward ({signal_name['Wheel_direction_FL']} is {constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD})".split()
            )
        else:
...
@testcase_definition(
    name="PDW acoustic warning.",
    description="Check if PDW acoustic warning gets weakened for objects not placed in the driving direction.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_acoustic_warning_front_obj_not_in_driv_dir,
            PDW_acoustic_warning_rear_obj_not_in_driv_dir,
        ]
