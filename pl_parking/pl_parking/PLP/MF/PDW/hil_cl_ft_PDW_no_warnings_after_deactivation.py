...
SIGNAL_DATA = "PDW_NO_WARNINGS_AFTER_DEACTIVATION"
...
@teststep_definition(
    step_number=1,
    name="PDW deactivation - no warnings sent",
    description=(
        "This step is checking if PDW switches to 'OFF' state"
        " and if no warnings are sent afterwards even if there are reasons for function autoactivation."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_deactivation_no_warnings_sent(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        # Acoustic warnings
        front_volume = read_data["Front_speaker_volume"]
        rear_volume = read_data["Rear_speaker_volume"]
        front_pitch = read_data["Front_speaker_pitch"]
        rear_pitch = read_data["Rear_speaker_pitch"]
        ...
        evaluation1 = " ".join(
            f"The evaluation is PASSED, PDW State {signal_name['PDW_State']} switched to 'OFF' state ({constants.HilCl.PDW.States.OFF})"
            f" within {constants.HilCl.PDW.FTTI.SYSTEM}us and acoustic and visual warnings related to the front side object detection weren't present. "
            " Throughout the entire evaluation there were reasons for activating the function"
            f"(front object detection within PDW distance range: {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}m).".split()
        )
            ...
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Find the moment when PDW is activated by reverse gear
                        if (
                            ...
                            if (
                                ...
                                # Check if PDW switched to 'activated by R-Gear'. Acoustic and visual output based only on front side object detection
                                if (
                                    ...
                                    trigger4 = False
                            if pdw_button[frame] == constants.HilCl.PDW.Button.PDW_TAP_ON and trigger4:
                                trigger5 = True
                                if (
                                    ...
                                    # Check if PDW switched to 'OFF' after deactivation by button
                                    if (
                                        pdw_state[frame] == constants.HilCl.PDW.States.OFF
                                        and front1[frame] == 0
                                        or front2[frame] == 0
                                        or front3[frame] == 0
                                        or front4[frame] == 0
        ...
        if trigger1 is False:
            ...
            preconditions = f"Check trigger3! Gear transmission not switched to reverse ({signal_name['Gear_auto']} != {constants.HilCl.PDW.Gear.Automatic.REVERSE} or {signal_name['Gear_man']} != {constants.HilCl.PDW.Gear.Manual.REVERSE}) ."
        elif trigger4 is False:
            preconditions = (
                f"Check trigger4! 1){signal_name['PDW_State']} did not switch to 'activated by R-Gear'({constants.HilCl.PDW.States.ACTIVATED_BY_R_GEAR}) "
                f"or 2) {signal_name['Front_speaker_volume']} or {signal_name['Front_speaker_pitch']} not greater than 0 "
                f"or 3){signal_name['Rear_speaker_volume']} or {signal_name['Rear_speaker_pitch']} not equal to 0."
            )
        elif trigger5 is False:
            preconditions = f"Check trigger5! PDW button({signal_name['PDW_Button']}) not pressed for deactivation (value {constants.HilCl.PDW.Button.PDW_TAP_ON})."
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation is FAILED, PDW State {signal_name['PDW_State']} did not switch to 'OFF' state ({constants.HilCl.PDW.States.OFF})"
                f" within {constants.HilCl.PDW.FTTI.SYSTEM}us or acoustic or visual warnings related to the front side object detection are still present. "
                " Throughout the entire evaluation there were reasons for activating the function"
                f"(front object detection within PDW distance range: {constants.HilCl.PDW.Thresholds.MIN_DISTANCE_DETECTION}m - {constants.HilCl.PDW.Thresholds.MAX_DISTANCE_DETECTION}m).".split()
            )
        else:
...
@testcase_definition(
    name="PDW function deactivation.",
    description="Check if PDW is behaving accordingly in terms of visual and acoustic warnings after the function was deactivated by button.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            Pdw_deactivation_no_warnings_sent,
        ]
