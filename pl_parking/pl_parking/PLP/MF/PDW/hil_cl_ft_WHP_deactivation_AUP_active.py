...
SIGNAL_DATA = "WHP_DEACTIVATION_BY_AUP"
...
@teststep_definition(
    step_number=1,
    name="WHP state deactivation by AUP",
    description=(
        "This step is checking if WHP is switching from ACTIVE to INACTIVE state after AUP function was activated."
    ),
    expected_result=BooleanResult(TRUE),
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_j0PGntIAEe2iKqc0KPO99Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_lWHOEPvsEeqIqKySVwTVNQ%2Fcomponents%2F_u4eQYMlKEe2iKqc0KPO99Q&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099",
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class WHP_deactivation_AUP_active(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" after AUP was activated ({signal_name['AUP_State']} switched to ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}).".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            if not start_waiting:
                ...
                if whp_state[frame] != constants.HilCl.WHP.State.FAILURE:
                    trigger1 = True
                    # Check WHP was activated before AUP gets activated
                    if (
                        ap_state[frame] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING
                        and trigger3 is None
                    ):
                        # Check if WHP button was pressed for function activation
                        if pdw_button[frame] == constants.HilCl.PDW.Button.WHP_TAP_ON:
                            trigger2 = True
                    ...
                    # Check if AUP gets activated
                        ...
                        if not FTTI_time:  # Start waiting for FTTI time
                            ...
                            # Check if WHP is INACTIVE after AUP was activated
        ...
        if trigger1 is False:
            ...
            preconditions = f" Check trigger2! WHP button ({signal_name['PDW_Button']}) not pressed for function activation ({constants.HilCl.PDW.Button.WHP_TAP_ON})."
        elif trigger3 is False:
            preconditions = (
                f" Check trigger3! WHP state ({signal_name['WHP_State']}) different than ACTIVE-{constants.HilCl.WHP.State.ACTIVE} after WHP button pressed."
                f" ({signal_name['PDW_Button']} - {constants.HilCl.PDW.Button.WHP_TAP_ON})."
            )
        elif trigger4 is False or trigger4 is None:
            f"Check trigger3! AUP did not get activated. ({signal_name['AUP_State']} not switched to ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING})."
        else:
...
@testcase_definition(
    name="WHP deactivation",
    description="Check if WHP(wheel protection)function gets deactivated.",
)
