...
SIGNAL_DATA = "WHP_DEFAULT_ENABLED"
...
@teststep_definition(
    step_number=1,
    name="WHP default state",
    description=("This step is checking if WHP is switching to INACTIVE state after the system is turned ON."),
    expected_result=BooleanResult(TRUE),
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_j0PGl9IAEe2iKqc0KPO99Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_lWHOEPvsEeqIqKySVwTVNQ%2Fcomponents%2F_u4eQYMlKEe2iKqc0KPO99Q&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099",
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class WHP_default_enabled_after_ignition(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" after the system was restarted ({signal_name['Veh_ignition']} - True (1)"
            f" and a certain time has passed ({constants.HilCl.PDW.FTTI.SYSTEM_INIT} us)".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            if not start_waiting:
                # Check if IG is ON
                if veh_ignition[frame] and trigger3 is None:
                    trigger1 = True
                    if not FTTI_time and trigger3 is None:  # Start waiting for FTTI time
                        start_waiting = 1
                        trigger1 = False
                    else:
                        ...
                        if whp_state[frame] != constants.HilCl.WHP.State.FAILURE:
                            ...
                            trigger2 = False
                            break
                        # CHeck WHP state to be INACTIVE before restarting the system
                        if whp_state[frame] != constants.HilCl.WHP.State.INACTIVE:
                            ...
                            trigger3 = True
                            FTTI_time = 0
                # Check if IG was set to OFF
                elif not veh_ignition[frame] and trigger3 and trigger4 is None:
                    trigger4 = True
                    # Check if IG was set to ON again
                elif veh_ignition[frame] and trigger4:
                    trigger5 = True
                    if not FTTI_time and eval_status_ok is None:  # Start waiting for FTTI time
                        ...
                        if whp_state[frame] != constants.HilCl.WHP.State.FAILURE:
                            ...
                            trigger2 = False
                            break
                        # Check if WHP is INACTIVE after system initialization time has passed
                        if whp_state[frame] != constants.HilCl.WHP.State.INACTIVE:
                ...
                if FTTI_time >= constants.HilCl.PDW.FTTI.SYSTEM_INIT:  # 1.5s - system init time
                    start_waiting = 0
        """Check if preconditions were ok"""
        if trigger1 is False or trigger1 is None:
            preconditions = (
                f" Check trigger1! Igniton of the system is not ON ({signal_name['Veh_ignition']} not True)."
            )
        if trigger2 is False:
            preconditions = f" Check trigger2! WHP state({signal_name['WHP_State']}) set to Failure ({constants.HilCl.WHP.State.FAILURE})."
        elif trigger3 is False:
            preconditions = f" Check trigger3! WHP ({signal_name['WHP_State']}) state different than INACTIVE-{constants.HilCl.WHP.State.INACTIVE} before system was restarted."
        elif trigger4 is None:
            preconditions = (
                f" Check trigger4! System was not turned OFF ({signal_name['Veh_ignition']} not set False)! "
            )
        elif trigger5 is None:
            preconditions = (
                f" Check trigger5! System was not restarted ({signal_name['Veh_ignition']} not set True) again! "
            )
        else:
        ...
        signal_summary["WHP state -activation if a certain condition was fulfilled."] = evaluation1
...
@testcase_definition(
    name="WHP state at ignition",
    description="Check if WHP(wheel protection)function is enabled.",
)
class WhpHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            WHP_default_enabled_after_ignition,
        ]
