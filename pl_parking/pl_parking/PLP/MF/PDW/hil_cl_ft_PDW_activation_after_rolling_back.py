...
SIGNAL_DATA = "PDW_REACTIVATION_AFTER_ROLLING_BACK"
...
@teststep_definition(
    step_number=1,
    name="PDW state transition to active state",
    description=("This step checks whether PDW state switches to an active state after vehicle is rolling backwards."),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_reactivation_after_rolling_back(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" PDW state switched to 'automatically activated' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) state after vehicle started rolling backwards"
            f" in NEUTRAL gear on a slope profile road(hill).".split()
        )
            ...
            if not start_waiting:
                ...
                if veh_ignition[frame] and pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Find the moment when vehicle is standstill
                        if (
                            veh_velocity[frame] < abs(constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS)
                            and fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INVALID
                            and rr_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INVALID
                        ):
                            trigger3 = True
                            # Find the moment when vehicle is stopped in neutral mode w/o EPB engaged
                            if (
                                (
                                ...
                                trigger4 = True
                        # After ego is driving backwards check if PDW state is set to "automatically activated" again
                        elif (
        ...
        if trigger1 is False:
            ...
            preconditions = "Check trigger2! No object detected!"
        elif trigger3 is None:
            preconditions = "Check trigger3! Vehicle not in standstill!"
        elif trigger4 is None:
            preconditions = f"Check trigger4! Gear not switched to NEUTRAL (automatic-{constants.HilCl.PDW.Gear.Automatic.NEUTRAL} , manual-{constants.HilCl.PDW.Gear.Manual.NEUTRAL}) or EPB(electronic park brake) was engaged!"
        elif trigger5 is False:
            preconditions = "Check trigger5! None of the preconditions were fulfilled, input (CM scenario) not correct!"
        else:
        ...
        if eval_status_ok is None:
            ...
            evaluation1 = " ".join(
                ...
                f" PDW state did not switch to 'automatically activated' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) state after vehicle started rolling backwards"
                f" in NEUTRAL gear on a slope profile road(hill).".split()
            )
        else:
            eval_cond = [True] * 1
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_reactivation_after_rolling_back,
        ]
