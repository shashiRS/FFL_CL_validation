...
class ValidationSignals(MDFSignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.PDW_DEACTV_BY_EPB: "MTS.ADC5xx_Device.CFG_DATA.MF_DWF_APP_Parameter.PDW_L_DEACTIV_BY_P_GEAR_NU",
...
@teststep_definition(
    step_number=1,
    name="PDW - constant value state",
    description=(
        "This step is checking that PDW state remains OFF after Ignition was turned ON,"
        " even if there are reasons for function self activation(objects detected in PDW range). "
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_deactivation_standstill_autoactivation_off(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        standstill_autoactv = read_data["PDW_standstill_autoactivation"].tolist()
        ...
        evaluation1 = " ".join(
            f"The evaluation of {signal_name['PDW_State']} is PASSED, PDW State remained OFF after Ignition ON "
            " even if there was a reason for function autoactivation (object detected in PDW range)."
            "'Standstill autoactivation' feature is deactivated. ".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            ...
            if not start_waiting:
                ...
                if veh_ignition[frame] and pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    trigger1 = True
                    # Check if 'standstill autoactivation' feature is disabled
                    if not standstill_autoactv[frame]:
                        trigger2 = True
                        # Check if the vehicle is standstill
                        if veh_velocity[frame] < abs(constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS):
                            trigger3 = True
                            # Check if there are objects detected by USS in PDW distance range
                            if ObjectDetection(frame):
                                ...
                                if (
                                    ...
                                    # EPB not engaged and status is valid
        ...
        if trigger1 is None:
            ...
            preconditions = "Check trigger2! 'Standstill autoactivation' is enabled (True)."
        elif trigger3 is None:
            preconditions = "Check trigger3! Vehicle not in standstill."
        elif trigger4 is None:
            preconditions = "Check trigger4! No object detected within PDW distance range."
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation of {signal_name['PDW_State']} is FAILED, PDW State did not remained set to OFF "
                f" after Ignition ON with 'standstill autoactivation' feature deactivated, after preconditions were met.".split()
            )
        else:
            eval_cond = [True] * 1

        signal_summary["PDW state -'standstill autoactivation' inactive case"] = evaluation1
        ...
        fig.add_trace(
            go.Scatter(
                x=time_signal, y=standstill_autoactv, mode="lines", name=signal_name["PDW_standstill_autoactivation"]
            )
        )
...
@testcase_definition(
    name="PDW - 'standstill autoactivation' feature off.",
    description="Test is checking the transition of the PDW state after Ignition ON in standstill with a certain feature deactivated. ",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_deactivation_standstill_autoactivation_off,
        ]
