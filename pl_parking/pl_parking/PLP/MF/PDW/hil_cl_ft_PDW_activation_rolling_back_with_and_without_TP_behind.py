...
SIGNAL_DATA = "PDW_REACTIVATION_AFTER_ROLLING_BACK_WITH/WO_TP_BEHIND"
...
class ValidationSignals(MDFSignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.ROAD: "CM.Vhcl.sRoad",
...
@teststep_definition(
    step_number=1,
    name="PDW state transition - rollback with TP",
    description=(
        "This step checks whether PDW state switches to an active state after vehicle is rolling backwards with a traffic participant behind."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_reactivation_after_rolling_back_with_TP_behind(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        road = read_data["Car_road"].tolist()
        ...
        FTTI_time = 0
        start_waiting = 0
        current_road_value = 0

        evaluation1 = " ".join(
            f"The evaluation is PASSED, "
            f" PDW state {signal_name['PDW_State']} switched to 'automatically activated' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) state after vehicle started rolling backwards"
            f" in NEUTRAL gear on a slope profile road(hill) and after the distance covered by the vehicle was greater than {constants.HilCl.PDW.Distance_covered.ROLL_BACKWARDS_WITH_TP}m"
            "with a traffic participant in PDW distance range in the rear side".split()
        )
        ...
        for frame in range(1, len(time_signal)):
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        trigger2 = True
                        # Find the moment when vehicle gear is set to neutral
                        if (
                            ...
                            trigger3 = True
                            # Find the moment when driver brake pedal is pressed
                            if pedal_brake[frame] == constants.HilCl.PDW.Driver_braking.ACTIVE and trigger5 is not True:
                                trigger4 = True
                                # Check PDW state before rolling backwards
                                if pdw_state[frame] == constants.HilCl.PDW.States.OFF:
                                    trigger5 = True
                                    current_road_value = road[frame]
                                else:
                                    trigger5 = False
                            # Find the moment when driver brake pedal is released
                            if pedal_brake[frame] == constants.HilCl.PDW.Driver_braking.INACTIVE and trigger5:
                                trigger6 = True
                                # Check if vehicle covered the distance mentioned in the requirement
                                if (
                                    current_road_value - road[frame]
                                ) > constants.HilCl.PDW.Distance_covered.ROLL_BACKWARDS_WITH_TP:
                                    if (
        ...
        if trigger1 is False:
            preconditions = f"Check trigger1! PDW state set to Failure ({constants.HilCl.PDW.States.FAILURE}) or Ignition not ON (True)!"
        elif trigger2 is None:
            preconditions = "Check trigger2! There are objects present behind ego vehicle within PDW distance range!"
        elif trigger3 is None:
            preconditions = f"Check trigger3! Gear transmission not switched to Neutral ({signal_name['Gear_auto']} != {constants.HilCl.PDW.Gear.Automatic.NEUTRAL}) or {signal_name['Gear_manual']} != {constants.HilCl.PDW.Gear.Manual.NEUTRAL})."
        elif trigger4 is None:
            preconditions = f"Check trigger4! Driver braking pedal was not pressed({signal_name['Pedal_brake']} != {constants.HilCl.PDW.Driver_braking.ACTIVE})."
        elif trigger5 is False:
            preconditions = f"Check trigger5! PDW state not OFF before rolling backwards({signal_name['PDW_State']} != {constants.HilCl.PDW.States.OFF} )"
        elif trigger6 is None:
            preconditions = f"Check trigger6! Driver braking pedal was not released({signal_name['Pedal_brake']} != {constants.HilCl.PDW.Driver_braking.INACTIVE})"
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation of {signal_name['PDW_State']} is FAILED, "
                f" PDW state {signal_name['PDW_State']} did not switch to 'automatically activated' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) state after vehicle started rolling backwards"
                f" in NEUTRAL gear on a slope profile road(hill) and after the distance covered by the vehicle was greater than {constants.HilCl.PDW.Distance_covered.ROLL_BACKWARDS_WITH_TP}"
                "with a traffic participant in PDW distance range in the rear side".split()
            )
        else:
            eval_cond = [True] * 1

        signal_summary["PDW state -activation for rolling backwards case."] = evaluation1
        ...
        fig.add_trace(go.Scatter(x=time_signal, y=road, mode="lines", name=signal_name["Car_road"]))
...
@teststep_definition(
    step_number=2,
    name="PDW state transition - rollback wo TP",
    description=(
        "This step checks whether PDW state switches to an active state after vehicle is rolling backwards with no traffic participant behind."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class PDW_reactivation_after_rolling_back_wo_TP_behind(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" PDW state {signal_name['PDW_State']} switched to 'automatically activated' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) state after vehicle started rolling backwards"
            f" in NEUTRAL gear on a slope profile road(hill) and after the distance covered by the vehicle was greater than {constants.HilCl.PDW.Distance_covered.ROLL_BACKWARDS_WITH_TP}m "
            "with no traffic participant in PDW distance range in the rear side.".split()
        )
                    ...
                    if not ObjectDetection(frame):
                        ...
                        if (
                            ...
                            if pedal_brake[frame] == constants.HilCl.PDW.Driver_braking.INACTIVE and trigger5:
                                ...
                                if (
                                    current_road_value - road[frame]
                                ) > constants.HilCl.PDW.Distance_covered.ROLL_BACKWARDS_NO_TP:
                                    if (
                                        not FTTI_time and eval_status_ok is None
                                    ):  # Wait FTTI time for functionality to change its own state as it should
                                        start_waiting = 1
...
@testcase_definition(
    name="PDW-function reactivation after rolling back.",
    description="Check if PDW state switch to an active state after vehicle started rolling backwards.",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_reactivation_after_rolling_back_with_TP_behind,
            PDW_reactivation_after_rolling_back_wo_TP_behind,
        ]
