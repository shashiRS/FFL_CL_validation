...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        INTERSECTS_SECTORLE = "INTERSECTS_SECTORLE{}"
        INTERSECTS_SECTORLE0 = "INTERSECTS_SECTORLE0"
        INTERSECTS_SECTORLE1 = "INTERSECTS_SECTORLE1"
        INTERSECTS_SECTORLE2 = "INTERSECTS_SECTORLE2"
        INTERSECTS_SECTORLE3 = "INTERSECTS_SECTORLE3"
    ...
    def __init__(self):
        ...
        intersect_sectors_left = {
            self.Columns.INTERSECTS_SECTORLE.format(x): f"AP.pdcpSectorsPort.sectorsLeft_{x}.intersectsDrvTube_nu"
            for x in range(4)
        }
        ...
        self._properties.update(intersect_sectors_left)
...
@teststep_definition(
    step_number=1,
    name="Intersects Driving Tube Left",  # this would be shown as a test step name in html report
    description=(
        "This test step evaluates if left sectors are inside the driving tube."
...
class IntersectsDrvTubeLeft(TestStep):
    ...
    def process(self, **kwargs):
        ...
        self.intersect_sector_l0 = signals[Signals.Columns.INTERSECTS_SECTORLE0]
        self.intersect_sector_l1 = signals[Signals.Columns.INTERSECTS_SECTORLE1]
        self.intersect_sector_l2 = signals[Signals.Columns.INTERSECTS_SECTORLE2]
        self.intersect_sector_l3 = signals[Signals.Columns.INTERSECTS_SECTORLE3]
        ...
        intersect_sig_name = [
            signals_obj._properties[Signals.Columns.INTERSECTS_SECTORLE0],
            signals_obj._properties[Signals.Columns.INTERSECTS_SECTORLE1],
            signals_obj._properties[Signals.Columns.INTERSECTS_SECTORLE2],
            signals_obj._properties[Signals.Columns.INTERSECTS_SECTORLE3],
        ]
        dist_sign_name = [
        ...
        interact_left = [
            self.intersect_sector_l0,
            self.intersect_sector_l1,
            self.intersect_sector_l2,
            self.intersect_sector_l3,
        ]
        small_dist_le = [
        ...
        idx_drt = idx
        if act_rgear:
            # Check if Driving tube is in left direction
            ...
            if verdict and self.dt_req.iat[idx_drt] == drvTubeDisplay.PDC_DRV_TUBE_REAR:
                for i, element in enumerate(small_dist_le[3]):
                    if element == min(small_dist_le[3]):
                        idx_int = i
                        break
                    if interact_left[3].iat[idx_int] == dt_intersection.INTERSECTS:
                ...
                if False in verification:
                    for x in range(4):
                        signal_summary[intersect_sig_name[x]] = (
                            f"The Driving Tube does not intersects with the most critical slice of the sector with value = {int(interact_left[x].iat[idx_int])}(intersectsDrvTube_nu)"
                        )
                    verdict = False
                else:
                    verdict = True
                    for x in range(4):
                        signal_summary[intersect_sig_name[x]] = (
                            f"The Driving Tube intersects with the most critical slice of the sector with value = {int(interact_left[x].iat[idx_int])}(intersectsDrvTube_nu)"
                        )
        elif act_btn:
            for j, element in enumerate(self.dt_req):
            ...
            if verdict and self.dt_req.iat[idx_drt] == drvTubeDisplay.PDC_DRV_TUBE_FRONT:
                for k in range(3):
                    for i, element in enumerate(small_dist_le[k]):
                        if element == min(small_dist_le[k]):
                            idx_int = i
                            break
                    if interact_left[k].iat[idx_int] == dt_intersection.INTERSECTS:
                        verification.append(True)
                    else:
                        verification.append(False)
                    if False in verification:
                        signal_summary[intersect_sig_name[k]] = (
                            f"The Driving Tube does not intersects with the most critical slice of the sector with value = {int(interact_left[k].iat[idx_int])}(intersectsDrvTube_nu)"
                        )
                        verdict = False
                    else:
                        verdict = True
                        signal_summary[intersect_sig_name[k]] = (
                            f"The Driving Tube intersects with the most critical slice of the sector with value = {int(interact_left[k].iat[idx_int])}(intersectsDrvTube_nu)"
                        )
        if not act_rgear:
            if not act_btn:
        ...
        for i in range(4):
            fig.add_trace(
                go.Scatter(
                    ...
                    y=interact_left[i],
                    mode="lines",
                    name=signals_obj._properties[Signals.Columns.INTERSECTS_SECTORLE.format(i)],
                )
            )
        for i in range(4):
            fig.add_trace(
...
@testcase_definition(
    name="Left Sector intersects Driving Tube",
    description=(
...
class PDWDrvTubeIntersectLeft(TestCase):
