...
SIGNAL_DATA = "PDW_ACTIVATED_BY_BUTTON"
...
@teststep_definition(
    ...
    description=(
        "This step is checking if PDW switches to 'activated by button' state"
        " after there is at least one object detected around the ego vehicle within PDW distance range and AUP gets activated."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class Pdw_autoactivation_to_activated_by_button(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" from 'automatically activated' state({signal_name['PDW_State']} set to {constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})."
            f" after the following condition was fulfilled ({signal_name['AUP_State']}"
                    ...
                    if ObjectDetection(frame):
                        ...
                        if ap_state[frame] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING:
                            if (
                                ...
                                # Check if PDW switched to 'automatically activated' based on the object detection
                        ...
                        if (
                            ...
                            if not FTTI_time:  # Wait FTTI time for functionality to change its own state as it should
                                ...
                                # Check if PDW switched to 'activated by button' based on the AUP activation
        ...
        if trigger1 is False:
            ...
            preconditions = "Check trigger2! No objects detected within PDW distance range therefore no reason for activation, input (CM scenario) not correct!."
        elif trigger3 is False:
            preconditions = f"Check trigger3! PDW state ({signal_name['PDW_State']}) not switched to 'automatically activated'({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) within {constants.HilCl.PDW.FTTI.SYSTEM}ms before AUP activation ."
        elif trigger4 is None:
            preconditions = f"Check trigger4! AUP did not get activated. ({signal_name['AUP_State']} not switched to 'ACTIVE' ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING})."
        else:
...
@testcase_definition(
    name="PDW function 'activated by button'.",
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            Pdw_autoactivation_to_activated_by_button,
        ]
