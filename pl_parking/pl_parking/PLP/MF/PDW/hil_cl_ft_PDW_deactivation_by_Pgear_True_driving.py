...
SIGNAL_DATA = "PDW_DEACTIVATION_BY_PGEAR_DRV"
...
@teststep_definition(
    step_number=1,
    name="PDW transition - active to OFF while driving",
    description=(
...
class PDW_deactivation_by_Pgear_driving(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f"PDW State switched to OFF from Autoactivation after Parking gear was engaged ('Deactivation by Pgear' feature enabled)".split()
        )
        ...
        for frame in range(0, len(time_signal)):
            ...
            if not start_waiting:
                ...
                if pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    trigger1 = True
                    # Check 'deactivation by Pgear' parameter is True
                    if pdw_deactv_by_pgear[frame]:
                        trigger2 = True
                        # Check if there are relevant object around the vehicle (in PDW distance range).
                        if ObjectDetection(frame):
                            trigger3 = True
                            if (  # Check if the vehicle is driving within PDW speed range
                                ...
                                if (
                                    ...
                                    or rr_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD
                                ):
                                    trigger5 = True
                                    if not FTTI_time and trigger6 is None:
                                        start_waiting = 1
                                    else:
                                        if (
                                            ...
                                            trigger6 = True
                                            FTTI_time = 0
                            elif veh_velocity[frame] < abs(constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS):
                                if (
                                    # vehicle standstill
                                    (
                                        fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT
                                        or rr_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.INIT
                                    )
                                    and (
                                        # gear in NEUTRAL gear
        ...
        if trigger1 is False:
            ...
            preconditions = "Check trigger2! 'Deactivation by Pgear' not enabled."
        elif trigger3 is False:
            preconditions = "Check trigger3! No object detected by USS."
        elif trigger4 is False:
            preconditions = "Check trigger4! Vehicle not driving within PDW speed range."
        elif trigger5 is False:
            preconditions = "Check trigger5! Vehicle not moving forward."
        elif trigger6 is False:
            preconditions = "Check trigger6! PDW state not switched to 'Automatically activation' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED})."
        elif trigger7 is False:
            preconditions = "Check trigger7! None of the preconditions were fulfilled, input (CM scenario) not correct."
        else:
        ...
        if eval_status_ok is None:
            ...
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"The evaluation of {signal_name['PDW_State']} is FAILED, PDW State did not switched to OFF from 'automatic activation' state "
                f" after Parking gear was engaged ('Deactivation by Pgear' feature enabled), after preconditions were met.".split()
            )
        else:
            eval_cond = [True] * 1

        signal_summary["PDW state -'deactivation by Pgear' active case"] = evaluation1
        ...
        fig.add_trace(
            go.Scatter(x=time_signal, y=pdw_deactv_by_pgear, mode="lines", name=signal_name["Deactv_by_Pgear"])
        )
...
@testcase_definition(
    name="PDW - 'deactivation by_Pgear(Parking gear)' feature active while driving.",
    description="Test is checking the PDW transition from active state to OFF state when a certain feature is active while our vehicle is driving. ",
)
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_deactivation_by_Pgear_driving,
        ]
