...
SIGNAL_DATA = "PDW_REACTIVATION_BY_EPB_ON_OFF_DRIVING"
...
class PDW_reactivation_by_Pgear_driving(TestStep):
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" after reactivation condition was fulfilled (electronic parking brake ON/OFF).".split()
        )
                ...
                if veh_ignition[frame] and pdw_state[frame] != constants.HilCl.PDW.States.FAILURE:
                    ...
                    if ObjectDetection(frame):
                        ...
                        if (
                            ...
                            # Check PDW state to be set to "automatically activated" beforehand and button is not pressed
                            if pdw_button[frame] == constants.HilCl.PDW.Button.PDW_TAP_ON:
                                button_pressed = True
                            if not button_pressed:
                                if (
                                    not FTTI_time and trigger1 is None
                                    ...
                                    if pdw_state[frame] != constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED:
                                        ...
                                        trigger1 = True
                                        FTTI_time = 0
                                # Check if PDW button was pressed for function deactivation
                            else:
                                if (
                                    not FTTI_time and trigger2 is None
                                ):  # Wait FTTI time for functionality to change its own state as it should
                                    start_waiting = 1
                                else:
                                    if pdw_state[frame] != constants.HilCl.PDW.States.OFF:  # PDW OFF
                                        ...
                                        trigger2 = True
                                        FTTI_time = 0
                        # Find the moment when vehicle is standstill after it drove a while
                        elif (
                            veh_velocity[frame] < abs(constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS)
                            and trigger2
                        ):
                            # Find the moment when the EPB is engaged
                            if (
                                (
                                    # gear not in reverse gear
                                    gear_manual[frame] != constants.HilCl.PDW.Gear.Manual.REVERSE
                                    and gear_automatic[frame] != constants.HilCl.PDW.Gear.Automatic.REVERSE
                                )
                                and (
                                    # gear not in parking gear
                                    gear_manual[frame] != constants.HilCl.PDW.Gear.Manual.PARK
                                    and gear_automatic[frame] != constants.HilCl.PDW.Gear.Automatic.PARK
                                    # EPB not engaged
                                )
                                and (
                                    gear_manual[frame] != constants.HilCl.PDW.Gear.Manual.INVALID
                                    and gear_automatic[frame] != constants.HilCl.PDW.Gear.Automatic.INVALID
                                )
                                and park_brake[frame]
                            ):
                                trigger3 = True
                        # After ego is driving again check if PDW state is set to "automatically activated" again
                        elif (
                            veh_velocity[frame] > constants.HilCl.PDW.Thresholds.MIN_SPEED_THRESHOLD_MPS
                            and veh_velocity[frame] < constants.HilCl.PDW.Thresholds.MAX_SPEED_THRESHOLD_MPS
                            and fl_wheel_direction[frame] == constants.HilCl.PDW.Veh_Motion.Wheel_dir.FORWARD
        ...
        if trigger1 is False:
            preconditions = f"Check trigger1! PDW state not switched to 'Automatically activation' ({constants.HilCl.PDW.States.AUTOMATICALLY_ACTIVATED}) beforehand."
        elif trigger2 is False:
            preconditions = f"Check trigger2! PDW state not switched to 'Automatically activation' ({constants.HilCl.PDW.States.OFF})."
        elif trigger3 is None:
            preconditions = "Check trigger3! No electronic parking brake was engaged."
        elif trigger4 is False:
            preconditions = (
                "Check trigger4! None of the preconditions were fulfilled, input (CM scenario) not correct!."
            )
        else:
        ...
        if eval_status_ok is None:
            ...
            evaluation1 = " ".join(
                ...
                f" after the following reactivation condition was fulfilled (electronic parking brake ON/OFF).".split()
            )
        else:
            eval_cond = [True] * 1
...
@testcase_definition(
    name="PDW-reactivation by EPB(electronic parking brake) ON-OFF ,driving situation.",
...
class PdwHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PDW_reactivation_by_Pgear_driving,
        ]
