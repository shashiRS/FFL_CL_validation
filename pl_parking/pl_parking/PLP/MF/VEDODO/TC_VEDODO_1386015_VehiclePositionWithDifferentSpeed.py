...
READER_NAME = "VedodoVehiclePositionDuringDifferentSpeed"
...
@teststep_definition(
    step_number=1,
    name="Test step to monitor position (x,y) signals continuously during different speed conditions",
    description="Test step to monitor position (x,y) signals continuously during different speed conditions "
    "and there should not be any jumps w.r.t previous data",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoMonitorVehiclePositionInDifferentSpeed(TestStep):
    """Test case to monitor position (x,y) signals continuously during different speed conditions.

    Objective
    ---------

    Monitor position (x,y) signals continuously during different speed conditions.

    Detail
    ------

    Check and monitor position (x,y) signals continuously during different speed conditions.
    """
    ...
    def process(self, **kwargs):
        ...
        for vy, ex, t in zip(est_velocity_y, est_x, time):
            if t and t % 10 == 0:
                if not abs(sum_x - pre_x) or not abs(sum_y - pre_y):
        ...
        if flag:
            ...
            evaluation = (
                f"Evaluation for Vehicle position (x,y) with different speed conditions is FAILED and there"
                f" are huge jumps are noticed w.r.t previous position, "
        ...
        x_plot = plot_graphs(time, est_x, CarMakerUrl.odoEstX, ref_x, CarMakerUrl.odoCmRefX, "Time[s]", "Distance[m]")
        y_plot = plot_graphs(time, est_y, CarMakerUrl.odoEstY, ref_y, CarMakerUrl.odoCmRefY, "Time[s]", "Distance[m]")
        v_plot = plot_graphs(
            time,
            est_velocity_x,
            CarMakerUrl.estVelocityX,
            cm_ego_velocity,
            CarMakerUrl.velocityX,
            "Time[s]",
            "Velocity[mps]",
        )
...
@verifies("ReqID_1386015")
@testcase_definition(
    name="Monitor position (x,y) signals continuously during different speed conditions",
    description="Test step to monitor position (x,y) signals continuously during different speed conditions and there "
    "should not be any jumps w.r.t previous data",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fj"
    "azz.conti.de%2Frm4%2Fresources%2FBI_3z4nVDaaEe6mrdm2_agUYg&oslc_config.context=https%3A%2F%2Fjazz.conti."
...
class VedodoMonitorVehiclePositionInDifferentSpeedTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoMonitorVehiclePositionInDifferentSpeed,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoMonitorVehiclePositionInDifferentSpeedTestCase,
        temp_dir=temp_dir,
