...
READER_NAME = "VedodoRelativeLateralPositionError"
...
class VedodoRelativeLateralPositionError(TestStep):
    ...
    def process(self, **kwargs):
        ...
        if max_lat_error > max_expected_lat_relative_error:
            evaluation_lat = " ".join(
                f"Evaluation for Lateral position error during normal driving conditions is FAILED."
                f"The maximum estimated relative error is {round(max_lat_error, 3)}m and  threshold is "
                f"{max_expected_lat_relative_error}m. The max deviation error is above the expected threshold.".split()
            )
            test_result_list.append(False)
        else:
            evaluation_lat = " ".join(
                f"Evaluation for Lateral position error during normal driving conditions is PASSED."
                f"The maximum estimated relative error is {round(max_lat_error, 3)}m and  threshold is "
                f"{max_expected_lat_relative_error}m. The max deviation error is below the expected threshold.".split()
            )
            test_result_list.append(True)

        signal_summary[CarMakerUrl.odoEstY] = evaluation_lat
        ...
        if max_relative_lat_error > max_expected_lat_relative_error:
            evaluation_relative_lat = " ".join(
                f"Evaluation for Lateral position error during normal driving conditions is FAILED."
                f"The maximum estimated relative error is {round(max_relative_lat_error, 3)}m and  threshold is "
                f"{max_expected_lat_relative_error}m. The max deviation error is above the expected threshold.".split()
            )
            test_result_list.append(False)
        else:
...
@testcase_definition(
    ...
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI_x7Zm0gDCEe-JmbR7EsJ0UA&componentURI=https%3A%2F%2Fjazz.conti.de"
...
class VedodoRelativeLateralPositionErrorNormalDrivingConditionsTestCase(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            VedodoRelativeLateralPositionError,
        ]
