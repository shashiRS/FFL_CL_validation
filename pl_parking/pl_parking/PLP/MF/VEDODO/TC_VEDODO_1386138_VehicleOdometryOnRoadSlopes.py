#!/usr/bin/env python3
"""
The ESC Odometry shall achieve the precision on sloped roads. The ESC Odometry shall detect when the car sets back
(or forward) due to road slope.
"""
...
READER_NAME = "VehicleOdometryOnRoadSlopes"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall achieve the precision on sloped roads. The ESC Odometry shall detect when the car "
    "sets back (or forward) due to road slope.",
    description="The ESC Odometry shall achieve the precision on sloped roads. The ESC Odometry shall detect when "
    "the car sets back (or forward) due to road slope.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleOdometryWithDifferentTirePressure(TestStep):
    """The ESC Odometry shall achieve the precision on sloped roads. The ESC Odometry shall detect when the car sets
    back (or forward) due to road slope.
    ...
    def process(self, **kwargs):
        ...
        gt_angle_yaw = list(df[VedodoSignals.Columns.gt_yawAngle])
        odo_angle_yaw = list(df[VedodoSignals.Columns.yawAngle_est])
        ...
        for yw_gt, yw_od, xgt, xod, ygt, yod, d, ang_gt, ang_od in zip(
            gt_yaw, odo_yaw, gt_x, odo_x, gt_y, odo_y, driven_dist, gt_angle_yaw, odo_angle_yaw
        ):
            total_dist += d
            if total_dist >= 2000:
                yaw_angle_deviate_list.append(abs(ang_gt - ang_od))
        ...
        if max_yaw_rate_deviate < max_expected_deviate_yawrate:
            evaluation_yaw = " ".join(
                f"Evaluation of estimated yaw rate, deviation w.r.t ground truth is {round(max_yaw_rate_deviate, 3)}"
                f"rad/s and the expected threshold is {max_expected_deviate_yawrate} rad/s, the deviation value is "
        ...
        if max_yaw_angle_deviate < max_expected_deviate_yaw_angle:
            ...
            test_result_yaw_angle = False

        if test_result_yaw and test_result_x and test_result_y and test_result_yaw_angle:
            test_result = fc.PASS
        ...
        signal_summary[CarMakerUrl.yawAngle] = evaluation_angle_yaw
...
@testcase_definition(
    ...
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z7DjzaaEe6mrdm2_agUYg&componentURI=https%3A%2F%2Fjazz.conti.de"
    "%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_2ewE0DK_Ee6mrdm2_agUYg&oslc.configura"
    "tion=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F30013",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class VedodoVehicleOdometryWithDifferentTirePressureTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoVehicleOdometryWithDifferentTirePressure,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoVehicleOdometryWithDifferentTirePressureTestCase,
        temp_dir=temp_dir,
