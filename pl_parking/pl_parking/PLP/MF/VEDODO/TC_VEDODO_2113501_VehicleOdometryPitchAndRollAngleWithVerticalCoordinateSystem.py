#!/usr/bin/env python3
"""The ESC Odometry shall provide the angles (pitch/roll) between the horizontal plane and the vehicle coordinate
system with an accuracy of 1 deg.
"""
...
READER_NAME = "VedodoVelocityOdometeryPitchAndRollAngleWithVerticalCoordinateSystem"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the angles (pitch/roll) between the horizontal plane and the vehicle "
    "coordinate system with an accuracy of 1 deg.",
    description="The ESC Odometry shall provide the angles (pitch/roll) between the horizontal plane and the "
    "vehicle coordinate system with an accuracy of 1 deg.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedPitchAndRollAngleWithVerticalCoordinateSystem(TestStep):
    """The ESC Odometry shall provide the angles (pitch/roll) between the horizontal plane and the vehicle coordinate
    system with an accuracy of 1 deg.
    ...
    def process(self, **kwargs):
        ...
        if max_roll_error < max_expected_roll_angle_error:
            evaluation_roll = " ".join(
                ...
                f"{round(max_roll_error, 3)}rad is below the given threshold {max_expected_roll_angle_error}rad".split()
            )
            test_result_roll = True
        else:
            evaluation_roll = " ".join(
                f"Evaluation of estimated Roll Angle with Gt is FAILED. The max deviation of Roll Angle "
                f"{round(max_roll_error, 3)}rad is above the given threshold {max_expected_roll_angle_error}rad".split()
            )
        ...
        if max_pitch_error < max_expected_pith_angle_error:
            evaluation_pitch = " ".join(
                ...
                f"{round(max_pitch_error, 3)}rad is within the given threshold {max_expected_pith_angle_error}rad"
                f"".split()
            )
            ...
            evaluation_pitch = " ".join(
                ...
                f"{round(max_pitch_error, 3)}rad is above the given threshold {max_expected_pith_angle_error}rad"
                f"".split()
            )
...
@testcase_definition(
    ...
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI__IiN1AIzEe-JmbR7EsJ0UA&componentURI=https%3A%2F%2Fjazz.conti.de"
...
class VedodoVehicleEstimatedPitchAndRollAngleWithVerticalCoordinateSystemTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoVehicleEstimatedPitchAndRollAngleWithVerticalCoordinateSystem,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoVehicleEstimatedPitchAndRollAngleWithVerticalCoordinateSystemTestCase,
        temp_dir=temp_dir,
