...
READER_NAME = "VedodoVelocityOdometerPositionsAndOrientation"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the position in (x,y) and Orientation.",
    description="The ESC Odometry shall provide the position in (x,y) and Orientation.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedPositionsAndYawAngle(TestStep):
    """The ESC Odometry shall provide the position in (x,y) and Orientation.
    ...
    def process(self, **kwargs):
        ...
        for yw_gt, yw_od, xgt, xod, ygt, yod, d in zip(gt_yaw, odo_yaw, gt_x, odo_x, gt_y, odo_y, driven_dist):
            total_dist += d
            if total_dist >= 2000:
                yaw_angle_deviate_list.append(abs(yw_gt - yw_od))
        ...
        if max_yaw_angle_deviate < max_expected_deviate_yawangle:
            evaluation_yaw = " ".join(
                f"Evaluation of estimated yaw angle, the deviation value w.r.t ground truth is {max_yaw_angle_deviate}"
                f"rad and the expected threshold is {round(max_expected_deviate_yawangle, 3)} rad, the deviation "
                f"value is below the threshold. Hence the result is PASSED.".split()
            )
            ...
            evaluation_yaw = " ".join(
                f"Evaluation of estimated yaw angle, the deviation value w.r.t ground truth is {max_yaw_angle_deviate}"
                f"rad and the expected threshold is {round(max_expected_deviate_yawangle, 3)} rad, the deviation "
                f"value is above the threshold. Hence the result is FAILED.".split()
            )
        ...
        if max_x_error < max_expected_x_error:
            evaluation_x = " ".join(
                f"Evaluation of estimated Position-X, the deviation value w.r.t ground truth is {max_x_error}m"
                f" and the expected threshold is {round(max_expected_x_error, 3)}m, the deviation value is below  "
            ...
            evaluation_x = " ".join(
                f"Evaluation of estimated Position-X, the deviation value w.r.t ground truth is {max_x_error}m"
                f" and the expected threshold is {round(max_expected_x_error, 3)}m, the deviation value is above  "
        ...
        if max_y_error < max_expected_y_error:
            evaluation_y = " ".join(
                f"Evaluation of estimated Position-Y, the deviation value w.r.t ground truth is {max_y_error}m"
                f" and the expected threshold is {round(max_expected_y_error, 3)}m, the deviation value is below  "
                f"the threshold. Hence the result is PASSED.".split()
            )
            test_result_y = True
        else:
            evaluation_y = " ".join(
                f"Evaluation of estimated Position-Y, the deviation value w.r.t ground truth is {max_y_error}m"
                f" and the expected threshold is {round(max_expected_y_error, 3)}m, the deviation value is above  "
                f"the threshold. Hence the result is FAILED.".split()
            )
            test_result_y = False
...
@verifies("ReqID_1386199")
@testcase_definition(
    name="The ESC Odometry shall provide the position in (x,y) and Orientation.",
    description="The ESC Odometry shall provide the position in (x,y) and Orientation.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%"
    "2Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z8RrTaaEe6mrdm2_agUYg&componentURI=https%3A%2F%2Fjazz.conti."
