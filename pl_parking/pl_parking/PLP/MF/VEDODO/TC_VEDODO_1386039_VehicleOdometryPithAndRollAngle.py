#!/usr/bin/env python3
"""This is the test case to check the Pitch Angle and Roll Angle"""
...
READER_NAME = "VedodoVelocityOdometerPitchAndRollAngle"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the Pitch Angle and Roll Angle.",
    description="The ESC Odometry shall provide the Pitch Angle and Roll Angle.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedPitchAndRollAngle(TestStep):
    """The ESC Odometry shall provide the Pitch Angle and Roll Angle.
    ...
    def process(self, **kwargs):
        ...
        if max_roll_error < max_expected_roll_angle_error:
            evaluation_roll = " ".join(
                f"Evaluation of estimated Roll Angle is PASSED w.r.t GroundTruth data and within the expected threshold"
                f" {max_expected_roll_angle_error}rad from the actual Roll Angle, max deviation value is "
                f"{max_roll_error}rad".split()
            )
            test_result_roll = True
        else:
            evaluation_roll = " ".join(
                f"Evaluation of estimated Roll Angle is FAILED w.r.t GroundTruth data and outside the expected "
                f"threshold {max_expected_roll_angle_error}rad from the Roll Angle, max deviation value is "
                f"{max_roll_error}rad".split()
            )
        ...
        if max_pitch_error < max_expected_pith_angle_error:
            evaluation_pitch = " ".join(
                f"Evaluation of estimated Pitch Angle is PASSED w.r.t GroundTruth data and within the expected "
                f"threshold {max_expected_pith_angle_error}rad from the actual Pitch Angle, max deviation value is "
                f"{max_pitch_error}rad".split()
            )
            test_result_pitch = True
        else:
            evaluation_pitch = " ".join(
                f"Evaluation of estimated Pitch Angle is FAILED w.r.t GroundTruth data and outside the expected "
                f"threshold {max_expected_pith_angle_error}rad from the actual Pitch Angle, max deviation value is "
                f"{max_pitch_error}rad".split()
            )
...
@verifies("ReqID_1386039")
@testcase_definition(
    name="The ESC Odometry shall provide the Pitch Angle and Roll Angle.",
    description="The ESC Odometry shall provide the Pitch Angle and Roll Angle.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2"
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z5OWjaaEe6mrdm2_agUYg&oslc_config.context=https%3A%2F%2Fjazz.c"
