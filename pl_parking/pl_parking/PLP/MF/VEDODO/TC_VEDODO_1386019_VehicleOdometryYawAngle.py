#!/usr/bin/env python3
"""This is the test case to check the YawAngle"""
...
READER_NAME = "VedodoVelocityOdometryawAngle"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the YawAngle.",
    description="The ESC Odometry shall provide the YawAngle.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedYawAngle(TestStep):
    """The ESC Odometry shall provide the YawAngle.
    ...
    def process(self, **kwargs):
        ...
        if max_yaw_angle_error < max_expected_yaw_angle_error:
            evaluation_yaw = " ".join(
                f"Evaluation of estimated yaw angle, deviation w.r.t ground truth is {round(max_yaw_angle_error, 3)}"
                f"rad and the expected threshold is {max_expected_yaw_angle_error} rad, the deviation value is "
...
@verifies("ReqID_1386019")
@testcase_definition(
    name="The ESC Odometry shall provide the YawAngle.",
    description="The ESC Odometry shall provide the YawAngle.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2"
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z4nWTaaEe6mrdm2_agUYg&oslc_config.context=https%3A%2F%2Fjazz.c"
...
class VedodoVehicleEstimatedYawAngleTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoVehicleEstimatedYawAngle,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoVehicleEstimatedYawAngleTestCase,
        temp_dir=temp_dir,
