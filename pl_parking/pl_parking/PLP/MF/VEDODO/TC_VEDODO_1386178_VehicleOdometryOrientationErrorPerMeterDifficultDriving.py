#!/usr/bin/env python3
"""This is the test case to check the Orientation error per meter during difficult driving condition"""
...
READER_NAME = "VedodoVelocityOdometryOrientationErrorPerMeterDifficultDriving"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the Orientation error per driven meter during difficult driving.",
    description="The ESC Odometry shall provide the Orientation error per driven meter during difficult driving.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedOrientationErrorPerDrivenMeterDifficultDriving(TestStep):
    """The ESC Odometry shall provide the Orientation error per driven meter during difficult driving.
    ...
    def process(self, **kwargs):
        ...
        total_gt_angle = 0
        total_ref_angle = 0
        diff_orientation = list()
        for d, gy, oy in zip(driven_dist, psi_gt, psi_estimated):
            if total_dist >= 1:
                diff_angle = abs(total_gt_angle - total_ref_angle)
                total_dist = 0
            else:
                ...
                total_dist += d
                diff_angle = 0
            diff_orientation.append(diff_angle)

        if yaw_angle_flag:
            evaluation = "The Orientation signal data is having valid data"

        max_error = max(diff_orientation)
        if max_error <= max_threshold_per_meter:
            evaluation_orientation = " ".join(
                f"Evaluation for Orientation error per driven meter is {round(max_error, 3)}rad and which "
                f"is below the max expected threshold {max_threshold_per_meter}rad, Hence the test result "
                f"is PASSED.".split()
            )
            test_result = fc.PASS
...
@verifies("ReqID_1386178")
@testcase_definition(
    name="The ESC Odometry shall provide the Orientation error per driven meter during difficult driving.",
    description="The relative orientation error shall not exceed 0.4 deg per driven meter during difficult "
    "driving conditions.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2"
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z7qqTaaEe6mrdm2_agUYg&componentURI=https%3A%2F%2Fjazz.conti.de"
...
class VedodoVehicleEstimatedOrientationErrorPerDrivenMeterDifficultDrivingTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoVehicleEstimatedOrientationErrorPerDrivenMeterDifficultDriving,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoVehicleEstimatedOrientationErrorPerDrivenMeterDifficultDrivingTestCase,
        temp_dir=temp_dir,
