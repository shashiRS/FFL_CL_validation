#!/usr/bin/env python3
"""This is the test case to check the Positions (x,y) and Yaw Rate"""
...
READER_NAME = "VedodoVelocityOdometerPositionsAndYawRate"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the position in (x,y) and Yaw Rate.",
    description="The ESC Odometry shall provide the position in (x,y) and Yaw Rate.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedPositionsAndYawRate(TestStep):
    """The ESC Odometry shall provide the position in (x,y) and Yaw Rate.
    ...
    def process(self, **kwargs):
        ...
        if max_yaw_rate_deviate < max_expected_deviate_yawrate:
            evaluation_yaw = " ".join(
                ...
                f"rad and the expected threshold is {max_expected_deviate_yawrate} rad, the deviation value is "
...
@verifies("ReqID_1386014")
@testcase_definition(
    name="The ESC Odometry shall provide the position in (x,y) and Yaw Rate.",
    description="The ESC Odometry shall provide the position in (x,y) and Yaw Rate.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F"
    "%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z4nUTaaEe6mrdm2_agUYg&oslc_config.context=https%3A%2F%2Fj"
    "azz.conti.de%2Fgc%2Fconfiguration%2F17099&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-proj"
    "ects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_2ewE0DK_Ee6mrdm2_agUYg",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class VedodoVehicleEstimatedPositionsAndYawRateTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoVehicleEstimatedPositionsAndYawRate,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoVehicleEstimatedPositionsAndYawRateTestCase,
        temp_dir=temp_dir,
