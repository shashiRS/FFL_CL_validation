#!/usr/bin/env python3
"""This is the test case shall predict the position (x, y) and the orientation up to 40 ms to the
future. The car can be assumed to continue with the same velocity and yaw rate.
"""
...
READER_NAME = "VedodoFuturePositionsAndOrientation"
...
class VedodoSignals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        odo_xPositionPredictBuffer3 = "odo_xPositionPredictBuffer3"
        odo_yPositionPredictBuffer3 = "odo_yPositionPredictBuffer3"
        odo_yawAnglePredictBuffer3 = "odo_yawAnglePredictBuffer3"
        YAW_ANGLE = "yawAngle"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.odo_xPositionPredictBuffer3: CarMakerUrl.odo_xPositionPredictBuffer3,
            self.Columns.odo_yPositionPredictBuffer3: CarMakerUrl.odo_yPositionPredictBuffer3,
            self.Columns.odo_yawAnglePredictBuffer3: CarMakerUrl.odo_yawAnglePredictBuffer3,
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall predict the position (x, y) and the orientation up to 40 ms to the future. "
    "The car can be assumed to continue with the same velocity and yaw rate",
    description="The ESC Odometry shall predict the position (x, y) and the orientation up to 40 ms to the future. "
    "The car can be assumed to continue with the same velocity and yaw rate ",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoPredictFuturePositionAndOrientation(TestStep):
    """The relative position error shall predict the position (x, y) and the orientation up to 40 ms to the future.
    The car can be assumed to continue with the same velocity and yaw rate.
    ...
    def process(self):
        ...
        x_estimated = list(df[VedodoSignals.Columns.odo_xPositionPredictBuffer3])[3:-buffer_delay]
        y_estimated = list(df[VedodoSignals.Columns.odo_yPositionPredictBuffer3])[3:-buffer_delay]
        orientation_estimated = list(df[VedodoSignals.Columns.odo_yawAnglePredictBuffer3])[3:-buffer_delay]
        ...
        if max_longi_error > max_expected_x_position_error:
            evaluation_long = (
                f"Evaluation for Longitudinal position up to 40ms to the future, the max "
        ...
        if max_lat_error > max_expected_y_position_error:
            evaluation_lat = (
                f"Evaluation for Lateral position up to 40ms to the future, the max "
        ...
        if max_orientation_error > max_expected_orientation_error:
            evaluation_orient = (
                f"Evaluation for Orientation up to 40ms to the future, the max longitudinal"
                f" deviation error is {round(max_orientation_error, 3)}rad and which is above the threshold "
                f"{max_expected_orientation_error}rad. Hence the test result is FAILED."
            )
            test_result_list.append(False)
        else:
            evaluation_orient = (
                f"Evaluation for Orientation up to 40ms to the future, the max longitudinal"
                f" deviation error is {round(max_orientation_error, 3)}rad and which is within the threshold "
                f"{max_expected_orientation_error}rad. Hence the test result is PASSED."
            )
            test_result_list.append(True)
        ...
        signal_summary[f"{CarMakerUrl.odo_xPositionPredictBuffer3} vs {CarMakerUrl.odoCmRefX}"] = evaluation_long
        signal_summary[f"{CarMakerUrl.odo_yPositionPredictBuffer3} vs {CarMakerUrl.odoCmRefY}"] = evaluation_lat
        signal_summary[f"{CarMakerUrl.odo_yawAnglePredictBuffer3} vs {CarMakerUrl.gt_yawAngle}"] = evaluation_orient
        ...
        x_plot = plot_graphs(
            ap_time,
            x_estimated,
            CarMakerUrl.odo_xPositionPredictBuffer3,
            gt_x,
        ...
        y_plot = plot_graphs(
            ap_time,
            y_estimated,
            CarMakerUrl.odo_yPositionPredictBuffer3,
            gt_y,
        ...
        orientation_plot = plot_graphs(
            ap_time,
            y_estimated,
            CarMakerUrl.odo_yawAnglePredictBuffer3,
            gt_y,
...
@verifies("ReqId-1386047")
@testcase_definition(
    name="The relative position error shall predict the position (x, y) and the orientation up to 40 ms to the future."
    "The car can be assumed to continue with the same velocity and yaw rate.",
    description="The relative position error shall predict the position (x, y) and the orientation up to "
    "40 ms to the future. The car can be assumed to continue with the same velocity and yaw rate.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_3z5OZzaaEe6mrdm2_agUYg&componentURI=https%3A%2F%2Fjazz.conti.de%2"
...
class VedodoPredictFuturePositionAndOrientationTestCase(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            VedodoPredictFuturePositionAndOrientation,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoPredictFuturePositionAndOrientationTestCase,
        temp_dir=temp_dir,
