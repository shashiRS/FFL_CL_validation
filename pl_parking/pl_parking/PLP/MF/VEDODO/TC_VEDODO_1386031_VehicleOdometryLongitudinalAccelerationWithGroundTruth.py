#!/usr/bin/env python3
"""This is the test case to check the Longitudinal Acceleration w.r.t Ground Truth"""
...
READER_NAME = "VedodoAccelerationOdometryLongitudinalAccelerationWithGt"
...
@teststep_definition(
    step_number=1,
    name="The ESC Odometry shall provide the longitudinal Acceleration w.r.t Ground Truth.",
    description="The ESC Odometry shall provide the Longitudinal Acceleration w.r.t Ground Truth.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(READER_NAME, VedodoSignals)
class VedodoVehicleEstimatedLongitudinalAccelerationWithGt(TestStep):
    """The ESC Odometry shall provide the Longitudinal Acceleration w.r.t Ground Truth.
    ...
    def process(self, **kwargs):
        ...
        ref_a = list(df[VedodoSignals.Columns.ax])
        motionStatus = list(df[VedodoSignals.Columns.motionStatus])
        a_flag = True
        evaluation = ""
        acceleration_dff_list = list()
        max_expected_threshold_error: float = 0.1
        ...
        for gv, ov, m in zip(ref_a, odo_a, motionStatus):
            if m:
        ...
        if max_error > max_expected_threshold_error and not a_flag:
            evaluation_error = " ".join(
                f"Evaluation for Longitudinal Acceleration, the allowed expected threshold is 1% and"
...
@verifies("ReqID_1386031")
@testcase_definition(
    name="The ESC Odometry shall provide the Longitudinal Acceleration w.r.t Ground Truth.",
    description="The ESC Odometry shall provide the Longitudinal Acceleration w.r.t Ground Truth.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2"
    "Fjazz.conti.de%2Frm4%2Fresources%2FBI_3z5OUzaaEe6mrdm2_agUYg&oslc_config.context=https%3A%2F%2Fjazz.c"
...
class VedodoVehicleEstimatedLongitudinalAccelerationWithGtTestCase(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            VedodoVehicleEstimatedLongitudinalAccelerationWithGt,
        ]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    _, testrun_id, cp = debug(
        VedodoVehicleEstimatedLongitudinalAccelerationWithGtTestCase,
        temp_dir=temp_dir,
