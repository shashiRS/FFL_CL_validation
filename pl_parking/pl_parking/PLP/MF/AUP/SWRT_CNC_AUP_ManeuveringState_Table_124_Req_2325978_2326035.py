...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_124_Req_2325978_2326035"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 124 Req 2325978 2326035",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the ESC failed) then the Automated Parking Core shall transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_124_Req_2325978_2326035_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            signals["status_esc_state"] = esc_state.apply(lambda x: "ESC_ACTIVE" if x == 1 else "ESC_INACTIVE")
            ...
            time_threshold_2 = find_time_threshold(esc_state, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"signal {Signals.Columns.ESC_STATE} == ESC_ACTIVE(1):<br>"
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.ESC_STATE} == ESC_ACTIVE(1) was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_124_Req_2325978_2326035",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the ESC failed) then the Automated Parking Core shall transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_124_Req_2325978_2326035_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_124_Req_2325978_2326035_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.escInformationPort_escState_nu</em> == ESC_ACTIVE(0) <br><br>
            </td>
        </tr>
        <tr>
            <td>
