...
SIGNAL_DATA = "DIRRECTION_PARALLE_LEFT"
...
@teststep_definition(
    step_number=1,
    name="Parallel Left Indicator during Scanning State",
    description="Check the behavior of the system after the driver select the left dirrection from HMI in case ego veh is on a parallel parking slot in Scanning Out state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class ParkOutDirrParallelLeftCheck(TestStep):
    """ParkOutDirrParallelLeftCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        eval_cond = [True] * 1
        evaluation1 = " ".join(
            f"The evaluation the left dirrection indicator signal is True, the system is providyng a constant visualisation to the intended parkingOut dirrection ({signal_name['AVG_Type']} =="
            f" {constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PARALLEL_LEFT}).".split()
        )
            ...
            if t2_idx is not None:
                ...
                if t3_idx is not None:
                    ...
                    for y in range(t3_idx,len(HMI_Info)):
                        if avg_type[y] != constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PARALLEL_LEFT:
                            test_result = fc.FAIL
                            eval_cond = [False] * 1
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['AVG_Type']} signal is FAILED,"
                                f" the system didn't provide a constant visualisation of the left dirrection indicator ( != {constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PARALLEL_LEFT})."
        ...
        signal_summary["Check the reaction of the system related to the dirrection indicator during ParkingOut state - parallel left."] = evaluation1
...
@testcase_definition(
    name="Dirrection Indicator in which the system shall start the parking out maneuver during scanning state - parallel left",
    description="The AP funtion shall continuously show the Left Dirrection Indicator in which the system shall start the parking out maneuver during scanning state",
...
class ParkOutDirrParallelLeft(TestCase):
    """ParkOutDirrParallelLeft Test Case."""
