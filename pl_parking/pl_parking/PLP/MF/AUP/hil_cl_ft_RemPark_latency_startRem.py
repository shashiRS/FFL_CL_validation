...
@teststep_definition(
    name="Latency observed in the car, start RP.",
...
class RPLatencyStartCheck(TestStep):
    """RPLatencyStartCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        """Evaluation part"""
        # Find the moment when driver press the remote controller in order to start the parking maneuver
        ...
        if t1_idx is not None:
            ...
            if trigger_AP_active is not None:
                ...
                if t2_idx is not None:
                    # Check if the system activate the function within {AP_G_MAX_LATENCY_REM_S}s
                    ...
                    if trigger_AP_within_range is not True:
                        ...
                        evaluation1 = " ".join(
                            ...
                            f" AP function not activated within {constants.HilCl.ApThreshold.AP_G_MAX_LATENCY_REM_S}s ( != {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}).".split()
                        )
                else:
                    test_result = fc.FAIL
                    eval_cond = [False] * 1
                ...
                evaluation1 = " ".join(
                            f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED,"
                            f" AVG never activated, preconditions not met (!={constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}).".split()
                        )
        else:
            test_result = fc.FAIL
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                f"TC Failed, the driver didn't pressed the Start button from remote controller(APHMIOutUserActionRem != {constants.HilCl.Hmi.Command.TAP_ON_START_REM_PARK_PHONE})".split()
            )
        ...
        signal_summary[
            "Check the reaction of the system while pressing the remote controller in order to start the parking maneuver."
        ] = evaluation1
...
@testcase_definition(
    name="Latency observed in the car - start RP.",
    description="The observed latency in the car shall be less than {AP_G_MAX_LATENCY_REM_S}s in case of start via RC.",
...
class RPLatencyStart(TestCase):
    """RPLatencyStart Test Case."""
