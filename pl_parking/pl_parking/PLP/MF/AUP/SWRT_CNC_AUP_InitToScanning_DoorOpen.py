...
ALIAS = "SWRT_CNC_AUP_InitToScanning_DoorOpen"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        VELOCITY = "Car.v"
        DM_BRAKE = "DM.Brake"
        DOOR_STATUS = "AP.doorStatusPort.status.driver_nu"
...
@teststep_definition(
    step_number=1,
    name="Init To Scanning Door Open",
    description=(
        "This test case verifies that the AutomatedParkingCore shall not transition from Init to Scanning when a vehicle state deactivation is present (door open)."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_InitToScanning_DoorOpen_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            door_driver = signals[Signals.Columns.DOOR_STATUS]
            ...
            time_threshold_1 = 1
            time_threshold_2 = find_time_threshold(door_driver, aup.DOOR_DRIVER.DOOR_STATUS_OPEN)
            time_threshold_3 = len(time) - 1

            if time_threshold_1 is not None and time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    f"The evaluation is PASSED for the interval between the start of measurement and when {Signals.Columns.DOOR_STATUS} "
                    f"reaches DOOR_STATUS_OPEN at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>, as expected. "
                    f"This confirms that the AUP core remained in the Init state until the driver's door was opened.".split()
                )
                ...
                if cond_negative:
                    evaluation_1 = " ".join(
                        f"The evaluation FAILED for the interval between the start of measurement and when {Signals.Columns.DOOR_STATUS} "
                        f"reached DOOR_STATUS_OPEN at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. "
                        f"Before the door was opened, the signal {Signals.Columns.CORE_STATE} transitioned to a state other than "
                        f"CORE_INIT (0), which is unexpected.".split()
                    )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value ({Signals.Columns.DOOR_STATUS} == DOOR_STATUS_OPEN (1)) was never found.".split()
                )

            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED for the interval between the start of measurement and when {Signals.Columns.DOOR_STATUS} "
                    f"reaches DOOR_STATUS_OPEN at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. "
                    f"This confirms that the AUP core remained in the Init state after the driver's door was opened.".split()
                )
                eval_cond_2 = True  # set the evaluation condition as TRUE
                core_state_interval_2 = core_state.iloc[time_threshold_2:time_threshold_3]
                cond_negative = (core_state_interval_2 != aup.CORE_STATUS.CORE_INIT).any()

                if cond_negative:
                    evaluation_2 = " ".join(
                        f"The evaluation FAILED for the interval between the start of measurement and when {Signals.Columns.DOOR_STATUS} "
                        f"reached DOOR_STATUS_OPEN at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. "
                        f"After the door was opened, the signal {Signals.Columns.CORE_STATE} transitioned to a state other than "
                        f"CORE_INIT (0), which is unexpected.".split()
                    )
                    eval_cond_2 = False
            ...
            expected_val_2 = "AP.PARKSMCoreStatusPort.parksmCoreState_nu == CORE_INIT (0)"
...
@testcase_definition(
    name="SWRT_CNC_AUP_InitToScanning_DoorOpen",
    description=(
...
class SWRT_CNC_AUP_InitToScanning_DoorOpen_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [SWRT_CNC_AUP_InitToScanning_DoorOpen_TS]  # in this list all the needed test steps are included
    ...
    <table>
        ...
        <tr>
            <td>
                ...
                - T2-T3: <em>AP.PARKSMCoreStatusPort.parksmCoreState_nu</em> == CORE_INIT (0)<br>
            </td>
        </tr>
    </table>
