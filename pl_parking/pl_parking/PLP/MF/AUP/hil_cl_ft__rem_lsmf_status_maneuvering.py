"""The remote controller HMI shall display remote controlled LSMF status information to the driver: active (maneuvering)"""
...
SIGNAL_DATA = "LSMF_REM_INFO_MANEUVERING"
...
@teststep_definition(
    name="Information about actual remote controlled LSMF state (maneuvering) on remmote HMI",
    description="Check information on remote HMI about actual state of remote controlled LSMF.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RemLsmfStatManeuveringCheck(TestStep):
    """RemLsmfStatManeuveringCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" MANEUVER_ACTIVE ({constants.HilCl.Hmi.APHMIGeneralScreenRemote.MANEUVER_ACTIVE}) state when LSMF controll active.".split()
        )
        ...
        if t_device_ready_idx is not None:
            ...
            if t_start_requset_idx is not None:
                ...
                states_dict = HilClFuntions.States(
                    rem_gen_screen_sig, t_start_requset_idx - 1, len(rem_gen_screen_sig), 1
                ...
                for key in states_dict:
                    if counter == 1:
                        ...
                        if states_dict[key] != constants.HilCl.Hmi.APHMIGeneralScreenRemote.MANEUVER_ACTIVE:
                            evaluation1 = " ".join(
                                ...
                                f" but requiered mode is MANEUVER_ACTIVE ({constants.HilCl.Hmi.APHMIGeneralScreenRemote.MANEUVER_ACTIVE}).".split()
                            )
                            eval_cond[0] = False
                            break
                        counter += 1
        ...
        signal_summary["Remote HMI shall show maneuver active state of LSMF"] = evaluation1
...
@testcase_definition(
    name="Remote controlled LSMF status state on remoted HMI, Maneuvering",
    description="The remote controller HMI shall display remote controlled LSMF status information to the driver: active (maneuvering)",
)
class RemLsmfStatManeuvering(TestCase):
    """RemLsmfStatManeuvering Test Case."""
