...
T2:
AP.vehicleOccupancyStatusPort.beltBuckleStatus.driver_nu = 2
...
ALIAS = "SWRT_CNC_AUP_ScanningToManeuvering_BeltBuckle"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        STATUS_BELT = "SignalManipulation.vehicleOccupancyStatusPort_beltBuckleStatus_driver_nu"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.STATUS_BELT: [
                "SignalManipulation.vehicleOccupancyStatusPort_beltBuckleStatus_driver_nu",
            ],
        }
...
@teststep_definition(
    step_number=1,
    name="Scanning To Maneuvering Belt Buckle",
    description=(
        "This test case verifies that the AutomatedParkingCore shall not transition from Scanning to Maneuvering when the driver's seat belt is unbuckled."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ScanningToManeuvering_BeltBuckle_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            status_belt = signals[Signals.Columns.STATUS_BELT]
            signals["status_core"] = core_state.apply(lambda x: fh.get_status_label(x, aup.CORE_STATUS))
            signals["status_belt"] = status_belt.apply(lambda x: fh.get_status_label(x, aup.BELT_DRIVER))
            ...
            time_threshold_2 = find_time_threshold(status_belt, aup.BELT_DRIVER.BELT_STATUS_OPEN)
            ...
            if time_threshold_2 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation PASSED at the moment when {Signals.Columns.STATUS_BELT} == BELT_STATUS_OPEN ({aup.BELT_DRIVER.BELT_STATUS_OPEN}) "
                    f"at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>, meeting the expected condition. <br>"
                    f"The vehicle remained in the SCANNING state prior to the driver’s seat belt being unbuckled. <br>".split()
                )
                ...
                if delay_negative:
                    evaluation_2 = " ".join(
                        f"The evaluation FAILED at the moment when {Signals.Columns.STATUS_BELT} == BELT_STATUS_OPEN ({aup.BELT_DRIVER.BELT_STATUS_OPEN}) "
                        f"(<b>{round(time.iloc[time_threshold_2], 3)} s</b>). <br>"
                        f"The signal {Signals.Columns.CORE_STATE} was not in the expected CORE_SCANNING state before the driver’s seat belt was unbuckled: <br>"
                        f" - At time <b>{round(time.iloc[time_threshold_2 - 1], 3)} s</b>, it was in state "
                        f"{signals['status_core'].iloc[time_threshold_2 - 1]} ({core_state.iloc[time_threshold_2 - 1]}). <br>".split()
                    )
                    eval_cond_1 = False

            else:
                evaluation_2 = " ".join(
                    f"The evaluation FAILED because the trigger value ({Signals.Columns.STATUS_BELT} == BELT_STATUS_OPEN ({aup.BELT_DRIVER.BELT_STATUS_OPEN})) was never found. <br>"
                    f"The vehicle did not transition to the expected state. <br>".split()
                )
                eval_cond_1 = False
            ...
            if time_threshold_3 is not None and time_threshold_4 is not None:
                ...
                if (interval != constants.AUPConstants.CORE_PARKING).all():
                    evaluation_3 = " ".join(
                        ...
                        f"from the moment of user action - TAP_ON_START_PARKING({aup.USER_ACTION.TAP_ON_START_PARKING}) at timestamp <b>{round(time.iloc[time_threshold_3], 3)} s</b> "
                        f"until the end of measurement at timestamp <b>{round(time.iloc[time_threshold_4], 3)} s</b>. <br>"
                        f"This confirms that the vehicle did not initiate an unexpected maneuver during this interval. <br>".split()
                    )
                    eval_cond_2 = True
                else:
                    evaluation_3 = " ".join(
                        f"The evaluation FAILED because the signal {Signals.Columns.CORE_STATE} transitioned to CORE_PARKING (2) "
                        f"within the interval between the user action - TAP_ON_START_PARKING({aup.USER_ACTION.TAP_ON_START_PARKING})  at timestamp <b>{round(time.iloc[time_threshold_3], 3)} s</b> "
                        f"and the end of measurement at timestamp <b>{round(time.iloc[time_threshold_4], 3)} s</b>. <br>"
                        f"This indicates an unexpected maneuver activation during this period. <br>".split()
                    )
                    eval_cond_2 = False
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=status_belt,
                    mode="lines",
                    name=Signals.Columns.STATUS_BELT,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_belt"],
                )
            )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ScanningToManeuvering_BeltBuckle",
    description=(
        "This test case verifies that the AutomatedParkingCore shall not transition from Scanning to Maneuvering when the driver's seat belt is unbuckled."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ScanningToManeuvering_BeltBuckle_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [SWRT_CNC_AUP_ScanningToManeuvering_BeltBuckle_TS]  # in this list all the needed test steps are included
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <em>AP.vehicleOccupancyStatusPort.beltBuckleStatus.driver_nu</em> = BELT_STATUS_OPEN (2) <br>
        ...
        <tr>
            <td>
                ...
                - T2: Delay (-0.01s): <em>AP.PARKSMCoreStatusPort.parksmCoreState_nu</em> = CORE_SCANNING (1) occurs just before T2.<br>
                - T3-T4: <em>AP.PARKSMCoreStatusPort.parksmCoreState_nu</em> != CORE_PARKING (2) between T3 and T4.
            </td>
        </tr>
    </table>
