...
T2: // env model traffic participants become unreliable
SignalManipulation.healthVector_envModelTrafficParticiapantsUnreliable = 1

T3: // heal condition
SignalManipulation.healthVector_envModelTrafficParticiapantsUnreliable = 0
...
AP.apUserInformationPort.generalUserInformation_nu == ENV_MODEL_TRAFFIC_PARTICIPANTS_UNRELIABLE (77)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_70_Req_2325842_2326005_2325843"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        TRAFFIC_PART = "SignalManipulation.healthVector_envModelTrafficParticiapantsUnreliable"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.TRAFFIC_PART: [
                "SignalManipulation.healthVector_envModelTrafficParticiapantsUnreliable",
            ],
            self.Columns.IGN_ON: [
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 70 Req 2325842 2326005 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the environmental model traffic participants are unreliable, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_70_Req_2325842_2326005_2325843_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            traffic_part = signals[Signals.Columns.TRAFFIC_PART]
            ...
            time_threshold_2 = find_time_threshold(traffic_part, 1)
            time_threshold_3 = find_time_threshold(traffic_part.iloc[time_threshold_2:], 0) + time_threshold_2
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.TRAFFIC_PART}</b> == 1: <br>"
                    ...
                    f"This confirms that AUPCore received the information that the environmental model traffic participants were unreliable and then it went into error <br>"
                ...
                delay_pos_user = (
                    user_inf.iloc[time_threshold_2 + 10] != aup.HMI_MESSAGE.ENV_MODEL_TRAFFIC_PARTICIPANTS_UNRELIABLE
                )
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before environmental model traffic participants were detected unreliable and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before environmental model traffic participants were detected unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due environmental model traffic participants unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"when {Signals.Columns.TRAFFIC_PART} == 1. This means {reason}".split()
                        )
                        eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TRAFFIC_PART} == 1 was never found.".split()
                )
            if time_threshold_2 is not None and time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED between the interval given by the signal {Signals.Columns.TRAFFIC_PART} == 1 "
                ...
                error_state = traffic_part.iloc[time_threshold_3 : (time_threshold_4 - 10)]
                ...
                if cond_neg_core or cond_neg_error:
                    if cond_neg_core and cond_neg_error:
                        evaluation_2 = " ".join(
                            f"The evaluation FAILED between the interval given by the signal {Signals.Columns.TRAFFIC_PART} == 1<br> "
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                        ...
                        evaluation_2 = " ".join(
                            ...
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                            f"<b>({round(time.iloc[time_threshold_2], 3)} - {round(time.iloc[time_threshold_5 - 10], 3)})s</b>,<br> "
                            f"because the signal {Signals.Columns.TRAFFIC_PART} wasn't set to 0.<br><br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TRAFFIC_PART} == 0 was never found between T2 and T4.<br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger values {Signals.Columns.TRAFFIC_PART} == 1 or {Signals.Columns.IGN_ON} == Ignition OFF (0) "
            ...
            AP.apUserInformationPort.generalUserInformation_nu == ENV_MODEL_TRAFFIC_PARTICIPANTS_UNRELIABLE (77)<br>"""
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=traffic_part,
                    mode="lines",
                    name=Signals.Columns.TRAFFIC_PART,
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_70_Req_2325842_2326005_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the environmental model traffic participants are unreliable, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_70_Req_2325842_2326005_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_70_Req_2325842_2326005_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //env model traffic participants become<br>
                    <em>SignalManipulation.healthVector_envModelTrafficParticiapantsUnreliable</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_envModelTrafficParticiapantsUnreliable</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == ENV_MODEL_TRAFFIC_PARTICIPANTS_UNRELIABLE (77) <br>
                T2 - T4:<br>
