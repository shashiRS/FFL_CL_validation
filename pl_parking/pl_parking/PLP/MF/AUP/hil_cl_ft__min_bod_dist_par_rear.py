...
SIGNAL_DATA = "BODY DISTANCE HIGH OBSTACLE PARALLEL REAR SIDE"
...
@teststep_definition(
    ...
    description="Body distance from high obstacle at final parking pose parallel rear side.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupAPMinBodDistParRearCheck(TestStep):
    """AupAPMinBodDistParRearCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_start_idx and t_success_idx:
            ego_pos = ego_x[t_success_idx]
            obj_pos = object_x[t_success_idx]
            if ego_pos - obj_pos > constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_SSIDE_HIGH_OBST_M:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    "The test for checking minimal body distance between rear side and high object for parallel parking is PASSED.".split()
                )
            else:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The test for checking minimal body distance between rear side and high object for parallel parking is FAILED. The final parking pose was too close to the high object, it was"
            ...
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The test for checking minimal body distance between rear side and high object for parallel parking is FAILED.".split()
            )
...
@testcase_definition(
    name="Minimum body distance from high object parallel rear",
    description="Body distance from high obstacle at final parking pose parallel rear side.",
)
class AupAPMinBodDistParRear(TestCase):
    """AupAPMinBodDistParRear Test Case."""
