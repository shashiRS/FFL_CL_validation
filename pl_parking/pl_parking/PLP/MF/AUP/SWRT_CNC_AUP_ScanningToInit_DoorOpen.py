...
INT_ET1_ET2:
AP.doorStatusPort.status.driver_nu == DOOR_STATUS_CLOSED (2)
...
ALIAS = "SWRT_CNC_AUP_ScanningToInit_DoorOpen"
...
@teststep_definition(
    step_number=1,
    name="Scanning To Init Door Open",
    description=(
        "This test case verifies that the AutomatedParkingCore shall transition from Scanning to Init when a vehicle state deactivation is present (door open)."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ScanningToInit_DoorOpen_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if time_threshold_1 is not None and time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    f"The evaluation is PASSED for the interval between the start of Scanning<b>{round(time.iloc[time_threshold_1], 3)} s</b> and when {Signals.Columns.DOOR_DRIVER} "
                    f"reaches the DOOR_STATUS_OPEN state at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. "
                    f"This confirms that the driver's door was CLOSED until it changed to OPEN.".split()
                )
                ...
                if cond_negative:
                    evaluation_1 = " ".join(
                        f"The evaluation FAILED in the interval between the start of Scanning {round(time.iloc[time_threshold_1], 3)} and when {Signals.Columns.DOOR_DRIVER} "
                        f"reaches the DOOR_STATUS_OPEN state at timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. "
                        f"Before the door was opened, the signal {Signals.Columns.DOOR_DRIVER} did not stay in the CLOSED state as expected. <br>".split()
                    )
                    eval_cond_1 = False
            else:
            ...
            if time_threshold_2 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED when {Signals.Columns.DOOR_DRIVER} reaches the DOOR_STATUS_OPEN state at "
                    f"timestamp <b>{round(time.iloc[time_threshold_2 + 10], 3)} s</b>, as expected. "
                    f"This indicates that the AUP core was in the Init state after the driver's door was opened.".split()
                )
                ...
                if cond_negative:
                    evaluation_2 = " ".join(
                        f"The evaluation FAILED when {Signals.Columns.DOOR_DRIVER} reached the DOOR_STATUS_OPEN state at "
                        f"timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. "
                        f"After the driver's door opened, the signal {Signals.Columns.CORE_STATE} did not transition to the expected "
            ...
            expected_val_1 = "AP.doorStatusPort.status.driver_nu == DOOR_STATUS_CLOSED (2)"
...
@testcase_definition(
    name="SWRT_CNC_AUP_ScanningToInit_DoorOpen",
    description=(
        "This test case verifies that the AutomatedParkingCore shall transition from Scanning to Init when a vehicle state deactivation is present (door open)."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ScanningToInit_DoorOpen_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [SWRT_CNC_AUP_ScanningToInit_DoorOpen_TS]  # in this list all the needed test steps are included
    ...
    <table>
        ...
        <tr>
            <td>
                ...
                - T1-T2: <em>AP.doorStatusPort.status.driver_nu</em> ==DOOR_STATUS_CLOSED (2) <br>
