"""RA Mode Transition, Precondition Mode to Automated Reversing, Brake released"""
...
SIGNAL_DATA = "RA_PRE_TO_AUT_BRAKE"
...
@teststep_definition(
    ...
    description="Check RA function switches from Precondition Mode to Automated Reversing if brake pedal is released.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupRaPreToAutCheckBrake(TestStep):
    """AupRaPreToAutCheckBrake Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_arp_idx and t_hmi_idx and t_standstill_idx and t_brake_idx and t_gear_idx and t_aut_rev_idx:
            if t_brake_idx <= t_aut_rev_idx:
                ...
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    "The test for checking Mode transition from Precondition Mode to Automated Reversing is FAILED.".split()
                )
                evaluation1.join(
                        "The system transitioned to Automated reverse before the brake pedal was released.".split())
        ...
        if t_arp_idx is None:
            evaluation1.join("The system did not transition to Automated Reversing Precondition Mode.".split())
        ...
        signal_summary[
            "Required RA state change: Precondition Mode to Automated Reversing. Reason: Brake pedal released"
        ] = evaluation1
...
@testcase_definition(
    name="RA Mode Transition, Precondition Mode to Automated Reversing, Brake released",
    description="The RA function shall transit from Automated Reversing Precondition Mode to Automated Reversing if brake pedal is released.",
)
class AupRaPreToAutBrake(TestCase):
    """AupRaPreToAutBrake Test Case."""
