...
SIGNAL_DATA = "MANEUVER_AREA_CHECK_PARALLEL"
...
@teststep_definition(
    name="Checking the maneuvering trajectory paralell",
    description="Check the maneuvering is correctly happening based on the maneuvering area constrain.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupManeuveringAreaParCheck(TestStep):
    """AupManeuveringAreaParCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t1_idx is not None and t2_idx is not None and t3_idx is not None:
            eval_cond, maneuver_area, ego = HilClFuntions.maneuver_area_check(
                positions_x=pos_x[t2_idx:t3_idx],
                positions_y=pos_y[t2_idx:t3_idx],
                heading_angle=ego_heading[t2_idx:t3_idx],
            ...
            eval_cond, maneuver_area, ego = HilClFuntions.maneuver_area_check(
                positions_x=pos_x[t3_idx - 1 : t3_idx],
                positions_y=pos_y[t3_idx - 1 : t3_idx],
                heading_angle=ego_heading[t3_idx - 1 : t3_idx],
                ego_length=constants.SlotOffer.VehicleDimensions.VEHICLE_LENGTH,
                ego_width=constants.SlotOffer.VehicleDimensions.VEHICLE_WIDTH,
                left_d=constants.HilCl.ManeuveringArea.PAR_IN_D_1,
                rear_d=constants.HilCl.ManeuveringArea.PAR_IN_D_2,
...
@testcase_definition(
    name="Maneuvering area check paralell",
    description="Checking, that the AUP function does not leave the specified manuevering area while performing parking in manuever.",
)
class AupManeuveringAreaPar(TestCase):
    """AupManeuverAreaCheck Test Case."""
