...
T2:
SignalManipulation.escInformationPort_tcsState_nu = {TCS_ACTIVE} (1)

T3:
T2 + AP_G_TCS_TIME_THRESH_S (0.5s)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_116_Req_2325960_2326034"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        TCS_STATE = "SignalManipulation.escInformationPort_tcsState_nu"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 116 Req 2325960 2326034",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that a TCS intervention is active for longer than 0.5 s."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_116_Req_2325960_2326034_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            tcs_state = signals[Signals.Columns.TCS_STATE]
            ...
            signals["status_tcs_state"] = tcs_state.apply(lambda x: "TCS_ACTIVE" if x == 1 else "TCS_NOT_ACTIVE")
            ...
            time_threshold_2 = find_time_threshold(tcs_state, TCS_ACTIVE)
            ...
            if time_threshold_3 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.TCS_STATE} is in TCS_ACTIVE (1) state: <br>"
                    ...
                    f"This confirms that AUPCore transitioned from Maneuvering to Terminate (Error or Pause-Finished) when the AUPCore received the<br>\
                        information that a TCS intervention is active for longer than {aup.AP_G_TCS_TIME_THRESH_S} s.".split()
                )
                eval_cond_1 = True
                    ...
                    if (delay_pos_ap or total_ap_finish != 1) or (delay_neg_ap or delay_neg_core):
                        if delay_neg_ap or delay_neg_core:
                            evaluation_1 = " ".join(
                                ...
                                f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                                f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                                f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received that a TCS intervention is active for longer than {aup.AP_G_TCS_TIME_THRESH_S} s.".split()
                            )
                            ...
                            evaluation_1 = " ".join(
                                ...
                                f"{Signals.Columns.TCS_STATE} is in TCS_ACTIVE (1) state: <br>"
                                f"- AUPCore didn't transition to Pause and then to Finished state when the AUPCore received the information that a TCS intervention is active for longer than {aup.AP_G_TCS_TIME_THRESH_S} s.".split()
                            )
                        eval_cond_1 = False
                ...
                if (delay_neg_ap or delay_neg_core or delay_pos_var or delay_pos_core) and not ap_pause:
                    if delay_neg_ap or delay_neg_core:
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                            f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information that a TCS intervention is active for longer than {aup.AP_G_TCS_TIME_THRESH_S} s.".split()
                        )
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_3 + 10]} ({var_esm.iloc[time_threshold_3 + 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 + 10]} ({core_state.iloc[time_threshold_3 + 10]}) <br><br>"
                            f"This means that AUPCore didn't transition to Error state when the AUPCore received the information that a TCS intervention is active for longer than {aup.AP_G_TCS_TIME_THRESH_S} s.".split()
                        )
                    eval_cond_1 = False
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TCS_STATE} == TCS_ACTIVE (1) was never found.".split()
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=tcs_state,
                    mode="lines",
                    name=Signals.Columns.TCS_STATE,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_tcs_state"],
                )
            )
            fig.add_trace(
                go.Scatter(
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_116_Req_2325960_2326034",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that a TCS intervention is active for longer than 0.5 s."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_116_Req_2325960_2326034_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_116_Req_2325960_2326034_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2:<em>SignalManipulation.escInformationPort_tcsState_nu</em> == TCS_ACTIVE (1) <br>
                T3: T2 +  AP_G_TCS_TIME_THRESH_S (0.5s)<br>
