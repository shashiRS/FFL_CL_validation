...
SIGNAL_DATA = "BODY DISTANCE HIGH OBSTACLE PARALLEL PASSENGER SIDE"
...
@teststep_definition(
    ...
    description="Body distance from high obstacle at final parking pose parallel passenger side.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupAPMinBodDistParPassCheck(TestStep):
    """AupAPMinBodDistParPassCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_start_idx and t_success_idx:
            ego_pos = ego_y[t_success_idx] - constants.HilCl.CarShape.CAR_R_AXLE_TO_SIDE
            obj_pos = object_y[t_success_idx]
            if ego_pos - obj_pos > constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_LSIDE_HIGH_OBST_PAR_M:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    "The test for checking minimal body distance between passenger side and high object for paralell parking is PASSED.".split()
                )
            else:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The test for checking minimal body distance between passenger side and high object for paralell parking is FAILED. The final parking pose was too close to the high object, it was"
                    f"{obj_pos-ego_pos} m while the maximum allowed is {constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_LSIDE_HIGH_OBST_PAR_M}".split()
                )

        else:
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The test for checking minimal body distance between passenger side and high object for paralell parking is FAILED.".split()
            )
...
@testcase_definition(
    name="Minimum body distance from high object parallel passenger",
    description="Body distance from high obstacle at final parking pose parallel passenger side.",
)
class AupAPMinBodDistParPass(TestCase):
    """AupAPMinBodDistParPass Test Case."""
