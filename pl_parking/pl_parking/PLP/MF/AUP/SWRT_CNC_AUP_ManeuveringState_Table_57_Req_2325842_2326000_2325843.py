...
T2: // left camera pre-proc becomes unreliable
SignalManipulation.healthVector_leftCamPreProcUnreliable = 1

T3: // heal condition
SignalManipulation.healthVector_leftCamPreProcUnreliable = 0
...
AP.apUserInformationPort.generalUserInformation_nu == LEFT_CAM_PRE_PROC_UNRELIABLE (49)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_57_Req_2325842_2326000_2325843"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 57 Req 2325842 2326000 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the left camera pre-processing is unreliable, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_57_Req_2325842_2326000_2325843_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"This confirms that AUPCore received the information that the left camera pre-processing was unreliable and then it went into error <br>"
                ...
                delay_pos_user = user_inf.iloc[time_threshold_2 + 10] != aup.HMI_MESSAGE.LEFT_CAM_PRE_PROC_UNRELIABLE
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before left camera pre-processing was detected unreliable and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before left camera pre-processing was detected unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due to left camera pre-processing unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
            ...
            AP.apUserInformationPort.generalUserInformation_nu == LEFT_CAM_PRE_PROC_UNRELIABLE (49)<br>"""
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_57_Req_2325842_2326000_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the left camera pre-processing is unreliable, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_57_Req_2325842_2326000_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_57_Req_2325842_2326000_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //left camera pre-proc becomes unreliable<br>
                    <em>SignalManipulation.healthVector_leftCamPreProcUnreliable</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_leftCamPreProcUnreliable</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == LEFT_CAM_PRE_PROC_UNRELIABLE (49) <br>
                T2 - T4:<br>
