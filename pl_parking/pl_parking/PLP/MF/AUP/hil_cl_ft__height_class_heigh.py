"""Check heigh object classification."""
...
SIGNAL_DATA = "HEIGHT_CLASSIFICATION_HEIGH"
...
@teststep_definition(
    ...
    description="Check heigh object classification",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class CommonHeighCheck(TestStep):
    """CommonHeighCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if len(detections_idx) > 0:
            ...
            evaluation1 = " ".join(
                "The evaluation is PASSED, AP function considers obstacle as heigh static object.".split()
            )

            # Check type of detected object
            for item in detections_idx:
                if height_class_sig[item] != constants.HilCl.HeightClases.HIGH_OBSTACLE:
                    evaluation1 = " ".join(
                        f"The evaluation is FAILED, AP function does not consider"
                        f" static object as heigh object at {time_sig[item]} us.".split()
                    )
                    eval_cond[0] = False
                    break
        else:
        ...
        signal_summary["Height classification: Heigh"] = evaluation1
...
@testcase_definition(
    name="Height classification: Heigh",
    description=f"The AP Function shall consider static objects higher than"
    f" {constants.HilCl.HeightLimits.AP_G_MAX_HEIGHT_BODY_TRAVER_M} m as high.",
)
class FtCommon(TestCase):
    """CommonHeigh Test Case."""

    custom_report = MfHilClCustomTestcaseReport
    # Important iformation:
    # Please use this script only with TestRun HIL_COMMON_HeightClassification_Heigh_ID1073780
