"""RC state switches from RC Getting Off to Init if deactivation is triggered by "The fuel tank cap is open"."""
import logging
import os
import sys
...
SIGNAL_DATA = "RCGETTINGOFF_TO_INIT_FUEL_CAP_OPEN"
...
class ValidationSignals(MDFSignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.TANK_CAP_STATE: "MTS.IO_CAN_Conti_Veh_CAN.VehInput05.TankCapOpen",
...
@teststep_definition(
    name="Transition mode check - RC Getting Off to Init, fuel tank cap is open",
    description="Check the transition from RC Getting Off to Init after the fuel tank cap is open.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RCGettingOffToInitFuelTankCapOpenCheck(TestStep):
    """RCGettingOffToInitFuelTankCapOpenCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t1_getting_off_idx is not None:
            # Find the moment when fuel cap is open
            for i in range(t1_getting_off_idx, len(HMI_Info)):
                if tank_cap_state[i] == constants.HilCl.TunkCap.OPEN:
                    t2_fuel_cap_open = i
                    break
            if t2_fuel_cap_open is not None:
                # Check if the system is in RC Getting Off until fuel cap is open
                for _ in range(t1_getting_off_idx, t2_fuel_cap_open):
                    if general_message_sig[cnt] != constants.HilCl.Hmi.APHMIGeneralMessage.LEAVE_VEHICLE:
                        trigger_state = False
                        break
                if trigger_state:
                    # taking the timestamp of t2_idx in order to check the reaction 0.5s after
                    t2_timestamp = time_signal[t2_fuel_cap_open]
                    for y in range(t2_fuel_cap_open, len(HMI_Info)):
                    ...
                    if t3_after_delay_added is not None:
                        ...
                        if HMI_Info[t3_after_delay_added] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE:
                            ...
                            evaluation1 = " ".join(
                                ...
                                f" system didn't change the state to Init after tank cap open( != {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE}).".split()
                            )
                    else:
                        test_result = fc.FAIL
                        eval_cond = [False] * 1
                    ...
                    eval_cond = [False] * 1
                    evaluation1 = " ".join(
                        f"TC Failed because the system is not in RC Getting Off before fuel cap open (APHMIGeneralMessage != {constants.HilCl.Hmi.APHMIGeneralMessage.LEAVE_VEHICLE})".split()
                    )
            else:
                test_result = fc.FAIL
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                            f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED,"
                            f" fuel cap not open (TankCapOpen != {constants.HilCl.TunkCap.OPEN}).".split()
                        )
        else:
            test_result = fc.FAIL
            eval_cond = [False] * 1
        ...
        signal_summary[
            "Check the reaction of the system during RC Getting Off if fuel tank cap is open"
        ] = evaluation1
        ...
        fig.add_trace(go.Scatter(x=time_signal, y=tank_cap_state, mode="lines", name=signal_name["Tank_cap_state"]))
...
@testcase_definition(
    name="RC Getting Off to Init transition, fuel tank cap is open.",
    description="The function shall switch from RC Getting Off to Init mode if fuel tank cap is open.",
...
class RCGettingOffToInitFuelTankCapOpen(TestCase):
    """RCGettingOffToInitFuelTankCapOpen Test Case."""
    ...
    def test_steps(self):
        ...
        return [
            RCGettingOffToInitFuelTankCapOpenCheck,
        ]
