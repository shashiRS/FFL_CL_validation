...
SIGNAL_DATA = "BODY DISTANCE HIGH OBSTACLE PERPENDICULAR DRIVER SIDE"
...
@teststep_definition(
    ...
    description="Body distance from high obstacle at final parking pose perpendicular driving side.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupAPMinBodDistPerpDrivCheck(TestStep):
    """AupAPMinBodDistPerpDrivCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_start_idx and t_success_idx:
            ...
            if ego_pos - obj_pos > constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_LSIDE_HIGH_OBST_PERP_M:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    "The test for checking minimal body distance between driver side and high object for perpendicular parking is PASSED.".split()
                )
            else:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The test for checking minimal body distance between driver side and high object for perpendicular parking is FAILED. The final parking pose was too close to the high object, it was"
            ...
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The test for checking minimal body distance between driver side and high object for perpendicular parking is FAILED.".split()
            )
...
@testcase_definition(
    name="Minimum body distance from high object perpendicular driving",
    description="Body distance from high obstacle at final parking pose perpendicular driving side.",
)
class AupAPMinBodDistPerpDriv(TestCase):
    """AupAPMinBodDistPerpDriv Test Case."""
