...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_161_Req_2711728_2326041"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 161 Req 2711728 2326041",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Terminate state and it receives the \
            information that a reversible error is present (right camera is blocked) then it should transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_161_Req_2711728_2326041_TS(TestStep):
            ...
            if time_threshold_2 is not None:
                ...
                if delay_neg_ap or delay_neg_core:
                    ...
                    evaluation_1 = " ".join(
                        ...
                        f"{Signals.Columns.RIGHT_CAM} == 0: <br>"
                        f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) <br>"
                        f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) <br>"
                        f"- the signal {Signals.Columns.VAR_PPC} == {signals['status_var_ppc'].iloc[time_threshold_2 + 10]} ({var_ppc.iloc[time_threshold_2 + 10]}) <br><br>"
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_161_Req_2711728_2326041",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Terminate state and it receives the \
            information that a reversible error is present (right camera is blocked) then it should transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_161_Req_2711728_2326041_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_161_Req_2711728_2326041_TS
