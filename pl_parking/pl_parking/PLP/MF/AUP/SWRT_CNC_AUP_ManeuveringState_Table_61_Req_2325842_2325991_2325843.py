...
T2: // vehicle communication error
SignalManipulation.healthVector_vehicleCommunicationError = 1

T3: // heal condition
SignalManipulation.healthVector_vehicleCommunicationError = 0
...
AP.apUserInformationPort.generalUserInformation_nu == VEHICLE_COMMUNICATION_ERROR (54)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_61_Req_2325842_2325991_2325843"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        VEH_COMM_ERR = "SignalManipulation.healthVector_vehicleCommunicationError"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.VEH_COMM_ERR: [
                "SignalManipulation.healthVector_vehicleCommunicationError",
            ],
            self.Columns.IGN_ON: [
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 61 Req 2325842 2325991 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the host platform has vehicle communication error, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_61_Req_2325842_2325991_2325843_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            veh_comm_err = signals[Signals.Columns.VEH_COMM_ERR]
            ...
            time_threshold_2 = find_time_threshold(veh_comm_err, 1)
            time_threshold_3 = find_time_threshold(veh_comm_err.iloc[time_threshold_2:], 0) + time_threshold_2
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.VEH_COMM_ERR}</b> == 1: <br>"
                    ...
                    f"This confirms that AUPCore received the information that the the host platform has vehicle communication error <br>and then it went into error "
                ...
                delay_pos_user = user_inf.iloc[time_threshold_2 + 10] != aup.HMI_MESSAGE.VEHICLE_COMMUNICATION_ERROR
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before vehicle communication error and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before vehicle communication error.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due to vehicle communication error.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"when {Signals.Columns.VEH_COMM_ERR} == 1. This means {reason}".split()
                        )
                        eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.VEH_COMM_ERR} == 1 was never found.".split()
                )
            if time_threshold_2 is not None and time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED between the interval given by the signal {Signals.Columns.VEH_COMM_ERR} == 1 "
                ...
                error_state = veh_comm_err.iloc[time_threshold_3 : (time_threshold_4 - 10)]
                ...
                if cond_neg_core or cond_neg_error:
                    if cond_neg_core and cond_neg_error:
                        evaluation_2 = " ".join(
                            f"The evaluation FAILED between the interval given by the signal {Signals.Columns.VEH_COMM_ERR} == 1<br> "
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                        ...
                        evaluation_2 = " ".join(
                            ...
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                            f"<b>({round(time.iloc[time_threshold_2], 3)} - {round(time.iloc[time_threshold_5 - 10], 3)})s</b>,<br> "
                            f"because the signal {Signals.Columns.VEH_COMM_ERR} wasn't set to 0.<br><br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.VEH_COMM_ERR} == 0 was never found between T2 and T4.<br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger values {Signals.Columns.VEH_COMM_ERR} == 1 or {Signals.Columns.IGN_ON} == Ignition OFF (0) "
            ...
            AP.apUserInformationPort.generalUserInformation_nu == VEHICLE_COMMUNICATION_ERROR (54)<br>"""
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=veh_comm_err,
                    mode="lines",
                    name=Signals.Columns.VEH_COMM_ERR,
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_61_Req_2325842_2325991_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the host platform has vehicle communication error, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_61_Req_2325842_2325991_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_61_Req_2325842_2325991_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //vehicle communication error<br>
                    <em>SignalManipulation.healthVector_vehicleCommunicationError</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_vehicleCommunicationError</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == VEHICLE_COMMUNICATION_ERROR (54) <br>
                T2 - T4:<br>
