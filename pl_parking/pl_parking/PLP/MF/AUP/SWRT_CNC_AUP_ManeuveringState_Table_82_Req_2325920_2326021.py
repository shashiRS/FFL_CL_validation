...
T2:
SignalManipulation.additionalBCMStatusPort_tankCapOpen_nu = 1
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_82_Req_2325920_2326021"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        TANK_CAP = "SignalManipulation.additionalBCMStatusPort_tankCapOpen_nu"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.TANK_CAP: [
                "SignalManipulation.additionalBCMStatusPort_tankCapOpen_nu",
            ],
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 82 Req 2325920 2326021",
    description=(
        "This test case verifies that AUP will transition from Scanning to Init when AUPCore receives the information \
            that the fuel tank cap is open."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_82_Req_2325920_2326021_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            tank_cap = signals[Signals.Columns.TANK_CAP]
            ...
            time_threshold_2 = find_time_threshold(tank_cap, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.TANK_CAP} == 1: <br>"
                    ...
                    f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 + 10]} ({ap_states.iloc[time_threshold_2 + 10]}) <br>"
                    f"This confirms that AUP transitioned from Scanning to Init when AUPCore received the information that \
                        the fuel tank cap was open.".split()
                )
                eval_cond_1 = True
                ...
                if delay_neg_ap or delay_pos_ap:
                    if delay_neg_ap:
                        evaluation_1 = " ".join(
                            ...
                            f"{Signals.Columns.TANK_CAP} == 1: <br>"
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 - 10]} ({ap_states.iloc[time_threshold_2 - 10]}) <br>"
                            f"This means that AUPCore wasn't in Scanning state when AUPCore received the infromation that the fuel tank cap was open.".split()
                        )
                    elif delay_pos_ap:
                        evaluation_1 = " ".join(
                            ...
                            f"{Signals.Columns.TANK_CAP} == 1: <br>"
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 + 10]} ({ap_states.iloc[time_threshold_2 + 10]}) <br>"
                            f"This means that AUPCore didn't transition to Init state when AUPCore received the infromation that the fuel tank cap was open".split()
                        )
                    eval_cond_1 = False
            else:
                self.result.measured_result = FALSE
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TANK_CAP} == 1 was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_82_Req_2325920_2326021",
    description=(
        "This test case verifies that AUP will transition from Scanning to Init when AUPCore receives the information \
            that the fuel tank cap is open."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_82_Req_2325920_2326021_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_82_Req_2325920_2326021_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.additionalBCMStatusPort_tankCapOpen_nu</em> == 1 <br><br>
            </td>
        </tr>
        <tr>
            <td>
