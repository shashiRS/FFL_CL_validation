...
INT_ET3_ET4:
AP.planningCtrlPort.apPlanningSpecification == APPS_PARK_OUT_UNTIL_CRITICAL_POINT_REACHED (4)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_RemoteParallelParkOutToCritical"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Remote Parallel Park Out To Critical",
    description=(
        "This test case verifies that in case of Remote Parking out parallel the AutomatedParkingCore shall trigger that the path is only followed until the critical road-side-edge or the neighboring obstacles is cleared."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_RemoteParallelParkOutToCritical_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if time_threshold_3 is not None:
                time_threshold_4 = (
                    find_time_threshold(ap_states.iloc[time_threshold_3:], aup.AP_STATES.AP_AVG_PAUSE)
                    + time_threshold_3
                )

            if time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_1 = " ".join(
                    f"The evaluation is PASSED for the interval between {Signals.Columns.AP_STATE} being AP_AVG_ACTIVE_OUT (4) (<b>{round(time.iloc[time_threshold_3], 3)} s</b>) and when {Signals.Columns.AP_STATE} "
                    f"reaches the AP_AVG_PAUSE (5) state at timestamp <b>{round(time.iloc[time_threshold_4], 3)} s</b>. "
                    f"This confirms that the path is only followed until the critical road-side-edge or the neighboring obstacles is cleared.".split()
                )
                eval_cond_1 = True  # set the evaluation condition as TRUE
                planning_spec_interval_1 = planning_spec.iloc[time_threshold_3:time_threshold_4]
                cond_negative = (
                    planning_spec_interval_1 != aup.PLANNING_SPECIFICATION.APPS_PARK_OUT_UNTIL_CRITICAL_POINT_REACHED
                ).any()

                if cond_negative:
                    evaluation_1 = " ".join(
                        f"The evaluation is FAILED for the interval between {Signals.Columns.AP_STATE} being AP_AVG_ACTIVE_OUT (4) (<b>{round(time.iloc[time_threshold_3], 3)} s</b>) and when {Signals.Columns.AP_STATE} "
                        f"reaches the AP_AVG_PAUSE (5) state at timestamp <b>{round(time.iloc[time_threshold_4], 3)} s</b>. "
                        f"This indicates that the path was not followed until the critical roadside edge or neighboring obstacles were cleared. <br>".split()
                    )
                    eval_cond_1 = False
            else:
            ...
            if time_threshold_2 is not None:
                evaluation_3 = " ".join(
                    f"The evaluation is PASSED when {Signals.Columns.DEVICE_CM_QUANT} reaches the REM_TAP_ON_PARK_OUT (25) state at "
                    f"timestamp <b>{round(time.iloc[time_threshold_2], 3)} s</b>. This indicates that remote park out is activated.".split()
                )
                ...
                eval_cond_3 = False
                evaluation_3 = " ".join("Evaluation not possible, the remote park out is not activated.".split())
            # set the verdict of the test step
            if eval_cond_2 and eval_cond_1 and eval_cond_3:
            ...
            expected_val_3 = "AP.remoteHMIOutputPort.userActionRemoteDeviceCMQuant_nu == REM_TAP_ON_PARK_OUT (25)"
            ...
            signal_summary["T1"] = [expected_val_3, evaluation_3, verdict3]
            signal_summary["T3 - T4"] = [expected_val_1, evaluation_1, verdict1]
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_RemoteParallelParkOutToCritical",
    description=(
        "This test case verifies that in case of Remote Parking out parallel the AutomatedParkingCore shall trigger that the path is only followed until the critical road-side-edge or the neighboring obstacles is cleared."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_RemoteParallelParkOutToCritical_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_RemoteParallelParkOutToCritical_TS
    ...
    <table>
        ...
        <tr>
            <td>
                ...
                - T3-T4: <em>AP.planningCtrlPort.apPlanningSpecification </em> == APPS_PARK_OUT_UNTIL_CRITICAL_POINT_REACHED (4) <br>
            </td>
        </tr>
    </table>
