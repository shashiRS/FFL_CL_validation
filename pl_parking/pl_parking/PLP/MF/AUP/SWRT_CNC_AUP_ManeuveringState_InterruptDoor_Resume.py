...
T4:
AP.doorStatusPort.status.driver_nu == 2 && AP.hmiOutputPort.userActionHeadUnit_nu == 20
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_InterruptDoor_Resume"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Interrupt Door Resume",
    description=(
        "This test case verifies that in Maneuvering state the AutomatedParkingCore core shall provide an information that the maneuver can be continued, when ALL conditions are fulfilled - received an information that NO vehicle state deactivation condition is triggered (driver door)."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_InterruptDoor_Resume_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                ...
                evaluation_2 = " ".join(
                    ...
                    f"({aup.DOOR_DRIVER.DOOR_STATUS_OPEN}) (<b>{round(time.iloc[time_threshold_2], 3)} s</b>) with expected values. <br>"
                    f"This indicates that the vehicle correctly stopped following the trajectory and transitioned into the MAKE_SECURE state "
                    f"when the driver's door was opened, as expected.".split()
                )
                ...
                if delay_negative or delay_posistive:
                    evaluation_2 = " ".join(
                        ...
                        f" - Before the driver's door was opened (<b>{round(time.iloc[time_threshold_2 - 10], 3)} s</b>), the signal was in state "
                        f"{signals['status_drv'].iloc[time_threshold_2 - 10]} ({drv_mode_req.iloc[time_threshold_2 - 10]}). <br>"
                        f" - After the driver's door was opened (<b>{round(time.iloc[time_threshold_2 + 10], 3)} s</b>), the signal transitioned to "
            ...
            if time_threshold_4 is not None:
                evaluation_3 = " ".join(
                    f"The evaluation PASSED at the moment when {Signals.Columns.DOOR_DRIVER} == DOOR_STATUS_CLOSED ({aup.DOOR_DRIVER.DOOR_STATUS_CLOSED}) "
                    ...
                    f"This indicates that the vehicle correctly resumed following the path after the driver's door was closed and the user "
                    f"requested to continue the maneuver.".split()
                )
                ...
                if delay_negative or delay_posistive:
                    ...
                    evaluation_3 = " ".join(
                        f"The evaluation FAILED at the moment when {Signals.Columns.DOOR_DRIVER} == DOOR_STATUS_CLOSED ({aup.DOOR_DRIVER.DOOR_STATUS_CLOSED}) "
                        ...
                        f"The signal {Signals.Columns.DRV_MODE_REQ} showed inconsistent behavior: <br>"
                        f" - Before the driver's door was closed (<b>{round(time.iloc[time_threshold_4 - 10], 3)} s</b>), the signal was in state "
                        f"{signals['status_drv'].iloc[time_threshold_4 - 10]} ({drv_mode_req.iloc[time_threshold_4 - 10]}). <br>"
                        f" - After the driver's door was closed and the user requested to continue (<b>{round(time.iloc[time_threshold_4 + 10], 3)} s</b>), "
                ...
                eval_cond_et4 = False
                evaluation_3 = " ".join(
                    f"Evaluation not possible, the trigger values ({Signals.Columns.DOOR_DRIVER} == DOOR_STATUS_CLOSED (2) and/or {Signals.Columns.ACT_HEAD_UNIT} == TAP_ON_CONTINUE(20)) were never found.".split()
                )
            ...
            signal_summary["T4"] = [expected_val_2, evaluation_3, verdict3]
            remark = " ".join(
                f"Check that at moment of {Signals.Columns.DOOR_DRIVER} == (1) ± 0.1s the following "
                ...
                f"Check that at moment of {Signals.Columns.DOOR_DRIVER} == (2) & {Signals.Columns.ACT_HEAD_UNIT} == (20) ± 0.1s the following "
                f"conditions are met: <br>"
                f"at delay -0.1s: AP.trajCtrlRequestPort.drivingModeReq_nu == MAKE_SECURE (3)<br>"
            ...
            fig.layout = go.Layout(
                ...
                xaxis_title="Time[s]",
                title="Maneuvering State Interrupt Door Resume Graphical Overview",
            )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_InterruptDoor_Resume",
    description=(
        "This test case verifies that in Maneuvering state the AutomatedParkingCore core shall provide an information that the maneuver can be continued, when ALL conditions are fulfilled - received an information that NO vehicle state deactivation condition is triggered (driver door)."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_InterruptDoor_Resume_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_InterruptDoor_Resume_TS
