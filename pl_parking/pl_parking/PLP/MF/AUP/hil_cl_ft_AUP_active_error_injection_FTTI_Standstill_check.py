...
SIGNAL_DATA = "INPUT_FAILURE_ACTIVE_AUP_SS_FTTI"
...
class System_safe_state_error_injected_AUP_active_SS_FTTI(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        if trigger1 is None:
            ...
            preconditions = (
                "<br>Evaluation stopped!"
                "<br>At least one of the following situations happened before error injection "
                f"<br>{signal_name['Hold_request']} was True. "
                f"<br>{signal_name['General_screen']} was set to Diag Error ({constants.HilCl.Hmi.APHMIGeneralScreen.DIAG_ERROR})."
                f"<br>{signal_name['Gear_switch_request']} was True. "
...
@testcase_definition(
    name="System transition to Safe State mode (within 700 miliseconds) while AUP was active",
...
class SafeStateHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            System_safe_state_error_injected_AUP_active_SS_FTTI,
        ]
