...
T2:
"AP.LaDMCStatusPort.driverIntervention_nu" == 1
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_41_Req_2325962_2326049"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 41 Req 2325962 2326049",
    description=(
        "This test case verifies that if AUP receives the information that a driver intervention and it is detected\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_41_Req_2325962_2326049_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            driver_intervention = signals[Signals.Columns.DRIVER_INTERVENTION]
            ...
            signals["status_intervention"] = driver_intervention.apply(
                lambda x: "Driver Intervention" if x == 1 else "No Driver Intervention"
            )
            ...
            time_threshold_2 = find_time_threshold(driver_intervention, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.DRIVER_INTERVENTION}</b> == 1(intervention) <br>"
                    ...
                    f"This confirms that Remote Control went from active to paused when AUP detected an intervention from the driver.".split()
                )
                ...
                if delay_neg_rm or delay_pos_rm or delay_neg_screen:
                    if delay_neg_rm and delay_pos_rm:
                        evaluation_1 = " ".join(
                            f"The evaluation is FAILED because <b>before</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                            f"the signal <b>{Signals.Columns.DRIVER_INTERVENTION}</b> == 1(intervention) <br>"
                            f"- the signal {Signals.Columns.AP_STATE} wasn't in AP_AVG_ACTIVE_IN (3) state<br><br>"
                            f"- the signal {Signals.Columns.SCREEN_NU} wasn't in REMOTE_APP_ACTIVE (3) state<br> AND <b>after</b> that timestamp<br>"
                            f"- the signal {Signals.Columns.AP_STATE} didn't transition to AP_AVG_PAUSE (5) state.".split()
                        )
                    elif delay_neg_rm or delay_neg_screen:
                        evaluation_1 = " ".join(
                            f"The evaluation is FAILED because <b>before</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                            f"the signal <b>{Signals.Columns.DRIVER_INTERVENTION}</b> == 1(intervention) <br>"
                            f"- the signal {Signals.Columns.AP_STATE} wasn't in RM_AVG_ACTIVE (3) state OR<br>"
                            f"- the signal {Signals.Columns.SCREEN_NU} wasn't in REMOTE_APP_ACTIVE (3) state.<br>"
                            f"This means that before AUP detected a driver intervention, the RC wasn't active.".split()
                        )
                    elif delay_pos_rm:
                        evaluation_1 = " ".join(
                            f"The evaluation is FAILED because <b>after</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                            f"the signal <b>{Signals.Columns.DRIVER_INTERVENTION}</b> == 1(intervention) <br>"
                            f"- the signal {Signals.Columns.AP_STATE} didn't transition to AP_AVG_PAUSE (5) state.<br>This means that "
                            f"after AUP detected a driver intervention, the RC wasn't paused.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.DRIVER_INTERVENTION}  == 1 was never found.".split()
                )

            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED because between the interval given by the signal <b>{Signals.Columns.DRIVER_INTERVENTION}</b> "
                    f"== 1 and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)}  "
                    ...
                    f"This confirms that after the RC is in paused state (due to a driver's intervention) it goes into finished state once.".split()
                )
                ...
                if total_rm_finished != 1:
                    evaluation_2 = " ".join(
                        f"The evaluation is FAILED because between the interval given by the signal <b>{Signals.Columns.DRIVER_INTERVENTION}</b>"
                        f" == 1 and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)}"
                        f"- {round(time.iloc[time_threshold_3], 3)})</b>:<br>- the signal {Signals.Columns.AP_STATE} doesn't reach "
                        f"the AP_AVG_FINISHED (8) state once.<br>This means that after the RC is in paused state (due to a driver's intervention)"
                        f" it didn't go into finished state once.".split()
                    )
                eval_cond_2 = False
            else:
                # create an evaluation text for the moment when the threshold isn't found, set test_result to NOT_ASSESSED
            ...
            fig.add_trace(
                go.Scatter(
                    x=time,
                    y=driver_intervention,
                    mode="lines",
                    name=Signals.Columns.DRIVER_INTERVENTION,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_intervention"],
                )
            )
            fig.add_trace(
                go.Scatter(
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_41_Req_2325962_2326049",
    description=(
        "This test case verifies that if AUP receives the information about a driver's intervention and it is detected\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_Table_41_Req_2325962_2326049_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_41_Req_2325962_2326049_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>AP.LaDMCStatusPort.driverIntervention_nu</em> = 1<br>
