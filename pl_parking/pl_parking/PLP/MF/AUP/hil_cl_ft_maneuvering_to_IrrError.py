"""Check the transition from Maneuvering to Irreversible Error."""
...
SIGNAL_DATA = "MANEUVERING_TO_IRRERROR"
...
@teststep_definition(
    step_number=1,
    name="Transition from Maneuvering state to Irreversible Error",
    description="Check the behavior of the system in case a irreversible error is inserted during the active state",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class ManeuverToIrrErrorCheck(TestStep):
    """ManeuverToIrrErrorCheck Test Step."""
    ...
    def process(self, **kwargs):
                ...
                for i in range(t1_idx,t2_idx):
                    if HMI_Info[i] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING:
                        ...
                        evaluation1 = " ".join(
                            ...
                            f" AVG not in Maneuvering state({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}) before inserting the error.".split()
                        )
                        break
        ...
        signal_summary["Check the reaction of the system in case a irreversible error is inserted into the system while Maneuvering state"] = evaluation1
...
@testcase_definition(
    name="AP state transition Maneuvering to IrrError",
    description="The AP funtion shall switch to Irreversible Error from Maneuvering after the error is inserted",
    doors_url="https://jazz.conti.de/rm4/resources/BI_-dT24eHDEe6Vytg2Rffy1A?oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324"
)
class ManeuverToIrrError(TestCase):
    """ManeuverToIrrError Test Case."""
