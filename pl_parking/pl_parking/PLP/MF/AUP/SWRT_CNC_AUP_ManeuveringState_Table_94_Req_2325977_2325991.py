...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_94_Req_2325977_2325991"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        VEH_COMM = "SignalManipulation.healthVector_vehicleCommunicationError"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 94 Req 2325977 2325991",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP irreversible error \
            (host platform has vehicle communication error) then the Automated Parking Core shall transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_94_Req_2325977_2325991_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            veh_comm = signals[Signals.Columns.VEH_COMM]
            ...
            time_threshold_2 = find_time_threshold(veh_comm, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"signal {Signals.Columns.VEH_COMM} == 1:<br>"
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.VEH_COMM} == 1 was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_94_Req_2325977_2325991",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP irreversible error \
            (host platform has vehicle communication error) then the Automated Parking Core shall transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_94_Req_2325977_2325991_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_94_Req_2325977_2325991_TS
