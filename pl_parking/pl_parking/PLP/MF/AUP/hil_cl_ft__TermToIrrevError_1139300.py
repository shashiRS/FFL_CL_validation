...
REGISTER = "TERM_TO_IRREV_ERROR"


@teststep_definition(
    step_number=1,
    name="TERM_TO_IRREV_ERROR",
...
class CheckTermToIrrevError(TestStep):
    """CheckTermToIrrevError Test Step."""
    ...
    def process(self, **kwargs):
        ...
        parking_ok_value = constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SUCCESS
        ...
        step_key = _strip_str("'Parking successful' phase shall be observable within the measurement")
        parking_ok_starts_subwindow = support.signal_equals(ap_state_signal, parking_ok_value)

        irreversible_error_detected_time = None
        error_injection_starts_time = None

        if not parking_ok_starts_subwindow:
            evaluation_steps[step_key] = _strip_str(
                "Failed; 'Parking successful' phase not registered during the measurement"
            )
            result_aggregator.append(False)
        else:
            parking_ok_starts_idx = parking_ok_starts_subwindow[0]
            highlight_eval_area_start = time_signal_in_sec[parking_ok_starts_idx]
            ...
            evaluation_steps[step_key] = _strip_str(
                f"'Parking successful' phase detected starting from {highlight_eval_area_start}[sec]<br/>(highlighted in summary plot)"
            )
            ...
            frame_timeout_signal = frame_timeout_signal[parking_ok_starts_idx:]
            ap_state_signal = ap_state_signal[parking_ok_starts_idx:]
            time_signal_in_sec = time_signal_in_sec[parking_ok_starts_idx:]
...
class TermToIrrevError(TestCase):
    """TermToIrrevError Test Case."""
