"""Check the behavior of the system after the remote controller is used by the driver outside the car and ignition fails."""
import logging
import os
import sys
...
SIGNAL_DATA = "RCOUTSIDEWAKEUP_TO_INIT_IGN_FAILS"
...
@teststep_definition(
    name="Transition mode check - RC Outside WakeUp to Init, Ignition failed",
    description="Check the transition from RC Outside WakeUp to Init after the Ignition is turned OFF.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RCOutsideWakeUpToInitCheck(TestStep):
    """RCOutsideWakeUpToInitCheck Test Step."""
    ...
    def process(self, **kwargs):
            ...
            if t2_idx is not None:
                ...
                if eval_cond[0] is not False:
                    ...
                    if t3_idx is not None:
                        ...
                        if HMI_Info[t3_idx] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE:
                            test_result = fc.FAIL
                            eval_cond = [False] * 1
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED,"
                                f" AVG not in Init state after Ignition has been turned OFF( != {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE}).".split()
                            )
                    else:
                        test_result = fc.FAIL
                        eval_cond = [False] * 1
                    ...
                    evaluation1 = " ".join(
                                f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED,"
                                f" AVG didn't had a constant WakeUp state before turning the Ignition OFF (!={constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_BEHAVIOR_INACTIVE}).".split()
                            )
            else:
                test_result = fc.FAIL
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                    f"TC Failed because the Ignition has not been turned OFF ({signal_name['Ignition']} != 0) after WakeUp state(AP[{constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_BEHAVIOR_INACTIVE}])".split()
                )
        else:
            test_result = fc.FAIL
            eval_cond = [False] * 1
        ...
        signal_summary[
            "Check the reaction of the system during WakeUp state if the Ignition failed"
        ] = evaluation1
...
@testcase_definition(
    name="RC Outside WakeUp to Init transition.",
    description="The function shall switch from RC Outside Wake Up to Init mode if turning on ignition failed.",
    doors_url="https://jazz.conti.de/rm4/resources/BI_E1rzYjHqEe6mrdm2_agUYg?oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F26252",
)
class RCOutsideWakeUpToInit(TestCase):
    """RCOutsideWakeUpToInit Test Case."""
