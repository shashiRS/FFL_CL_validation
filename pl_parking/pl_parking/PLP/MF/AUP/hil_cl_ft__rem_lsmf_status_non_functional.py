"""The remote controller HMI shall display remote controlled LSMF status information to the driver: non-functional"""
...
SIGNAL_DATA = "LSMF_REM_INFO_NON_FUNCTIONAL"
...
@teststep_definition(
    name="Information about actual remote controlled LSMF state (non functional) on remmote HMI",
    description="Check information on remote HMI about actual state of remote controlled LSMF.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RemLsmfStatNonFuncCheck(TestStep):
    """RemLsmfStatNonFuncCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        t_start_requset_idx = None
        t_issue_injected_idx = None

        evaluation1 = " ".join(
            f"The evaluation of {signal_name['Rem_general_screen']} signal is PASSED, signals shows"
            f" INTERNAL_SYSTEM_ERROR ({constants.HilCl.Hmi.APHMIGeneralMessageRemote.INTERNAL_SYSTEM_ERROR}) state when"
            " LSMF controll is non functional because of irreversible error.".split()
        )
        ...
        if t_device_ready_idx is not None:
            # Find start maneuvering request from driver
            for cnt in range(t_device_ready_idx, len(rem_user_act_sig)):
                if rem_user_act_sig[cnt] == constants.HilCl.Hmi.RemoteCommands.TAP_ON_START_PARKING:
                    t_start_requset_idx = cnt
                    break

            if t_start_requset_idx is not None:

                # Find issue injection
                for cnt in range(t_start_requset_idx, len(issue_state_sig)):
                    if issue_state_sig[cnt] == constants.HilCl.CarMaker.UssError.TIMEOUT_ENABLED:
                        t_issue_injected_idx = cnt
                        break

                if t_issue_injected_idx is not None:
                    # Check signal states
                    eval_cond = [True] * 1
                    counter = 0
                    ...
                    states_dict = HilClFuntions.States(
                        rem_gen_message_sig, t_issue_injected_idx - 1, len(rem_gen_message_sig), 1
                    )  # t_issue_injected_idx - 1 to skip not know system reaction time
                    ...
                    for key in states_dict:
                        if counter == 1:
                            ...
                            if key < t_start_requset_idx:
                                evaluation1 = " ".join(
                                    f"The evaluation of {signal_name['Rem_general_message']} signal is FAILED, signal switches to {actual_value} ({actual_number}) at {time_signal[key]} us"
                                    f" before issue injection event (at {time_signal[t_issue_injected_idx]} us).".split()
                                )
                                eval_cond[0] = False
                                break

                            if states_dict[key] != constants.HilCl.Hmi.APHMIGeneralMessageRemote.INTERNAL_SYSTEM_ERROR:
                                evaluation1 = " ".join(
                                    f"The evaluation of {signal_name['Rem_general_message']} signal is FAILED, state of signal is {actual_value} ({actual_number}) at {time_signal[key]} us"
                                    f" but requiered mode is INTERNAL_SYSTEM_ERROR ({constants.HilCl.Hmi.APHMIGeneralMessageRemote.INTERNAL_SYSTEM_ERROR}).".split()
                                )
                                eval_cond[0] = False
                                break
                            counter += 1
                ...
                evaluation1 = " ".join(
                    ...
                    f" Signal never switched to TAP_ON_START_PARKING ({constants.HilCl.Hmi.RemoteCommands.TAP_ON_START_PARKING})."
                    " Start of maneuver was never requested by driver via remote device."
                    " It is not possible to continue evaluation in this case. This event is needed to evaluation.".split()
                )
        ...
        signal_summary["Remote HMI shall show maneuver non functional state of LSMF"] = evaluation1
...
@testcase_definition(
    name="Remote controlled LSMF status state on remoted HMI, Non functional",
    description="The remote controller HMI shall display remote controlled LSMF status information to the driver: non functional",
)
class RemLsmfStatNonFunc(TestCase):
    """RemLsmfStatNonFunc Test Case."""
