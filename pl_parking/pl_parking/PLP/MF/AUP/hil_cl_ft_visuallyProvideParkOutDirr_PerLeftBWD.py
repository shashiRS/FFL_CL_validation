...
SIGNAL_DATA = "DIRRECTION_PERPENDICULAR_LEFT_BWD"
...
@teststep_definition(
    step_number=1,
    name="Perpendicular backward Left Indicator during Scanning State",
    description="Check the behavior of the system after the driver select the left backward dirrection from HMI in case ego veh is on a perpendicular parking slot in Scanning Out state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class ParkOutDirrPerpBWDLeftCheck(TestStep):
    """ParkOutDirrPerpBWDLeftCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        eval_cond = [True] * 1
        evaluation1 = " ".join(
            f"The evaluation the left backward dirrection indicator signal is True, the system is providyng a constant visualisation to the intended parkingOut dirrection ({signal_name['AVG_Type']} =="
            f" {constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PERPENDICULAR_LEFT_BWD}).".split()
        )
        ...
        if t1_idx is not None:
            ...
            for i in range(t1_idx,len(HMI_Info)):
                if user_act_sig[i] ==  constants.HilCl.Hmi.Command.TAP_ON_PARKING_SPACE_LEFT_1:
                    t2_idx = i
                    break
            ...
            if t2_idx is not None:
                ...
                if t3_idx is not None:
                    ...
                    for y in range(t3_idx,len(HMI_Info)):
                        if avg_type[y] != constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PERPENDICULAR_LEFT_BWD:
                            test_result = fc.FAIL
                            eval_cond = [False] * 1
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['AVG_Type']} signal is FAILED,"
                                f" the system didn't provide a constant visualisation of the left backward dirrection indicator ( != {constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PERPENDICULAR_LEFT_BWD})."
                ...
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                f"TC Failed because the driver didn't select the parkOut dirrection ({signal_name['User_action']} != {constants.HilCl.Hmi.Command.TAP_ON_PARKING_SPACE_LEFT_1})".split()
                )
        else:
            eval_cond = [False] * 1
            evaluation1 = " ".join(
        ...
        signal_summary["Check the reaction of the system related to the dirrection indicator during ParkingOut state - perpendicular left backward."] = evaluation1
...
@testcase_definition(
    name="Dirrection Indicator in which the system shall start the parking out maneuver during scanning state - perpendicular backward left",
    description="The AP funtion shall continuously show the Left Backward Dirrection Indicator in which the system shall start the parking out maneuver during scanning state",
...
class ParkOutDirrPerpBWDLeft(TestCase):
    """ParkOutDirrPerpBWDLeft Test Case."""
