...
T2:
"SignalManipulation.engineCtrlStatusPort_engineOn_nu" == 0
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_44_Req_2325962_2326047"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        IGNITION = "SignalManipulation.additionalBCMStatusPort_ignitionOn_nu"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 44 Req 2325962 2326047",
    description=(
        "This test case verifies that if AUP receives the information that Main Start/Stop Button was operated and it is detected\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_44_Req_2325962_2326047_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            ignition = signals[Signals.Columns.IGNITION]
            ...
            signals["status_ignition"] = ignition.apply(
                lambda x: "Ignition detected" if x == 0 else "No Ignition detected"
            )
            ...
            time_threshold_2 = find_time_threshold(ignition, 0)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.IGNITION}</b> == 0 <br>"
                    ...
                    f"This confirms that Remote Control went from active to paused when AUP detected the trigger.".split()
                )
                ...
                if delay_neg_rm or delay_pos_rm or delay_neg_screen:
                    if delay_neg_rm and delay_pos_rm and delay_neg_screen:
                        evaluation_1 = " ".join(
                            ...
                            f"the signal <b>{Signals.Columns.IGNITION}</b> == 1(intervention) <br>"
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"This means that before AUP detected a trigger, the RC wasn't active.".split()
                        )
                    elif delay_pos_rm:
                        evaluation_1 = " ".join(
                            ...
                            f"after AUP detected the trigger, the RC wasn't paused.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.IGNITION}  == 0 was never found.".split()
                )

            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED because between the interval given by the signal <b>{Signals.Columns.IGNITION}</b> "
                    f"== 0 and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)}  "
                    ...
                    f"This confirms that after the RC is in paused state it goes into finished state once.".split()
                )
                ...
                if total_rm_finished != 1:
                    evaluation_2 = " ".join(
                        f"The evaluation is FAILED because between the interval given by the signal <b>{Signals.Columns.IGNITION}</b>"
                        f" == 0 and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)}"
                        f"- {round(time.iloc[time_threshold_3], 3)})</b>:<br>- the signal {Signals.Columns.AP_STATE} doesn't reach "
                        f"the AP_AVG_FINISHED (8) state once.<br>This means that after the RC is in paused state "
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_44_Req_2325962_2326047",
    description=(
        "This test case verifies that if AUP receives the information that Main Start/Stop Button was operated and it is detected\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_44_Req_2325962_2326047_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_44_Req_2325962_2326047_TS
