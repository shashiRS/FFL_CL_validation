"""executing a perpendicular parking in maneuver so that the ego vehicle reaches the final parking position with a maximum amount of strokes"""
...
__author__ = "uif99923"
SIGNAL_DATA = "PARK_IN_MAX_MAN_TERM_NUM_OF_STROKES_PERP"
...
@teststep_definition(
    name="Maximum number of strokes in case of perpendicular parking",
    description="Check state change if number of strokes exceeds the limit",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class MaxNumOfStrokesPerpParkInCheck(TestStep):
    """MaxNumOfStrokesPerpParkInCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        for cnt in range(0, len(state_on_hmi_sig)):
            if state_on_hmi_sig[cnt] == constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SCANNING_IN:
                t_scanning_in_start_idx = cnt
                break

        if t_scanning_in_start_idx is not None:
            ...
            if t_start_man_idx and t_end_man_idx:
                ...
                if number_of_strokes > constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_PERP_NU:
                    test_result = fc.FAIL

                    step_key = " ".join(
                        f"System executed parking with {number_of_strokes} stroke(s),"
                        f" Limit: {constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_PERP_NU}".split()
                    )
                    evaluation_steps[step_key] = " ".join(
                        f"The evaluation is FAILED."
                        f" Number of strokes is {number_of_strokes} at end of maneuver,"
                        f" this value is greater than AP_G_MAX_NUM_STROKES_PERP_NU ({constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_PERP_NU}) and"
                    ...
                    evaluation_steps[step_key] = " ".join(
                        ...
                        f" Number of strokes is {number_of_strokes} at end of maneuver,"
                        f" this value is not greater than AP_G_MAX_NUM_STROKES_PERP_NU ({constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_PERP_NU}) and"
                        f" {signal_name['State_on_HMI']} signal is PPC_SUCCESS ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SUCCESS})"
                        f" at end of active maneuver ({time_signal[t_end_man_idx]} us)".split()
                    )
            ...
            if t_end_man_idx is None:
                ...
                step_key = " ".join(
                    ...
                    f" {signal_name['State_on_HMI']} switches out from"
                    f" PPC_PERFORM_PARKING ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING})".split()
                )
                evaluation_steps[step_key] = " ".join(
            ...
            step_key = " ".join(
                f"System shall finish Maneuvering mode:"
                f" {signal_name['State_on_HMI']} switches into"
                f" PPC_SCANNING_IN ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SCANNING_IN})".split()
            )
            evaluation_steps[step_key] = " ".join(
                f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED, signal never switch into PPC_SCANNING_IN ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SCANNING_IN})."
                " It is not possible to continue evaluation in this case. This event is needed to evaluation.".split()
            )
...
@testcase_definition(
    name="Parking In: Maneuvering to Terminate, Number of strokes exceeds, Perpendicular parking ",
    description="Perpendicular parking slot: The AP function shall be capable of executing a parking in maneuver so that the ego vehicle reaches the final parking in pose with a maximum amount of strokes for the type of the selected parking slot "
    f"<br>Maximum number of strokes:"
    f"<br>Perpendicular parking: {constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_PERP_NU}",
)
class MaxNumOfStrokesPerpParkIn(TestCase):
    """MaxNumOfStrokesPerpParkIn Test Case."""
    ...
    def test_steps(self):
        ...
        return [
            MaxNumOfStrokesPerpParkInCheck,
        ]
