...
T2: // right camera becomes unreliable
SignalManipulation.healthVector_rightCamVisionUnreliable = 1

T3: // heal condition
SignalManipulation.healthVector_rightCamVisionUnreliable = 0
...
AP.apUserInformationPort.generalUserInformation_nu == RIGHT_CAM_VISION_UNRELIABLE (45)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_54_Req_2325842_2325997_2325843"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        RIGHT_CAM = "SignalManipulation.healthVector_rightCamVisionUnreliable"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 54 Req 2325842 2325997 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the right camera vision is unreliable, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_54_Req_2325842_2325997_2325843_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"This confirms that AUPCore received the information that the right camera vision was unreliable and then it went into error <br>"
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before right camera was detected unreliable and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before right camera was detected unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due to right camera unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
            ...
            AP.apUserInformationPort.generalUserInformation_nu == RIGHT_CAM_VISION_UNRELIABLE (45)<br>"""
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_54_Req_2325842_2325997_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the right camera vision is unreliable, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_54_Req_2325842_2325997_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_54_Req_2325842_2325997_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //right camera becomes unreliable<br>
                    <em>SignalManipulation.healthVector_rightCamVisionUnreliable</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_rightCamVisionUnreliable</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == RIGHT_CAM_VISION_UNRELIABLE (45) <br>
                T2 - T4:<br>
