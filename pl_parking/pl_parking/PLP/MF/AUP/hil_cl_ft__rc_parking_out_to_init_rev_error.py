"""Mode transition, RC Outside Parking Out to Init, Reversible error"""
...
SIGNAL_DATA = "RC_PARK_OUTTO_INIT_REV_ERROR"
...
@teststep_definition(
    name="RC reaction for reversible error in RC Outside Parking Out state",
...
class CommonRcGetOffToInitRevErrorCheck(TestStep):
    ...
    def process(self, **kwargs):
        ...
        if t_begining_rc_idx is not None:
            ...
            # Find ABS activity
            for cnt in range(t_begining_rc_idx, len(abs_sig)):
            ...
            if t_abs_act_idx is not None:
                ...
                states_dict = HilClFuntions.States(state_on_hmi_sig, t_begining_rc_idx, len(state_on_hmi_sig), 1)
                ...
                for key in states_dict:
                    if counter == 1:
                        ...
                        if (
                            ...
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['State_on_HMI']} signal is PASSED, state of signal is {actual_value} ({actual_number}) at {time_signal[key]} us and this is the requiered state.".split()
                            )
                            break
                    else:
                        counter += 1
        ...
        signal_summary["Required state change: RC Outside Parking Out to Init. Reason: Reversible error"] = evaluation1
...
@testcase_definition(
    name="Mode transition, RC Outside Parking Out to Init, Reversible error detected",
    description="The function shall switch from RC Outside Parking Out to Init mode, if the function has a reversible error ",
)
