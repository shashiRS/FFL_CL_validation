...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_155_Req_2711728_2326035"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 155 Req 2711728 2326035",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Terminate state and it receives the information that a\
            reversible error is present (ESC intervention is active for longer than AP_G_ESC_TIME_THRESH_S (0.5s))\
                then it should transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_155_Req_2711728_2326035_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            signals["status_esc_state"] = esc_state.apply(lambda x: fh.get_status_label(x, aup.ESC_STATE))
            ...
            time_threshold_2 = find_time_threshold(esc_state, aup.ESC_STATE.ESC_ACTIVE)
            time_threshold_3 = int(time_threshold_2 + aup.AP_G_ESC_TIME_THRESH_S * 100)

            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    f"The trigger value {Signals.Columns.ESC_STATE} == ESC_ACTIVE (1) has been found.".split()
                )
                eval_cond_1 = True
            else:
            ...
            if time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    ...
                    f"{Signals.Columns.ESC_STATE} == ESC_ACTIVE (1): <br>"
                ...
                if delay_neg_ap or delay_neg_core:
                    ...
                    evaluation_2 = " ".join(
                        ...
                        f"{Signals.Columns.ESC_STATE} == 0: <br>"
                        f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) <br>"
                        f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) <br>"
                        f"- the signal {Signals.Columns.VAR_PPC} == {signals['status_var_ppc'].iloc[time_threshold_2 + 10]} ({var_ppc.iloc[time_threshold_2 + 10]}) <br><br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = (
                    "Evaluation not possible, the trigger value T2 + AP_G_ESC_TIME_THRESH_S (0.5s) was never found."
                )
            ...
            expected_val_1 = "SignalManipulation.escInformationPort_escState_nu == ESC_ACTIVE (1)"
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_155_Req_2711728_2326035",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Terminate state and it receives the information that a\
            reversible error is present (ESC intervention is active for longer than AP_G_ESC_TIME_THRESH_S (0.5s))\
                then it should transition to Error state."
...
class SWRT_CNC_AUP_ManeuveringState_Table_155_Req_2711728_2326035_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_155_Req_2711728_2326035_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.escInformationPort_escState_nu</em> == ESC_ACTIVE (1)<br>
                T3:<br>
                    T2 + AP_G_ESC_TIME_THRESH_S (0.5s)<br><br>
            </td>
        </tr>
        <tr>
            <td>
