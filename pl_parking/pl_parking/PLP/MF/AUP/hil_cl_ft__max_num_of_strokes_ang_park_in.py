"""executing an angular parking in maneuver so that the ego vehicle reaches the final parking position with a maximum amount of strokes"""
...
__author__ = "uif99923"
SIGNAL_DATA = "PARK_IN_MAX_MAN_TERM_NUM_OF_STROKES_ANGULAR"
...
class MaxNumOfStrokesAngParkInCheck(TestStep):
    """MaxNumOfStrokesAngParkInCheck Test Step."""
            ...
            if t_start_man_idx and t_end_man_idx:
                ...
                if number_of_strokes > constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_ANG_NU:
                    ...
                    evaluation_steps[step_key] = " ".join(
                        ...
                        f" this value is greater than AP_G_MAX_NUM_STROKES_ANG_NU ({constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_ANG_NU}) and"
                    ...
                    evaluation_steps[step_key] = " ".join(
                        ...
                        f" Number of strokes is {number_of_strokes} at end of maneuver,"
                        f" this value is not greater than AP_G_MAX_NUM_STROKES_ANG_NU ({constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_ANG_NU}) and"
                        f" {signal_name['State_on_HMI']} signal is PPC_SUCCESS ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SUCCESS})"
                        f" at end of active maneuver ({time_signal[t_end_man_idx]} us)".split()
                    )
...
@testcase_definition(
    name="Parking In: Maneuvering to Terminate, Number of strokes exceeds, Angular parking ",
    description="Angular parking slot: The AP function shall be capable of executing a parking in maneuver so that the ego vehicle reaches the final parking in pose with a maximum amount of strokes for the type of the selected parking slot "
    f"<br>Maximum number of strokes:"
    f"<br>Angular parking: {constants.HilCl.ApThreshold.AP_G_MAX_NUM_STROKES_ANG_NU}",
)
class MaxNumOfStrokesAngParkIn(TestCase):
    """MaxNumOfStrokesAngParkIn Test Case."""
