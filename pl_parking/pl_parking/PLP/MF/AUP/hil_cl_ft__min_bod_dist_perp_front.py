...
SIGNAL_DATA = "BODY DISTANCE HIGH OBSTACLE PERPENDICULAR FRONT SIDE"
...
@teststep_definition(
    ...
    description="Body distance from high obstacle at final parking pose perpendicular front side.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupAPMinBodDistPerpFrontCheck(TestStep):
    """AupAPMinBodDistPerpFrontCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_start_idx and t_success_idx:
            ego_pos = ego_y[t_success_idx] + constants.HilCl.CarShape.CAR_R_AXLE_TO_FRONT + constants.HilCl.CarShape.CAR_R_AXLE_TO_HITCH
            obj_pos = object_y[t_success_idx]
            if obj_pos - ego_pos > constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_SSIDE_HIGH_OBST_M:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    "The test for checking minimal body distance between front side and high object for perpendicular parking is PASSED.".split()
                )
            else:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The test for checking minimal body distance between front side and high object for perpendicular parking is FAILED. The final parking pose was too close to the high object, it was"
            ...
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The test for checking minimal body distance between front side and high object for perpendicular parking is FAILED.".split()
            )
...
@testcase_definition(
    name="Minimum body distance from high object perpendicular front",
    description="Body distance from high obstacle at final parking pose perpendicular front side.",
)
class AupAPMinBodDistPerpFront(TestCase):
    """AupAPMinBodDistPerpFront Test Case."""
