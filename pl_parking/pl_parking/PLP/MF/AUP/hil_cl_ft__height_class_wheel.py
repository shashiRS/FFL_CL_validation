"""Check wheel traverasble object classification."""
...
SIGNAL_DATA = "HEIGHT_CLASSIFICATION_WHEEL_TRAV"
...
@teststep_definition(
    ...
    description="Check wheel traverasble object classification",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class CommonWheelTravCheck(TestStep):
    """CommonWheelTravCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if len(detections_idx) > 0:
            ...
            evaluation1 = " ".join(
                "The evaluation is PASSED, AP function considers obstacle as wheel traversable static object.".split()
            )

            # Check type of detected object
            for item in detections_idx:
                if height_class_sig[item] != constants.HilCl.HeightClases.WHEEL_TRAVERSABLE:
                    evaluation1 = " ".join(
                        f"The evaluation is FAILED, AP function does not consider"
                        f" static object as wheel traversable object at {time_sig[item]} us.".split()
                    )
                    eval_cond[0] = False
                    break
        else:
        ...
        signal_summary["Height classification: Wheel traversable"] = evaluation1
...
@testcase_definition(
    ...
    description=f"The AP Function shall consider static objects higher than"
    f" {constants.HilCl.HeightLimits.AP_G_MIN_HEIGHT_OBSTACLE_M} m"
    f" but lower or equal than"
    f" {constants.HilCl.HeightLimits.AP_G_MAX_HEIGHT_WHEEL_TRAVER_M} m as wheel traversable.",
)
class FtCommon(TestCase):
    """CommonWheelTrav Test Case."""

    custom_report = MfHilClCustomTestcaseReport

    # Important iformation:
    # Please use this script only with TestRun HIL_COMMON_HeightClassification_Wheel_ID1073776

    @property
    def test_steps(self):
        """Define the test steps."""
        return [
            CommonWheelTravCheck,
        ]
