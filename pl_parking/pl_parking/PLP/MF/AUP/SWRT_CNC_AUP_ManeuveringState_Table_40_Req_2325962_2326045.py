...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_40_Req_2325962_2326045"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 40 Req 2325962 2326045",
    description=(
        "This test case verifies that if AUP receives the information that a seat is occupied and it is detected\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_40_Req_2325962_2326045_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            seat_occ = signals[Signals.Columns.SEAT_OCC]
            ...
            signals["status_seat_occ"] = seat_occ.apply(lambda x: fh.get_status_label(x, aup.SEAT_OCCUPANCY))
            ...
            time_threshold_2 = find_time_threshold(seat_occ, aup.SEAT_OCCUPANCY.OCC_STATUS_OCCUPIED)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.SEAT_OCC}</b> == OCC_STATUS_OCCUPIED (3) <br>"
                    ...
                    f"This confirms that Remote Control went from active to paused when AUP detected that a seat was occupied.".split()
                )
                ...
                if delay_neg_rm or delay_pos_rm or delay_neg_screen:
                    if delay_neg_rm and delay_pos_rm and delay_neg_screen:
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"This means that before AUP detected that a seat was occupied, the RC wasn't active.".split()
                        )
                    elif delay_pos_rm:
                        evaluation_1 = " ".join(
                            ...
                            f"after AUP detected that a seat was occupied, the RC wasn't paused.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.SEAT_OCC}  == OCC_STATUS_OCCUPIED (3) was never found.".split()
                )

            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED because between the interval given by the signal <b>{Signals.Columns.SEAT_OCC}</b> "
                    f"== OCC_STATUS_OCCUPIED (3) and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)}  "
                    ...
                    f"This confirms that after the RC is in paused state (due to a seat being occupied) it goes into finished state once.".split()
                )
                ...
                if total_rm_finished != 1:
                    evaluation_2 = " ".join(
                        f"The evaluation is FAILED because between the interval given by the signal <b>{Signals.Columns.SEAT_OCC}</b>"
                        f" == OCC_STATUS_OCCUPIED (3) and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)}"
                        f"- {round(time.iloc[time_threshold_3], 3)})</b>:<br>- the signal {Signals.Columns.AP_STATE} doesn't reach "
                        f"the AP_AVG_FINISHED (8) state once.<br>This means that after the RC is in paused state (due to a seat being occupied)"
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_40_Req_2325962_2326045",
    description=(
        "This test case verifies that if AUP receives the information that a seat is occupied and it is detected\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_40_Req_2325962_2326045_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_40_Req_2325962_2326045_TS
