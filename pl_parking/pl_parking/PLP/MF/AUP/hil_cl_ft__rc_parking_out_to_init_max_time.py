"""RC Outside Parking Out to Init, Timeout"""
...
SIGNAL_DATA = "RC_PARK_OUTTO_INIT_TIMEOUT"
...
@teststep_definition(
    name="System reaction on too long waitingtime",
    description="Check system reaction if driver does not start maneuver more than AP_G_MAX_WAIT_TIME_REM_S",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class CommonRcGetOffToInitTimeCheck(TestStep):
    """CommonRcGetOffToInitTimeCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_scanning_out_idx is not None:
            # Find when tap on park out
            for cnt in range(t_scanning_out_idx, len(rem_ser_act_sig)):
                if rem_ser_act_sig[cnt] == constants.HilCl.Hmi.RemCommand.REM_TAP_ON_PARK_OUT:
                    t_tap_park_out_idx = cnt
                    break

            if t_tap_park_out_idx is not None:
                ...
                # Collect states after PPC_SCANNING_OUT
                ...
                for key in states_dict:
                    if counter == 1:
                        ...
                        delta_time = time_signal[key] - time_signal[t_tap_park_out_idx]

                        if key < t_tap_park_out_idx:
                            evaluation1 = " ".join(
                                ...
                                f" before REM_TAP_ON_PARK_OUT event.".split()
                            )
                            eval_cond[0] = False
                            break

                        if delta_time < constants.HilCl.ApThreshold.AP_G_MAX_WAIT_TIME_REM_S * 1e6:
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED, signal switches to {actual_value} ({actual_number}) at {time_signal[key]} us."
                                f" State checnged after REM_TAP_ON_PARK_OUT event but earlier then the AP_G_MAX_WAIT_TIME_REM_S ({constants.HilCl.ApThreshold.AP_G_MAX_WAIT_TIME_REM_S} sec).".split()
                            )
                            eval_cond[0] = False
                            break
                ...
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['Rem_user_act']} signal is FAILED, signal never switched to REM_TAP_ON_PARK_OUT ({constants.HilCl.Hmi.RemCommand.REM_TAP_ON_PARK_OUT})."
        ...
        signal_summary["Required state change: RC Outside Parking Out to Init. Reason: Timeout"] = evaluation1
...
@testcase_definition(
    name="Mode transition, RC Outside Parking Out to Init, Timeout",
    description=f"The function was in RC Outside Parking Out mode longer than AP_G_MAX_WAIT_TIME_REM_S ({constants.HilCl.ApThreshold.AP_G_MAX_WAIT_TIME_REM_S}) sec"
    " without the driver trying to start the maneuver with the remote device.",
)
class CommonRcGetOffToInitTime(TestCase):
    """CommonRcGetOffToInitTime Test Case."""
