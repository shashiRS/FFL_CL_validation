...
T2:
SignalManipulation.trailerStatusPort_trailerAttached_nu = 1
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_80_Req_2325920_2326019"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        TRAIL_ATT = "SignalManipulation.trailerStatusPort_trailerAttached_nu"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.TRAIL_ATT: [
                "SignalManipulation.trailerStatusPort_trailerAttached_nu",
            ],
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 80 Req 2325920 2326019",
    description=(
        "This test case verifies that AUP will transition from Scanning to Init when AUPCore receives the information \
            that a trailer is attached to the ego vehicle."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_80_Req_2325920_2326019_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            trail_att = signals[Signals.Columns.TRAIL_ATT]
            ...
            time_threshold_2 = find_time_threshold(trail_att, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.TRAIL_ATT} == 1: <br>"
                    ...
                    f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 + 10]} ({ap_states.iloc[time_threshold_2 + 10]}) <br>"
                    f"This confirms that AUP transitioned from Scanning to Init when AUPCore received the information that a \
                        trailer was attached to the ego vehicle.".split()
                )
                eval_cond_1 = True
                ...
                if delay_neg_ap or delay_pos_ap:
                    if delay_neg_ap:
                        evaluation_1 = " ".join(
                            ...
                            f"{Signals.Columns.TRAIL_ATT} == 1: <br>"
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 - 10]} ({ap_states.iloc[time_threshold_2 - 10]}) <br>"
                            f"This means that AUPCore wasn't in Scanning state when AUPCore received the infromation that a trailer \
                                was attached to the ego vehicle.".split()
                        )
                    elif delay_pos_ap:
                        evaluation_1 = " ".join(
                            ...
                            f"{Signals.Columns.TRAIL_ATT} == 1: <br>"
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 + 10]} ({ap_states.iloc[time_threshold_2 + 10]}) <br>"
                            f"This means that AUPCore didn't transition to Init state when AUPCore received the infromation that a trailer \
                                was attached to the ego vehicle.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TRAIL_ATT} == 1 was never found.".split()
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=trail_att,
                    mode="lines",
                    name=Signals.Columns.TRAIL_ATT,
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_80_Req_2325920_2326019",
    description=(
        "This test case verifies that AUP will transition from Scanning to Init when AUPCore receives the information \
            that a trailer is attached to the ego vehicle."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_80_Req_2325920_2326019_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_80_Req_2325920_2326019_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.trailerStatusPort_trailerAttached_nu</em> == 1 <br><br>
            </td>
        </tr>
        <tr>
            <td>
