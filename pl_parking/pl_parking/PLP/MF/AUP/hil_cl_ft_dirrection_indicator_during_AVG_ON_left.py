...
SIGNAL_DATA = "DIRRECTION_INDICATOR_AVG_LEFT"
...
@teststep_definition(
    step_number=1,
    name="Left dirrection Indicator during AVG ON",
...
class DirrIndicatorDuringAVGonCheck(TestStep):
    ...
    def process(self, **kwargs):
        ...
        eval_cond = [True] * 1
        evaluation1 = " ".join(
            f"The evaluation the left turn indicator signal is True, the system is providyng a constant visualisation to the expected final pose ({signal_name['Turn_sig_lever']} =="
            " 1).".split()
        )
            ...
            if t2_idx is not None:
                ...
                if t3_idx is not None:
                    for i in range(t2_idx,t3_idx):
                        if turn_sig_lever[i] != constants.HilCl.TurnSignallLever.TURN_LEVER_ENGAGED_LEFT:
                            test_result = fc.FAIL
                            eval_cond = [False] * 1
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['Turn_sig_lever']} signal is FAILED,"
                                " the system didn't provide a constant visualisation of the left turn indicator (1).".split()
                            )
                            break
                else:
                    eval_cond = [False] * 1
...
@testcase_definition(
    name="Left Dirrection Indicator during AVG ON",
    description="The AP funtion shall continuously show the left dirrection indicator in the direction of the intended final pose",
