...
SIGNAL_DATA = "MANEUVER_AREA_CHECK_PERP_B"
...
@teststep_definition(
    name="Checking the maneuvering trajectory perpendicular backward",
    description="Check the maneuvering is correctly happening based on the maneuvering area constrain.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupManeuveringPerpBAreaCheck(TestStep):
    """AupManeuveringPerpBAreaCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        # Find when the maneuver is finished
        for cnt in range(t1_idx or 0, len(p_core_state_sig)):
            if (
                p_core_state_sig[cnt] == constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SUCCESS
            ):  # create ignition class
                t3_idx = cnt
                break
        ...
        if t1_idx is not None and t2_idx is not None and t3_idx is not None:
            eval_cond, maneuver_area, ego = HilClFuntions.maneuver_area_check(
                positions_x=pos_x[t2_idx : t3_idx + 1],
                positions_y=pos_y[t2_idx : t3_idx + 1],
                heading_angle=ego_heading[t2_idx : t3_idx + 1],
                ...
                left_d=constants.HilCl.ManeuveringArea.PERP_IN_B_D_2,
                rear_d=constants.HilCl.ManeuveringArea.PERP_IN_B_D_3,
                right_d=constants.HilCl.ManeuveringArea.PERP_IN_B_D_4,
                front_d=constants.HilCl.ManeuveringArea.PERP_IN_B_D_1,
                parking_pos_x=pos_x[t3_idx],
                parking_pos_y=pos_y[t3_idx],
                parking_heading_angle=ego_heading[t3_idx],
            )

            # maneuver_area_plot = HilClFuntions.create_slider_graph(ego, maneuver_area, timestamps=())
        # The maneuver is done with one stroke only
        elif t1_idx is not None and t2_idx is None and t3_idx is not None:
            eval_cond, maneuver_area, ego = HilClFuntions.maneuver_area_check(
                positions_x=pos_x[t3_idx - 1 : t3_idx + 1],
                positions_y=pos_y[t3_idx - 1 : t3_idx + 1],
                heading_angle=ego_heading[t3_idx - 1 : t3_idx + 1],
                ego_length=constants.SlotOffer.VehicleDimensions.VEHICLE_LENGTH,
                ego_width=constants.SlotOffer.VehicleDimensions.VEHICLE_WIDTH,
                left_d=constants.HilCl.ManeuveringArea.PERP_IN_B_D_2,
                rear_d=constants.HilCl.ManeuveringArea.PERP_IN_B_D_3,
        ...
        if all(eval_cond):
            test_result = fc.PASS
            evaluation1 = " ".join(
                "The evaluation is PASSED, AP function does not leave the designated maneuvering area.".split()
            )
        else:
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The evaluation is FAILED, AP function leaves the designated maneuvering area.".split()
            )
        ...
        signal_summary["Maneuver area test while performing automated parking"] = evaluation1
...
@testcase_definition(
    name="Maneuvering area check perpendicular backward",
    description="Checking, that the AUP function does not leave the specified manuevering area while performing perpendicular parking in manuever.",
)
class AupManeuveringPerpBArea(TestCase):
    """AupManeuveringArea Test Case."""
