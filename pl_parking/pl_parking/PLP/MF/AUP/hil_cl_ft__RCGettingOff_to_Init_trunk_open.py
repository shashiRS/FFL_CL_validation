"""RC state switches from RC Getting Off to Init if deactivation is triggered (ID 965315) by "The trunk is open"."""
import logging
import os
import sys
...
SIGNAL_DATA = "RCGETTINGOFF_TO_INIT_TRUNK"
...
class ValidationSignals(MDFSignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.DOOR_STATE: "MTS.IO_CAN_Conti_Veh_CAN.VehInput01.DoorOpen",
...
@teststep_definition(
    name="Transition mode check - RC Getting Off to Init, trunk open",
    description="Check the transition from RC Getting Off to Init after the trunk is open.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RCGettingOffToInitTrunkOpenCheck(TestStep):
    """RCGettingOffToInitTrunkOpenCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        eval_cond = [True] * 1
        evaluation1 = " ".join(
            f"The evaluation of RC State {signal_name['State_on_HMI']} signal is PASSED,"
        ...
        if t1_getting_off_idx is not None:
            # Find the moment when trunk is open
            for i in range(t1_getting_off_idx, len(HMI_Info)):
                if door_state[i] == constants.HilCl.DoorOpen.TRUNK_OPEN:
                    t2_trunk_open = i
                    break
            if t2_trunk_open is not None:
                # Check if the system is in RC Getting Off until trunk is open
                for _ in range(t1_getting_off_idx, t2_trunk_open):
                    if general_message_sig[cnt] != constants.HilCl.Hmi.APHMIGeneralMessage.LEAVE_VEHICLE:
                        trigger_state = False
                        break
                if trigger_state:
                    # taking the timestamp of t2_idx in order to check the reaction 0.5s after
                    t2_timestamp = time_signal[t2_trunk_open]
                    for y in range(t2_trunk_open, len(HMI_Info)):
                    ...
                    if t3_after_delay_added is not None:
                        ...
                        if HMI_Info[t3_after_delay_added] != constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE:
                            ...
                            evaluation1 = " ".join(
                                ...
                                f" system didn't change the state to Init after trunk open( != {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE}).".split()
                            )
                    else:
                        test_result = fc.FAIL
                        eval_cond = [False] * 1
                    ...
                    eval_cond = [False] * 1
                    evaluation1 = " ".join(
                        f"TC Failed because the system is not in RC Getting Off before trunk open (APHMIGeneralMessage != {constants.HilCl.Hmi.APHMIGeneralMessage.LEAVE_VEHICLE})".split()
                    )
            else:
                test_result = fc.FAIL
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                            f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED,"
                            f" trunk not open (DoorOpen != {constants.HilCl.DoorOpen.TRUNK_OPEN}).".split()
                        )
        else:
            test_result = fc.FAIL
            eval_cond = [False] * 1
        ...
        signal_summary[
            "Check the reaction of the system during RC Getting Off if the trunk is open"
        ] = evaluation1
...
@testcase_definition(
    name="RC Getting Off to Init transition, trunk open.",
    description="The function shall switch from RC Getting Off to Init mode if the trunk is open.",
...
class RCGettingOffToInitTrunkOpen(TestCase):
    """RCGettingOffToInitTrunkOpen Test Case."""
    ...
    def test_steps(self):
        ...
        return [
            RCGettingOffToInitTrunkOpenCheck,
        ]
