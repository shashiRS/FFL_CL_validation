"""The HMI shall provide a visual or sound alert to the driver in case of error mode"""
...
SIGNAL_DATA = "LSMF_SOUND_VISUAL_ALERT_OR"
...
@teststep_definition(
    name="Provided alert(s) in case of error. Relation between alert(s): OR",
    description="Check provided alert(s) when error appears.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class SotifDrivAlertOrCheck(TestStep):
    """SotifDrivAlertOrCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_maneuvering_idx is not None:
            ...
            if t_issue_injection_idx is not None:
                ...
                if t_issue_remove_idx is not None:
                    ...
                    # Sound alert
                    for cnt in range(t_issue_injection_idx, t_issue_remove_idx + 1):
                        if (front_pitch_sig[cnt] > 0 and front_volume_sig[cnt] > 0) or (
                            rear_pitch_sig[cnt] > 0 and rear_volume_sig[cnt] > 0
                        ):
                            sound_alert_detected = True
                            break

                    if visual_alert_detected or sound_alert_detected:
                        eval_cond = [True] * 1

                        evaluation1 += " ".join(
                            "The evaluation is PASSED. HMI provides a visual or sound alert to the driver in case of error mode".split()
                        )
                        ...
                        eval_cond = [False] * 1
                        evaluation1 = " ".join(
                            "The evaluation is FAILED. HMI don't provide a visual nor sound alert to the driver in case of error mode".split()
                        )
        ...
        signal_summary["The HMI shall provide a visual or sound alert to the driver in case of error mode"] = (
            evaluation1
        )
...
@testcase_definition(
    name="Driver alert in case or error. Relation between alert(s): OR",
    description="The HMI shall provide a visual or sound alert to the driver in case of error mode",
)
class SotifDrivAlertOr(TestCase):
    """SotifDrivAlertOr Test Case."""
