...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_128_Req_2325978_2326039"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 128 Req 2325978 2326039",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the rear camera failed) then the Automated Parking Core shall transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_128_Req_2325978_2326039_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"signal {Signals.Columns.REAR_CAM} == 1:<br>"
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.REAR_CAM} == 1 was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_128_Req_2325978_2326039",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the rear camera failed) then the Automated Parking Core shall transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_128_Req_2325978_2326039_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_128_Req_2325978_2326039_TS
