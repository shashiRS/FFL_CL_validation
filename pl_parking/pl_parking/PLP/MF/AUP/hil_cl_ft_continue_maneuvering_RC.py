"""Ego vehicle shall continue the interrupted maneuvering if the driver select it via RC."""
import logging
import os
import sys
...
@teststep_definition(
    ...
    description="Check if ego car continue the interrupted maneuvering if the driver select it via RC.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class ContinueInterrManRCCheck(TestStep):
    """ContinueInterrManRCCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            ...
            f" after the driver select the continue option from RC.".split()
        )
        ...
        if t1_idx is not None:
            ...
            for cnt in range(t1_idx, len(user_act_rem)):
                if user_act_rem[cnt] == constants.HilCl.Hmi.RemCommand.REM_TAP_ON_CONTINUE:
                    t2_idx = cnt
                    break
            ...
            if t2_idx is not None:
                ...
                evaluation1 = " ".join(
                    ...
                    f" ({signal_name['User_action']} != {constants.HilCl.Hmi.RemCommand.REM_TAP_ON_CONTINUE}).".split()
                )
        else:
            test_result = fc.FAIL
            eval_cond = [False] * 1
...
@testcase_definition(
    name="Continue Interrupted Maneuvering via RC",
    description=f"In case the driver select Continue Option from RC ({constants.HilCl.Hmi.RemCommand.REM_TAP_ON_CONTINUE}) after AVG has been interrupted, the parking maneuver shall"
                f" continue (AVG = {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING})",
    doors_url="https://jazz.conti.de/rm4/resources/BI_tE5Rw91eEe62R7UY0u3jZg?oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17892",
)
class ContinueInterrManRC(TestCase):
    """ContinueInterrManRC Test Case."""
