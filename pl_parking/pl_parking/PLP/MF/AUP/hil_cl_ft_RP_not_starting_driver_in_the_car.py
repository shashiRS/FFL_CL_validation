"""The system shall not start the low speed maneuvering procedure if the driver is in the car  (remote parking only)."""
import logging
import os
import sys
...
SIGNAL_DATA = "RP_NOT_START_DRIVER_DET"
...
@teststep_definition(
    name="RP not starting driver present",
    description="Check if the system is not starting the remote parking maneuver if the driver is detected in the car.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RPNotStartingDriverInTheCarCheck(TestStep):
    """RPNotStartingDriverInTheCarCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t1_idx is not None:
            ...
            for cnt in range(t1_idx, len(user_act_rem)):
                if Finger_pos_X[cnt] == constants.HilCl.Hmi.RemFingerPosition.DM_FINGER_POS_X and \
                   ...
                   seat_occupancy_state_driver[cnt] == constants.HilCl.Seat.DRIVERS_OCCUPIED:
                    t2_idx = cnt
                    break

            if t2_idx is not None:
                # check if after 5s the AVG didn't start the parking maneuver
                for i in range (t2_idx, len(HMI_Info)):
                    if HMI_Info[i] == constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING:
                        ...
                        evaluation1 = " ".join(
                            ...
                            f" AVG start the parking maneuver while driver still in the car (state == {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING})".split()
                        )
                        break
            else:
                test_result = fc.FAIL
        ...
        signal_summary[
            "Check the reaction of the system in case a driver is present in the car (remote parking only)."
        ] = evaluation1
...
@testcase_definition(
    name="Not Starting RP during driver presence",
    description=f"In case the driver is still in the car, the system shall not start the low speed parking maneuver"
...
class RPNotStartingDriverInTheCar(TestCase):
    """RPNotStartingDriverInTheCar Test Case."""
    ...
    def test_steps(self):
        ...
        return [
            RPNotStartingDriverInTheCarCheck,
        ]
