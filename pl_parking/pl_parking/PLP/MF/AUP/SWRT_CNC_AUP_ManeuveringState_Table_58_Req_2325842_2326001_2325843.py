...
T2: // right camera pre-proc becomes unreliable
SignalManipulation.healthVector_rightCamPreProcUnreliable = 1

T3: // heal condition
SignalManipulation.healthVector_rightCamPreProcUnreliable = 0
...
AP.apUserInformationPort.generalUserInformation_nu == RIGHT_CAM_PRE_PROC_UNRELIABLE (50)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_58_Req_2325842_2326001_2325843"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 58 Req 2325842 2326001 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the right camera pre-processing is unreliable, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_58_Req_2325842_2326001_2325843_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            time_threshold_3 = find_time_threshold(right_cam.iloc[time_threshold_2:], 0) + time_threshold_2
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.RIGHT_CAM}</b> == 1: <br>"
                    ...
                    f"This confirms that AUPCore received the information that the right camera pre-processing was unreliable and then it went into error <br>"
                ...
                delay_pos_user = user_inf.iloc[time_threshold_2 + 10] != aup.HMI_MESSAGE.RIGHT_CAM_PRE_PROC_UNRELIABLE
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before right camera pre-processing was detected unreliable and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before right camera pre-processing was detected unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due to right camera pre-processing unreliable.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"when {Signals.Columns.RIGHT_CAM} == 1. This means {reason}".split()
                        )
                        eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.RIGHT_CAM} == 1 was never found.".split()
                )
            if time_threshold_2 is not None and time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED between the interval given by the signal {Signals.Columns.RIGHT_CAM} == 1 "
                ...
                error_state = right_cam.iloc[time_threshold_3 : (time_threshold_4 - 10)]
                ...
                if cond_neg_core or cond_neg_error:
                    if cond_neg_core and cond_neg_error:
                        evaluation_2 = " ".join(
                            f"The evaluation FAILED between the interval given by the signal {Signals.Columns.RIGHT_CAM} == 1<br> "
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                        ...
                        evaluation_2 = " ".join(
                            ...
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                            f"<b>({round(time.iloc[time_threshold_2], 3)} - {round(time.iloc[time_threshold_5 - 10], 3)})s</b>,<br> "
                            f"because the signal {Signals.Columns.RIGHT_CAM} wasn't set to 0.<br><br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.RIGHT_CAM} == 0 was never found between T2 and T4.<br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger values {Signals.Columns.RIGHT_CAM} == 1 or {Signals.Columns.IGN_ON} == Ignition OFF (0) "
            ...
            AP.apUserInformationPort.generalUserInformation_nu == RIGHT_CAM_PRE_PROC_UNRELIABLE (50)<br>"""
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_58_Req_2325842_2326001_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the right camera pre-processing is unreliable, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_58_Req_2325842_2326001_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_58_Req_2325842_2326001_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //right camera pre-proc becomes unreliable<br>
                    <em>SignalManipulation.healthVector_rightCamPreProcUnreliable</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_rightCamPreProcUnreliable</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == RIGHT_CAM_PRE_PROC_UNRELIABLE (50) <br>
                T2 - T4:<br>
