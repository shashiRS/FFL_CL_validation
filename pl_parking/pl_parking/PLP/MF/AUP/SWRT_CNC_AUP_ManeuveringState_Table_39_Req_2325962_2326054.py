...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_39_Req_2325962_2326054"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        DOOR = "AP.doorStatusPort.status.driver_nu"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 39 Req 2325962 2326054",
    description=(
        "This test case verifies that if AUP receives the information that the vehicle door is open\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_39_Req_2325962_2326054_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            door = signals[Signals.Columns.DOOR]
            ...
            signals["status_door"] = door.apply(lambda x: fh.get_status_label(x, aup.DOOR_DRIVER))
            ...
            time_threshold_2 = find_time_threshold(door, aup.DOOR_DRIVER.DOOR_STATUS_OPEN)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.DOOR}</b> == DOOR_STATUS_OPEN (1)<br>"
                    ...
                    f"This confirms that Remote Control went from active to paused when AUP detected that the vehicle door was open.".split()
                )
                ...
                if delay_neg_rm or delay_neg_screen or delay_pos_rm:
                    if delay_neg_rm and delay_pos_rm and delay_neg_screen:
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"This means that before AUP detected that the vehicle door was open, the RC wasn't active.".split()
                        )
                    elif delay_pos_rm:
                        evaluation_1 = " ".join(
                            ...
                            f"when AUP detected that the vehicle door was open, the RC wasn't paused.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.DOOR} == DOOR_STATUS_OPEN (1) was never found.".split()
                )

            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED because between the interval given by the signal {Signals.Columns.DOOR} == DOOR_STATUS_OPEN (1) "
                    ...
                    f"This confirms that after the RC is in paused state (due to vehicle door open) it goes into finished state once.".split()
                )
                ...
                if total_rm_finished != 1:
                    evaluation_2 = " ".join(
                        f"The evaluation is FAILED because between the interval given by the signal {Signals.Columns.DOOR} == 1"
                        f" and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)} - {round(time.iloc[time_threshold_3], 3)})</b>:<br>"
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_39_Req_2325962_2326054",
    description=(
        "This test case verifies that if AUP receives the information that the vehicle door is open\
            while Remote Control is active, then the Remote Control should go into paused state then to finished."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_39_Req_2325962_2326054_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_39_Req_2325962_2326054_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>AP.doorStatusPort.status.driver_nu</em> = DOOR_STATUS_OPEN (1)<br>
