"""Check low object classification."""
...
SIGNAL_DATA = "HEIGHT_CLASSIFICATION_LOW"
...
@teststep_definition(
    ...
    description="Check low object classification",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class CommonLowCheck(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        """Evaluation part"""
        # Check max value of detected_static_obj_sig
        if max(detected_static_obj_sig) > 0:
            # Find when syxtem detect a static object. Collect rising edges
            detections_idx = HilClFuntions.RisingEdge(detected_static_obj_sig, 0)
            # Type of static shall be checked later
            type_check = True
        ...
        if type_check:
            ...
            evaluation1 = " ".join(
                "The evaluation is PASSED, AP function considers detected static objects as low.".split()
            )
            # Check type of detected object
            for item in detections_idx:
                if height_class_sig[item] != constants.HilCl.HeightClases.UNKNOWN:
                    evaluation1 = " ".join(
                        f"The evaluation is FAILED, AP function does not consider"
                        f" static object as low object at {time_sig[item]} us.".split()
                    )
                    eval_cond[0] = False
                    break
            ...
            eval_cond = [True] * 1
            evaluation1 = " ".join(
                "The evaluation is PASSED, AP funtion des not detect any object during TestRun.".split()
            )

        signal_summary["Height classification Low"] = evaluation1
...
@testcase_definition(
    name="Height classification Low",
    description=(
        f"The AP Function shall consider static objects lower or equal than"
        f" {constants.HilCl.HeightLimits.AP_G_MIN_HEIGHT_OBSTACLE_M} m as low."
    ),
)
class FtCommon(TestCase):
    ...
    # Information from Marcel Horvath:
    # Low == If something is not detected or detected as height unknown

    # Important iformation:
    # Please use this script only with TestRun HIL_COMMON_HeightClassification_Low_ID1073773
