...
SIGNAL_DATA = "DIRRECTION_PERPENDICULAR_RIGHT_BWD"
...
@teststep_definition(
    step_number=1,
    name="Perpendicular backward Right Indicator during Scanning State",
    description="Check the behavior of the system after the driver select the right backward dirrection from HMI in case ego veh is on a perpendicular parking slot in Scanning Out state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class ParkOutDirrPerpBWDRightCheck(TestStep):
    """ParkOutDirrPerpBWDRightCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        eval_cond = [True] * 1
        evaluation1 = " ".join(
            f"The evaluation the right backward dirrection indicator signal is True, the system is providyng a constant visualisation to the intended parkingOut dirrection ({signal_name['AVG_Type']} =="
            f" {constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PERPENDICULAR_RIGHT_BWD}).".split()
        )
            ...
            if t2_idx is not None:
                ...
                if t3_idx is not None:
                    ...
                    for y in range(t3_idx,len(HMI_Info)):
                        if avg_type[y] != constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PERPENDICULAR_RIGHT_BWD:
                            test_result = fc.FAIL
                            eval_cond = [False] * 1
                            evaluation1 = " ".join(
                                f"The evaluation of {signal_name['AVG_Type']} signal is FAILED,"
                                f" the system didn't provide a constant visualisation of the right backward dirrection indicator ( != {constants.HilCl.Hmi.APHMIGeneralAVGType.PARK_OUT_PERPENDICULAR_RIGHT_BWD})."
        ...
        signal_summary["Check the reaction of the system related to the dirrection indicator during ParkingOut state - perpendicular right backward."] = evaluation1
...
@testcase_definition(
    name="Dirrection Indicator in which the system shall start the parking out maneuver during scanning state - perpendicular backward right",
    description="The AP funtion shall continuously show the Right Backward Dirrection Indicator in which the system shall start the parking out maneuver during scanning state",
...
class ParkOutDirrPerpBWDRight(TestCase):
    """ParkOutDirrPerpBWDRight Test Case."""
