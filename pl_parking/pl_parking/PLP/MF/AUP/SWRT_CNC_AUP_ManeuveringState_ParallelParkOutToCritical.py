...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_ParallelParkOutToCritical"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Parallel Park Out To Critical",
    description=(
        "This test case verifies that in case of Parking out parallel and the driver is inside the vehicle the AutomatedParkingCore shall trigger that the path is only followed until the critical road-side-edge or the neighboring obstacles is cleared."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_ParallelParkOutToCritical_TS(TestStep):
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_ParallelParkOutToCritical",
    description=(
        "This test case verifies that in case of Parking out parallel and the driver is inside the vehicle the AutomatedParkingCore shall trigger that the path is only followed until the critical road-side-edge or the neighboring obstacles is cleared."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_ParallelParkOutToCritical_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_ParallelParkOutToCritical_TS
