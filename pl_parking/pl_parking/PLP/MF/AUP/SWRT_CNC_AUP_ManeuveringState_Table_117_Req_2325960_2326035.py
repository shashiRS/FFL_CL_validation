...
T2:
SignalManipulation.escInformationPort_escState_nu = {ESC_ACTIVE} (1)

T3:
T2 + AP_G_ESC_TIME_THRESH_S (0.5s)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_117_Req_2325960_2326035"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        ESC_STATE = "SignalManipulation.escInformationPort_escState_nu"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.ESC_STATE: [
                "SignalManipulation.escInformationPort_escState_nu",
            ],
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 117 Req 2325960 2326035",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that that an ESC intervention is active for longer than 0.5 s."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_117_Req_2325960_2326035_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            esc_state = signals[Signals.Columns.ESC_STATE]
            ...
            signals["status_esc_state"] = esc_state.apply(lambda x: "ESC_ACTIVE" if x == 1 else "ESC_NOT_ACTIVE")
            ...
            time_threshold_2 = find_time_threshold(esc_state, ESC_ACTIVE)
            ...
            if time_threshold_3 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.ESC_STATE} is in ESC_ACTIVE (1) state: <br>"
                    ...
                    f"This confirms that AUPCore transitioned from Maneuvering to Terminate (Error or Pause-Finished) when the AUPCore received the<br>\
                        information that an ESC intervention is active for longer than {aup.AP_G_ESC_TIME_THRESH_S} s.".split()
                )
                eval_cond_1 = True
                    ...
                    if (delay_pos_ap or total_ap_finish != 1) or (delay_neg_ap or delay_neg_core):
                        if delay_neg_ap or delay_neg_core:
                            evaluation_1 = " ".join(
                                ...
                                f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                                f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                                f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received that  an ESC intervention is active for longer than {aup.AP_G_ESC_TIME_THRESH_S} s.".split()
                            )
                            ...
                            evaluation_1 = " ".join(
                                ...
                                f"{Signals.Columns.ESC_STATE} is in ESC_ACTIVE (1) state: <br>"
                                f"- AUPCore didn't transition to Pause and then to Finished state when the AUPCore received the information that an ESC intervention is active for longer than {aup.AP_G_ESC_TIME_THRESH_S} s.".split()
                            )
                        eval_cond_1 = False
                ...
                if (delay_neg_ap or delay_neg_core or delay_pos_var or delay_pos_core) and not ap_pause:
                    if delay_neg_ap or delay_neg_core:
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                            f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information that an ESC intervention is active for longer than {aup.AP_G_ESC_TIME_THRESH_S} s.".split()
                        )
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_3 + 10]} ({var_esm.iloc[time_threshold_3 + 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 + 10]} ({core_state.iloc[time_threshold_3 + 10]}) <br><br>"
                            f"This means that AUPCore didn't transition to Error state when the AUPCore received the information that an ESC intervention is active for longer than {aup.AP_G_ESC_TIME_THRESH_S} s.".split()
                        )
                    eval_cond_1 = False
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.ESC_STATE} == ESC_ACTIVE (1) was never found.".split()
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=esc_state,
                    mode="lines",
                    name=Signals.Columns.ESC_STATE,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_esc_state"],
                )
            )
            fig.add_trace(
                go.Scatter(
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_117_Req_2325960_2326035",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that an ESC intervention is active for longer than 0.5 s."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_Table_117_Req_2325960_2326035_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_117_Req_2325960_2326035_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2:<em>SignalManipulation.escInformationPort_escState_nu</em> == ESC_ACTIVE (1) <br>
                T3: T2 +  AP_G_ESC_TIME_THRESH_S (0.5s)<br>
