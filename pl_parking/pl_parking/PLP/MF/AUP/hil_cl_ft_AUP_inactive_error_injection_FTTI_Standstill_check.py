...
SIGNAL_DATA = "INPUT_FAILURE_INACTIVE_AUP_SS"
...
@teststep_definition(
    step_number=1,
    name="System input failure while AUP is inactive",
    description=(
...
class System_safe_state_error_injected_AUP_inactive_SS(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        for frame in range(0, len(time_signal)):
            ...
            if veh_ignition[frame] and trigger1 is None:
                ...
                if init_time >= constants.HilCl.PDW.FTTI.SYSTEM_INIT:
                    # Check the status of AUP to be INACTIVE
                    if aup_state[frame] == constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE:
                        trigger1 = True
                        t1_idx = frame
                        break
                # Increase initialization time
                init_time += time_signal[frame] - time_signal[frame - 1]
        if t1_idx:
        ...
        if t2_idx:
            for frame in range(t2_idx, len(time_signal)):
                ...
                if (
                    ...
                    and aup_state[frame] == constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE
        ...
        if trigger1 is None:
            preconditions = (
                ...
                f"AUP state ({signal_name['AUP_State']}) not set to PPC_INACTIVE ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_INACTIVE})"
            )
...
@testcase_definition(
    name="System transition to Safe State mode (within 700 miliseconds) while AUP was inactive",
...
class SafeStateHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            System_safe_state_error_injected_AUP_inactive_SS,
        ]
