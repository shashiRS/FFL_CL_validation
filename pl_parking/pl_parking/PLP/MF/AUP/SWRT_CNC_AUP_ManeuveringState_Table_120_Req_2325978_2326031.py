...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_120_Req_2325978_2326031"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 120 Req 2325978 2326031",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the LoActor failed) then the Automated Parking Core shall transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_120_Req_2325978_2326031_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"signal {Signals.Columns.LODMC_STATE} == LODMC_NOT_AVAILABLE(0):<br>"
                    ...
                    f"This confirms that the Automated Parking Core transitioned from Init to Error when the Health <br>\
                        Vector reported an AUP reversible error.".split()
                )
                eval_cond_1 = True
                ...
                if delay_neg_core or delay_neg_std:
                    evaluation_1 = " ".join(
                        ...
                        f"This means that the Automated Parking Core wasn't in Init state before the Health Vector reported an AUP reversible error.".split()
                    )
                    eval_cond_1 = False
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.LODMC_STATE} == LODMC_NOT_AVAILABLE(0) was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_120_Req_2325978_2326031",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the LoActor failed) then the Automated Parking Core shall transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_120_Req_2325978_2326031_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_120_Req_2325978_2326031_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.loDMCStatusPort_loDMCSystemState_nu</em> == LODMC_NOT_AVAILABLE(0) <br><br>
            </td>
        </tr>
        <tr>
            <td>
