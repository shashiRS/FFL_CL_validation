...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_87_Req_2325960_2326040"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 87 Req 2325960 2326040",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that the left camera is blocked (AP function has a reversible error)."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_87_Req_2325960_2326040_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.LEFT_CAM} == 1: <br>"
                    ...
                    f"This confirms that AUPCore transitioned from Maneuvering to Terminate (Error or Pause-Finished) when the AUPCore received the<br>\
                        information that the left camera was blocked (AP function has a reversible error).".split()
                )
                eval_cond_1 = True
                ...
                if ap_pause:
                    ...
                    # Verify the transiton from Maneuvering to Pause - Finished
                    if (delay_pos_ap or total_ap_finish != 1) or (delay_neg_ap or delay_neg_core):
                        if delay_neg_ap or delay_neg_core:
                            evaluation_1 = " ".join(
                                ...
                                f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 - 10]} ({ap_states.iloc[time_threshold_2 - 10]}) <br>"
                                f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) <br><br>"
                                f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information that the left camera was blocked.".split()
                            )
                            ...
                            evaluation_1 = " ".join(
                                ...
                                f"{Signals.Columns.LEFT_CAM} == 1: <br>"
                                f"- AUPCore didn't transition to Pause and then to Finished state when the AUPCore received the information that the left camera was blocked.".split()
                            )
                        eval_cond_1 = False
                ...
                if (delay_neg_ap or delay_neg_core or delay_pos_var or delay_pos_core) and not ap_pause:
                    if delay_neg_ap or delay_neg_core:
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) <br><br>"
                            f"This means that AUPCore didn't transition to Error state when the AUPCore received the information that the left camera was blocked.".split()
                        )
                    eval_cond_1 = False
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_87_Req_2325960_2326040",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that the left camera is blocked (AP function has a reversible error)."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_87_Req_2325960_2326040_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_87_Req_2325960_2326040_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.healthVector_leftCamBlockage</em> == 1 <br><br>
            </td>
        </tr>
        <tr>
            <td>
