...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_153_Req_2711728_2326033"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        ACC_STATUS = "SignalManipulation.accInformationPort_accStatus_nu"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 153 Req 2711728 2326033",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Terminate state and it receives the \
            information that a reversible error is present (ACC System is active) then it should transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_153_Req_2711728_2326033_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            acc_status = signals[Signals.Columns.ACC_STATUS]
            ...
            signals["status_acc_status"] = acc_status.apply(lambda x: fh.get_status_label(x, aup.ACC_STATUS))
            ...
            time_threshold_2 = find_time_threshold(acc_status, aup.ACC_STATUS.ACC_ACTIVE)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.ACC_STATUS} == ACC_ACTIVE (3): <br>"
                ...
                if delay_neg_ap or delay_neg_core:
                    ...
                    evaluation_1 = " ".join(
                        ...
                        f"{Signals.Columns.ACC_STATUS} == 0: <br>"
                        f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) <br>"
                        f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) <br>"
                        f"- the signal {Signals.Columns.VAR_PPC} == {signals['status_var_ppc'].iloc[time_threshold_2 + 10]} ({var_ppc.iloc[time_threshold_2 + 10]}) <br><br>"
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.ACC_STATUS} == ACC_ACTIVE (3) was never found.".split()
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=acc_status,
                    mode="lines",
                    name=Signals.Columns.ACC_STATUS,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_acc_status"],
                )
            )
            fig.add_trace(
                go.Scatter(
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_153_Req_2711728_2326033",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Terminate state and it receives the \
            information that a reversible error is present (ACC System is active) then it should transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_153_Req_2711728_2326033_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_153_Req_2711728_2326033_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.accInformationPort_accStatus_nu</em> == ACC_ACTIVE (3)<br>
                T3:<br>
