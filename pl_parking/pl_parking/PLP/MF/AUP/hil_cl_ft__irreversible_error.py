...
@teststep_definition(
    name="System reaction on irreversible error",
    description="Check system reaction on USS timeout",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupIrrErrorCheck(TestStep):
    """AupIrrErrorCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation of {signal_name['State_on_HMI']} signal is PASSED, state of signal switches to PPC_IRREVERSIBLE_ERROR ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_IRREVERSIBLE_ERROR})"
            f" after USS Timeout injection.".split()
        )
        ...
        signal_summary["Required state change: Maneuvering to Irreversible error mode. Reason: USS Timeout"] = (
            evaluation1
        )
...
@testcase_definition(
    name="Irreversible error detection",
    description="The AP function shall have an irreversible error, if USS Timeout appears.",
)
class AupIrrError(TestCase):
    """AupIrrError Test Case."""
