...
SIGNAL_DATA = "BODY DISTANCE HIGH OBSTACLE PARALLEL FRONT SIDE"
...
@teststep_definition(
    ...
    description="Body distance from high obstacle at final parking pose parallel front side.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupAPMinBodDistParFrontCheck(TestStep):
    """AupAPMinBodDistParFrontCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_start_idx and t_success_idx:
            ego_pos = ego_x[t_success_idx] + constants.HilCl.CarShape.CAR_R_AXLE_TO_FRONT + constants.HilCl.CarShape.CAR_R_AXLE_TO_HITCH
            obj_pos = object_x[t_success_idx]
            if obj_pos - ego_pos > constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_SSIDE_HIGH_OBST_M:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    "The test for checking minimal body distance between front side and high object for parallel parking is PASSED.".split()
                )
            else:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The test for checking minimal body distance between front side and high object for parallel parking is FAILED. The final parking pose was too close to the high object, it was"
            ...
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The test for checking minimal body distance between front side and high object for parallel parking is FAILED.".split()
            )
...
@testcase_definition(
    name="Minimum body distance from high object parallel front",
    description="Body distance from high obstacle at final parking pose parallel front side.",
)
class AupAPMinBodDistParFront(TestCase):
    """AupAPMinBodDistParFront Test Case."""
