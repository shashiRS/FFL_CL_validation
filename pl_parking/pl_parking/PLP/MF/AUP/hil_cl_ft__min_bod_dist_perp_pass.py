...
SIGNAL_DATA = "BODY DISTANCE HIGH OBSTACLE PERPENDICULAR PASSENGER SIDE"
...
@teststep_definition(
    ...
    description="Body distance from high obstacle at final parking pose perpendicular passenger side.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class AupAPMinBodDistPerpPassCheck(TestStep):
    """AupAPMinBodDistPerpPassCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        ego_x = read_data["Car_sroad"].tolist()
        object_x = read_data["Obj_sroad"].tolist()
        ...
        if t_start_idx and t_success_idx:
            ego_pos = ego_x[t_success_idx] + constants.HilCl.CarShape.CAR_R_AXLE_TO_SIDE
            obj_pos = object_x[t_success_idx]
            if obj_pos - ego_pos > constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_LSIDE_HIGH_OBST_PERP_M:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    "The test for checking minimal body distance between passenger side and high object for perpendicular parking is PASSED.".split()
                )
            else:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The test for checking minimal body distance between passenger side and high object for perpendicular parking is FAILED. The final parking pose was too close to the high object, it was"
                    f"{obj_pos-ego_pos} m while the maximum allowed is {constants.ConstantsAUPBodyDistFinalPose.AP_G_DIST_MIN_LSIDE_HIGH_OBST_PERP_M}".split()
                )

        else:
            test_result = fc.FAIL
            evaluation1 = " ".join(
                "The test for checking minimal body distance between passenger side and high object for perpendicular parking is FAILED.".split()
            )
        ...
        if test_result == fc.FAIL:
            ...
            fig.add_trace(go.Scatter(x=time_signal, y=object_x, mode="lines", name=signal_name["Obj_sroad"]))
            fig.add_trace(go.Scatter(x=time_signal, y=ego_x, mode="lines", name=signal_name["Car_sroad"]))
...
@testcase_definition(
    name="Minimum body distance from high object perpendicular passenger",
    description="Body distance from high obstacle at final parking pose perpendicular passenger side.",
)
class AupAPMinBodDistPerpPass(TestCase):
    """AupAPMinBodDistPerpPass Test Case."""
