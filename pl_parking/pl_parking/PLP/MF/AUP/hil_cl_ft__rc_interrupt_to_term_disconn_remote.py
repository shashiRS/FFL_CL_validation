"""RC Interrupt to Terminate, Disconnected remote device"""
...
SIGNAL_DATA = "RC_MAN_INT_TO_TERM_DISC_REM_DEVICE"
...
@teststep_definition(
    name="System reaction on lost connection with remote device in case of interrupted mode",
...
class CommonRcManIntTermDisconnCheck(TestStep):
    """CommonRcManIntTermDisconnCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_device_connected_idx is not None:
            ...
            if t_interrupt_idx is not None:
                ...
                for cnt in range(t_interrupt_idx, len(rem_device_con_sig)):
                ...
                if t_device_disconnected_idx is not None:
                    ...
                    for cnt in range(t_device_disconnected_idx, len(rem_gen_message_sig)):
                        if rem_gen_message_sig[cnt] == constants.HilCl.Hmi.APHMIGeneralMessageRemote.PARKING_FAILED:
                            t_failed_msg_sig = cnt
                            break

                    if t_failed_msg_sig is not None:

                        eval_cond = [True] * 1

                        delta = time_signal[t_failed_msg_sig] - time_signal[t_device_disconnected_idx]  # [us]
                        ...
                        if delta < constants.HilCl.ApThreshold.AP_G_MAX_TIME_REM_DISCONNECT_S * 1e6:
                            ...
                            evaluation1 = " ".join(
                                ...
                                f" to PARKING_FAILED ({constants.HilCl.Hmi.APHMIGeneralMessageRemote.PARKING_FAILED}) at {time_signal[t_failed_msg_sig]} us after remote device disconnected event ({time_signal[t_device_disconnected_idx]} us)."
                                f" Time gap between error mesage and issue injection is {round(delta * 1e-6, 4)} secnod(s) but system shall switch from RC Maneuvering Interrupted to Terminate if the remote device"
                                f" is disconnected from the ego vehicle for longer than {constants.HilCl.ApThreshold.AP_G_MAX_TIME_REM_DISCONNECT_S} second(s)".split()
                            )
                    ...
                    eval_cond = [False] * 1
                    evaluation1 = " ".join(
                        f"The evaluation of {signal_name['Rem_device_connected']} signal is FAILED, signal never switched to FALSE ({constants.HilCl.Hmi.BTDevConnected.FALSE}) after PPC_PARKING_PAUSE event."
                ...
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['State_on_HMI']} signal is FAILED, signal never switched to PPC_PARKING_PAUSE ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PARKING_PAUSE})."
                    " It is not possible to continue evaluation in this case. This event is needed to evaluation.".split()
                )
        else:
            test_result = fc.FAIL
        ...
        signal_summary[
            f"Required state change: RC Maneuvering Interrupted to Terminate, Reason: Disconnected remote device for longer than {constants.HilCl.ApThreshold.AP_G_MAX_TIME_REM_DISCONNECT_S} second(s)"
        ] = evaluation1
...
@testcase_definition(
    name="RC Maneuvering Interrupted to Terminate, Disconnected remote device",
    description=f"The system shall switch from RC Maneuvering Interrupted to Terminate if the remote device"
...
class CommonRcManIntTermDisconn(TestCase):
    """CommonRcManIntTermDisconn Test Case."""
    ...
    def test_steps(self):
        ...
        return [
            CommonRcManIntTermDisconnCheck,
        ]
