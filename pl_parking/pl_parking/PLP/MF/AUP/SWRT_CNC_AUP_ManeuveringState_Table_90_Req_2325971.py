...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_90_Req_2325971"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 90 Req 2325971",
    description=(
        "This test case verifies that in case or Remote Control, RM shall transition from Maneuvering to Terminate (Pause then Finished) if \
            the ego vehicle engine is turned OFF."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_90_Req_2325971_TS(TestStep):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"This confirms that RM transitioned from Maneuvering to Pause and then to Finished when the ignition was turned OFF".split()
                )
                eval_cond_1 = True
                ...
                if delay_neg_rm or delay_pos_rm or delay_neg_screen:
                    if delay_neg_rm or delay_neg_screen:
                        evaluation_1 = " ".join(
                            ...
                            f"This means that RM wasn't in Maneuvering state when the ignition was turned OFF".split()
                        )
                    elif delay_pos_rm:
                        evaluation_1 = " ".join(
                            ...
                            f"This means that RM didn't transition to Pause state when the ignition was turned OFF.".split()
                        )
                    eval_cond_1 = False
            else:
            ...
            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    ...
                    f"This confirms that RM transitioned from Maneuvering to Pause and then to Finished (once) when the ignition was turned OFF".split()
                )
                eval_cond_2 = True
                ...
                if total_ap_finish != 1:
                    evaluation_2 = " ".join(
                        ...
                        f"- the signal {Signals.Columns.AP_STATE} != AP_AVG_FINISHED (8) once <br><br>"
                        f"This means that RM didn't transition from Pause to Finished once when the ignition was turned OFF.".split()
                    )
                    eval_cond_2 = False
            else:
                self.result.measured_result = FALSE
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_90_Req_2325971",
    description=(
        "This test case verifies that in case or Remote Control, RM shall transition from Maneuvering to Terminate (Pause then Finished) if \
            the ego vehicle engine is turned OFF."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_90_Req_2325971_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_90_Req_2325971_TS
