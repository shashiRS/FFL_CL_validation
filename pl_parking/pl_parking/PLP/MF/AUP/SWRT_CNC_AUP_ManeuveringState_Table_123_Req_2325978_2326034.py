...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_123_Req_2325978_2326034"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 123 Req 2325978 2326034",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the TCS failed) then the Automated Parking Core shall transition to Error state."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_123_Req_2325978_2326034_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            signals["status_tcs_state"] = tcs_state.apply(lambda x: "TCS_ACTIVE" if x == 1 else "TCS_INACTIVE")
            ...
            time_threshold_2 = find_time_threshold(tcs_state, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"signal {Signals.Columns.TCS_STATE} == TCS_ACTIVE(1):<br>"
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.TCS_STATE} == TCS_ACTIVE(1) was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_123_Req_2325978_2326034",
    description=(
        "This test case verifies that in case the Automated Parking Core is in Init state and the Health Vector reports an AUP reversible error \
            (the the TCS failed) then the Automated Parking Core shall transition to Error state."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_123_Req_2325978_2326034_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_123_Req_2325978_2326034_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.escInformationPort_tcsState_nu</em> == TCS_ACTIVE(0) <br><br>
            </td>
        </tr>
        <tr>
            <td>
