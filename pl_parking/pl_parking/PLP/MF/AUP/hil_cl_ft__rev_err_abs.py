...
class AupRevErrAbsCheck(TestStep):
    ...
    def process(self, **kwargs):
        ...
        if t_parking_idx is not None:
            ...
            for cnt in range(t_parking_idx, len(abs_state_sig)):
                if abs_state_sig[cnt] == constants.HilCl.FunctionActivate.ABS_ACTIVE:
                    t_act_idx = cnt
                    break
            ...
            if t_act_idx is not None:
                ...
                # Find ABS deact
                for cnt in range(t_act_idx, len(abs_state_sig)):
                    if abs_state_sig[cnt] != constants.HilCl.FunctionActivate.ABS_ACTIVE:
                        t_deact_idx = cnt
                        break
                ...
                if t_deact_idx is not None:
                            ...
                            if counter == 1:
                                ...
                                if (
                                    ...
                                    evaluation1 = " ".join(
                                        ...
                                        f" ({constants.HilCl.ApThreshold.AP_G_ABS_TIME_THRESH_S}) sec. System shall not detect reversible error in this case.".split()
                                    )
                        ...
                        for cnt in range(t_act_idx, len(state_on_hmi_sig)):
                            if (
                                ...
                                evaluation1 = " ".join(
                                    ...
                                    f" ({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_REVERSIBLE_ERROR}) at {time_signal[cnt]} us but ABS intervention is"
                    ...
                    eval_cond = [False] * 1
                    evaluation1 = " ".join(
                        f"The evaluation of {signal_name['ABS_state']} signal is FAILED, value of signal never switched out from ABS_ACTIVE ({constants.HilCl.FunctionActivate.ABS_ACTIVE})."
                ...
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['ABS_state']} signal is FAILED, value of signal never switched to ABS_ACTIVE ({constants.HilCl.FunctionActivate.ABS_ACTIVE})."
                    " It is not possible to continue evaluation in this case. This event is needed to evaluation.".split()
                )
        else:
            test_result = fc.FAIL
