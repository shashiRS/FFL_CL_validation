...
T2:
SignalManipulation.trunkLidStatusPort_open_nu = 1
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_81_Req_2325920_2326020"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        TRUNK = "SignalManipulation.trunkLidStatusPort_open_nu"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.TRUNK: [
                "SignalManipulation.trunkLidStatusPort_open_nu",
            ],
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 81 Req 2325920 2326020",
    description=(
        "This test case verifies that AUP will transition from Scanning to Init when AUPCore receives the information \
            that the trunk of the ego vehicle is open."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_81_Req_2325920_2326020_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            trunk = signals[Signals.Columns.TRUNK]
            ...
            time_threshold_2 = find_time_threshold(trunk, 1)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"This confirms that AUP transitioned from Scanning to Init when AUPCore received the information that \
                        the trunk of the ego vehicle was open.".split()
                )
                eval_cond_1 = True
                ...
                if delay_neg_ap or delay_pos_ap:
                    if delay_neg_ap:
                        evaluation_1 = " ".join(
                            ...
                            f"{Signals.Columns.TRUNK} == 1: <br>"
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 - 10]} ({ap_states.iloc[time_threshold_2 - 10]}) <br>"
                            f"This means that AUPCore wasn't in Scanning state when AUPCore received the infromation that the trunk \
                                of the ego vehicle was open.".split()
                        )
                    elif delay_pos_ap:
                        evaluation_1 = " ".join(
                            ...
                            f"{Signals.Columns.TRUNK} == 1: <br>"
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 + 10]} ({ap_states.iloc[time_threshold_2 + 10]}) <br>"
                            f"This means that AUPCore didn't transition to Init state when AUPCore received the infromation that the trunk \
                                of the ego vehicle was open.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TRUNK} == 1 was never found.".split()
                )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_81_Req_2325920_2326020",
    description=(
        "This test case verifies that AUP will transition from Scanning to Init when AUPCore receives the information \
            that the trunk of the ego vehicle is open."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_81_Req_2325920_2326020_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_81_Req_2325920_2326020_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.trunkLidStatusPort_open_nu</em> == 1 <br><br>
            </td>
        </tr>
        <tr>
            <td>
