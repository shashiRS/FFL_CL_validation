"""The remote controller HMI shall display remote controlled LSMF status information to the driver: active (standby)"""
...
SIGNAL_DATA = "LSMF_REM_INFO_STANDBY"
...
@teststep_definition(
    name="Information about actual remote controlled LSMF state (standby) on remmote HMI",
    description="Check information on remote HMI about actual state of remote controlled LSMF.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class RemLsmfStatStandyCheck(TestStep):
    """RemLsmfStatStandyCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        evaluation1 = " ".join(
            f"The evaluation of {signal_name['Rem_general_message']} signal is PASSED, signal shows LEAVE_VEHICLE"
            f" ({constants.HilCl.Hmi.APHMIGeneralMessageRemote.LEAVE_VEHICLE}) after driver sent TAP_ON_START_PARKING"
            f" ({constants.HilCl.Hmi.RemoteCommands.TAP_ON_START_PARKING}) but didn't leave the car.".split()
        )
        ...
        if t_device_ready_idx is not None:
            ...
            if t_start_requset_idx is not None:
                ...
                states_dict = HilClFuntions.States(
                    rem_gen_message_sig, t_start_requset_idx - 1, len(rem_gen_message_sig), 1
                )  # t_start_requset_idx - 1 to skip not know system reaction time
                    ...
                    if counter == 1:
                        ...
                        if key < t_start_requset_idx:
                            evaluation1 = " ".join(
                                ...
                                f" before TAP_ON_START_PARKING ({constants.HilCl.Hmi.RemoteCommands.TAP_ON_START_PARKING}) sent via remote device event.".split()
                            )
                            eval_cond[0] = False
                            break

                        if states_dict[key] != constants.HilCl.Hmi.APHMIGeneralMessageRemote.LEAVE_VEHICLE:
                            evaluation1 = " ".join(
                                ...
                                f" but requiered mode is LEAVE_VEHICLE ({constants.HilCl.Hmi.APHMIGeneralMessageRemote.LEAVE_VEHICLE}).".split()
                            )
                            eval_cond[0] = False
                            break
                        counter += 1
        ...
        signal_summary["Remote HMI shall show stanby state of LSMF"] = evaluation1
...
@testcase_definition(
    name="Remote controlled LSMF status state on remoted HMI, Standby",
    description="The remote controller HMI shall display remote controlled LSMF status information to the driver: active (stanby)",
)
class RemLsmfStatStandy(TestCase):
    """RemLsmfStatStandy Test Case."""
