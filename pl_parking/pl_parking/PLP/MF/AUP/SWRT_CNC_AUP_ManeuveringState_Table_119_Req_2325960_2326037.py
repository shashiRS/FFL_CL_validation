...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_119_Req_2325960_2326037"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 119 Req 2325960 2326037",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that an EBD intervention is active for longer than 0.5 s."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_119_Req_2325960_2326037_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            signals["status_ebd_state"] = ebd_state.apply(lambda x: "EBD_ACTIVE" if x == 1 else "EBD_NOT_ACTIVE")
            ...
            time_threshold_2 = find_time_threshold(ebd_state, EBD_ACTIVE)
            ...
            if time_threshold_3 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.EBD_STATE} is in EBD_ACTIVE (1) state: <br>"
                    ...
                    f"This confirms that AUPCore transitioned from Maneuvering to Terminate (Error or Pause-Finished) when the AUPCore received the<br>\
                        information  that an EBD intervention is active for longer than {aup.AP_G_EBD_TIME_THRESH_S } s.".split()
                )
                eval_cond_1 = True
                    ...
                    if (delay_pos_ap or total_ap_finish != 1) or (delay_neg_ap or delay_neg_core):
                        if delay_neg_ap or delay_neg_core:
                            evaluation_1 = " ".join(
                                ...
                                f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                                f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                                f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received  that an EBD intervention is active for longer than {aup.AP_G_EBD_TIME_THRESH_S } s.".split()
                            )
                            ...
                            evaluation_1 = " ".join(
                                ...
                                f"{Signals.Columns.EBD_STATE} is in EBD_ACTIVE (1) state: <br>"
                                f"- AUPCore didn't transition to Pause and then to Finished state when the AUPCore received the information  that an EBD intervention is active for longer than {aup.AP_G_EBD_TIME_THRESH_S } s.".split()
                            )
                        eval_cond_1 = False
                ...
                if (delay_neg_ap or delay_neg_core or delay_pos_var or delay_pos_core) and not ap_pause:
                    if delay_neg_ap or delay_neg_core:
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                            f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information  that an EBD intervention is active for longer than {aup.AP_G_EBD_TIME_THRESH_S } s.".split()
                        )
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_3 + 10]} ({var_esm.iloc[time_threshold_3 + 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 + 10]} ({core_state.iloc[time_threshold_3 + 10]}) <br><br>"
                            f"This means that AUPCore didn't transition to Error state when the AUPCore received the information  that an EBD intervention is active for longer than {aup.AP_G_EBD_TIME_THRESH_S } s.".split()
                        )
                    eval_cond_1 = False
...
class SWRT_CNC_AUP_ManeuveringState_Table_119_Req_2325960_2326037_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_119_Req_2325960_2326037_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2:<em>SignalManipulation.escInformationPort_ebdState_nu  </em> == EBD_ACTIVE (1) <br>
                T3: T2 +  AP_G_EBD_TIME_THRESH_S  (0.5s)<br>
