...
T2:
SignalManipulation.loDMCStatusPort_loDMCSystemState_nu = {LODMC_NOT_AVAILABLE} (0)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_113_Req_2325960_2326031"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        LODMC_STATE = "SignalManipulation.loDMCStatusPort_loDMCSystemState_nu"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 113 Req 2325960 2326031",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that the LoActor reports a reversible error."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_113_Req_2325960_2326031_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            lodmc_state = signals[Signals.Columns.LODMC_STATE]
            ...
            signals["status_lodmc_state"] = lodmc_state.apply(lambda x: fh.get_status_label(x, aup.LODMC_SYSTEM_STATE))
            ...
            time_threshold_2 = find_time_threshold(lodmc_state, aup.LODMC_SYSTEM_STATE.LODMC_NOT_AVAILABLE)
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.LODMC_STATE} is in LODMC_NOT_AVAILABLE (0) state: <br>"
                    ...
                    f"This confirms that AUPCore transitioned from Maneuvering to Terminate (Error or Pause-Finished) when the AUPCore received the<br>\
                        information that the LoActor reports a reversible error.".split()
                )
                eval_cond_1 = True
                    ...
                    if (delay_pos_ap or total_ap_finish != 1) or (delay_neg_ap or delay_neg_core):
                        if delay_neg_ap or delay_neg_core:
                            evaluation_1 = " ".join(
                                ...
                                f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 - 10]} ({ap_states.iloc[time_threshold_2 - 10]}) <br>"
                                f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) <br><br>"
                                f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information that the LoActor had an error.".split()
                            )
                            ...
                            evaluation_1 = " ".join(
                                ...
                                f"{Signals.Columns.LODMC_STATE} is in LODMC_NOT_AVAILABLE (0) state: <br>"
                                f"- AUPCore didn't transition to Pause and then to Finished state when the AUPCore received the information that the LoActor had an error.".split()
                            )
                        eval_cond_1 = False
                ...
                if (delay_neg_ap or delay_neg_core or delay_pos_var or delay_pos_core) and not ap_pause:
                    if delay_neg_ap or delay_neg_core:
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_2 - 10]} ({ap_states.iloc[time_threshold_2 - 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) <br><br>"
                            f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information that the LoActor reports a reversible error..".split()
                        )
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) <br><br>"
                            f"This means that AUPCore didn't transition to Error state when the AUPCore received the information that the LoActor reports a reversible error..".split()
                        )
                    eval_cond_1 = False
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.LODMC_STATE} == LODMC_NOT_AVAILABLE (0) was never found.".split()
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=lodmc_state,
                    mode="lines",
                    name=Signals.Columns.LODMC_STATE,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_lodmc_state"],
                )
            )
            fig.add_trace(
                go.Scatter(
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_113_Req_2325960_2326031",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that the LoActor reports a reversible error."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_113_Req_2325960_2326031_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_113_Req_2325960_2326031_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>SignalManipulation.loDMCStatusPort_loDMCSystemState_nu</em> == LODMC_NOT_AVAILABLE (0) <br>
