"""Check HMI Display for LSMF Active - Maneuvering"""
...
SIGNAL_DATA = "LSMF_ACTIVE_MANEUVERING"
...
@teststep_definition(
    name="LSMF Active - Maneuvering Check",
    description="Verify that the HMI displays the maneuvering state when the AP function is active.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class LsmfActiveManeuveringCheck(TestStep):
    """LsmfActiveManeuveringCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        if t_start_parking_idx is None:
            ...
            # Step 2: Detect when the system transitions to PPC_PERFORM_PARKING state
            ...
            if t_maneuvering_idx is None:
                test_result = fc.FAIL
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['State_on_HMI']} is FAILED, the system never transitioned to PPC_PERFORM_PARKING [{constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}] (maneuvering state) after TAP_ON_START_PARKING [{constants.HilCl.Hmi.Command.TAP_ON_START_PARKING}].".split()
                )
            else:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['State_on_HMI']} is PASSED, the system transitioned to PPC_PERFORM_PARKING [{constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}] (maneuvering state) at {time_signal[t_maneuvering_idx]} [us]"
                    f" after TAP_ON_START_PARKING [{constants.HilCl.Hmi.Command.TAP_ON_START_PARKING}] action at {time_signal[t_start_parking_idx]} [us].".split()
                )
        ...
        signal_summary["LSMF Active - Maneuvering Evaluation"] = evaluation1
...
@testcase_definition(
    name="LSMF Active - Maneuvering Check",
    description="Verify that the HMI displays maneuvering status when AP function is active.",
)
class LsmfActiveManeuveringTest(TestCase):
    """LsmfActiveManeuveringTest Test Case."""
