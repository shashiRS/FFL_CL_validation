...
REGISTER = "SCAN_TO_IRREV_ERROR"


@teststep_definition(
    step_number=1,
    name="SCAN_TO_IRREV_ERROR",
...
class CheckScanToIrrevError(TestStep):
    """CheckScanToIrrevError Test Step."""
    ...
    def process(self, **kwargs):
        ...
        scanning_in_value = constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_SCANNING_IN
        ...
        step_key = _strip_str("'Scanning in' phase shall be observable within the measurement")

        scanning_in_starts_subwindow = support.signal_equals(ap_state_signal, scanning_in_value)

        irreversible_error_detected_time = None
        error_injection_starts_time = None

        # --- CHECK FOR SCANNING IN SEGMENT ---
        if not scanning_in_starts_subwindow:
            evaluation_steps[step_key] = _strip_str("Failed; 'Scanning in' phase not registered during the measurement")
            result_aggregator.append(False)
        else:
            scanning_in_starts_idx = scanning_in_starts_subwindow[0]
            highlight_eval_area_start = time_signal_in_sec[scanning_in_starts_idx]
            ...
            evaluation_steps[step_key] = _strip_str(
                f"'Scanning in' phase detected starting from {highlight_eval_area_start}[sec]<br/>(highlighted in summary plot)"
            )

            # --- CHECK FOR ERROR INJECTION SEGMENT WITHIN SCANNING SUBWINDOW---
            step_key = _strip_str(
            ...
            frame_timeout_signal = frame_timeout_signal[scanning_in_starts_idx:]
            ap_state_signal = ap_state_signal[scanning_in_starts_idx:]
            time_signal_in_sec = time_signal_in_sec[scanning_in_starts_idx:]
...
class ScanToIrrevError(TestCase):
    """ScanToIrrevError Test Case."""
