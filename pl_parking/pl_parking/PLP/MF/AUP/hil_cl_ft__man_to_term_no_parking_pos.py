"""Check the reaction of the system in case the selected parking slot is not available anymore during AVG ON."""
...
SIGNAL_DATA = "MANEUVERING_TO_TERMINATE_NO_PARKING_POSSIBLE"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering to Terminate, no parking possible",
    description="Check the behavior of the system in case the length of the selected parking slot is not sufficient anymore after AVG start",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class ManToTermNoParkingPossibleCheck(TestStep):
    """ManToTermNoParkingPossibleCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        eval_cond = [True] * 1
        evaluation1 = " ".join(
            f"The evaluation of ParkingProcedureCtrlState is PASSED, signal switch to PPC_BEHAVIOR_INACTIVE ({signal_name['State_on_HMI']} =="
        ...
        for cnt in range(0, len(HMI_Info)):
            if (
                HMI_Info[cnt] == constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING and
        ...
        # If AVG not activated, TC Failed
        if t1_idx is not None:
            ...
            if t2_idx is not None:
                # Check if we have a constant AVG state = 8
                ...
                if eval_cond[0] is not False:
                    ...
                    evaluation1 = " ".join(
                        f"TC Failed because AVG ({signal_name['State_on_HMI']}) don't have a constant active state (!= {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING} before precondition met).".split()
                        )
            else:
                eval_cond = [False] * 1
                evaluation1 = " ".join(
                "TC Failed because reference vehicle didn't move to the selected parking slot.".split()
                )
        else:
            eval_cond = [False] * 1
            evaluation1 = " ".join(
                    f"TC Failed because AVG never activated( {signal_name['State_on_HMI']} !="
                    f" {constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_PERFORM_PARKING}).".split()
                )

        signal_summary["Check the reaction of the system in case AVG activated and minimum length is not sufficient anymore."] = evaluation1
...
@testcase_definition(
    name="Maneuvering to Terminate No Parking Possible",
    description=f"The system shall switch from Maneuvering to Terminate({constants.HilCl.Hmi.ParkingProcedureCtrlState.PPC_BEHAVIOR_INACTIVE}) after reference vehicle start the movement.",
    doors_url="https://jazz.conti.de/rm4/resources/BI_jtlo4eeKEe6f_fKhM-zv_g?oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F26252"
)
class ManToTermNoParkingPossible(TestCase):
    """ManToTermNoParkingPossible Test Case."""
    ...
    def test_steps(self):
        ...
        return [
            ManToTermNoParkingPossibleCheck,
        ]
