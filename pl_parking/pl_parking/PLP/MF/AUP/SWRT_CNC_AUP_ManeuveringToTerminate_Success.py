...
T2:
AP.targetPosesPort.selectedPoseData.reachedStatus == TP_REACHED (1)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringToTerminate_Success"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        REACHED_STS = "AP.targetPosesPort.selectedPoseData.reachedStatus"
...
@teststep_definition(
    ...
    description=(
        "This test case verifies that the AutomatedParkingCore shall transition from Maneuvering to \
            Terminate if the ego vehicle reaches the target pose."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringToTerminate_Success_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            reached_sts = signals[Signals.Columns.REACHED_STS]
            ...
            signals["status_reached_sts"] = reached_sts.apply(lambda x: fh.get_status_label(x, aup.REACHED_STATUS))
            ...
            # Find the triggers time and make sure that they are in sequence T1 < T2 < T3 < T4
            time_threshold_1 = find_time_threshold(core_state, aup.CORE_STATUS.CORE_PARKING)

            # Check if target pose == TP_REACHED is present in erg after AUP state Parking
            if time_threshold_1 is not None:
                time_threshold_2 = (
                    find_time_threshold(reached_sts[time_threshold_1:], aup.REACHED_STATUS.TP_REACHED)
                    + time_threshold_1
                )
                # Check if state Pasue is present in erg after target pose == TP_REACHED
                if time_threshold_2 is not None:
                    core_pause_check = (core_state.iloc[time_threshold_2:] == aup.CORE_STATUS.CORE_PAUSE).any()
                    if core_pause_check:
                        time_threshold_3 = (
                            find_time_threshold(core_state[time_threshold_2:], aup.CORE_STATUS.CORE_PAUSE)
                            + time_threshold_2
                        )
                        # Check if state Finish is present in erg after state Pause
                        if time_threshold_3 is not None:
                            core_finish_check = (
                                core_state.iloc[time_threshold_3:] == aup.CORE_STATUS.CORE_FINISH
                            ).any()
                            if core_finish_check:
                                time_threshold_4 = (
                                    find_time_threshold(core_state[time_threshold_3:], aup.CORE_STATUS.CORE_FINISH)
                                    + time_threshold_3
                                )
                            else:
            ...
            if time_threshold_1 is not None and time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.CORE_STATE} == CORE_PARKING (2) and {Signals.Columns.REACHED_STS} == TP_REACHED (1): <br>"
                    ...
                    f"This confirms that AUP transitioned to Maneuvering state when the automated parking maneuvering was selected and stayed <br>\
                        in this state until the ego vehicle went standstill in the selected parking slot.".split()
                )
                eval_cond_1 = True
                ...
                if core_park or ap_active:
                    evaluation_1 = " ".join(
                        ...
                        f"This means that the AUP didn't transition to Maneuvering state after the automated parking maneuvering was selected and/or AUP dind't <br>\
                            stayed in this state until the ego vehicle stopped.".split()
                    )
                    eval_cond_1 = False
                ...
                self.result.measured_result = FALSE
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.REACHED_STS} == TP_REACHED (1) after <br>\
                        {Signals.Columns.CORE_STATE} == CORE_PARKING (2) was never found.".split()
                )

            if time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED because between the interval <b>[{round(time.iloc[time_threshold_3], 3)},{round(time.iloc[time_threshold_4], 3)}) s</b> given by the signals "
                    ...
                    f"This confirms that AUP transitioned to Pause state after the ego vehicle reached the target pose and AUP stayed <br>\
                        in this state until it transitioned to Finish state.".split()
                )
                eval_cond_2 = True

                interval_core = core_state.iloc[time_threshold_3 : (time_threshold_4 - 1)]
                core_pause = (interval_core != aup.CORE_STATUS.CORE_PAUSE).any()

                interval_ap = ap_states.iloc[time_threshold_3 : (time_threshold_4 - 1)]
                ap_pause = (interval_ap != aup.AP_STATES.AP_AVG_PAUSE).any()

                if core_pause or ap_pause:
                    evaluation_2 = " ".join(
                        f"The evaluation is FAILED because between the interval <b>[{round(time.iloc[time_threshold_3], 3)},{round(time.iloc[time_threshold_4], 3)}) s</b> given by the signals "
                        f"{Signals.Columns.CORE_STATE} == CORE_PARKING (2) and {Signals.Columns.CORE_STATE} == CORE_FINISH (4): <br>"
                        f"- the signal {Signals.Columns.AP_STATES} != CORE_PAUSE (3) <br>"
                        f"- the signal {Signals.Columns.CORE_STATE} != AP_AVG_PAUSE (5) <br><br>"
                        f"This means that the AUP didn't transition to Pause state after the ego vehicle reached the target pose or <br>\
                            AUP didn't remained in that state until it transitioned to Finish.".split()
                    )
                    eval_cond_2 = False
            elif time_threshold_3 is None:
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the triggers {Signals.Columns.CORE_STATE} == CORE_PAUSE (3) after <br>\
                        {Signals.Columns.REACHED_STS} == TP_REACHED (1) was never found.".split()
                )
            elif time_threshold_4 is None:
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger {Signals.Columns.CORE_STATE} == CORE_FINISH (4) after <br>\
                        {Signals.Columns.CORE_STATE} == CORE_PAUSE (3) was never found.".split()
                )

            if time_threshold_4 is not None:
                evaluation_3 = " ".join(
                    f"The evaluation is PASSED because at the timestamp <b>{round(time.iloc[time_threshold_4], 3)} s</b> given by the signal "
                    f"{Signals.Columns.CORE_STATE} == CORE_FINISH (4) <br>"
                    f"the signal {Signals.Columns.AP_STATES} ==  AP_AVG_FINISHED (8) <br><br>"
                    f"This confirms that AUP transitioned from Pause to Finished state after the ego vehicle reached the target pose.".split()
                )
                eval_cond_3 = True

                ap_finished = ap_states.iloc[time_threshold_4] != aup.AP_STATES.AP_AVG_FINISHED

                if ap_finished:
                    evaluation_3 = " ".join(
                        f"The evaluation is FAILED because at the timestamp <b>{round(time.iloc[time_threshold_4], 3)} s</b> given by the signal "
                        f"{Signals.Columns.CORE_STATE} == CORE_FINISH (4) <br>"
                        f"the signal {Signals.Columns.AP_STATES} !=  AP_AVG_FINISHED (8) <br><br>"
                        f"This means that AUP didn't transition from Pause to Finished state after the ego vehicle reached the target pose.".split()
                    )
            else:
                self.result.measured_result = FALSE
                evaluation_3 = " ".join(
            ...
            signal_summary["T3-T4"] = [expected_val_2, evaluation_2, verdict2]
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=reached_sts,
                    mode="lines",
                    name=Signals.Columns.REACHED_STS,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_reached_sts"],
                )
            )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringToTerminate_Success",
    description=(
        "This test case verifies that the AutomatedParkingCore shall transition from Maneuvering to \
            Terminate if the ego vehicle reaches the target pose."
    ),
...
class SWRT_CNC_AUP_ManeuveringToTerminate_Success_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [SWRT_CNC_AUP_ManeuveringToTerminate_Success_TS]  # in this list all the needed test steps are included
    ...
    <table>
        <tr>
            <td>
                ...
                T2:<br>
                    <em>AP.targetPosesPort.selectedPoseData.reachedStatus</em> == TP_REACHED (1)<br>
                T3:<br>
