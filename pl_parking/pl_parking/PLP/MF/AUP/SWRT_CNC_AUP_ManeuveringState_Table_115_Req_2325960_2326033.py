...
T2:
SignalManipulation.accInformationPort_accStatus_nu = {ACC_ACTIVE} (3)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_115_Req_2325960_2326033"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        ACC_STATE = "SignalManipulation.accInformationPort_accStatus_nu"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 115 Req 2325960 2326033",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that the ACC System is active."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_115_Req_2325960_2326033_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            acc_state = signals[Signals.Columns.ACC_STATE]
            ...
            signals["status_acc_state"] = acc_state.apply(lambda x: "ACC_ACTIVE" if x == 3 else "ACC_NOT_ACTIVE")
            ...
            time_threshold_2 = find_time_threshold(acc_state, ACC_ACTIVE)
            ...
            if time_threshold_3 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"{Signals.Columns.ACC_STATE} is in ACC_ACTIVE (3) state: <br>"
                    ...
                    f"This confirms that AUPCore transitioned from Maneuvering to Terminate (Error or Pause-Finished) when the AUPCore received the<br>\
                        information that the ACC System is active.".split()
                )
                eval_cond_1 = True
                    ...
                    if (delay_pos_ap or total_ap_finish != 1) or (delay_neg_ap or delay_neg_core):
                        if delay_neg_ap or delay_neg_core:
                            evaluation_1 = " ".join(
                                ...
                                f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                                f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                                f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received that the ACC System is active.".split()
                            )
                            ...
                            evaluation_1 = " ".join(
                                ...
                                f"{Signals.Columns.ACC_STATE} is in ACC_ACTIVE (3) state: <br>"
                                f"- AUPCore didn't transition to Pause and then to Finished state when the AUPCore received the information that the ACC System is active.".split()
                            )
                        eval_cond_1 = False
                ...
                if (delay_neg_ap or delay_neg_core or delay_pos_var or delay_pos_core) and not ap_pause:
                    if delay_neg_ap or delay_neg_core:
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.AP_STATES} == {signals['status_ap_states'].iloc[time_threshold_3 - 10]} ({ap_states.iloc[time_threshold_3 - 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 - 10]} ({core_state.iloc[time_threshold_3 - 10]}) <br><br>"
                            f"This means that AUPCore wasn't in Maneuvering state when the AUPCore received the information that the ACC System is active.".split()
                        )
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"- the signal {Signals.Columns.VAR_ESM} == {signals['status_var_esm'].iloc[time_threshold_3 + 10]} ({var_esm.iloc[time_threshold_3 + 10]}) <br>"
                            f"- the signal {Signals.Columns.CORE_STATE} == {signals['status_core_state'].iloc[time_threshold_3 + 10]} ({core_state.iloc[time_threshold_3 + 10]}) <br><br>"
                            f"This means that AUPCore didn't transition to Error state when the AUPCore received the information that the ACC System is active.".split()
                        )
                    eval_cond_1 = False
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.ACC_STATE} == ACC_ACTIVE (3) was never found.".split()
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=acc_state,
                    mode="lines",
                    name=Signals.Columns.ACC_STATE,
                    hovertemplate="Time: %{x}<br>Value: %{y}<br>Status: %{text}<extra></extra>",
                    text=signals["status_acc_state"],
                )
            )
            fig.add_trace(
                go.Scatter(
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_115_Req_2325960_2326033",
    description=(
        "This test case verifies that AUPCore shall transition from Maneuvering to Terminate (Error or Pause-Finished) if \
            the AUPCore receives the information that the ACC System is active."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_115_Req_2325960_2326033_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_115_Req_2325960_2326033_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2:<em>SignalManipulation.accInformationPort_accStatus_nu</em> == ACC_ACTIVE (3) <br>
