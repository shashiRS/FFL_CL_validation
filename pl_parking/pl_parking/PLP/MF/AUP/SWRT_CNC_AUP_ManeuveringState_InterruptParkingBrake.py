...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_InterruptParkingBrake"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Interrupt Parking Brake",
    description=(
        "This test case verifies that in Maneuvering state the AutomatedParkingCore core shall stop to follow the path and secure the vehicle in case a driver intervention is detected (driver engages the parking brake)."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_InterruptParkingBrake_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if time_threshold_2 is not None:
                ...
                evaluation_2 = " ".join(
                    f"The evaluation PASSED at the moment when {Signals.Columns.DM_BRAKE_PARK} == 1 "
                    f"(<b>{round(time.iloc[time_threshold_2], 3)} s</b>). <br>"
                    f"At this point, the vehicle correctly followed the trajectory before the driver engaged the parking brake. <br>"
                    f"After the event, the vehicle stopped following the path and successfully secured itself, as expected.".split()
                )
                ...
                if delay_negative or delay_posistive:
                    evaluation_2 = " ".join(
                        f"The evaluation FAILED at the moment when {Signals.Columns.DM_BRAKE_PARK} == 1 "
                        f"(<b>{round(time.iloc[time_threshold_2], 3)} s</b>). <br>"
                        f"The signal {Signals.Columns.DRV_MODE_REQ} did not behave as expected: <br>"
                        f" - Before the driver engaged the parking brake (<b>{round(time.iloc[time_threshold_2 - 10], 3)} s</b>), "
                        f"the signal was in state {signals['status_drv'].iloc[time_threshold_2 - 10]} "
                        f"({drv_mode_req.iloc[time_threshold_2 - 10]}). <br>"
                        f" - After the driver engaged the parking brake (<b>{round(time.iloc[time_threshold_2 + 10], 3)} s</b>), "
            ...
            fig.layout = go.Layout(
                ...
                xaxis_title="Time[s]",
                title="Interrupt Parking Brake Graphical Overview",
            )
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_InterruptParkingBrake",
    description=(
        "This test case verifies that in Maneuvering state the AutomatedParkingCore core shall stop to follow the path and secure the vehicle in case a driver intervention is detected (driver engages the parking brake)."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_InterruptParkingBrake_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_InterruptParkingBrake_TS
