...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_36_Req_2325962_2326050"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 36 Req 2325962 2326050",
    description=(
        "This test case verifies that if AUP receives the information that the accelerator pedal is operated (and the threshold is exeeded) \
            while Remote Control is active, then the Remote Control will be paused and then finished."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_36_Req_2325962_2326050_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            gas = signals[Signals.Columns.GAS]
            ...
            # Checking for T2 after second 18 in scenario to ignore the values before the manipulation
            time_threshold_2 = (
                find_time_threshold(gas.iloc[1800:], hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC / 100) + 1800
            )
            time_threshold_3 = len(time) - 1

            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    f"The evaluation is PASSED because right <b>before</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                    f"the signal <b>{Signals.Columns.GAS}</b> > {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100}<br>"
                    f"- the signal {Signals.Columns.AP_STATE} is in AP_AVG_ACTIVE_IN (3) state;"
                    ...
                    f"This confirms that AUP detects that the accelerator pedal is operated and the RC is paused due to this.".split()
                )
                ...
                if delay_neg_rm or delay_pos_rm or delay_neg_screen:
                    if delay_neg_rm and delay_pos_rm and delay_neg_screen:
                        evaluation_1 = " ".join(
                            f"The evaluation is FAILED because right <b>before</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                            f"the signal <b>{Signals.Columns.GAS}</b> > {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100}<br>"
                            f"- the signal {Signals.Columns.AP_STATE} wasn't in AP_AVG_ACTIVE_IN (3) state<br> AND <b>after</b> that timestamp<br>"
                            f"- the signal {Signals.Columns.SCREEN_NU} wasn't in REMOTE_APP_ACTIVE (3) state<br> AND <b>after</b> that timestamp<br>"
                            f"- the signal {Signals.Columns.AP_STATE} isn't in AP_AVG_PAUSE (5) state, as expected.<br>This means that when "
                            f"AUP detected that the accelerator pedal was operated, the RC didn't transition from active to paused.".split()
                        )
                    elif delay_neg_rm or delay_neg_screen:
                        evaluation_1 = " ".join(
                            f"The evaluation is FAILED because right <b>before</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                            f"the signal <b>{Signals.Columns.GAS}</b> > {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100}<br>"
                            f"- the signal {Signals.Columns.AP_STATE} wasn't in AP_AVG_ACTIVE_IN (3) state OR<br>"
                            f"- the signal {Signals.Columns.SCREEN_NU} wasn't in REMOTE_APP_ACTIVE (3) state.<br>"
                            f"This means that before AUP detected that the accelerator pedal was operated the RC wasn't active.".split()
                        )
                    elif delay_pos_rm:
                        evaluation_1 = " ".join(
                            f"The evaluation is FAILED because <b>after</b> the timestamp <b>{round(time.iloc[time_threshold_2], 3)}s</b> when "
                            f"the signal <b>{Signals.Columns.GAS}</b> > {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100}<br>"
                            f"- the signal {Signals.Columns.AP_STATE} isn't in AP_AVG_PAUSE (3) state, as expected.<br>This means that when AUP detected"
                            f" that the accelerator pedal was operated, the RC wasn't paused.".split()
                        )
                    eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value ({Signals.Columns.GAS} > {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100}"
                    f"was never found.".split()
                )

            if time_threshold_2 is not None and time_threshold_3 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED because between the interval given by the signal {Signals.Columns.GAS} being "
                    f"> {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100} and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)} - "
                    ...
                    f"This confirms that after the RC is in paused state (due to operation of acceleration pedal) it will go into finished state once.".split()
                )
                ...
                mask_rm_finished = rm_interval.rolling(2).apply(
                    lambda x: x[0] != aup.RM_STATE.RM_AVG_FINISHED and x[1] == aup.AP_STATES.AP_AVG_FINISHED, raw=True
                )
                total_rm_finished = mask_rm_finished.sum()

                if total_rm_finished != 1:
                    evaluation_2 = " ".join(
                        f"The evaluation is FAILED because between the interval given by the signal {Signals.Columns.GAS} being "
                        f"> {hil.ApThreshold.AP_G_THROTTLE_THRESH_PERC/100} and the end of the measurement <b>({round(time.iloc[time_threshold_2], 3)} - "
                        f"{round(time.iloc[time_threshold_3], 3)})</b>:<br>- the signal {Signals.Columns.AP_STATE} doesn't reach"
                        f" the AP_AVG_FINISHED (8) state once.<br>This means that after the RC is in paused state (due to operation of acceleration pedal)"
                        f" it doesn't go into finished state once.".split()
                    )
                eval_cond_2 = False
            else:
                # create an evaluation text for the moment when the threshold isn't found, set test_result to NOT_ASSESSED
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_36_Req_2325962_2326050",
    description=(
        "This test case verifies that if AUP receives the information that the accelerator pedal is operated (and the threshold is exeeded) \
            while Remote Control is active, then the Remote Control will be paused and then finished."
    ),
...
class SWRT_CNC_AUP_ManeuveringState_Table_36_Req_2325962_2326050_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_36_Req_2325962_2326050_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: <br>
                    <em>DM.Gas > DIV(AP_G_THROTTLE_THRESH_PERC, 100) (10 %) <br><br>
