...
T2: // AVGA failure
SignalManipulation.healthVector_avgaFailed = 1

T3: // heal condition
SignalManipulation.healthVector_avgaFailed = 0
...
AP.apUserInformationPort.generalUserInformation_nu == AVGA_FAILED (84)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_63_Req_2325842_2326007_2325843"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        AVGA_FAILURE = "SignalManipulation.healthVector_avgaFailed"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.AVGA_FAILURE: [
                "SignalManipulation.healthVector_avgaFailed",
            ],
            self.Columns.IGN_ON: [
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 63 Req 2325842 2326007 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that AVGA (automated vehicle guidance arbiter) failed, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_63_Req_2325842_2326007_2325843_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            avga_failure = signals[Signals.Columns.AVGA_FAILURE]
            ...
            time_threshold_2 = find_time_threshold(avga_failure, 1)
            time_threshold_3 = find_time_threshold(avga_failure.iloc[time_threshold_2:], 0) + time_threshold_2
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.AVGA_FAILURE}</b> == 1: <br>"
                    ...
                    f"This confirms that AUPCore received the information that the AVGA failed <br>and then it went into error "
                ...
                delay_pos_user = user_inf.iloc[time_threshold_2 + 10] != aup.HMI_MESSAGE.AVGA_FAILED
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before AVGA failed and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before AVGA failed.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due to AVGA failure.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"when {Signals.Columns.AVGA_FAILURE} == 1. This means {reason}".split()
                        )
                        eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.AVGA_FAILURE} == 1 was never found.".split()
                )
            if time_threshold_2 is not None and time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED between the interval given by the signal {Signals.Columns.AVGA_FAILURE} == 1 "
                ...
                error_state = avga_failure.iloc[time_threshold_3 : (time_threshold_4 - 10)]
                ...
                if cond_neg_core or cond_neg_error:
                    if cond_neg_core and cond_neg_error:
                        evaluation_2 = " ".join(
                            f"The evaluation FAILED between the interval given by the signal {Signals.Columns.AVGA_FAILURE} == 1<br> "
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                        ...
                        evaluation_2 = " ".join(
                            ...
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                            f"<b>({round(time.iloc[time_threshold_2], 3)} - {round(time.iloc[time_threshold_5 - 10], 3)})s</b>,<br> "
                            f"because the signal {Signals.Columns.AVGA_FAILURE} wasn't set to 0.<br><br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.AVGA_FAILURE} == 0 was never found between T2 and T4.<br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger values {Signals.Columns.AVGA_FAILURE} == 1 or {Signals.Columns.IGN_ON} == Ignition OFF (0) "
            ...
            AP.apUserInformationPort.generalUserInformation_nu == AVGA_FAILED (84)<br>"""
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=avga_failure,
                    mode="lines",
                    name=Signals.Columns.AVGA_FAILURE,
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_63_Req_2325842_2326007_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that AVGA (automated vehicle guidance arbiter) failed, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_63_Req_2325842_2326007_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_63_Req_2325842_2326007_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //AVGA failure<br>
                    <em>SignalManipulation.healthVector_avgaFailed</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_avgaFailed</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == AVGA_FAILED (84) <br>
                T2 - T4:<br>
