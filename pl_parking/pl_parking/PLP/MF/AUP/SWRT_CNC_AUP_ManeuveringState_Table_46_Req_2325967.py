...
OR "AP.psmDebugPort.stateVarDM_nu" == {DM_DRIVER_MANEUVERING} (1)
"""
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_46_Req_2325967"
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 46 Req 2325967",
    description=(
        "This test case verifies that if AUP receives the information that the accelerator pedal is operated (and the threshold is exceeded)."
    ),
    expected_result=BooleanResult(TRUE),
)
@register_signals(ALIAS, Signals)
class SWRT_CNC_AUP_ManeuveringState_Table_46_Req_2325967_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            t2 = gas.iloc[moment_of_manipulation:] > (aup.AP_G_THROTTLE_THRESH_PERC / 100)
            if np.any(t2):
                time_threshold_2 = np.argmax(t2) + moment_of_manipulation
            ...
            if time_threshold_2 is not None:
                ...
                delay_pos_ap = ap_state.iloc[time_threshold_2 + 10] == aup.AP_STATES.AP_INACTIVE
                delay_pos_dm = state_var_dm.iloc[time_threshold_2 + 10] == aup.STATE_VAR_DM.DM_DRIVER_MANEUVERING
                ...
                evaluation_1 = " ".join(
                    ...
                    f"This confirms that AUP receives the information that the accelerator pedal is operated (and the threshold is exceeded).".split()
                )
                ...
                if delay_neg or delay_pos:
                    ...
                    evaluation_1 = " ".join(
                        ...
                        f"- the signal {Signals.Columns.AP_STATE} is in {signals['status_ap_state'].iloc[time_threshold_2+10]} ({ap_state.iloc[time_threshold_2+10]})state<br>"
                        f"- the signal {Signals.Columns.STATE_VAR_DM} is in {signals['status_var_dm'].iloc[time_threshold_2+10]} ({state_var_dm.iloc[time_threshold_2+10]})state<br>"
                        f"This confirms that AUP didn't received the information that the accelerator pedal is operated (and the threshold is exceeded).".split()
                    )
            else:
            ...
            expected_val_1 = """delay -0.1s:\
                ...
                delay 0.1s: <br>\
                "AP.planningCtrlPort.apStates" == AP_INACTIVE (0) OR <br>\
                    "AP.psmDebugPort.stateVarDM_nu" == DM_DRIVER_MANEUVERING (1)<br>"""
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_46_Req_2325967",
    description=(
        "This test case verifies that if AUP receives the information that the accelerator pedal is operated (and the threshold is exceeded)."
    ),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class SWRT_CNC_AUP_ManeuveringState_Table_46_Req_2325967_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_46_Req_2325967_TS
    ...
    <table>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    <em>AP.planningCtrlPort.apStates</em> == AP_INACTIVE (0) OR<br>
                    <em>AP.psmDebugPort.stateVarDM_nu</em> == DM_DRIVER_MANEUVERING (1) <br>
