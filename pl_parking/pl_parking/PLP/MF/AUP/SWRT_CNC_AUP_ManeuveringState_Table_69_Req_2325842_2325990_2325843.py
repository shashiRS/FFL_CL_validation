...
T2: // temperature warning
SignalManipulation.healthVector_hostTemperatureWarning = 1

T3: // heal condition
SignalManipulation.healthVector_hostTemperatureWarning = 0
...
AP.apUserInformationPort.generalUserInformation_nu == HOST_TEMPERATURE_WARNING (53)
...
ALIAS = "SWRT_CNC_AUP_ManeuveringState_Table_69_Req_2325842_2325990_2325843"
...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        TEMP_WRN = "SignalManipulation.healthVector_hostTemperatureWarning"
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.TEMP_WRN: [
                "SignalManipulation.healthVector_hostTemperatureWarning",
            ],
            self.Columns.IGN_ON: [
...
@teststep_definition(
    step_number=1,
    name="Maneuvering State Table 69 Req 2325842 2325990 2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the host platform has a temperature warning, which is \
...
class SWRT_CNC_AUP_ManeuveringState_Table_69_Req_2325842_2325990_2325843_TS(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            temp_wrn = signals[Signals.Columns.TEMP_WRN]
            ...
            time_threshold_2 = find_time_threshold(temp_wrn, 1)
            time_threshold_3 = find_time_threshold(temp_wrn.iloc[time_threshold_2:], 0) + time_threshold_2
            ...
            if time_threshold_2 is not None:
                evaluation_1 = " ".join(
                    ...
                    f"the signal <b>{Signals.Columns.TEMP_WRN}</b> == 1: <br>"
                    ...
                    f"This confirms that AUPCore received the information that the host platform has a temperature warning <br>and then it went into error "
                ...
                delay_pos_user = user_inf.iloc[time_threshold_2 + 10] != aup.HMI_MESSAGE.HOST_TEMPERATURE_WARNING
                ...
                if delay_neg_core or delay_neg_esm or delay_pos_core or delay_pos_esm or delay_pos_user:
                    if delay_neg_core and delay_neg_esm and delay_pos_core and delay_pos_esm and delay_pos_user:
                        ...
                        reason = " ".join(
                            f"AUPCore wasn't error free before a temperature warning was detected and it didn't transition to error state<br>"
                        ...
                        if delay_neg_core or delay_neg_esm:
                            timeline = "before"
                            reason = " ".join(
                                f"AUPCore wasn't error free before a temperature warning was detected.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 - 10]} ({core_state.iloc[time_threshold_2 - 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 - 10], 3)}s {Signals.Columns.VAR_ESM } was in "
                            ...
                            timeline = "after"
                            reason = " ".join(
                                f"AUPCore didn't transition to error state due to temperature warning.<br>"
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.CORE_STATE} was in "
                                f"{signals['status_core_state'].iloc[time_threshold_2 + 10]} ({core_state.iloc[time_threshold_2 + 10]}) state "
                                f"- At time <b>{round(time.iloc[time_threshold_2 +  10], 3)}s {Signals.Columns.VAR_ESM } was in "
                                f"{signals['status_var_esm'].iloc[time_threshold_2 + 10]} ({var_esm.iloc[time_threshold_2 + 10]}) state"
                        ...
                        evaluation_1 = " ".join(
                            ...
                            f"when {Signals.Columns.TEMP_WRN} == 1. This means {reason}".split()
                        )
                        eval_cond_1 = False
            else:
                ...
                evaluation_1 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TEMP_WRN} == 1 was never found.".split()
                )
            if time_threshold_2 is not None and time_threshold_3 is not None and time_threshold_4 is not None:
                evaluation_2 = " ".join(
                    f"The evaluation is PASSED between the interval given by the signal {Signals.Columns.TEMP_WRN} == 1 "
                ...
                error_state = temp_wrn.iloc[time_threshold_3 : (time_threshold_4 - 10)]
                ...
                if cond_neg_core or cond_neg_error:
                    if cond_neg_core and cond_neg_error:
                        evaluation_2 = " ".join(
                            f"The evaluation FAILED between the interval given by the signal {Signals.Columns.TEMP_WRN} == 1<br> "
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                        ...
                        evaluation_2 = " ".join(
                            ...
                            f"and right before the signal {Signals.Columns.IGN_ON} == Ignition OFF (0) "
                            f"<b>({round(time.iloc[time_threshold_2], 3)} - {round(time.iloc[time_threshold_5 - 10], 3)})s</b>,<br> "
                            f"because the signal {Signals.Columns.TEMP_WRN} wasn't set to 0.<br><br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger value {Signals.Columns.TEMP_WRN} == 0 was never found between T2 and T4.<br>"
                ...
                self.result.measured_result = FALSE
                evaluation_2 = " ".join(
                    f"Evaluation not possible, the trigger values {Signals.Columns.TEMP_WRN} == 1 or {Signals.Columns.IGN_ON} == Ignition OFF (0) "
            ...
            AP.apUserInformationPort.generalUserInformation_nu == HOST_TEMPERATURE_WARNING (53)<br>"""
...
@testcase_definition(
    name="SWRT_CNC_AUP_ManeuveringState_Table_69_Req_2325842_2325990_2325843",
    description=(
        "This test case verifies that if AUPCore receives the information that the host platform has a temperature warning, which is \
            an irreversible error, AUPCore shall go into error state and stay in this state until the ignition is switched off.\
                Also, information about the reason of the error shall be provided by AUPCore."
...
class SWRT_CNC_AUP_ManeuveringState_Table_69_Req_2325842_2325990_2325843_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SWRT_CNC_AUP_ManeuveringState_Table_69_Req_2325842_2325990_2325843_TS
    ...
    <table>
        <tr>
            <td>
                ...
                T2: //a temperature warning<br>
                    <em>SignalManipulation.healthVector_hostTemperatureWarning</em> == 1 <br>
                T3: //heal condition<br>
                    <em>SignalManipulation.healthVector_hostTemperatureWarning</em> == 0 <br>
        ...
        <tr>
            <td>
                ...
                delay 0.1s: <br>
                    ...
                    <em>AP.apUserInformationPort.generalUserInformation_nu</em> == HOST_TEMPERATURE_WARNING (53) <br>
                T2 - T4:<br>
