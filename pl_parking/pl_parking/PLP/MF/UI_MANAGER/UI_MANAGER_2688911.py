...
@teststep_definition(
    step_number=1,
    name="Inform about optical warning deactivation",  # this would be shown as a test step name in html report
    description=(
        "HMIManager shall perform arbitration between the PDWCore and Infotainment with information about deactivation of optical warning."
...
class TestStepUI_MANAGER(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            T1 = None
            table_title = "Check if HMIManager disables optical warnings for {} sectors"
            remark = f"<b>Signals evaluated:</b><br><br>\
                       Input: <b>{signals_obj._properties[Signals.Columns.HMI_OUTPUT_USER_ACTION]} == {constants.UiManager.UserAction.get_variable_name(constants.UiManager.UserAction.TAP_ON_HIDE_PDW)}</b><br>"
            ...
            signal_summary = {
                s: None for s in signals_obj.sides
            ...
            # pdc_series = signals[Signals.Columns.PDW_SYS_STATE]
            hmi_series = signals[Signals.Columns.HMI_OUTPUT_USER_ACTION]

            hmi_series_events = hmi_series.rolling(2).apply(
                lambda x: x[1] != x[0] and x[1] == constants.UiManager.UserAction.TAP_ON_HIDE_PDW, raw=True
            )
            ...
            series_dict = {
                Signals.Columns.SECTOR_DISTANCE_HMI.format(side, x).format(side, x): signals[
                    Signals.Columns.SECTOR_DISTANCE_HMI.format(side, x)
                ]
                for side in signals_obj.sides
                for x in range(4)
            }
            # series_dict.update({Signals.Columns.SECTOR_DISTANCE_HMI.format(side, x).format(side, x): signals[Signals.Columns.SECTOR_DISTANCE_HMI.format(side, x)] for side in signals_obj.sides for x in range(4)})
            ...
            if timestamp_pdw_events:
                for ts in timestamp_pdw_events:
                    for side in signals_obj.sides:
                        ...
                        transition_message.clear()
                        list_of_signals = [x for x in series_dict.keys() if side in x]
                        for s in list_of_signals:

                            # pdw_sys_state = pdc_series.loc[ts]
                            T1 = list(signals.index).index(ts)
                            sub_series = series_dict[s].iloc[T1 : T1 + max_delay + 1]
                            if not sub_series[sub_series >= constants.UiManager.PDW_DEFAULT_MINIMUM_DISTANCE].empty:
                                ...
                                transition_message.append(
                                    f"HMIManager did disable warnings, after {it_took} ms, within {max_delay * 10} ms of user input."
                                )
                            else:
                                transition_events.append(False)
                                transition_message.append(
                                    f"HMIManager did not disable warnings, within {max_delay * 10} ms of user input."
                                )
            ...
            for side in signals_obj.sides:
                fig = go.Figure()
                remark += f"Output: <b>AP.hmiInputPort.pdcSectors.{side.lower()}_%.smallestDistance_m >= {constants.UiManager.PDW_DEFAULT_MINIMUM_DISTANCE} m (DEFAULT_MINIMUM_DISTANCE)</b><br>"
                sig_sum = fh.build_html_table(pd.DataFrame(signal_summary[side]), remark, table_title.format(side))
                ...
                for s in list_of_signals:
                    fig.add_trace(
                        go.Scatter(
                            x=ap_time,
                            y=signals[s].values.tolist(),
                            mode="lines",
                            name=signals_obj._properties[s],
                            # text=[
                            #     f"Timestamp: {ap_time[idx]} <br>"
                            #     f"HMI USer Action:<br>"
...
@testcase_definition(
    name="UI_MANAGER_2688911",
    description="HMIManager shall perform arbitration between the PDWCore and Infotainment with information about deactivation of optical warning.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_Zdx8oHpsEe-9FrFCDLx05w&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_ns_CwE4gEe6M5-WQsF_-tQ&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
