...
@teststep_definition(
    step_number=1,
    name="Check HMIHandler input for Tone H",  # this would be shown as a test step name in html report
    description=(
        "HMIManager shall forward an input to InstrumentClusterDisplay about the acoustical warning that needs to be displayed."
...
class TestStepUI_MANAGER(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            tone_h_events = tone_h.rolling(2).apply(lambda x: x[1] != x[0], raw=True)
            ...
            if timestamp_tone_h_events:
                for ts in timestamp_tone_h_events:
                    ...
                    transition_message.append(f"Tone H transitioned from {tone_h.iloc[T1-1]} to {tone_h.iloc[T1]},.")
                signal_summary = {
                    "Timestamp [s]": {idx: ap_time.loc[x] for idx, x in enumerate(timestamp_tone_h_events)},
            ...
            table_title = "HMIManager shall forward an input to InstrumentClusterDisplay about the acoustical warning that needs to be displayed"
            remark = f"<b>Signals evaluated:</b><br><br>\
                       Input: <b>{signals_obj._properties[tone_h.name]}</b>"
...
@testcase_definition(
    name="UI_MANAGER_2688895",
    description="HMIManager shall forward an input to InstrumentClusterDisplay about the acoustical warning that needs to be displayed.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_Zdr2AHpsEe-9FrFCDLx05w&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_ns_CwE4gEe6M5-WQsF_-tQ&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
