...
@teststep_definition(
    step_number=1,
    name="Inform about LSCA deactivation",  # this would be shown as a test step name in html report
    description=(
        "The HMIHandler shall give the input to the LSCACore about the deactivation from infotainment settings."
...
class TestStepUI_MANAGER(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            pdc_series = signals[Signals.Columns.PDC_USER_INTERACTION_LSCA_ACTIVE]
            hmi_series = signals[Signals.Columns.HMI_OUTPUT_USER_ACTION]

            hmi_series_events = hmi_series.rolling(2).apply(
                lambda x: x[1] != x[0] and x[1] == constants.UiManager.UserAction.TAP_ON_LSCA, raw=True
            )
            transition_events = []
            transition_message = []
            ...
            if timestamp_pdw_events:
                for ts in timestamp_pdw_events:
                    ...
                    if not sub_series[sub_series == constants.UiManager.LscaCore.LSCA_DISABLED].empty:
                        transition_events.append(True)

                        end_idx = sub_series[
                            sub_series == constants.UiManager.LscaCore.LSCA_DISABLED
                        ].first_valid_index()
                        ...
                        transition_message.append(
                            f"HMIManager did forward the input about the LSCA function disabled state, after {it_took} ms, within {max_delay * 10} ms of user input."
                        )
                    else:
                        transition_events.append(False)
                        transition_message.append(
                            f"HMIManager did not forward the input about the LSCA function disabled state, within {max_delay * 10} ms of user input."
                        )
            ...
            table_title = "Check if HMIManager forwards the input about the LSCA function's state"
            remark = f"<b>Signals evaluated:</b><br><br>\
                       Input: <b>{signals_obj._properties[Signals.Columns.HMI_OUTPUT_USER_ACTION]} == {constants.UiManager.UserAction.get_variable_name(constants.UiManager.UserAction.TAP_ON_LSCA)}</b><br> \
                       Output: <b>{signals_obj._properties[Signals.Columns.PDC_USER_INTERACTION_LSCA_ACTIVE]}== {constants.UiManager.LscaCore.get_variable_name(constants.UiManager.LscaCore.LSCA_DISABLED)}</b>"
            ...
            fig.add_trace(
                go.Scatter(
                    x=ap_time,
                    y=signals[Signals.Columns.PDC_USER_INTERACTION_LSCA_ACTIVE].values.tolist(),
                    mode="lines",
                    name=signals_obj._properties[Signals.Columns.PDC_USER_INTERACTION_LSCA_ACTIVE],
                    text=[
                        f"Timestamp: {ap_time[idx]} <br>"
                        f"PDC UserInteraction:<br>"
                        f"{constants.UiManager.LscaCore.get_variable_name(state)}"
                        for idx, state in enumerate(
                            signals[Signals.Columns.PDC_USER_INTERACTION_LSCA_ACTIVE].values.tolist()
                        )
                    ],
                    hoverinfo="text",
                )
...
@testcase_definition(
    name="UI_MANAGER_2688809",
    description="The HMIHandler shall give the input to the LSCACore about the deactivation from infotainment settings.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_ZdQYMHpsEe-9FrFCDLx05w&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_ns_CwE4gEe6M5-WQsF_-tQ&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
