...
class Signals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        TIME = "TimeStamp"
        # HMI_INPUT_SYS_STATE = "HMI System State"
        PDW_SYS_STATE = "PDW_SYS_STATE"
        HMI_INPUT_SYS_STATE = "HMI_INPUT_SYS_STATE"
    ...
    def __init__(self):
        ...
        self._properties = {
            self.Columns.HMI_INPUT_SYS_STATE: "AP.hmiInputPort.general.pdcSystemState_nu",
...
@teststep_definition(
    step_number=1,
    name="Inform driver about PDW deactivation",  # this would be shown as a test step name in html report
    description=(
...
class TestStepUI_MANAGER(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            pdw_series = signals[Signals.Columns.PDW_SYS_STATE]
            hmi_series = signals[Signals.Columns.HMI_INPUT_SYS_STATE]
            pdw_events = pdw_series.rolling(2).apply(
                lambda x: x[1] > x[0] and x[1] == constants.UiManager.pdwSystemState.PDW_OFF,
                raw=True,
            )
            transition_events = []
            transition_message = []
                ...
                for ts in timestamp_pdw_events:
                    ...
                    if not sub_series[sub_series == pdw_sys_state].empty:
                        ...
                        transition_message.append(
                            f"HMI Handler informed the driver about the deactivation of PDW:  \
                    {constants.UiManager.pdwSystemState.get_variable_name(pdw_sys_state)} after {it_took} ms, within {max_delay * 10} ms of PDW Transition."
                        ...
                        transition_message.append(
                            f"HMI Handler did not inform the driver about the current shutdown cause in case PDW system state is OFF: \
            ...
            remark = f"<b>Signals evaluated:</b><br><br>\
                        <b>{signals_obj._properties[Signals.Columns.PDW_SYS_STATE]}</b> == <b>{signals_obj._properties[Signals.Columns.HMI_INPUT_SYS_STATE]}</b>"
            ...
            fig.add_trace(
                go.Scatter(
                    x=ap_time,
                    y=signals[Signals.Columns.HMI_INPUT_SYS_STATE].values.tolist(),
                    mode="lines",
                    name=signals_obj._properties[Signals.Columns.HMI_INPUT_SYS_STATE],
                    text=[
                        f"Timestamp: {ap_time[idx]} <br>"
                        f"HMI SYSTEM STATE:<br>"
                        f"{constants.UiManager.pdwSystemState.get_variable_name(state)}"
                        for idx, state in enumerate(signals[Signals.Columns.HMI_INPUT_SYS_STATE].values.tolist())
                    ],
                    hoverinfo="text",
                )
            )
