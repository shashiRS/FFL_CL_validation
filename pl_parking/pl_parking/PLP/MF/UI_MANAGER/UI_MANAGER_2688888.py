...
@teststep_definition(
    step_number=1,
    name="Mute/unmute PDW warnings",  # this would be shown as a test step name in html report
    description=(
        "In case that the driver wants to mute or unmute the audio warning of PDW, the HMIManager shall forward the input about the selection of the acoustical warning (turn ON or OFF)."
        ...
        try:
            ...
            if timestamp_pdw_events:
                for ts in timestamp_pdw_events:
                    ...
                    if not sub_series[
                        sub_series == constants.UiManager.PDCUserActionHeadUnit.PDC_TAP_ON_MUTE_BTN
                    ].empty:
                        ...
                        transition_message.append(
                            f"HMIManager did not forward the input, within {max_delay * 10} ms of user input."
                        )
            ...
            remark = f"<b>Signals evaluated:</b><br><br>\
                       ...
                       Output: <b>{signals_obj._properties[Signals.Columns.PDC_USER_ACTION]} == {constants.UiManager.PDCUserActionHeadUnit.PDC_TAP_ON_MUTE_BTN}</b>"
...
@testcase_definition(
    name="UI_MANAGER_2688888",
    description="In case that the driver wants to mute or unmute the audio warning of PDW, the HMIManager shall forward the input about the selection of the acoustical warning (turn ON or OFF).",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_ZdpZwXpsEe-9FrFCDLx05w&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_ns_CwE4gEe6M5-WQsF_-tQ&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
