"""Park Markers Sorting by Distance from Ego Vehicle"""
...
SIGNAL_DATA = "SI_ParkMarkerDistanceValidation"
signals_obj = SISignals()


@teststep_definition(
    step_number=1,
    name="ParkMarkerDistanceSortingWorldCoordinatesTC",
    description="Verify park markers are sorted by distance from the ego vehicle.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SISignals)
class ParkMarkerDistanceTypeSorting(TestStep):
    """Test Step for sorting park markers by distance from the ego vehicle."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            # Extract the number of park markers and timestamps
            number_of_ParkMarkers = read_data["envModelPort.numberOfParkMarkings_u8"]

            unique_number_of_ParkMarkers = number_of_ParkMarkers.unique().tolist()

            if len(unique_number_of_ParkMarkers) == 0:
                _log.debug("No park markers found.")
            ...
            for timestamp in filteredTimestamp:
                ...
                for vertex in range(16):
                    obj_0_x_col = f"envModelPort.parkingSpaceMarkings_0_{vertex}.x"
                    obj_0_y_col = f"envModelPort.parkingSpaceMarkings_0_{vertex}.y"
                    obj_1_x_col = f"envModelPort.parkingSpaceMarkings_1_{vertex}.x"
                    obj_1_y_col = f"envModelPort.parkingSpaceMarkings_1_{vertex}.y"

                    obj_2_x_col = f"envModelPort.parkingSpaceMarkings_2_{vertex}.x"
                    obj_2_y_col = f"envModelPort.parkingSpaceMarkings_2_{vertex}.y"
                    obj_3_x_col = f"envModelPort.parkingSpaceMarkings_3_{vertex}.x"
                    obj_3_y_col = f"envModelPort.parkingSpaceMarkings_3_{vertex}.y"
            ...
            Description_list = [
                f"Marker 1 min distance: {d0} m, Marker 2 min distance: {d1} m,Marker 3 min distance: {d2} m,Marker 4 min distance: {d3} m, from ego vehicle."
            ...
            table_remark = "<b>Park Markers should be sorted by their distance from the ego vehicle.</b><br><br>"
            if (
                list_min_distance_for_obj_0_all_vertices
                ...
                if res1:
                    ...
                    sig_sum = pd.DataFrame(
                        {
                            ...
                            "Distance to Marker 1 [m]": list_min_distance_for_obj_0_all_vertices,
                            "Distance to Marker 2 [m]": list_min_distance_for_obj_1_all_vertices,
                            "Distance to Marker 3 [m]": list_min_distance_for_obj_2_all_vertices,
                            "Distance to Marker 4 [m]": list_min_distance_for_obj_3_all_vertices,
                    ...
                    sig_sum_html = ft_helper.create_hidden_table(
                        sig_sum_html, 1, button_text="Park Marker Distances Table"
                    )
                ...
                # Plotting the distances and number of park markers over time
                fig = go.Figure()

                # Plot distances of Marker 1 to 4
                fig.add_trace(
                    go.Scatter(
                        x=list_ap_time_in_sec,
                        y=list_min_distance_for_obj_0_all_vertices,
                        mode="lines+markers",
                        name="Distance to Marker 1",
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="lines+markers",
                        name="Distance to Marker 4",
                        line=dict(color="orange"),
                        marker=dict(size=8),
                    )
                )

                # Plot number of park markers
                fig.add_trace(
                    go.Scatter(
                        x=list_ap_time_in_sec,
                        y=number_of_ParkMarkers,
                        mode="lines+markers",
                        name="Number of Park Markers",
                        line=dict(color="red"),
                        marker=dict(size=8),
                    )
                )
                fig.update_layout(
                    title="Distance of Park Markers and Marker Count Over Time",
                    xaxis_title="Timestamp (seconds)",
                    yaxis_title="Distance (m) / Marker Count",
                    template="plotly_white",
                    font=dict(size=14),
                )

                plot_titles.append("Park Marker Distances and Counts")
                plots.append(fig)
...
@verifies("ReqId_1406039")
@testcase_definition(
    name="ParkMarkerDistanceSortingWorldCoordinatesTC",
    description="Test Case for park marker distance sorting.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9tz87DpwEe6GONELpfdJ2Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36325&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class ParkMarkerDistanceSortingTC(TestCase):
    """Test Case for verifying sorting of park markers by distance."""

    @property
    def test_steps(self):
        """Define the test steps."""
        return [
            ParkMarkerDistanceTypeSorting,
        ]


def main(data_folder, temp_dir=None, open_explorer=True):
    """Main entry point for debugging the test case."""
    test_bsigs = r"D:\Parking_SI\carmaker\Latest\adc5_platform_meta\out\bricks_mf_sil_performance_vs2017_debug_prod\package\tests\SIL\CarMaker\SimOutput\SISim_UC_Rear_Prk_Limiters.testrun.erg"

    debug(
        ParkMarkerDistanceSortingTC,
        test_bsigs,
        temp_dir=temp_dir,
