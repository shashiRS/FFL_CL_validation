...
"""req id : 1405840
Two parking markings that are separated by a free space that is:
1. Wide enough to fit a perpendicular parking space

2. Wider than the maximum width for a perpendicular parking space (larger than):

( {AP_V_WIDTH_M} + {AP_G_PERP_SLOT_MAX_OFFSET_W_SCAN_M} ) during Scanning Mode  /or/

( {AP_V_WIDTH_M} + {AP_G_PERP_SLOT_MAX_OFFSET_W_PARK_M} ) during Parking Mode

3. Not wide enough to fit a parallel parking space (smaller than):

( {AP_V_LENGTH_M} + {AP_G_PAR_SLOT_MIN_OFFSET_L_M} )



then no parking box shall be defined.
...
READER_NAME = "SI_PerpendicularSpaceWidthValidationWithTwoParkingMarkings"
signals_obj = SISignals()


@teststep_definition(
    name="Check for SI_PerpendicularSpaceWidthValidationWithTwoParkingMarkings",
    description="Two parking markings that are separated by a free space "
    "that is Wide enough to fit a perpendicular parking space.",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_PerpendicularSpaceWidthValidationWithTwoParkingMarkings(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            new_collected_info = ft_helper.process_data(read_data[num_valid_pb])
        ...
        from detected parking slots found in between the delimiters (from signal <b>parkingBoxPort.parkingBoxes_% )</b> <br><br>"
            ...
            for timestamp_val in new_collected_info.keys():
                ...
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_pb":
                    for park_box_id in new_collected_info[timestamp_val]["pb_with_marker_match"]:
                        ...
                        is_whole_delim_area_covered_bool = (
                            round((AC_width_delimiter - AC_width_pb), 2)
                            >= 0  #
                        ...
                        if is_wide_enough_bool and not is_wider_than_min_parallel_bool:
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>This slot was not supposed to be detected</b><br>Wider than the minimum width to fit a parallel parking space (greater than <b>{threshold.MIN_LEN_PARALLEL} [m]</b>).\
                                The car was in Apstate {ft_helper.ap_state_dict(the_state_of_ap)}"
            ...
            if res:
                if all([x == "PASS" for x in res]):
                    ...
                    for ts in numValidPbTimestamps:
                        if ts not in new_collected_info.keys():
                            ts = min(new_collected_info.keys(), key=lambda x: abs(x - ts))
            ...
            for key, val in new_collected_info.items():
                ...
                if origin_reset:
                    new_origin_x, new_origin_y = ft_helper.transform_coordinates_using_new_origin(
                ...
                if val.get("pb", None):  # Only create the parking box trace if coordinates exist
                    for pb_id, val_coords in val["pb"].items():
                        ...
                        if origin_reset:
                            val_coords_to_plot = ft_helper.transform_coordinates_using_new_origin(
                ...
                for pb__id, marker_info in val["markers"].items():
                    ...
                    for marking in marker_info["coordinates"]:
                        ...
                        if origin_reset:
                            marking_to_plot = ft_helper.transform_coordinates_using_new_origin(old_origin, marking)
...
@verifies("ReqId_1405840")
@testcase_definition(
    name="SI Perpendicular space width validation with two parking markings",
    description="Two parking markings that are separated by a free space "
    "that is Wide enough to fit a perpendicular parking space",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9twSiTpwEe6GONELpfdJ2Q&vvc.configuration=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fcm%2Fstream%2F_G3kr8DgnEe6mrdm2_agUYg&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_PerpendicularSpaceWidthValidationWithTwoParkingMarkingsTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_PerpendicularSpaceWidthValidationWithTwoParkingMarkings,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SI_PerpendicularSpaceWidthValidationWithTwoParkingMarkingsTC,
        # test_bsigs,
        temp_dir=temp_dir,
