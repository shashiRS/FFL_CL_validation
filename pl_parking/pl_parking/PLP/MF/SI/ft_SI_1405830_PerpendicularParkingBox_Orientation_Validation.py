"""Validate Perpendicualr Parking Box Orientation According to ego vehicle"""
...
SIGNAL_DATA = "PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle"
signals_obj = SISignals()


@teststep_definition(
    step_number=1,
    name="Check for SI PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle",
    description="This test step checks whether the detected perpendicular parking box orientation aligns with the orientation of the ego vehicle.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SISignals)
class PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            yaw_range = f"[-{ConstantsSI.AP_G_PERP_MAX_ORI_ANGLE_RAD};{ConstantsSI.AP_G_PERP_MAX_ORI_ANGLE_RAD}]"
            ...
            table_remark = f"The SceneInterpretation shall detect free perpendicular parking boxes, \
                            if the orientation of the road side edge of the box is between \
                            [-{ConstantsSI.AP_G_PERP_MAX_ORI_ANGLE_RAD};{ConstantsSI.AP_G_PERP_MAX_ORI_ANGLE_RAD}] \
                            with respect to the orientation of the ego vehicle while passing the parking box."
            ...
            for timestamp_val in new_collected_info.keys():
                for parking_box_id in new_collected_info[timestamp_val]["pb"]:
                    if parking_box_id in first_collinearity:
                        continue
                    ...
                    vertices_pb = [vertices_pb_list[0], vertices_pb_list[3]]

                    box_orientation_in_rad = ft_helper.cal_orientation_pb(vertices_pb)
                    new_collected_info[timestamp_val]["Orientation_pb"][parking_box_id] = box_orientation_in_rad

                    left_mirror = DrawCarLayer.CarCoordinates.LEFT_MIRROR_REAR_LEFT_CORNER
                    right_mirror = DrawCarLayer.CarCoordinates.RIGHT_MIRROR_REAR_LEFT_CORNER
                    ...
                    left_car_point_for_collinearity = {
                        "x": left_mirror[0] - CAR_MIRROR_BACK_OFFSET,
                        "y": left_mirror[1],
                    }
                    right_car_point_for_collinearity = {
                        "x": right_mirror[0] - CAR_MIRROR_BACK_OFFSET,
                        "y": right_mirror[1],
                    }
                    transformed_left_car_point_for_collinearity = ft_helper.translate_and_rotate(
                        left_car_point_for_collinearity,
                    ...
                    transformed_right_car_point_for_collinearity = ft_helper.translate_and_rotate(
                        right_car_point_for_collinearity,
                        new_collected_info[timestamp_val]["ego_vehicle"][0][0],
                        new_collected_info[timestamp_val]["ego_vehicle"][0][1],
                        new_collected_info[timestamp_val]["ego_vehicle"][0][2],
                    ...
                    if abs(vertices_for_collinearity[1]["y"] - transformed_left_car_point_for_collinearity["y"]) < abs(
                        vertices_for_collinearity[1]["y"] - transformed_right_car_point_for_collinearity["y"]
                    ):
                        are_points_collinear_left = ft_helper.are_points_collinear(
                            vertices_for_collinearity[0],
                            vertices_for_collinearity[1],
                            transformed_left_car_point_for_collinearity,
                        )
                    else:
                        are_points_collinear_right = ft_helper.are_points_collinear(
                            vertices_for_collinearity[0],
                            vertices_for_collinearity[1],
                            transformed_right_car_point_for_collinearity,
                        )

                    if are_points_collinear_left or are_points_collinear_right:
                        first_collinearity[parking_box_id] = timestamp_val
                        ego_vehicle_orientation_in_rad = new_collected_info[timestamp_val]["ego_vehicle"][0][2]
                        relative_orientation_in_rad = ft_helper.calculate_relative_orientation(
                            box_orientation_in_rad, ego_vehicle_orientation_in_rad
                        )
                        if (
                            -ConstantsSI.AP_G_PERP_MAX_ORI_ANGLE_RAD
                            <= relative_orientation_in_rad
                            <= ConstantsSI.AP_G_PERP_MAX_ORI_ANGLE_RAD
                        ):
                            new_collected_info[timestamp_val]["verdict"][parking_box_id] = "PASS"
                            new_collected_info[timestamp_val]["description"][
                                parking_box_id
                            ] += f"The relative orientation for the parking box with the ID {parking_box_id} is <b>{round(relative_orientation_in_rad, 4)}</b> [rad]; within the range {yaw_range} [rad]"
                        else:
                            new_collected_info[timestamp_val]["verdict"][parking_box_id] = "FAIL"
                            new_collected_info[timestamp_val]["description"][parking_box_id] += (
                                f"The relative orientation for the parking box with the ID {parking_box_id} is <b>{round(relative_orientation_in_rad, 4)}</b> [rad];<br>"
                                f"Not in the range of {yaw_range} [rad]"
                            )
                    else:
                        new_collected_info[timestamp_val]["verdict"][parking_box_id] = "Unknown"
                        new_collected_info[timestamp_val]["description"][
                            parking_box_id
                        ] += f"There is no collinearity between the parking box with ID {parking_box_id} and the ego vehicle."
                # if any(new_collected_info[timestamp_val]["verdict"][pb_id] == "PASS" for pb_id in new_collected_info[timestamp_val]["pb"]):
                #     break
                # else:
                #     continue
                # break
            ...
            res = [
                val["verdict"][x]
                for val in new_collected_info.values()
                for x in val["verdict"].keys()
                if val["verdict"][x] in ["PASS", "FAIL"]
            ]
            valid_timestamps = [
                ts
                for ts in new_collected_info.keys()
                if any(x in ["PASS", "FAIL"] for x in new_collected_info[ts]["verdict"].values())
            ]
            if res:
                for ts in valid_timestamps:
                    for pb_id in new_collected_info[ts]["Orientation_pb"].keys():
                        sig_sum["Timestamp [s]"].append(new_collected_info[ts]["ts"])
                        sig_sum["Orientation_pb [rad]"].append(new_collected_info[ts]["Orientation_pb"][pb_id])
                        sig_sum["Orientation_ego_vehicle [rad]"].append(new_collected_info[ts]["ego_vehicle"][0][2])
                        sig_sum["description"].append(new_collected_info[ts]["description"][pb_id])
                        sig_sum["Result"].append(ft_helper.get_result_color(new_collected_info[ts]["verdict"][pb_id]))
...
@verifies("ReqId_1405830")
@testcase_definition(
    name="PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle_TC",
...
class PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle_TC(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle,
        ]
...
def main(data_folder, temp_dir=None, open_explorer=True):
    ...
    debug(
        PerpendicularParkingBoxOrientation_Validation_with_ego_vehicle_TC,
        # test_bsigs,
        temp_dir=temp_dir,
