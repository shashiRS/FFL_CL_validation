#!/usr/bin/env python3
"""req id : 1405843
Given two parking markings that are separated by a free space that is:



1. Wide enough to fit a parallel parking space (larger or equal to):

({AP_V_LENGTH_M} + {AP_G_PAR_SLOT_MIN_OFFSET_L_M})

2. Not wider than the maximum width for a parallel parking space (smaller or equal to):

({AP_V_LENGTH_M} + {AP_G_PAR_SLOT_MAX_OFFSET_L_SCAN_M}) during Scanning Mode

({AP_V_LENGTH_M} + {AP_G_PAR_SLOT_MAX_OFFSET_L_PARK_M}) during Parking Mode



then a parallel parking box shall be defined covering the entire width of the free space
...
READER_NAME = "SI_ParallelSpaceWidthValidation"
signals_obj = SISignals()


@teststep_definition(
    name="Check for SI_ParallelSpaceWidthValidation",
    description="parallel parking box shall be defined covering the entire width of the free space.",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_ParallelSpaceWidthValidation(TestStep):
    """Example test step."""
            ...
            for timestamp_val in new_collected_info.keys():
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_only":
                    for park_box_id in new_collected_info[timestamp_val]["markers"].keys():
                        ...
                        max_par_park_threshold = (
                            threshold.MAX_LEN_PARALLEL_PARK_IN
                            if the_state_of_ap == ConstantsTrajpla.AP_AVG_ACTIVE_IN
                            else threshold.MAX_LEN_PARALLEL_SCAN_IN
                        )
                        ...
                        is_wide_enough_bool = AC_width_delimiter > threshold.MIN_LEN_PARALLEL
                        is_wider_than_max_par_lenght_bool = (
                            AC_width_delimiter > threshold.MAX_LEN_PARALLEL_PARK_IN
                            if the_state_of_ap == ConstantsTrajpla.AP_AVG_ACTIVE_IN
                            else AC_width_delimiter > threshold.MAX_LEN_PARALLEL_SCAN_IN
                        )

                        if is_wide_enough_bool and not is_wider_than_max_par_lenght_bool:
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += "<b>There is no detection from SI.</b><br>"
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"Wide enough to fit a parallel parking space (between <b>{threshold.MIN_LEN_PARALLEL}</b> and <b>{max_par_park_threshold} [m]</b>).\
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>This slot was not detected</b><br>Smaller than the minimum space for a paralel parking space (smaller than <b>{threshold.MIN_LEN_PARALLEL} [m]</b>).\
                                The car was in Apstate {ft_helper.ap_state_dict(the_state_of_ap)}"
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>This slot was not supposed to be detected</b><br>Wider than the maximum lenght for a parallel parking space(larger than <b>{max_par_park_threshold} [m].</b>)\
                                The car was in Apstate {ft_helper.ap_state_dict(the_state_of_ap)}"
                ...
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_pb":
                    for park_box_id in new_collected_info[timestamp_val]["pb_with_marker_match"]:
                        ...
                        # Remark: during maneuvering it is allowed that the parking box shrinks (due to update of environment), to improve overall robustness of the parking maneuver.
                        is_pb_inside_delim_area = (
                            round((AC_width_delimiter-AC_width_pb),2) >= 0 #
                            #AC_width_pb == AC_width_delimiter
                            if the_state_of_ap == ConstantsTrajpla.AP_SCAN_IN
                            else AC_width_pb <= AC_width_delimiter
                        )
                        is_whole_delim_area_covered_bool = is_pb_inside_delim_area  #and overlap_percentage >= 99.6
                        is_wide_enough_bool = AC_width_delimiter > threshold.MIN_LEN_PARALLEL
                        is_wider_than_max_par_lenght_bool = (
                            AC_width_delimiter > threshold.MAX_LEN_PARALLEL_PARK_IN
                            if the_state_of_ap == ConstantsTrajpla.AP_AVG_ACTIVE_IN
...
@verifies("ReqId_1405843")
@testcase_definition(
    name="SI_ParallelSpaceWidthValidationTC",
    description="parallel parking box shall be defined covering the entire width of the free space.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9twSijpwEe6GONELpfdJ2Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg&vvc.configuration=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fcm%2Fstream%2F_G3kr8DgnEe6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_ParallelSpaceWidthValidationTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_ParallelSpaceWidthValidation,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SI_ParallelSpaceWidthValidationTC,
        # test_bsigs,
        temp_dir=temp_dir,
