...
class SI_Prevention_of_Parking_Overhang_Over_Body_Traversable_Objects_Delimiting_Free_Spaces(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            collected_info = None
            signal_summary = {}
            num_valid_pb = df[SISignals.Columns.NUMVALIDPARKINGBOXES_NU] == 1
            ...
            if not df[num_valid_pb].empty:
                collected_info = ft_helper.process_from_two_objects(df[num_valid_pb])
            ...
            if collected_info:
                ...
                for _, value in collected_info.items():
                    ...
                    pb = go.Scatter(
                        ...
                        text=[
                            f"Timestamp: {value['ts']}<br>" f"X: {value['pb'][::2][x]}<br>" f"Y: {ycoord}"
                            for x, ycoord in enumerate(value["pb"][1::2])
                        ],
                        hoverinfo="text",
                    )
                    space = go.Scatter(
                        x=value["possible_space"][::2],
                        y=value["possible_space"][1::2],
                        mode="lines",
                        fill="toself",  # Fill the polygon
                        fillcolor="rgba(0, 0, 255, 0.5)",  # Fill color with transparency
                        line=dict(color="blue"),
                        name="Space evaluated",
                        text=[
                            f"Timestamp: {value['ts']}<br>" f"X: {value['possible_space'][::2][x]}<br>" f"Y: {ycoord}"
                            for x, ycoord in enumerate(value["pb"][1::2])
                        ],
                        hoverinfo="text",
                    )
                ...
                signal_summary = ({"Evaluation not possible": "No valid parking boxes found"},)
...
@verifies("ReqId_1405860")
@testcase_definition(
    name="SI Prevention of Parking Overhang Over Body Traversable Objects Delimiting Free Spaces",
    description="Test that in case of body traversable delimiters the resulting parking box does not overlap.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9tw5izpwEe6GONELpfdJ2Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F30013&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg",
)
@register_inputs("/parking")
class SI_Prevention_of_Parking_Overhang_Over_Body_Traversable_Objects_Delimiting_Free_Spaces_TC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_Prevention_of_Parking_Overhang_Over_Body_Traversable_Objects_Delimiting_Free_Spaces,
        ]
