"""This module contains the test step to check if the detected
parking boxes are long enough to fit a vehicle in between the delimiters.
The delimiters can be extended with a certain length.
"""
...
READER_NAME = "SI_ExtendLengthofParkingBox"
signals_obj = SISignals()


@teststep_definition(
    name="Check for SI_ExtendLengthofParkingBox",
    description="Two parking markings that are separated by a free space "
    "that is deep/long enough to fit a parking space.",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_ExtendLengthofParkingBox(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            # max(df["staticObject_2_objShape_m.array_y_dir_0"])
            s = list(df.columns)
            s.sort()
            ...
            table_remark = "Verify if the detected parking boxes are sufficiently long/deep to accommodate a vehicle between the delimiters. \
            Note that the delimiters can be extended if necessary.<br><br>"
            ...
            df_scenario = df["parkingBoxes_0.parkingScenario_nu"].copy()
            for timestamp in df.index:
                if df_scenario[timestamp] not in sum(parking_type_map.values(), []):
                    df_scenario = df_scenario[~df_scenario.index.isin([timestamp])]
            for key, values in parking_type_map.items():
                if df_scenario.values[0] in values:
                    parking_type = key
                    break
            ...
            for timestamp_val in new_collected_info.keys():
                if new_collected_info[timestamp_val]["ap_state"] == ConstantsTrajpla.AP_AVG_ACTIVE_IN:
                    DEPTH_PARALLEL_PARKING_BOX_PARK = (
                        threshold.AP_V_WIDTH_M + threshold.AP_SI_PAR_SLOT_MIN_OFFSET_DEPTH_PARK_M
                    )
                    DEPTH_NONPARALLEL_PARKING_BOX_PARK = (
                        threshold.AP_V_LENGTH_M + threshold.AP_SI_NONPAR_SLOT_MIN_OFFSET_DEPTH_PARK_M
                    )
                else:
                    DEPTH_PARALLEL_PARKING_BOX_PARK = (
                        threshold.AP_V_WIDTH_M + threshold.AP_SI_PAR_SLOT_MIN_OFFSET_DEPTH_SCAN_M
                    )
                    DEPTH_NONPARALLEL_PARKING_BOX_PARK = (
                        threshold.AP_V_LENGTH_M + threshold.AP_SI_NONPAR_SLOT_MIN_OFFSET_DEPTH_SCAN_M
                    )

                AP_PARKING_SPACE_DEPTH = (
                    DEPTH_PARALLEL_PARKING_BOX_PARK
                    if parking_type == "parallel"
                    else DEPTH_NONPARALLEL_PARKING_BOX_PARK
                )
                ...
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_only":
                    for park_box_id in new_collected_info[timestamp_val]["markers"].keys():
                        ...
                        EXTENDED_MARKER_LINE = (
                            AB_lenght_marker + threshold.AP_G_PAR_MAX_ROAD_OVERHANG_M
                            if parking_type == "parallel"
                            else AB_lenght_marker + threshold.AP_G_PER_MAX_ROAD_OVERHANG_M # same value applies to angular parking type
                        )

                        is_long_enough_bool = AB_lenght_marker >= AP_PARKING_SPACE_DEPTH
                        is_long_enough_with_extension_bool = EXTENDED_MARKER_LINE >= AP_PARKING_SPACE_DEPTH

                        if is_long_enough_bool or is_long_enough_with_extension_bool:
                            space_type = "space" if is_long_enough_bool else "EXTENDED space"
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>There is no detection from SI.</b><br>Long enough to fit a {space_type} for a {parking_type} spot (greater than <b>{AP_PARKING_SPACE_DEPTH} [m]</b>)."
                            new_collected_info[timestamp_val]["verdict"][park_box_id] = "PASS"
                        else:
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>There is no detection from SI.</b><br>Not long enough to fit a {space_type} for a {parking_type} spot (smaller than <b>{AP_PARKING_SPACE_DEPTH} [m]</b>)."
                            new_collected_info[timestamp_val]["verdict"][park_box_id] = "FAIL"

                        x_coords_box_to_plot, y_coords_box_to_plot = zip(
                            *new_collected_info[timestamp_val]["markers"][park_box_id]["coordinates"][0]
                        )
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_pb":
                    for park_box_id in new_collected_info[timestamp_val]["pb_with_marker_match"]:
                        ...
                        furthest_marker_point = max(
                            new_collected_info[timestamp_val]["markers"][park_box_id]["coordinates"][0]
                            + new_collected_info[timestamp_val]["markers"][park_box_id]["coordinates"][1],
                            key=lambda point: abs(point[1]),
                        )
                        furthest_marker_line = None
                        for line in new_collected_info[timestamp_val]["markers"][park_box_id]["coordinates"]:
                            if furthest_marker_point in line:
                                furthest_marker_line = line
                                break
                        ...
                        if are_point_beyond_marking_line:
                            new_collected_info[timestamp_val]["description"][park_box_id] += (
                                "<b>A parking box was detected by SI.</b><br>"
                            ...
                            MARKER_LINE = round(
                                math.sqrt(
                                    (furthest_marker_line[1][0] - furthest_marker_line[0][0]) ** 2
                                    + (furthest_marker_line[1][1] - furthest_marker_line[0][1]) ** 2
                                ),
                                3,
                            )
                            EXTENDED_MARKER_LINE = (
                                MARKER_LINE + threshold.AP_G_PAR_MAX_ROAD_OVERHANG_M
                                if parking_type == "parallel"
                                else MARKER_LINE + threshold.AP_G_PER_MAX_ROAD_OVERHANG_M # same value applies to angular parking type
                            )

                            marker_line_check = MARKER_LINE >= AP_PARKING_SPACE_DEPTH
                            extended_marker_line_check = EXTENDED_MARKER_LINE >= AP_PARKING_SPACE_DEPTH
                            marker = MARKER_LINE if marker_line_check else EXTENDED_MARKER_LINE

                            if marker_line_check or extended_marker_line_check:
                                space_type = "marker" if marker_line_check else "EXTENDED marker"
                                depth_of_pb = new_collected_info[timestamp_val]["parking_box_lenght_value"][park_box_id]
                                if depth_of_pb <= MARKER_LINE:
                                    new_collected_info[timestamp_val]["description"][park_box_id] += (
                                        f"<b>A {parking_type} parking box was detected by SI.</b><br>"
                                        f"A parking spot was offered<br>"
                                        f"The parking box is long/deep enough for a vehicle to park in.<br>"
                                        f"Length value of the <b>marker</b> is {MARKER_LINE} [m].<br>"
                                    )
                                    new_collected_info[timestamp_val]["verdict"][park_box_id] = "PASS"
                                elif depth_of_pb <= EXTENDED_MARKER_LINE:
                                    new_collected_info[timestamp_val]["description"][park_box_id] += (
                                        f"<b>A {parking_type} parking box was detected by SI.</b><br>"
                                        f"A parking spot was offered<br>"
                                        f"The parking box is long/deep enough for a vehicle to park in.<br>"
                                        f"Length value of the <b>EXTENDED marker</b> is {EXTENDED_MARKER_LINE} [m].<br>"
                                    )
                                    new_collected_info[timestamp_val]["verdict"][park_box_id] = "PASS"
                                else:
                                    new_collected_info[timestamp_val]["description"][park_box_id] += (
                                        f"<b>A {parking_type} parking box was detected by SI.</b><br>"
                                        f"A parking spot cannot be offered<br>"
                                        f"Length of the <b>{space_type}</b> is {marker} [m].<br>"
                                        f"A parking box could've been offered in the spot (<b>{marker} > {AP_PARKING_SPACE_DEPTH}</b>).<br>"
                                        f"FAIL REASON: The length of scanned parking box is greater than the length of the {space_type} (<b>{depth_of_pb} > {marker}</b>).<br>"
                                    )
                                    new_collected_info[timestamp_val]["verdict"][park_box_id] = "FAIL"
                            else:
                                new_collected_info[timestamp_val]["description"][park_box_id] += (
                                    f"<b>A {parking_type} parking box was detected by SI.</b><br>"
                                    f"A vehicle cannot be parked in the space given.<br>"
                                    f"Length of the <b>extended delimiter</b> is {marker} [m].<br>"
                                    f"Minimum required length/depth for a parking space is {AP_PARKING_SPACE_DEPTH} [m]."
                                )
                                new_collected_info[timestamp_val]["verdict"][park_box_id] = "FAIL"
...
@verifies("ReqId_1405882")
@testcase_definition(
    name="SI Extended Parking Box Length",
    description="Verify if a parking box is sufficiently long to fit within a delimited parking space.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9txgkTpwEe6GONELpfdJ2Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36325",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_ExtendLengthofParkingBoxTC(TestCase):
