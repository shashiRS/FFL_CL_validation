...
from pl_parking.PLP.MF.constants import DrawCarLayer
...
READER_NAME = "SI_1405856"
signals_obj = SISignals()


@teststep_definition(
    name="Check for parking slots between two cars",
    description="Given a parking delimiter located between two vehicles, a parking box shall be identified. \
            If the space between the delimiter and either car is not large enough to accommodate a parking box, \
            the offered space shall be limited to the area constrained by the two vehicles.",
...
class SI_TestStep(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            parking_type = ft_helper.get_parking_scenario_from_name(os.path.basename(self.artifacts[0].file_path))
            # ft_helper.process_from_two_objects(df)
            collected_info = ft_helper.process_from_two_objects_with_delimiter_between(df, parking_type)
            # ft_helper.process_data_three_obj_shapes(df)
            sig_sum = {
                "Timestamp [s]": [],
            ...
            for key, val in collected_info.items():
                ...
                if val.get("pb", None):  # Only create the parking box trace if coordinates exist
                    ...
                    parking_box_trace = go.Scatter(
                        ...
                        name="Parking Box" if val["verdict"] else "Fail reason:<br>" f" {val['fail_reason']}",
                        text=[
...
@verifies("ReqId_1405856")
@testcase_definition(
    name="SI Parking box detection between static objects",
    description="""Consider the case where there is enough space between two obstacles to include a parking slot of any type:<br><br>

                - perpendicular parking,<br>
                - parallel parking,<br>
                - angular parking.<br><br>

                If a parking marking is found between the obstacles, that shall not invalidate the parking box.""",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9tw5iDpwEe6GONELpfdJ2Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
