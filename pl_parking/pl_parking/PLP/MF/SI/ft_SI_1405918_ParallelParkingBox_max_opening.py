"""
This script defines a test case for evaluating the SceneInterpretation (SI) module's ability to provide a parallel parking box with a maximum opening

Classes:
    - SI_ParallelParkingBox_Max_Opening: A test step that evaluates the SI module's performance in providing a parallel parking box with a minimum opening.
    - SI_ParallelParkingBox_Max_OpeningTC: A test case that includes the `SI_ParallelParkingBox_Max_Opening` test step.
"""
...
READER_NAME = "ParallelParkingBox_Max_Opening"
signals_obj = SISignals()


@teststep_definition(
    name="Check for ParallelParkingBox_Max_Opening",
    description="The SceneInterpretation shall provide an parallel parking box that has a minimum opening.",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_ParallelParkingBox_Max_Opening(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            new_collected_info = ft_helper.process_data_initial(read_data)
            df = read_data.as_plain_df
            MAX_PAR_WDTH_PARK_IN = threshold.AP_V_LENGTH_M + threshold.AP_G_PAR_SLOT_MAX_OFFSET_L_PARK_M
            MAX_PAR_WDTH_SCAN_IN = threshold.AP_V_LENGTH_M + threshold.AP_G_PAR_SLOT_MAX_OFFSET_L_SCAN_M
            MIN_LENGTH_PARALLEL_PARKING = threshold.AP_V_LENGTH_M + threshold.AP_G_PAR_SLOT_MIN_OFFSET_L_M
            ...
            table_remark = f"The SceneInterpretation shall limit the maximum opening of a parallel parking box to \
                ( {threshold.AP_V_LENGTH_M} + {threshold.AP_G_PAR_SLOT_MAX_OFFSET_L_SCAN_M} ) during Scanning Mode  /or/ \
                ( {threshold.AP_V_LENGTH_M} + {threshold.AP_G_PAR_SLOT_MAX_OFFSET_L_PARK_M} ) during Parking Mode"
            ...
            for timestamp_val in new_collected_info.keys():
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_only":
                    for park_box_id in new_collected_info[timestamp_val]["markers"].keys():
                        AC_width_delimiter = float(
                            new_collected_info[timestamp_val]["delim_width"][park_box_id]
                            .split("Raw value: <b>")[1]
                            .split("</b>")[0]
                        )
                        ...
                        max_par_park_threshold = (
                            MAX_PAR_WDTH_PARK_IN
                            if the_state_of_ap == ConstantsTrajpla.AP_AVG_ACTIVE_IN
                            else MAX_PAR_WDTH_SCAN_IN
                        )

                        is_wide_enough_bool = AC_width_delimiter > MIN_LENGTH_PARALLEL_PARKING
                        is_wider_than_max_par_bool = (
                            AC_width_delimiter > MAX_PAR_WDTH_PARK_IN
                            if the_state_of_ap == ConstantsTrajpla.AP_AVG_ACTIVE_IN
                            else AC_width_delimiter > MAX_PAR_WDTH_SCAN_IN
                        )
                        if is_wide_enough_bool and not is_wider_than_max_par_bool:
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"Wide enough to fit a parallel parking space (between <b>{MIN_LENGTH_PARALLEL_PARKING}</b> and <b>{max_par_park_threshold} [m]</b>).\
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>This slot was not detected</b><br>Wider than the maximum width for a parallel parking space(larger than <b>{max_par_park_threshold} [m].</b>)\
                                The car was in Apstate {ft_helper.ap_state_dict(the_state_of_ap)}"
                ...
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_pb":
                    for park_box_id in new_collected_info[timestamp_val]["pb_with_marker_match"]:
                        ...
                        AC_width_pb = float(
                            new_collected_info[timestamp_val]["pb_width"][park_box_id]
                            .split("Raw value: <b>")[1]
                            .split("</b>")[0]
                        )
                        ...
                        is_whole_delim_area_covered_bool = round(AC_width_pb, 1) == round(AC_width_delimiter, 1)
                        is_wide_enough_bool = AC_width_pb > MIN_LENGTH_PARALLEL_PARKING
                        is_wider_than_max_par_bool = (
                            AC_width_pb > MAX_PAR_WDTH_PARK_IN
                            if the_state_of_ap == ConstantsTrajpla.AP_AVG_ACTIVE_IN
                            else AC_width_pb > MAX_PAR_WDTH_SCAN_IN
                        )
                        ...
                        if is_wide_enough_bool and not is_wider_than_max_par_bool:
                            ...
                            if is_whole_delim_area_covered_bool:
                                ...
                                new_collected_info[timestamp_val]["description"][park_box_id] = (
                                    "<b>Value of width between delimiters is not matching the value of width of the parking box.</b>.<br>"
                                    + new_collected_info[timestamp_val]["description"][park_box_id]
                                )
                                new_collected_info[timestamp_val]["verdict"][park_box_id] = "FAIL"
                        else:
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>This slot was not supposed to be detected</b><br>Wider than the maximum width for a parallel parking space(larger than <b>{max_par_park_threshold} [m].</b>)\
                                The car was in Apstate {ft_helper.ap_state_dict(the_state_of_ap)}"
...
@verifies("ReqId_1405918")
@testcase_definition(
    name="SI_ParallelParkingBox_Max_OpeningTC",
    description="The SceneInterpretation shall provide an parallel parking box that has a minimum opening.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9tyHqjpwEe6GONELpfdJ2Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_ParallelParkingBox_Max_OpeningTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_ParallelParkingBox_Max_Opening,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    """
    # test_bsigs = r"D:\reporting-ui\workspace\workspace\pl_parking\recordings\SI_recc\mine\21_11_2024_ang_perp_par\SISim_UC_Angled_Empty_Prk_Mrk_Defined.testrun.erg"

    debug(
        SI_ParallelParkingBox_Max_OpeningTC,
        # test_bsigs,
        temp_dir=temp_dir,
