...
Given two parking markings that are separated by a free space that is:
    ...
    1. Wider than the maximum width for a parallel parking space (larger than)
...
READER_NAME = "SI_ParallelSpaceParkingMarkingsWidthValidation"
signals_obj = SISignals()


@teststep_definition(
    name="Check for SI_ParallelSpaceParkingMarkingsWidthValidation",
    description="Given two parking markings that are separated by a free space that is Wider"
    "than the maximum width for a parallel parking space.",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_ParallelSpaceParkingMarkingsWidthValidation(TestStep):
    """Example test step."""
            ...
            for timestamp_val in new_collected_info.keys():
                if new_collected_info[timestamp_val].get("marker_match", "") == "marker_only":
                    for park_box_id in new_collected_info[timestamp_val]["markers"].keys():
                        ...
                        if not is_wider_than_max_par_lenght_bool:
                            ...
                            new_collected_info[timestamp_val]["description"][
                                park_box_id
                            ] += f"<b>This slot was not detected</b><br>Wider than the maximum width to fit a parallel parking space (greater than <b>{max_par_park_threshold} [m]</b>).\
                                The car was in Apstate {ft_helper.ap_state_dict(the_state_of_ap)}"
...
@verifies("ReqId_1405842")
@testcase_definition(
    name="SI_ParallelSpaceParkingMarkingsWidthValidationTC",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9twSizpwEe6GONELpfdJ2Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg&vvc.configuration=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fcm%2Fstream%2F_G3kr8DgnEe6mrdm2_agUYg",
    description="Given two parking markings that are separated by a free space that is Wider"
    "than the maximum width for a parallel parking space",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_ParallelSpaceParkingMarkingsWidthValidationTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_ParallelSpaceParkingMarkingsWidthValidation,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SI_ParallelSpaceParkingMarkingsWidthValidationTC,
        # test_bsigs,
        temp_dir=temp_dir,
