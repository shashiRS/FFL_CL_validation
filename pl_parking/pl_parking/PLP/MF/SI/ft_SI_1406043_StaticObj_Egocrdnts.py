"""Static Objects Sorting by Distance from Ego Vehicle"""
...
SIGNAL_DATA = "SI_StaticObjectDistanceValidation"
signals_obj = SISignals()


@teststep_definition(
    step_number=1,
    name="StaticObjectDistanceSortingEgoCoordinatesTC",
    description="Verify static objects are sorted by distance from the ego vehicle.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SISignals)
class StaticObjectDistanceTypeSorting(TestStep):
    """Test Step for sorting static objects (parking boxes) by distance from the ego vehicle."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            # Extract the number of static objects and timestamps
            number_of_StaticObjects = read_data["collEnvModelPort.numberOfStaticObjects_u8"]

            unique_number_of_StaticObjects = number_of_StaticObjects.unique().tolist()

            if len(unique_number_of_StaticObjects) == 0:
                _log.debug("No static objects found.")
            ...
            for timestamp in filteredTimestamp:
                ...
                for vertex in range(16):
                    obj_0_x_col = f"collEnvModelPort.staticObjects_0.objShape_m_{vertex}.x"
                    obj_0_y_col = f"collEnvModelPort.staticObjects_0.objShape_m_{vertex}.y"
                    obj_1_x_col = f"collEnvModelPort.staticObjects_1.objShape_m_{vertex}.x"
                    obj_1_y_col = f"collEnvModelPort.staticObjects_1.objShape_m_{vertex}.y"
                    obj_2_x_col = f"collEnvModelPort.staticObjects_2.objShape_m_{vertex}.x"
                    obj_2_y_col = f"collEnvModelPort.staticObjects_2.objShape_m_{vertex}.y"
                    obj_3_x_col = f"collEnvModelPort.staticObjects_3.objShape_m_{vertex}.x"
                    obj_3_y_col = f"collEnvModelPort.staticObjects_3.objShape_m_{vertex}.y"
                ...
                # Transform the ego vehicle coordinates to the ego coordinates
                # vehicle_polygon_in_ego_coordinate = ft_helper.transform_ego_to_ego(
                #     ego_vehicle_position, ego_yaw_rad, vehicle_polygon
                # )
                vehicle_polygon_in_world_coordinate = vehicle_polygon
                # print(f"vehicle_polygon_in_ego_coordinate is : {vehicle_polygon_in_ego_coordinate}")
            ...
            table_remark = "<b>Static Objects should be sorted by their distance from the ego vehicle.</b><br><br>"
            if (
                list_min_distance_for_obj_0_all_vertices
                ...
                if res1:
                    ...
                    sig_sum_html = ft_helper.create_hidden_table(
                        sig_sum_html, 1, button_text="Static Object Distances Table"
                    )
                ...
                # Plotting the distances and number of static objects over time
                fig = go.Figure()
                ...
                # Plot number of static objects
                fig.add_trace(
                    go.Scatter(
                        x=list_ap_time_in_sec,
                        y=number_of_StaticObjects,
                        mode="lines+markers",
                        name="Number of Static Objects",
                        line=dict(color="red"),
                        marker=dict(size=8),
                    )
                )
                fig.update_layout(
                    title="Distance of Static Objects and Object Count Over Time",
                ...
                plot_titles.append("Static Object Distances and Counts")
                plots.append(fig)
...
@verifies("ReqId_1406043")
@testcase_definition(
    name="StaticObjectDistanceSortingEgoCoordinatesTC",
    description="Test Case for static object distance sorting.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9tz87jpwEe6GONELpfdJ2Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36325",
)
@register_inputs("/Playground_2/TSF-Debug")
class StaticObjectDistanceSortingTC(TestCase):
    """Test Case for verifying sorting of static objects by distance."""

    @property
    def test_steps(self):
        """Define the test steps."""
        return [
            StaticObjectDistanceTypeSorting,
        ]


def main(data_folder, temp_dir=None, open_explorer=True):
    """Main entry point for debugging the test case."""
    # test_bsigs = r"D:\Parking_SI\carmaker\Latest\adc5_platform_meta\out\bricks_mf_sil_performance_vs2017_debug_prod\package\tests\SIL\CarMaker\SimOutput\SISim_UC_Rear_Static_Obstacles.testrun.erg"
    test_bsigs = r"D:\Parking_SI\ergs\Rear_Obstacles\SISim_UC_Rear_Static_Obstacles.testrun.erg"

    debug(
        StaticObjectDistanceSortingTC,
        test_bsigs,
        temp_dir=temp_dir,
