#!/usr/bin/env python3
"""
Once a parking box has been detected it shall be provided in the output until the distance between
the parking box and the ego vehicle is larger than:

( {AP_G_SCAN_OFFER_SLOTS_LO_DIST_M} )
...
READER_NAME = "SI_Distance_between_PB_and_ego_Vehicle_larger_than_threshold_value"
signals_obj = SISignals()


@teststep_definition(
    step_number=1,
    name="Check for SI_Distance_between_PB_and_ego_Vehicle_larger_than_threshold_value",
    description="Once a parking box has been detected it shall be provided in the output until the distance between"
    "the parking box and the ego vehicle is larger than AP_G_SCAN_OFFER_SLOTS_LO_DIST_M",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_Distance_between_PB_and_ego_Vehicle_larger_than_threshold_value(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            Max_Threshold_Distance = ConstantsSI.AP_G_SCAN_OFFER_SLOTS_LO_DIST_M

            pb_with_marker_match = {}
            table_remark = f"Once a parking box has been detected it shall be provided in the output until the distance between the parking box and the ego vehicle is larger than <br> {Max_Threshold_Distance} [m]. <br><br>"
...
@verifies("ReqId_1841553")
@testcase_definition(
    name="SI_Distance_between_PB_and_ego_Vehicle_largerthanTC",
    description="Once a parking box has been detected it shall be provided in the output until the distance between"
    "the parking box and the ego vehicle is larger than:"
    "( {AP_G_SCAN_OFFER_SLOTS_LO_DIST_M} )",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_qZDygLrAEe6Sn6sUDJb62g&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_Distance_between_PB_and_ego_Vehicle_larger_than_threshold_valueTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_Distance_between_PB_and_ego_Vehicle_larger_than_threshold_value,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SI_Distance_between_PB_and_ego_Vehicle_larger_than_threshold_valueTC,
        # test_bsigs,
        temp_dir=temp_dir,
