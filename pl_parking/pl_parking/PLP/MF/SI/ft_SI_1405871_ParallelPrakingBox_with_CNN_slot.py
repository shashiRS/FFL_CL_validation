"""
The road side edge of a parallel parking box shall not be moved by more than
 {AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M} closer to the road than the road side edge of the CNN slot.
"""
...
READER_NAME = "SI_ParallelPrakingBox_with_CNN_slot"
signals_obj = SISignals()


@teststep_definition(
    step_number=1,
    name="Check for SI_ParallelPrakingBox_with_CNN_slot",
    description="The road side edge of a parallel parking box shall not be moved by more than {AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M} closer to the road than the road side edge of the CNN slot.",
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(READER_NAME, SISignals)
class SI_ParallelPrakingBox_with_CNN_slot(TestStep):
    """Example test step."""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            table_remark = f"The road side edge of a parallel parking box shall not be moved by more than <br> {f'{VAL_AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M}'} [m] closer to the road than the road side edge of the CNN slot. <br><br>"
            ...
            for timestamp_val in filtered_timestamps:
                ...
                if pb_from_ts:
                    ...
                    collect_data_dict[timestamp_val]["Edge of the CNN slot"][
                        park_box_id
                    ] = VAL_AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M
                    ...
                    # print(
                    # f"At {timestamp_val} AB_depth_pb is : {AB_depth_pb} , min_distance_between_CNN_slots is : {min_distance_between_CNN_slots} and difference is : {abs(difference)}"
                    # )

                    if AB_depth_pb >= (min_distance_between_CNN_slots - VAL_AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M):
                        ...
                        parking_edge >= cnn_edge and there difference { AB_depth_pb - min_distance_between_CNN_slots } [m], which falls in The maximum allowed distance the parking edge can move closer to the road  {VAL_AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M} [m]."
                        ...
                        parking_edge <= cnn_edge and there difference { AB_depth_pb - min_distance_between_CNN_slots } [m], which does not falls in The maximum allowed distance the parking edge can move closer to the road  {VAL_AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M} [m]."
...
@verifies("ReqId_1405871")
@testcase_definition(
    name="SI_ParallelPrakingBox_with_CNN_slotTC",
    description="The road side edge of a parallel parking box shall not be moved by more than {AP_SI_CNN_ROAD_ALIGN_LIMIT_TO_ROAD_PARA_M} closer to the road than the road side edge of the CNN slot.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_9tw5lDpwEe6GONELpfdJ2Q&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_N-ji4CuIEe6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
@register_inputs("/Playground_2/TSF-Debug")
class SI_ParallelPrakingBox_with_CNN_slotTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            SI_ParallelPrakingBox_with_CNN_slot,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SI_ParallelPrakingBox_with_CNN_slotTC,
        # test_bsigs,
        temp_dir=temp_dir,
