"""
SWRT_CNC_TRJPLA_AngledFrontOpeningMaxTargetPose.py
Test Scenario: ParkIn maneuver followed by ParkOut maneuver to/from a Parking box of scenarioType ANGLED_PARKING_OPENING_TOWARDS_FRONT.
(Script expects scenarioType ANGLED_PARKING_OPENING_TOWARDS_FRONT for Parking slot 0. Test scenario Single or all angled parking slots of ANGLED_PARKING_OPENING_TOWARDS_FRONT can be used.)
"""
...
@teststep_definition(
    step_number=1,
    name="Maximum target pose angled front opening scenario]",
    description="TRJPLA shall provide maximum 1 target pose per angled parking box with opening towards front",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class AngledParkingFrontOpening(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            for idx, val in enumerate(parking_scenario_0):
                if val == ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT:
                    par_pose = idx
                    break
            test1 = []
            test2 = []
                ...
                if par_pose is not None:
                    for idx in range(t2, len(ap_state)):
                        if parking_scenario_0.iloc[
                            idx
                        ] == ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT and (
                            ...
                            if num_valid_poses.iloc[idx] > 0:
                                if ap_state.iloc[idx] == ConstantsTrajpla.ApStates.AP_SCAN_IN and len(test1) == 0:
                                    test1.append(idx)
                                if ap_state.iloc[idx] == ConstantsTrajpla.ApStates.AP_SCAN_OUT and len(test2) == 0:
                                    test2.append(idx)
                                ...
                                if (
                                    ...
                                    frame_test_status.append(
                                        f"More than {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE} target pose/angled parking box with opening towards front"
                                    )
                            else:
                    ...
                    evaluation = " ".join(
                        ...
                        f"{signal_name['parkingScenario_nu0']} was never equal to ANGLED_PARKING_OPENING_TOWARDS_FRONT"
                        f"({ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT})".split()
                    )
            else:
                frame_test_result.append(None)
                evaluation = " ".join(
            ...
            expected_val = (
                f"Number of target poses per Parking Box of ScenarioType ANGLED_PARKING_OPENING_TOWARDS_FRONT"
                f"({ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT}) "
                f"<= {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE}"
            )

            signal_summary["Number of target poses per Parking Box of type ANGLED_PARKING_OPENING_TOWARDS_FRONT"] = [
                expected_val,
                evaluation,
                verdict,
            ]
            ...
            remark = (
                f"Check if Number of target poses per parking box of type ANGLED_PARKING_OPENING_TOWARDS_FRONT"
                f"({ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT}) <= {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE}."
            )
            ...
            # add graph for relatedParkingBoxId
            fig = go.Figure()
...
@testcase_definition(
    ...
    "<br>TRJPLA shall provide maximum 1 target pose per angled parking box with opening towards front (ApParkingBoxPort.parkingBoxes.parkingScenario_nu == ANGLED_PARKING_OPENING_TOWARDS_FRONT) in TargetPosesPort.targetPoses.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_Sr7cYHPTEe6YqIugsJ-rgQ&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17100",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class AngledParkingFrontOpeningTC(TestCase):
    """Example test case."""
