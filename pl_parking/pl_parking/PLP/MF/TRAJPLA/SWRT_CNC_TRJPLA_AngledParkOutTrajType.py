...
@teststep_definition(
    step_number=1,
    name="Planned traj type.",
    description=" TRJPLA shall send traj type based on selected pose type.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TrjplaTrajType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        for idx, val in enumerate(pose_type):
            if (
                val == ConstantsTrajpla.T_ANGLED_PARKING_STANDARD_OUT
                or val == ConstantsTrajpla.T_ANGLED_PARKING_REVERSE_OUT
            ):
                par_pose = idx
                break
            ...
            if par_pose is not None:
                for idx in range(t2, len(traj_valid)):
                    if traj_valid[idx] == ConstantsTrajpla.TRAJ_VALID and (
                        pose_type[idx] == ConstantsTrajpla.T_ANGLED_PARKING_STANDARD_OUT
                        or pose_type[idx] == ConstantsTrajpla.T_ANGLED_PARKING_REVERSE_OUT
                    ):
                        if traj_type[idx] == ConstantsTrajpla.PAR_PARK_OUT_TRAJ and eval_cond:
                            evaluation = " ".join(
                                f"The evaluation of {signal_name['trajType_nu']} is PASSED, with values ="
                                f" {ConstantsTrajpla.PAR_PARK_OUT_TRAJ} when {signal_name['poseType0']} == "
                                f"{ConstantsTrajpla.T_ANGLED_PARKING_STANDARD_OUT} or "
                                f" {ConstantsTrajpla.T_ANGLED_PARKING_REVERSE_OUT}".split()
                            )
                            eval_cond = True
                        else:
                            evaluation = " ".join(
                                f"The evaluation of {signal_name['trajType_nu']} and is FAILED, with values ="
                                f" {traj_type[idx]} when {signal_name['poseType0']} == "
                                f"{pose_type[idx]} at frame  {idx}.".split()
                            )
                            eval_cond = False
                            break
            else:
                ...
                evaluation = " ".join(
                    ...
                    f"{signal_name['poseType0']} was never equal to {ConstantsTrajpla.T_ANGLED_PARKING_STANDARD_OUT} "
                    f" or {ConstantsTrajpla.T_ANGLED_PARKING_REVERSE_OUT}".split()
                )
        else:
            eval_cond = False
            evaluation = " ".join(
        ...
        signal_summary[f"{signal_name['trajType_nu']} and {signal_name['poseType0']}"] = evaluation
        ...
        # testStatusList = pose_Type
        fig = go.Figure(
            data=[
                go.Table(
                    header=dict(
                        values=["Frames", "trajValid_nu", "Selected Target Pose Type", "TrajType"],
                        fill_color="paleturquoise",
                    ),
                    cells=dict(
                        values=[
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                "This testcase will verfiy that If a valid path is available for the given environmental model "
                "and the Selected_Target_Pose is of type T_ANGLED_PARKING_STANDARD_OUT or "
                "T_ANGLED_PARKING_REVERSE_OUT, TRJPLA shall send <PlannedTrajPort.trajType_nu> = ANG_PARK_OUT_TRAJ. "
            ],
