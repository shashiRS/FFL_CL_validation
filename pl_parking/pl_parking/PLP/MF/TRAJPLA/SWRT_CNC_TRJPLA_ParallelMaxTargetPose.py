...
class TrjplaParallelParkingScan(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for idx, val in enumerate(parking_scenario_0):
            if val == ConstantsTrajpla.PARALLEL_PARKING:
                par_pose = idx
                break
        ...
        if t2 is not None:
            if par_pose is not None:
                for idx in range(t2, len(ap_state)):
                    if parking_scenario_0[idx] == ConstantsTrajpla.PARALLEL_PARKING and (
                        ap_state[idx] == ConstantsTrajpla.AP_SCAN_IN or ap_state[idx] == ConstantsTrajpla.AP_SCAN_OUT
                    ):
                        if num_valid_poses[idx] <= ConstantsTrajpla.PARALLEL_PARK_MAX_TARGET_POSE:
                            evaluation = " ".join(
                            ...
                            frame_data_dict[idx] = "More than 1 target pose/parallel parking box"
                    else:
                ...
                evaluation = " ".join(
                    ...
                    f"{signal_name['numValidPoses']} was never equal to {ConstantsTrajpla.PARALLEL_PARKING}".split()
                )
        else:
            test_result_dict[0] = False
            evaluation = " ".join(
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                ...
                "target pose per parallel parking box (<ApParkingBoxPort.parkingBoxes.parkingScenario"
                "_nu == PARALLEL_PARKING) in <TargetPosesPort.targetPoses>"
            ],
...
@testcase_definition(
    ...
    description="This testcase will verify that TRJPLA shall provide maximum 1 target pose per parallel parking box",
)
