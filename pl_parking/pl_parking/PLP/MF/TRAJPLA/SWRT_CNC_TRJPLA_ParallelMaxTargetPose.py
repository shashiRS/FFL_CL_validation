...
SWRT_CNC_TRJPLA_ParallelMaxTargetPose
...
SIGNAL_DATA = "MF_PARALLEL_MAX_TP"
...
@teststep_definition(
    step_number=1,
    name="Maximum target pose parallel scenario",
    description="TRJPLA shall provide maximum 1 target pose per parallel parking box",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TrjplaParallelParkingScan(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            expected_val = (
                f"Number of target poses per Parking box of ScenarioType PARALLEL_PARKING"
                f"({ConstantsTrajpla.ScenarioType.PARALLEL_PARKING}) "
                f"<= {ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE}"
            )

            signal_summary["Number of target poses per parking box of type PARALLEL_PARKING"] = [
                expected_val,
                evaluation,
                verdict,
            ]
            ...
            remark = (
                f"Check if Number of target poses per parking box of type PARALLEL_PARKING "
                f"<= {ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE}"
            )
...
@verifies("1612315")
@testcase_definition(
    name="SWRT_CNC_TRJPLA_ParallelMaxTargetPose",
    ...
    "<br>TRJPLA shall provide maximum 1 target pose per parallel parking box "
    "<br> (ApParkingBoxPort.parkingBoxes.parkingScenario_nu == PARALLEL_PARKING) in TargetPosesPort.targetPoses.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_EUEtYHPQEe6YqIugsJ-rgQ&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class TrjplaParallelMaxTargetPoseTC(TestCase):
    """Example test case."""
