...
class TrjplaTrajType(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for idx, val in enumerate(pose_type):
            if val == ConstantsTrajpla.T_PARALLEL_PARKING:
                par_pose = idx
                break
            ...
            if par_pose is not None:
                for idx in range(t2, len(traj_valid)):
                    if (
                        ...
                        and pose_type[idx] == ConstantsTrajpla.T_PARALLEL_PARKING
                    ):
                        if traj_type[idx] == ConstantsTrajpla.PAR_PARK_IN_TRAJ and eval_cond:
                            evaluation = " ".join(
                                f"The evaluation of {signal_name['trajType_nu']} and is PASSED, with values ="
                                f" {traj_type[idx]} when {signal_name['poseType0']} == "
                                f"{pose_type[idx]}.".split()
                            )
                            eval_cond = True
                        else:
                            evaluation = " ".join(
                                ...
                                f" {traj_type[idx]} when {signal_name['poseType0']} == "
                                f"{pose_type[idx]} at frame  {idx} s.".split()
                            )
                            eval_cond = False
                        break
            else:
                ...
                evaluation = " ".join(
                    ...
                    f"{signal_name['poseType0']} was never equal to {ConstantsTrajpla.T_PARALLEL_PARKING}".split()
                )
        else:
            eval_cond = False
            evaluation = " ".join(
        ...
        signal_summary["Target Pose"] = evaluation
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                "This test will verify If a valid path is available (<PlannedTrajPort.trajValid_nu>  == true)"
...
class ParallelParkTrajTypeTC(TestCase):
    """Example test case."""
