...
class ParParkOutTrajType(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for idx, val in enumerate(pose_type):
            if val == ConstantsTrajpla.T_PAR_PARKING_OUT:
                par_pose = idx
                break
        ...
        if t2 is not None:
            ...
            if par_pose is not None:
                for idx in range(t2, len(traj_valid)):
                    if (
                        traj_valid[idx] == ConstantsTrajpla.TRAJ_VALID
                        and pose_type[idx] == ConstantsTrajpla.T_PAR_PARKING_OUT
                    ):
                        if traj_type[idx] == ConstantsTrajpla.PAR_PARK_OUT_TRAJ and eval_cond:
                            evaluation = " ".join(
                                ...
                                f"{ConstantsTrajpla.T_PAR_PARKING_OUT}.".split()
                            )
                            eval_cond = True
                        else:
                            evaluation = " ".join(
                ...
                evaluation = " ".join(
                    ...
                    f"{signal_name['poseType0']} was never equal to {ConstantsTrajpla.T_PAR_PARKING_OUT}".split()
                )
        else:
            eval_cond = False
            evaluation = " ".join(
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                "This testcase will verfiy that If a valid path is available for the given environmental model and the"
