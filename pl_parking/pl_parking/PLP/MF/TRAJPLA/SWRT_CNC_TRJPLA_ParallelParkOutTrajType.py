...
SWRT_CNC_TRJPLA_ParallelParkOutTrajType.py
Test Scenario: Scenario should have a Park out maneuver from a parallel parking box
"""
...
SIGNAL_DATA = "MF_PARALLEL_PARKOUT_TRAJ"
...
@teststep_definition(
    step_number=1,
    name="Planned trajectory type parallel Park Out",
    description=f"TRJPLA shall send planned traj type PAR_PARK_OUT_TRAJ({ConstantsTrajpla.PlannedTrajType.PAR_PARK_OUT_TRAJ}), "
    f"when selected target pose type is T_PAR_PARKING_OUT({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT})",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class ParParkOutTrajType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            for idx, val in enumerate(pose_type):
                if val == ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT:
                    par_pose = idx
                    break
                ...
                if par_pose is not None:
                    for idx in range(t2, len(traj_valid)):
                        if (
                            ...
                            and pose_type.iloc[idx] == ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT
                        ):
                            if traj_type.iloc[idx] == ConstantsTrajpla.PlannedTrajType.PAR_PARK_OUT_TRAJ:
                                ...
                                frame_test_status.append(
                                    f"Fail: trajType_nu != PAR_PARK_OUT_TRAJ({ConstantsTrajpla.PlannedTrajType.PAR_PARK_OUT_TRAJ})"
                                )
                    ...
                    evaluation = " ".join(
                        ...
                        f"T_PAR_PARKING_OUT({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT})".split()
                    )
            else:
                frame_test_result.append(None)
                evaluation = " ".join(
            ...
            expected_val = f"{signal_name['trajType_nu']} = PAR_PARK_OUT_TRAJ({ConstantsTrajpla.PlannedTrajType.PAR_PARK_OUT_TRAJ})"

            signal_summary[
                f"{signal_name['trajType_nu']} = PAR_PARK_OUT_TRAJ({ConstantsTrajpla.PlannedTrajType.PAR_PARK_OUT_TRAJ}), when "
                f"<br>{signal_name['poseType0']} == "
                f"T_PAR_PARKING_OUT({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT})"
            ] = [expected_val, evaluation, verdict]

            remark = (
                f"Check if {signal_name['trajType_nu']} = "
                f"PAR_PARK_OUT_TRAJ({ConstantsTrajpla.PlannedTrajType.PAR_PARK_OUT_TRAJ}), "
                f"<br>when a valid path is available ({signal_name['trajValid_nu']} = True(1)) and "
                f"<br>the selected target pose is of type T_PAR_PARKING_OUT({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT})"
            )
...
@verifies("1500188")
@testcase_definition(
    name="SWRT_CNC_TRJPLA_ParallelParkOutTrajType",
    description="This testcase will verfiy that If a valid path is available for the given environmental model and the"
    " Selected_Target_Pose is of type T_PAR_PARKING_OUT, TRJPLA shall send PlannedTrajPort.trajType_nu ="
    " PAR_PARK_OUT_TRAJ.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_sqEiAEc0Ee68PtCWeWkWbA&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class ParParkOutTrajTypeTC(TestCase):
    """Example test case."""
