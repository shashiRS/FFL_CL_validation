...
class AngledParkingBackOpening(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for idx, val in enumerate(parking_scenario_0):
            if val == ConstantsTrajpla.ANGLED_PARKING_OPENING_TOWARDS_BACK:
                par_pose = idx
                break
        ...
        if t2 is not None:
            if par_pose is not None:
                for idx in range(t2, len(ap_state)):
                    if parking_scenario_0[idx] == ConstantsTrajpla.ANGLED_PARKING_OPENING_TOWARDS_BACK and (
                ...
                evaluation = " ".join(
                    ...
                    f"{ConstantsTrajpla.ANGLED_PARKING_OPENING_TOWARDS_BACK}".split()
                )
        else:
            test_result_dict[0] = False
            evaluation = " ".join(
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                ...
                "target pose per angled parking box with opening towards back in <TargetPosesPort.targetPoses>"
            ],
...
@testcase_definition(
    ...
    description=(
        "Verify that TRJPLA shall provide maximum 1 target pose per angled parking box with opening towards back"
    ),
)
