...
SIGNAL_DATA = "MF_PERPENDICULAR_PARKIN_TRAJ"
...
@teststep_definition(
    step_number=1,
    name="Planned trajectory type perpendicular Park In",
    description=f"TRJPLA shall send planned traj type PERP_PARK_IN_TRAJ({ConstantsTrajpla.PlannedTrajType.PERP_PARK_IN_TRAJ}), "
    f"when selected target pose type is T_PERP_PARKING_FWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD})"
    f" or T_PERP_PARKING_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD}).",
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            for idx, val in enumerate(pose_type):
                if (
                    val == ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD
                    or val == ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD
                ):
                    par_pose = idx
                    break
                ...
                if par_pose is not None:
                    for idx in range(t2, len(traj_valid)):
                        if traj_valid.iloc[idx] == ConstantsTrajpla.TRAJ_VALID and (
                            pose_type.iloc[idx] == ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD
                            or pose_type.iloc[idx] == ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD
                        ):
                    ...
                    evaluation = " ".join(
                        ...
                        f"T_PERP_PARKING_FWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD}) or "
                        f"T_PERP_PARKING_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD})".split()
                    )
            else:
                frame_test_result.append(None)
                evaluation = " ".join(
            ...
            signal_summary[
                ...
                f"T_PERP_PARKING_FWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD}) or "
                f"T_PERP_PARKING_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD})"
            ...
            remark = (
                ...
                f"<br>the selected target pose is of type T_PERP_PARKING_FWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD}) "
                f"or T_PERP_PARKING_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD})"
            )
...
@verifies("1500178")
@testcase_definition(
    name="SWRT_CNC_TRJPLA_PerpendicularParkInTrajType",
    ...
    "and the Selected_Target_Pose is of type T_PERP_PARKING_FWD or T_PERP_PARKING_BWD, "
    "TRJPLA shall send PlannedTrajPort.trajType_nu = PERP_PARK_IN_TRAJ.",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_juR9oEcyEe68PtCWeWkWbA&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class TrjplaPerpTrajTypeTC(TestCase):
    """Example test case."""
