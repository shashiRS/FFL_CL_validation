...
class TrjplaTrajType(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for idx, val in enumerate(pose_type):
            if val == ConstantsTrajpla.T_PERP_PARKING_FWD or val == ConstantsTrajpla.T_PERP_PARKING_BWD:
                par_pose = idx
                break
            ...
            if par_pose is not None:
                for idx in range(t2, len(traj_valid)):
                    if traj_valid[idx] == ConstantsTrajpla.TRAJ_VALID and (
                        pose_type[idx] == ConstantsTrajpla.T_PERP_PARKING_FWD
                        or pose_type[idx] == ConstantsTrajpla.T_PERP_PARKING_BWD
                    ):
                        if traj_type[idx] == ConstantsTrajpla.PERP_PARK_IN_TRAJ and eval_cond:
                            evaluation = " ".join(
                                f"The evaluation of {signal_name['trajType_nu']} is PASSED, with values ="
                                f" {ConstantsTrajpla.PERP_PARK_IN_TRAJ} when {signal_name['poseType0']} == "
                                f"{ConstantsTrajpla.T_PERP_PARKING_FWD} or {ConstantsTrajpla.T_PERP_PARKING_BWD}.".split()
                            )
                            eval_cond = True
                        else:
                            evaluation = " ".join(
                                ...
                                f" {ConstantsTrajpla.PERP_PARK_IN_TRAJ} when {signal_name['poseType0']} =="
                                f" {ConstantsTrajpla.T_PERP_PARKING_FWD} or {ConstantsTrajpla.T_PERP_PARKING_BWD} at"
                                f" frame  {idx} s.".split()
                            )
                            eval_cond = False
                            break
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                ...
                " is of type T_PERP_PARKING_FWD or T_PERP_PARKING_BWD, TRJPLA"
                " shall send <PlannedTrajPort.trajType> = PERP_PARK_IN_TRAJ."
            ],
