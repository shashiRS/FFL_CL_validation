"""moco outside position on path before first path point"""
...
ALIAS = "SWRT_CNC_MOCO_OutsidePositionOnPathBeforeFirstPathPoint"
...
class Step1(TestStep):
    """moco outside position on path before first path point test setup"""
        ...
        try:
            ...
            if not df_filtered.empty:
                ...
                if all(passed):
                    ...
                    test_result = fc.PASS
                    eval_text = f"The position on path is outside the planned path (=before first path point) more than AP_C_LEAVING_PATH_BEFORE_M ({constants.MoCo.Parameter.AP_C_LEAVING_PATH_BEFORE_M}) and the component output mfControlStatusPort.longitudinalPathControlFailed_nu=true."
                else:
                    self.result.measured_result = FALSE
                    test_result = fc.FAIL
                    eval_text = f"The position on path is outside the planned path (=before first path point) more than AP_C_LEAVING_PATH_BEFORE_M ({constants.MoCo.Parameter.AP_C_LEAVING_PATH_BEFORE_M}) and the component output mfControlStatusPort.longitudinalPathControlFailed_nu=false."

                eval_0 = " ".join(
                    f"Ensure that if the position on path is outside the planned path (=before first path point) more than AP_C_LEAVING_PATH_BEFORE_M ({constants.MoCo.Parameter.AP_C_LEAVING_PATH_BEFORE_M}) the component shall output mfControlStatusPort.longitudinalPathControlFailed_nu=true.".split()
                )
...
@tag("SWRT_CNC_MOCO_OutsidePositionOnPathBeforeFirstPathPoint")
@verifies("1407075")
@testcase_definition(
    name="MOCO Outside Position On Path Before First Path Point",
...
class SWRT_CNC_MOCO_OutsidePositionOnPathBeforeFirstPathPoint(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_OutsidePositionOnPathBeforeFirstPathPoint,
        *test_bsigs,
        temp_dir=temp_dir,
