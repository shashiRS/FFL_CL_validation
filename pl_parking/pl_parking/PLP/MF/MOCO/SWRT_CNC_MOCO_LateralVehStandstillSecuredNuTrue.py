"""moco lateral veh standstill secured nu true"""
...
ALIAS = "SWRT_CNC_MOCO_LateralVehStandstillSecuredNuTrue"
...
class Step1(TestStep):
    """moco lateral veh standstill secured nu true test setup"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if not df_filtered.empty:
                ...
                if (df["calculated_veh_secured_standstill"] == 1).any():
                    ...
                    eval_text = "calculated_veh_secured_standstill is not set"
                    self.result.measured_result = NAN
...
@tag("SWRT_CNC_MOCO_LateralVehStandstillSecuredNuTrue")
@verifies("1974778")
@testcase_definition(
    name="MOCO Lateral Veh Standstill Secured Nu True",
    description="Ensure that if the current steer angle(s) is/are close to static and  close to the requested steer angle(s), the component "
    "shall output the vehicle standstill secured information (mfControlStatusPort.vehStandstillSecured_nu=true)",
    group="steer",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class SWRT_CNC_MOCO_LateralVehStandstillSecuredNuTrue(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_LateralVehStandstillSecuredNuTrue,
        *test_bsigs,
        temp_dir=temp_dir,
