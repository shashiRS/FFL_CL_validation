"""moco lateral control finished false"""
...
from pl_parking.PLP.MF.MOCO.helpers import (
    ...
    get_steer_angle_not_close_to_static_trigger,
)
...
ALIAS = "SWRT_CNC_MOCO_LateralControlFinishedFalse"
...
class Step1(TestStep):
    """moco lateral control finished false test setup"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            # "Determine steer angle not close to static when is triggered"
            df = get_steer_angle_not_close_to_static_trigger(df)

            "TestStep"
            "Filter data by lateral control request"
            df_filtered = df[(df["activateLaCtrl"] == 1)]

            new_df_filtered = df_filtered[(df_filtered["steer_angle_not_close_to_static"] == 1)]

            passed = []

            if not new_df_filtered.empty:
                for _, car_pos_row in new_df_filtered.iterrows():
                    if car_pos_row["lateralControlFinished_nu"] == 0:
                        passed.append(True)
                    else:
                        passed.append(False)
                if all(passed):
                    ...
                    test_result = fc.PASS
                    eval_text = " ".join(
                        f"Current steer angle is not close to Requested steer angle and "
                        f"lateralControlFinished_nu is false, Satisfied the requirement"
                    ...
                    eval_text = " ".join(
                        f"Current steer angle is not close to Requested steer angle and "
                        f"Didn't satisfy the requirement"
                        f"Conditions: {test_result}.".split()
                    )

                eval_0 = " ".join(
                    " If the current steer angle(s) is/are not close to static, ensure that "
                    "the component shall output the not finished lateral control information.".split()
                )
                # Set table dataframe
                ...
                fig.add_trace(
                    go.Scatter(
                        x=df.index.values.tolist(),
                        y=df["steer_angle_not_close_to_static"].values.tolist(),
                        mode="lines",
                        name="steer_angle_not_close_to_static_trigerred",
                    )
                )
                ...
                eval_text = "Pre-Condition not satisfied"
                self.result.measured_result = NAN
...
@tag("SWRT_CNC_MOCO_LateralControlFinishedFalse")
@verifies("1407013")
@testcase_definition(
    name="MOCO Lateral Control Finished False",
    description="If the current steer angle(s) is/are not close to static, ensure that the "
    "component shall output the not finished lateral control information(mfControlStatusPort.lateralControlFinished_nu)",
    group="steer",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class SWRT_CNC_MOCO_LateralControlFinishedFalse(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_LateralControlFinishedFalse,
        *test_bsigs,
        temp_dir=temp_dir,
