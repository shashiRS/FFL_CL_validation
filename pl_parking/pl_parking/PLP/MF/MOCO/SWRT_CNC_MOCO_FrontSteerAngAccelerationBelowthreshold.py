...
ALIAS = "SWRT_CNC_MOCO_FrontSteerAngAccelerationBelowthreshold"
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if not df_filtered.empty:
                ...
                for ts, _ in df_filtered.iterrows():
                ...
                front_steer_angle = df_filtered["frontSteerAngReq_rad_sec2"]

                front_steer_angle_belowThreshold = (
                    abs(front_steer_angle) < constants.MoCo.Parameter.AP_C_PC_MAX_STEER_ACC_RADPS2
                )
                true_count = front_steer_angle_belowThreshold.value_counts()
                if true_count[True] == len(front_steer_angle_belowThreshold):
                    passed = True
                else:
                    passed = False
                if passed:
...
@tag("SWRT_CNC_MOCO_FrontSteerAngAccelerationBelowthreshold")
@verifies("1407004")
@testcase_definition(
...
class SWRT_CNC_MOCO_FrontSteerAngAccelerationBelowthreshold(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_FrontSteerAngAccelerationBelowthreshold,
        *test_bsigs,
        temp_dir=temp_dir,
