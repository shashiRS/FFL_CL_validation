...
ALIAS = "SWRT_CNC_MOCO_LateralMaxLatError"
...
class Step1(TestStep):
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            "TestStep"
            "Filter data by lateral control request,longitudinal control request and lateral operation mode control by path"
            df_filtered = df[
                (df["activateLaCtrl"] == 1)
            ...
            if not df_filtered.empty:
                ...
                for _, car_pos_row in df_filtered.iterrows():
                    ...
                    if (abs(lateral_deviation) > constants.MoCo.Parameter.AP_C_PC_FAIL_MAX_LAT_ERROR_M and
                ...
                if all(passed):
                    ...
                    test_result = fc.FAIL
                    eval_text = " ".join(
                        f"Lateral request control is active and absolute lateral deviation from planned path is greater than "
                ...
                eval_0 = " ".join(
                    f"Lateral control request sent and if absolute lateral deviation from planned path is greater "
                    f"than AP_C_PC_FAIL_MAX_LAT_ERROR_M({constants.MoCo.Parameter.AP_C_PC_FAIL_MAX_LAT_ERROR_M})."
                    f"And the component output is requested to stop the vehicle(loDMCCtrlRequestPort.holdReq_nu=LODMC_HOLD_REQ_ON)."
                    .split()
                )
...
@tag("SWRT_CNC_MOCO_LateralMaxLatError")
@verifies("1407123")
@testcase_definition(
...
class SWRT_CNC_MOCO_LateralMaxLatError(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_LateralMaxLatError,
        *test_bsigs,
        temp_dir=temp_dir,
