...
ALIAS = "SWRT_CNC_MOCO_FrontSteerAngleRequestLimitMaxSteerAngleVelocity"
            ...
            if not df_filtered.empty:
                ...
                for ts, car_pos_row in df_filtered.iterrows():
                    if first_iteration == 1:
                        front_steer_angle_rad_sec = steeranglegrad
                        df.at[ts, "frontSteerAngReq_rad_sec"] = steeranglegrad
                        first_iteration = 0
                    else:
                        front_steer_angle_rad_sec = car_pos_row["frontSteerAngReq_rad_sec"]

                    if (abs(front_steer_angle_rad_sec)) <= (constants.MoCo.Parameter.AP_V_MAX_STEER_ANG_VEL_RADPS +
                                                            constants.MoCo.Parameter.floatingPointThreshold):
                        passed.append(True)
                    else:
                        passed.append(False)
                if all(passed):
...
@tag("SWRT_CNC_MOCO_FrontSteerAngleRequestLimitMaxSteerAngleVelocity")
@verifies("1407000")
@testcase_definition(
...
class SWRT_CNC_MOCO_FrontSteerAngleRequestLimitMaxSteerAngleVelocity(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_FrontSteerAngleRequestLimitMaxSteerAngleVelocity,
        *test_bsigs,
        temp_dir=temp_dir,
