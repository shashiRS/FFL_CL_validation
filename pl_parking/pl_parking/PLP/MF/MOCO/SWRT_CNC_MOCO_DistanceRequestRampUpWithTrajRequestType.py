"""moco distance request ramp up with traj request type"""
...
ALIAS = "SWRT_CNC_MOCO_DistanceRequestRampUpWithTrajRequestType"
...
class Step1(TestStep):
    """moco distance request ramp up with traj request type test setup"""
            ...
            if not df_filtered.empty:
                ...
                for _, car_pos_row in df_filtered.iterrows():
                    if (
                        ...
                        or car_pos_row["newSegmentStarted_nu"]
                        or car_pos_row["calculated_standstill"]
                    ):
                        if round(car_pos_row["newDistanceToStopReq_m"], 3) != round(
                            car_pos_row["distanceToStopReq_m"], 3
                        ):
                            passed.append(False)
                        else:
                            passed.append(True)
                ...
                if all(passed):
                    ...
                    test_result = fc.PASS
                    eval_text = f"The component performs the distance request ramp up feature if configured via parameter AP_C_VL_RAMP_UP_DIST_NU ({constants.MoCo.Parameter.AP_C_VL_RAMP_UP_DIST_NU}) and loCtrlRequestType == LOCTRL_BY_TRAJECTORY."
                else:
                    self.result.measured_result = FALSE
                    test_result = fc.FAIL
                    eval_text = f"The component is not performed the distance request ramp up feature if configured via parameter AP_C_VL_RAMP_UP_DIST_NU({constants.MoCo.Parameter.AP_C_VL_RAMP_UP_DIST_NU}) and loCtrlRequestType == LOCTRL_BY_TRAJECTORY."

                eval_0 = " ".join(
                    f"The component shall perform the distance request ramp up feature if configured via "
                    f"parameter AP_C_VL_RAMP_UP_DIST_NU({constants.MoCo.Parameter.AP_C_VL_RAMP_UP_DIST_NU})and loCtrlRequestType == LOCTRL_BY_TRAJECTORY".split()
                )
...
@tag("SWRT_CNC_MOCO_DistanceRequestRampUpWithTrajRequestType")
@verifies("1407074")
@testcase_definition(
    name="MOCO Distance Request Ramp Up With Traj Request Type",
    description="Ensure that if the component shall perform the distance request ramp up feature "
    "if configured via parameter {AP_C_VL_RAMP_UP_DIST_NU} and loCtrlRequestType = LOCTRL_BY_TRAJECTORY.",
    group="RampUp",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class SWRT_CNC_MOCO_DistanceRequestRampUpWithTrajRequestType(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MOCO_DistanceRequestRampUpWithTrajRequestType,
        *test_bsigs,
        temp_dir=temp_dir,
