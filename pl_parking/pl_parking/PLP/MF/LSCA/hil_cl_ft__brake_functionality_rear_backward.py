"""LSCA disable brake functionality in backward mode when trailer is attached"""
...
SIGNAL_DATA = "LSCA_DISABLE_REAR_BRAKE_BACKWARD"
...
@teststep_definition(
    name="LSCA disable brake functionality in backward mode",
    description="In case of a trailer being attached to the ego vehicle, the LSCA function shall disable the brake functionality for the rear side of the ego vehicle (when moving backwards)",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ValidationSignals)
class LscaDisableBrakeBackwardCheck(TestStep):
    """LscaDisableBrakeBackwardCheck Test Step."""
    ...
    def process(self, **kwargs):
        ...
        """Evaluation part"""
        # Check when trailer is attached and driving backward
        ...
        if trailer_attached:
            ...
            if brake_detected:
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['Lsca_brake']} signal is FAILED. LSCA incorrectly activated the brakes while driving backward with the trailer attached.".split()
                )
            else:
                test_result = fc.PASS
                evaluation1 = " ".join(
                    f"The evaluation of {signal_name['Lsca_brake']} signal is PASSED. LSCA did not activate the brakes while driving backward with the trailer attached.".split()
                )
        else:
            evaluation1 = " ".join(
        ...
        signal_summary["LSCA Disable Rear Brake Backward Mode"] = evaluation1

        self.sig_sum = HilClFuntions.hil_convert_dict_to_pandas(signal_summary)
        plot_titles.append("LSCA Trailer Brake Disable Evaluation (Backward Mode)")
        ...
        plot_titles.append("LSCA Brake Activation Failure (Backward Mode)")
        plots.append(fig)
        remarks.append("")
...
@testcase_definition(
    name="LSCA disable brake functionality in backward mode with trailer attached",
    description="When the trailer is attached and the vehicle moves backward, the LSCA should not intervene by activating the rear brakes.",
)
class LscaDisableBrakeBackward(TestCase):
    """LscaDisableBrakeBackward Test Case."""
