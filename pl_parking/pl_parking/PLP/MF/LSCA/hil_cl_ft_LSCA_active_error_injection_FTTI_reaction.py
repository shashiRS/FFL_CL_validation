...
SIGNAL_DATA = "INPUT_FAILURE_LSCA_ACT_SS_FTTI"
...
class System_safe_state_error_injected_LSCA_active_SS(TestStep):
    """Example test step"""
    ...
    def process(self, **kwargs):
        ...
        for frame in range(0, len(time_signal)):
            # Wait until REVERSE gear(LSCA activated) was engaged and there is an obstacle in the rear side of the ego within LSCA activation range while and ego vehicle is approaching.
            if (
                (
                ...
                and (
                    (
                        ...
                        and uss8[frame] < constants.HilCl.PDW.Thresholds.MAX_LSCA_DISTANCE
                    )
                    or (
                        uss9[frame] > constants.HilCl.PDW.Thresholds.MIN_LSCA_DISTANCE
                        and uss9[frame] < constants.HilCl.PDW.Thresholds.MAX_LSCA_DISTANCE
                    )
                )
            ...
            for frame in range(t2_idx, len(time_signal)):
                ...
                if FTTI_T1:
                    ...
                    if time_signal[frame] <= (FTTI_T1 + constants.HilCl.PDW.FTTI.SAFETY_SS):
                        ...
                        if all(eval_check(frame, val_list)):
                            eval_status_ok = True
                            break
                        else:
        ...
        if trigger1 is None:
            preconditions = (
                ...
                "<br>Condition 1 - Gear not switched to REVERSE:"
                ...
                "<br>Condition 2 - No object reported by rear USS within LSCA activation range:"
                f"<br>{signal_name['USS8_dist_to_object']} not between {constants.HilCl.PDW.Thresholds.MIN_LSCA_DISTANCE} - {constants.HilCl.PDW.Thresholds.MAX_LSCA_DISTANCE}"
                f"<br> or {signal_name['USS9_dist_to_object']} not between {constants.HilCl.PDW.Thresholds.MIN_LSCA_DISTANCE} - {constants.HilCl.PDW.Thresholds.MAX_LSCA_DISTANCE}"
                "<br>Condition 3 - Vehicle not moving:"
...
@testcase_definition(
    name="System transition to Safe State mode (within 700 miliseconds) while LSCA was active",
...
class SafeStateHil(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            System_safe_state_error_injected_LSCA_active_SS,
        ]
