...
SIGNAL_DATA = "MF_RECALCULATED_PARKOUT_RELATED_PARKING_BOX_ID"
...
@teststep_definition(
    step_number=1,
    name="ParkOut Recalculated Related Parking Box id",
...
class TaposdParkOutRecalculatedRelatedParkingBoxId(TestStep):
    """defining tetstep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                        ...
                        if (
                            ...
                            if relatedParkingBoxID[0].iloc[idx] == relatedParkingBoxID[0].iloc[idx - 1]:
                                evaluation = " ".join(
                                    ...
                                    "relatedParkingBoxID is PASSED where recalculated target pose with the same associated parking Box ID as the original target pose in Park Out Maneuver".split()
                                )
                                eval_cond = True
                                frame_data_dict[idx] = "Pass"
                            else:
                                evaluation = " ".join(
                                    ...
                                    f"relatedParkingBoxID is FAILED at frame {idx} where recalculated target pose does not have same associated parking Box ID as the original target pose in Park Out Maneuver".split()
                                )
                                eval_cond = False
                                frame_data_dict[idx] = "fail"
                                break
            ...
            expected_val = "AP.targetPosesPort.targetPoses[%].relatedParkingBoxID of recalculated target pose should have same associated parking Box ID as the original target pose in Park Out Maneuver"
...
@verifies("2442503")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_PArkOutRecalculatedRelatedParkingBoxId",
    description="Verify Park Out Recalculated related parking box id",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_R63vADqpEe-sxr1i_55q4Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class TaposdParkOutRecalculatedRelatedParkingBoxIdTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdParkOutRecalculatedRelatedParkingBoxId,
        ]
