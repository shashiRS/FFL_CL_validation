...
SIGNAL_DATA = "MF_RECALCULATED_POSE_ID"
...
class TaposdRecalculatedPoseId(TestStep):
    """defining tetstep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                        ...
                        if (
                            ...
                            if poseID[0].iloc[idx] == poseID[0].iloc[idx - 1]:
                                evaluation = " ".join(
                                    ...
                                    "PoseID is PASSED where a recalculated target pose with the same pose ID as the original target pose".split()
                                )
                                eval_cond = True
                                frame_data_dict[idx] = "Pass"
                            else:
                                evaluation = " ".join(
                                    ...
                                    f"PoseID is FAILED at frame {idx} where a recalculated target pose does not have same pose ID as the original target pose".split()
                                )
                                eval_cond = False
                                frame_data_dict[idx] = "fail"
                                break
            ...
            expected_val = "AP.targetPosesPort.targetPoses[].poseId of recalculated target pose should have same pose ID as the original target pose"
...
@verifies("2436294")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_RecalculatedPoseId",
    description="Verify Recalculated pose id",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI__CtkMDoOEe-sxr1i_55q4Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdRecalculatedPoseIdTC(TestCase):
    """Example test case."""
