...
SIGNAL_DATA = "MF_RECALCULATED_POSE_TYPE"
...
class TaposdRecalculatedPoseType(TestStep):
    """defining tetstep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                        ...
                        if (
                            ...
                            if poseType[0].iloc[idx] == poseType[0].iloc[idx - 1]:
                                evaluation = " ".join(
                                    ...
                                    "PoseType is PASSED where a recalculated target pose of the same pose type as the original target pose".split()
                                )
                                eval_cond = True
                                frame_data_dict[idx] = "Pass"
                            else:
                                evaluation = " ".join(
                                    ...
                                    f"PoseType is FAILED at frame {idx} where a recalculated target pose does not have same pose type as the original target pose".split()
                                )
                                eval_cond = False
                                frame_data_dict[idx] = "fail"
                                break
            ...
            expected_val = "AP.targetPosesPort.targetPoses[%].type of recalculated target pose should have same pose type as the original target pose"
...
@verifies("2436296")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_RecalculatedPosetype",
    description="Verify Recalculated pose type",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_X-rShjoPEe-sxr1i_55q4Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class TaposdRecalculatedPoseTypeTC(TestCase):
    """Example test case."""
