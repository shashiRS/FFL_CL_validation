...
SIGNAL_DATA = "MF_PARALLEL_OUT_TP_TYPE"
...
@teststep_definition(
    ...
    description="For a given parking box  in slot zero of the parking box port of scenario type PARALLEL_PARKING, TAPOSD shall provide 1 target pose of type T_PAR_PARKING_OUT.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TaposdScanOutParallelTargetPoseType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                            ...
                            if num_valid_poses.iloc[idx] > 0:
                                for i in range(int(num_valid_poses.iloc[idx])):
                                    if (
                                        ...
                                        and pose_type[i].iloc[idx] == ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT
                                    ):
                                        pose_count += 1
                                ...
                                if pose_count == ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE:
                                    ...
                                    if not evaluation:
                                        evaluation = " ".join(
                                            f"FAILED: Number of target poses of type T_PAR_PARKING_OUT"
            ...
            expected_val = (
                f"Number of target poses of type T_PAR_PARKING_OUT"
                f"({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT}) for Parking Box 0"
                f" = {ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE}"
            )

            signal_summary[
                "Number of target poses of type T_PAR_PARKING_OUT"
                f"({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT}) <br>"
            ...
            remark = (
                f"Check if Number of target poses of pose type T_PAR_PARKING_OUT({ConstantsTrajpla.PoseType.T_PAR_PARKING_OUT})"
                f" per parking box of type PARALLEL_PARKING"
                f" == {ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE}"
            )
...
@verifies("2440676")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_ScanOutParallelTargetPoseType",
    description="This testcase will verify that for a given parking box  in slot zero of the parking box port of scenario type PARALLEL_PARKING the Component shall provide 1 target pose of type T_PAR_PARKING_OUT.",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdScanOutParallelTargetPoseTypeTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdScanOutParallelTargetPoseType,
        ]
