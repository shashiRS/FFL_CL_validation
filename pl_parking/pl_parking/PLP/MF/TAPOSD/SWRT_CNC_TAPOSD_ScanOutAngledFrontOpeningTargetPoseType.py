...
SIGNAL_DATA = "MF_ANGLED_FRONT_OUT_TP_TYPE"
...
@teststep_definition(
    ...
    description="For a given parking box  in slot zero of the parking box port of scenario type ANGLED_PARKING_OPENING_TOWARDS_FRONT, TAPOSD shall provide 1 target pose of type T_ANGLED_PARKING_REVERSE_OUT, in SCAN OUT mode",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TaposdScanOutAngledFrontOpeningTargetPoseType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                            ...
                            if num_valid_poses.iloc[idx] > 0:
                                for i in range(int(num_valid_poses.iloc[idx])):
                                    if (
                                        ...
                                        == ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE_OUT
                                    ):
                                        pose_count += 1
                                ...
                                if pose_count == ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE:
                                    ...
                                    if not evaluation:
                                        evaluation = " ".join(
                                            f"FAILED: Number of target poses of type T_ANGLED_PARKING_REVERSE_OUT"
            ...
            expected_val = (
                f"Number of target poses of type T_ANGLED_PARKING_REVERSE_OUT"
                f"({ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE_OUT}) for Parking Box 0"
                f" = {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE}"
            )

            signal_summary[
                "Number of target poses of type T_ANGLED_PARKING_REVERSE_OUT"
                f"({ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE_OUT}) <br>"
            ...
            remark = (
                f"Check if Number of target poses of pose type T_ANGLED_PARKING_REVERSE_OUT({ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE_OUT})"
                f" per parking box of type ANGLED_PARKING_OPENING_TOWARDS_FRONT"
                f" == {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE}"
            )
...
@verifies("2442267")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_ScanOutAngledFrontOpeningTargetPoseType",
    description=(
        "The testcase should verify that For a given parking box  in slot zero of the parking box port of scenario type ANGLED_PARKING_OPENING_TOWARDS_FRONT, TAPOSD shall provide 1 target pose of type T_ANGLED_PARKING_REVERSE_OUT, in SCAN OUT mode."
    ),
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdScanOutAngledFrontOpeningTargetPoseTypeTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdScanOutAngledFrontOpeningTargetPoseType,
        ]
