...
SIGNAL_DATA = "MF_ANGLED_FRONT_IN_TP_TYPE"
...
@teststep_definition(
    ...
    description="For a given parking box of scenario type ANGLED_PARKING_OPENING_TOWARDS_FRONT, TAPOSD shall provide 1 target pose of type T_ANGLED_PARKING_REVERSE, in SCAN IN mode",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TaposdScanInAngledFrontOpeningTargetPoseType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                    ...
                    for idx in range(t2, len(ap_state)):
                        if parking_scenario_0.iloc[
                            ...
                            if num_valid_poses.iloc[idx] > 0:
                                for i in range(int(num_valid_poses.iloc[idx])):
                                    if (
                                        ...
                                        and pose_type[i].iloc[idx] == ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE
                                    ):
                                        pose_count += 1
                            else:
                            ...
                            if pose_count == ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE:
                                ...
                                if not evaluation:
                                    evaluation = " ".join(
                                        f"FAILED: Number of target poses of type T_ANGLED_PARKING_REVERSE"
            ...
            expected_val = (
                f"Number of target poses of type T_ANGLED_PARKING_REVERSE"
                f"({ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE}) for Parking Box 0"
                f" = {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE}"
            )

            signal_summary[
                "Number of target poses of type T_ANGLED_PARKING_REVERSE"
                f"({ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE}) <br>"
                f"for parking box 0 of Scenario type ANGLED_PARKING_OPENING_TOWARDS_FRONT"
                f"({ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT})"
            ] = [expected_val, evaluation, verdict]

            remark = (
                f"Check if Number of target poses of pose type T_ANGLED_PARKING_REVERSE({ConstantsTrajpla.PoseType.T_ANGLED_PARKING_REVERSE})"
                f" per parking box of type ANGLED_PARKING_OPENING_TOWARDS_FRONT({ConstantsTrajpla.ScenarioType.ANGLED_PARKING_OPENING_TOWARDS_FRONT})"
                f" == {ConstantsTrajpla.Parameter.ANGLED_PARK_MAX_TARGET_POSE}"
            )
...
@verifies("2435156")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_ScanInAngledFrontOpeningTargetPoseType",
    description=(
        "The testcase should verify that, For a given parking box of scenario type ANGLED_PARKING_OPENING_TOWARDS_FRONT, the Component shall provide 1 target pose of type T_ANGLED_PARKING_REVERSE, in SCAN IN mode."
    ),
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdScanInAngledFrontOpeningTargetPoseTypeTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdScanInAngledFrontOpeningTargetPoseType,
        ]
