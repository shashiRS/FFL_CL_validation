...
SIGNAL_DATA = "MF_PERPENDICULAR_TP_TYPE"
...
@teststep_definition(
    ...
    description="For a given parking box of scenario type PERPENDICULAR_PARKING, TAPOSD shall provide 2 target poses: 1 target pose of type T_PERP_PARKING_FWD and 1 target pose of type T_PERP_PARKING_BWD, in SCAN IN mode",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TaposdScanInPerpendicularTargetPoseType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
                ...
                if par_pose is not None:
                    for idx in range(t2, len(ap_state)):
                        if parking_scenario_0.iloc[idx] == ConstantsTrajpla.ScenarioType.PERPENDICULAR_PARKING and (
                            ...
                            pose_count = 0
                            perp_fwd_pose_count = 0
                            perp_bwd_pose_count = 0
                            if num_valid_poses.iloc[idx] > 0:
                                for i in range(int(num_valid_poses.iloc[idx])):
                                    if related_parking_box_id[i].iloc[idx] == parking_box_id_0.iloc[idx]:
                                        pose_count += 1
                                        if pose_type[i].iloc[idx] == ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD:
                                            perp_fwd_pose_count += 1
                                        if pose_type[i].iloc[idx] == ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD:
                                            perp_bwd_pose_count += 1
                            else:
                            ...
                            if (
                                ...
                                and perp_fwd_pose_count == ConstantsTrajpla.Parameter.PERP_PARK_FWD_MAX_TARGET_POSE
                                and perp_bwd_pose_count == ConstantsTrajpla.Parameter.PERP_PARK_BWD_MAX_TARGET_POSE
                            ):
                                frame_test_result.append(True)
                                frame_test_status.append(
                                    f"Pass: Target Poses Count for Parking Box 0 = {pose_count}<br> "
                                    f"T_PERP_PARKING_FWD Target Poses Count: {perp_fwd_pose_count}<br>"
                                    f"T_PERP_PARKING_BWD Target Poses Count: {perp_bwd_pose_count}"
                                )
                            else:
                                if not evaluation:
                                    evaluation = " ".join(
                                        f"FAILED: Number of target poses for Parking Box 0 = {pose_count}<br>"
                                        f"Number of target poses of type T_PERP_PARKING_FWD for Parking Box 0 = {perp_fwd_pose_count}<br>"
                                        f"Number of target poses of type T_PERP_PARKING_BWD for Parking Box 0 = {perp_bwd_pose_count}<br>"
                                        f" at frame {idx}".split()
                                    )
                                frame_test_result.append(False)
                                frame_test_status.append(
                                    f"Fail: Target Poses Count = {pose_count}<br>"
                                    f"T_PERP_PARKING_FWD Target Poses Count: {perp_fwd_pose_count}<br>"
                                    f"T_PERP_PARKING_BWD Target Poses Count: {perp_bwd_pose_count}"
                                )
                        else:
            ...
            expected_val = (
                f"Number of target poses for Parking Box 0 = {ConstantsTrajpla.Parameter.PERP_PARK_MAX_TARGET_POSE}<br>"
                f"Number of target poses of type T_PERP_PARKING_FWD for Parking Box 0 = {ConstantsTrajpla.Parameter.PERP_PARK_FWD_MAX_TARGET_POSE}<br>"
                f"Number of target poses of type T_PERP_PARKING_BWD for Parking Box 0 = {ConstantsTrajpla.Parameter.PERP_PARK_BWD_MAX_TARGET_POSE}"
            )

            signal_summary[
                "Number of target poses of type T_PERP_PARKING_FWD"
                f"({ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD})  and T_PERP_PARKING_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD})<br>"
            ...
            remark = (
                ...
                f"check if {ConstantsTrajpla.Parameter.PERP_PARK_FWD_MAX_TARGET_POSE} target pose of "
                f"pose type T_PERP_PARKING_FWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_FWD}) and "
                f"{ConstantsTrajpla.Parameter.PERP_PARK_BWD_MAX_TARGET_POSE} target pose of "
                f"pose type T_PERP_PARKING_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_BWD})"
                f"are available"
            )
...
@verifies("2435153")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_ScanInPerpendicularTargetPoseType",
    description=(
        "This testcase will verify that For a given parking box of scenario type PERPENDICULAR_PARKING, TAPOSD provides 2 target poses: 1 target pose of type T_PERP_PARKING_FWD and 1 target pose of type T_PERP_PARKING_BWD, in SCAN IN mode"
    ),
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdScanInPerpendicularTargetPoseTypeTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdScanInPerpendicularTargetPoseType,
        ]
