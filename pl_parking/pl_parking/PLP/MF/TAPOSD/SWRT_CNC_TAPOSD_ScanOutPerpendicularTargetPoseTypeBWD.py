...
SIGNAL_DATA = "MF_PERPENDICULAR_BWD_TP_TYPE"
...
@teststep_definition(
    ...
    description="For a given parking box  in slot zero of the parking box port WHICH HAS "
    "scenario type PERPENDICULAR_PARKING WHERE the yaw angle of the current ego vehicle is "
    "within a range of +-90deg relative to the road-to-curb direction of the associated parking box (parked in forward), "
    "THERE TAPOSD shall provide 2 target poses of the type T_PERP_PARKING_OUT_BWD with different target sides, in SCANOUT mode",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TaposdScanOutParpendicularBwdTargetPoseType(TestStep):
    """Defining teststep"""
        ...
        try:
            ...
            if t2 is not None:
                            ...
                            if num_valid_poses.iloc[idx] > 0:
                                for i in range(int(num_valid_poses.iloc[idx])):
                                    if (
                                        ...
                                        and pose_type[i].iloc[idx] == ConstantsTrajpla.PoseType.T_PERP_PARKING_OUT_BWD
                                    ):
                                        pose_count += 1
                                ...
                                if (
                                    ...
                                    if not evaluation:
                                        evaluation = " ".join(
                                            f"FAILED: Number of target poses of type T_PERP_PARKING_OUT_BWD for Parking Box 0 = {pose_count}<br>"
                                            f"Number of T_PERP_PARKING_OUT_BWD poses with target side TS_RIGHT_SIDE = {rgt_side_pose_count}<br>"
                                            f"Number of T_PERP_PARKING_OUT_BWD poses with target side TS_LEFT_SIDE = {lft_side_pose_count}<br>"
                                            f" at frame {idx}".split()
                                        )
                                    frame_test_result.append(False)
                                    frame_test_status.append(
                                        f"Fail: Target Poses T_PERP_PARKING_OUT_BWD Count  = {pose_count}<br>"
                                        f"T_PERP_PARKING_OUT_BWD Target Poses with TS_RIGHT_SIDE Count: {rgt_side_pose_count}<br>"
                                        f"T_PERP_PARKING_OUT_BWD Target Poses with TS_LEFT_SIDE Count: {lft_side_pose_count}"
                                    )
                            else:
                ...
                evaluation = " ".join(
                    ...
                    f"== AP_SCAN_OUT({ConstantsTrajpla.ApStates.AP_SCAN_OUT}) was nev]er found.".split()
                )
            ...
            expected_val = (
                f"Number of target poses type T_PERP_PARKING_OUT_BWD for Parking Box 0 = {ConstantsTrajpla.Parameter.PERP_PARK_MAX_TARGET_POSE}<br>"
                f"Number of T_PERP_PARKING_OUT_BWD target poses of Side TS_RIGHT_SIDE = 1<br>"
                f"Number of T_PERP_PARKING_OUT_BWD target poses of Side TS_LEFT_SIDE = 1"
            )

            signal_summary[
                "Number of target poses of type T_PERP_PARKING_OUT_BWD"
                f"({ConstantsTrajpla.PoseType.T_PERP_PARKING_OUT_BWD}) <br>"
            ...
            remark = (
                ...
                f"where the vehicle is parked in forward, "
                f"check if {ConstantsTrajpla.Parameter.PERP_PARK_MAX_TARGET_POSE} target poses of "
                f"pose type T_PERP_PARKING_OUT_BWD({ConstantsTrajpla.PoseType.T_PERP_PARKING_OUT_FWD}), "
...
@verifies("2442253")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_ScanOutParpendicularBwdTargetPoseType",
    description=(
        "This testcase will verify that For a given parking box  in slot zero of the parking box port WHICH HAS "
        "scenario type PERPENDICULAR_PARKING WHERE the yaw angle of the current ego vehicle is "
        "within a range of +-90deg relative to the road-to-curb direction of the associated parking box (parked in forward), "
        "THERE TAPOSD shall provide 2 target poses of the type T_PERP_PARKING_OUT_BWD with different target sides, in SCAN OUT mode"
    ),
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdScanOutParpendicularBwdTargetPoseTypeTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdScanOutParpendicularBwdTargetPoseType,
        ]
