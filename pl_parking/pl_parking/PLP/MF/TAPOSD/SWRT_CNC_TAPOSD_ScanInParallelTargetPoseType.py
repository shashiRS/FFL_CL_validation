...
SWRT_CNC_TAPOSD_ScanInParallelTargetPoseType
Script assumes that parking box 0 has parkingScenario of parallel parking
"""
...
SIGNAL_DATA = "MF_PARALLEL_IN_TP_TYPE"
...
@teststep_definition(
    step_number=1,
    name="Parallel Park In Target pose type",
    description="For a given parking box of scenario type PARALLEL_PARKING, TAPOSD shall provide 1 target pose "
    "of type T_PARALLEL_PARKING, in SCAN IN mode.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MfSignals)
class TaposdScanInParallelTargetPoseType(TestStep):
    """Defining teststep"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            for idx, val in enumerate(parking_scenario_0):
                if val == ConstantsTrajpla.ScenarioType.PARALLEL_PARKING:
                    par_pose = idx
                    break
            ...
            if t2 is not None:
                if par_pose is not None:
                    for idx in range(t2, len(ap_state)):
                        if parking_scenario_0.iloc[idx] == ConstantsTrajpla.ScenarioType.PARALLEL_PARKING and (
                            ...
                            if num_valid_poses.iloc[idx] > 0:
                                for i in range(int(num_valid_poses.iloc[idx])):
                                    if (
                                        ...
                                        and pose_type[i].iloc[idx] == ConstantsTrajpla.PoseType.T_PARALLEL_PARKING
                                    ):
                                        pose_count += 1
                            else:
                            ...
                            if pose_count == ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE:
                                frame_test_result.append(True)
                                frame_test_status.append(f"Pass: Target Poses Count for Parking Box 0 = {pose_count}")
                            else:
                                if not evaluation:
                                    evaluation = " ".join(
                                        f"FAILED: Number of target poses of type T_PARALLEL_PARKING"
                    ...
                    evaluation = " ".join(
                        ...
                        f"{signal_name['parkingScenario_nu0']} was never equal to PARALLEL_PARKING"
                        f"({ConstantsTrajpla.ScenarioType.PARALLEL_PARKING})".split()
                    )
            else:
                frame_test_result.append(None)
                evaluation = " ".join(
            ...
            expected_val = (
                f"Number of target poses of type T_PARALLEL_PARKING"
                f"({ConstantsTrajpla.PoseType.T_PARALLEL_PARKING}) for Parking Box 0"
                f" = {ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE}"
            )

            signal_summary[
                "Number of target poses of type T_PARALLEL_PARKING"
                f"({ConstantsTrajpla.PoseType.T_PARALLEL_PARKING}) <br>"
                f"for parking box 0 of Scenario type PARALLEL_PARKING"
                f"({ConstantsTrajpla.ScenarioType.PARALLEL_PARKING})"
            ] = [expected_val, evaluation, verdict]

            remark = (
                f"Check if Number of target poses of pose type T_PARALLEL_PARKING({ConstantsTrajpla.PoseType.T_PARALLEL_PARKING})"
                f" per parking box of type PARALLEL_PARKING({ConstantsTrajpla.ScenarioType.PARALLEL_PARKING})"
                f" == {ConstantsTrajpla.Parameter.PARALLEL_PARK_MAX_TARGET_POSE}"
            )
...
@verifies("2435149")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_ScanInParallelTargetPoseType",
    description="This testcase will verify that for a given parking box of scenario type PARALLEL_PARKING, TAPOSD shall provide 1 target pose of type T_PARALLEL_PARKING, in SCAN IN mode.",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class TaposdScanInParallelTargetPoseTypeTC(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [
            TaposdScanInParallelTargetPoseType,
        ]
