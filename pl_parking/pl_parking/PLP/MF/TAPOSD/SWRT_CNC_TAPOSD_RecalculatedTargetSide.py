...
SIGNAL_DATA = "MF_RECALCULATED_TARGET_SIDE"
...
@teststep_definition(
    step_number=1,
    name="Recalculated Target side",
    description="This testcase will verify That the Component shall provide a recalculated target pose with the same target side <targetPosesPort.targetPoses.targetSide> as"
...
class TaposdRecalculatedTargetSide(TestStep):
    """defining tetstep"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if t2 is not None:
                if (
                    num_valid_poses.iloc[idx - 1] > 0
                    and ap_state.iloc[idx - 1] == ConstantsTrajpla.ApStates.AP_SCAN_IN
                        ...
                        if (
                            ...
                            if target_side[0].iloc[idx] == target_side[0].iloc[idx - 1]:
                                evaluation = " ".join(
                                    ...
                                    "TargetSide is PASSED where a recalculated target pose has the same target side as the original target pose".split()
                                )
                                eval_cond = True
                                frame_data_dict[idx] = "Pass"
                            else:
                                evaluation = " ".join(
                                    ...
                                    f"TargetSide is FAILED at frame {idx} where a recalculated target pose does not have same target side as the original target pose".split()
                                )
                                eval_cond = False
                                frame_data_dict[idx] = "fail"
                                break
            ...
            expected_val = "AP.targetPosesPort.targetPoses[%].type of recalculated target pose should have the same target side as the original target pose"
...
@verifies("2436297")
@testcase_definition(
    name="SWRT_CNC_TAPOSD_RecalculatedTargetSide",
    description="Verify Recalculated target side",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_cIoKcDoPEe-sxr1i_55q4Q&oslc_config.context=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F36324&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_p9KzlDK_Ee6mrdm2_agUYg",
)
@register_inputs("/Playground_2/TSF-Debug")
class TaposdRecalculatedTargetSideTC(TestCase):
    """Example test case."""
