...
@teststep_definition(
    step_number=1,
    name="PMSD AL Signal State",
    description="PMSD shall only process input signals if their signal state is AL_SIG_STATE_OK",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, CemSignals)
class TestStepFtPMSDSigStateOk(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for column_name in pmsd_input_data:
            for esig_value in pmsd_input_data[column_name]:
                if esig_value == ConstantsCem.AL_SIG_STATE_OK:
                    for column_name in output_df:
                        if (
                            output_df[column_name]
                            .isin([ConstantsCem.AL_SIG_STATE_INIT, ConstantsCem.AL_SIG_STATE_OK])
                            .all()
                        ):
                            result_list.append(1)
                        else:
                            result_list.append(0)
                            fail_dict[column_name] = "Invalid"

        if fail_dict:
            evaluation = (
                "PMSD output signal state is Invalid even if the input signal state is Valid for these " "signals ",
                fail_dict.keys(),
            )
        else:
            evaluation = "PMSD only processes input signals if their signal state is AL_SIG_STATE_OK."
        ...
        signal_summary = pd.DataFrame(
            {
                "Evaluation": {
                    "1": "PMSD shall only process input signals if their signal state is AL_SIG_STATE_OK.",
                },
                "Result": {
                    "1": evaluation,
                },
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1987894"],
            fc.TESTCASE_ID: ["86634"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: [
                "This test case checks that PMSD shall only process input signals if their signal state "
                "is AL_SIG_STATE_OK"
            ],
...
@verifies("1987894")
@testcase_definition(
    name="SWRT_CNC_PMSD_SigStateOk",
    description="This test case checks that PMSD shall only process input signals if their signal state "
    "is AL_SIG_STATE_OK",
    doors_url="",
)
@register_inputs("/parking")
# @register_inputs("/parking")
class FtPMSDSigStateOk(TestCase):
    """Signal state ok test case."""

    @property
    def test_steps(self):
        """Define the test steps."""
        return [TestStepFtPMSDSigStateOk]
