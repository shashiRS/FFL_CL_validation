#!/usr/bin/env python3
"""Defining  signal status control output in Reset operational mode cases"""
import logging
import os
import tempfile
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_ResetModeControlOutputSignalState",
    description="This test step confirms that the component signals status on its control output when component is "
    "triggered in reset operation mode and initialization is successful.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ft.PCESignals)
class PCEResetControlOutputSignalStateTestStep(TestStep):
    ...
    def process(self):
        ...
        eval_0 = "Check if initialization is successful on control outputs"
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1407385", "1407384"],
            fc.TESTCASE_ID: ["55841"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: [
                "Ensures that the component signals status on its control output when component is triggered in "
                "reset operation mode and initialization is successful."
            ],
...
@verifies("L3_PCE_1407385, L3_PCE_1407384")
@testcase_definition(
    name="SWRT_CNC_PCE_ResetModeControlOutputSignalState",
    description="This test ensures that the component signals status on its control output when component is triggered"
    "in reset operation mode and initialization is successful.",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class PCEResetModeControlOutputSignalState(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCEResetControlOutputSignalStateTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    # test_bsigs = r"D:\csv's\2024.08.29_at_13.44.23_camera-mi_13_meascounter_timestamp_compstate.bsig"
    debug(
        PCEResetModeControlOutputSignalState,
        test_bsigs,
        temp_dir=temp_dir,
