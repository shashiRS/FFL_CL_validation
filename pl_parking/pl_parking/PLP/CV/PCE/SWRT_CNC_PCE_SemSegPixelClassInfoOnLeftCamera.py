...
SIGNAL_DATA = "SEMSEG_LEFT_SIGNALS"
...
class PCESEMSEGSignals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        SEMSEG_IMAGEDATA_LSC = "SemSeg_imageData_lsc"
        SEMSEG_IMAGEDATAOFFSET_LSC = "SemSeg_imageDataOffset_lsc"
    ...
    def get_properties(self):
        ...
        signal_dict = {
            ...
            self.Columns.SEMSEG_IMAGEDATA_LSC: ".GRAPPA_LSC_IMAGE.GrappaSemSegLscImage.imageData",
            self.Columns.SEMSEG_IMAGEDATAOFFSET_LSC: ".GRAPPA_LSC_IMAGE.GrappaSemSegLscImage.imageDataOffset",
        }
        return signal_dict
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_SEMSEG_PIXELCLASSINFO_ONLEFTCAM",
    description="This test step confirms that the component provides the semseg class information",
    expected_result=BooleanResult(TRUE),
)
class PCESemsegClassInfoOnLeftCameraTestStep(TestStep):
    ...
    def process(self):
        ...
        signal_check = []
        signal_check.append("semseg_sig_status_lsc")
        signal_check.append("Base_ctrl_opmode_lsc")
        ...
        if signal_availability:
            ...
            evaluation_desc = "LSC Semseg pixel classes"
            # Filters df for 'semseg_sig_status == AL_SIG_STATE_OK' and 'Base_ctrl_opmode is == GS_BASE_OM_RUN'
            df_filtered_check = df[
                (df["semseg_sig_status_lsc"] == SigStatus.AL_SIG_STATE_OK)
                & (df["Base_ctrl_opmode_lsc"] == BaseCtrl.GS_BASE_OM_RUN)
            ]
            df_filtered = df_filtered_check.drop_duplicates(subset=["semseg_timestamp_lsc"], keep="last")
            # Hold the timestamps where the semseg class information is not in expected range
            if not df_filtered.empty:
                # if 'semseg_sig_status == AL_SIG_STATE_OK' and 'Base_ctrl_opmode is == GS_BASE_OM_RUN'
                for _, row in df_filtered.iterrows():
                    pce = PCESemsegClassInfoOnLeftCameraTestStep()
                    # get the semseg image data offset in every frame
                    self.idx = int(row["SemSeg_imageDataOffset_lsc"])
                    ...
                    Semseg_Imagedata = list(
                        map(lambda x: pce.get_semseg_imagedata(row=row, camera="lsc"), range(self.idx, maximum_index))
                    )
                ...
                if all(list_passed):
                    ...
                    evaluation_result = "valid class is provided for all pixels on LSC semseg data output"
                    test_result = fc.PASS
                    ...
                    evaluation_result = (
                        "valid class information is not provided for any one pixel on LSC semseg data output"
                    )
            ...
            df_filtered_check = df[["mts_ts", "semseg_timestamp_lsc", "semseg_sig_status_lsc", "Base_ctrl_opmode_lsc"]]
            df_plot = df_filtered_check[
                (df_filtered_check["semseg_sig_status_lsc"] == SigStatus.AL_SIG_STATE_OK)
                & (df_filtered_check["Base_ctrl_opmode_lsc"] == BaseCtrl.GS_BASE_OM_RUN)
            ]
            df_plot = df_plot.drop_duplicates(subset=["semseg_timestamp_lsc"], keep="last")
            fig = go.Figure()
            # Semseg status
            fig.add_trace(
                go.Scatter(
                    x=df_plot["semseg_timestamp_lsc"],
                    y=df_plot["semseg_sig_status_lsc"],
                    mode="lines",
                    name="semseg_sig_status_lsc",
                )
            )
            # Base control opmode
            fig.add_trace(
                go.Scatter(
                    x=df_plot["semseg_timestamp_lsc"],
                    y=df_plot["Base_ctrl_opmode_lsc"],
                    mode="lines",
                    name="Base_ctrl_opmode_lsc",
                )
            )
...
class PCESemsegPixelClassInfoOnLeftCamera(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCESemsegClassInfoOnLeftCameraTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    # test_bsigs = r"D:\csv's\2024.12.04_at_14.23.36_camera-mi_9056_R7_SemSeg.bsig"
    test_bsigs = r"D:\csv's\2024.12.04_at_14.11.29_camera-mi_9056_R7_SemSeg.bsig"
    debug(
        PCESemsegPixelClassInfoOnLeftCamera,
        test_bsigs,
        temp_dir=temp_dir,
