...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_ParkingSlotOrientationConfidence",
...
class PCEParkingSlotOrientationConfidenceTestStep(TestStep):
                ...
                if poly_detections[cam] is True:
                    ...
                    if availability_of_associated_detections[cam] is True:
                        ...
                        if validity_of_keypoints and not df_filtered.empty:
                            ...
                            valid_orientation_confidence, invalid_orientation_conf_timestamps = (
                                ft.check_parking_slot_orientation_confidence(
                                    df_filtered, cam, ef, dict_obj_keypoints[cam]
                                )
                            )
                            if valid_orientation_confidence:  # if subclass confidence is valid
                                ...
                                evaluation_result[cam] = (
                                    f"Most Confident subclass is not provided at timestamps: {invalid_orientation_conf_timestamps}"
                                )
...
class PCEParkingSlotOrientationConfidence(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCEParkingSlotOrientationConfidenceTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        PCEParkingSlotOrientationConfidence,
        test_bsigs,
        temp_dir=temp_dir,
