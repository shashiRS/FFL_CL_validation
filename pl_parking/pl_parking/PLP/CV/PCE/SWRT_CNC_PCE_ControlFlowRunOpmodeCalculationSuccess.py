#!/usr/bin/env python3
"""Defining  signal status control output in run operational mode cases"""
import logging
import os
import tempfile
...
from pl_parking.PLP.CV.PCE.constants import BaseCtrl, CompCtrl, SigStatus
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_RUNMODECALCULATIONSUCCESS",
    description="This test step confirms that the component signals success on its control output when component is"
    " triggered in run operation mode and calculation is done successfully.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, ft.PCESignals)
class PCERunModeCalculationSuccessTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera in camera_list:
            ...
            evaluation_desc[camera] = f"{camera.swapcase()} Calculation status in run opmode"
            # list the signals needed for test
            dict_signal_check.append(f"algo_sigstate_{camera}")
            dict_signal_check.append(f"algo_compstate_{camera}")
        ...
        eval_0 = "PCE calculation in run opmode."
        ...
        if signal_availability:
            ...
            for camera in camera_list:
                ...
                if not df_filtered.empty:
                    # Check calculation status of FC
                    if (df_filtered[f"algo_sigstate_{camera}"] == SigStatus.AL_SIG_STATE_OK).all() and (
                        df_filtered[f"algo_compstate_{camera}"] == CompCtrl.GS_COMP_STATE_SUCCESS
                    ).all():
                        # Set camera result to True when Calculation is successful
                        dict_camera_result[camera] = True
                        evaluation_result[camera] = f"{camera.swapcase()} PCE Calculation is successful in run opmode"
                        verdict_result[camera] = "passed"
                    else:
                        # Set fc_result to False when Calculation is not successful
                        dict_camera_result[camera] = False
                        evaluation_result[camera] = (
                            f"{camera.swapcase()} PCE Calculation is not successful in run opmode"
                        )
            ...
            if all(ele is True for ele in dict_camera_result.values()):
                # perform following when Calculation is successful for all the camera instances
                # sets test to Pass when signal status on control output is OK
                test_result = fc.PASS
                self.result.measured_result = TRUE
            elif any(ele is False for ele in dict_camera_result.values()):
                # perform following when Calculation is not successful for any of the camera instances
                # sets test to Fail when signal status on control output is not OK
                test_result = fc.FAIL
            ...
            for cam in camera_list:
                pce_data = df[
                    [
                        f"algo_sigstate_{cam}",
                        f"algo_compstate_{cam}",
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1407393", "1407389"],
            fc.TESTCASE_ID: ["55846"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: [
                "Ensures that the component signals success on its control output when component is "
                "triggered in run operation mode and calculation is done successfully.",
            ],
...
@verifies("L3_PCE_1407393, L3_PCE_1407389")
@testcase_definition(
    name="SWRT_CNC_PCE_ControlFlowRunOpmodeCalculationsuccess",
    description="This test ensures that the component signals success on its control output when component is"
    " triggered in run operation mode and calculation is done successfully.",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class PCERunModeCalculationSuccess(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCERunModeCalculationSuccessTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    """
    """Need to update bsig"""
    # test_bsigs = r"V:\Grappa_bsig's\2024.08.29_at_13.44.23_camera-mi_13_meascounter_timestamp_compstate.bsig"
    test_bsigs = r"V:\Grappa_bsig's\2024.09.26_at_11.18.23_camera-mi_020.bsig"
    debug(
        PCERunModeCalculationSuccess,
        test_bsigs,
        temp_dir=temp_dir,
