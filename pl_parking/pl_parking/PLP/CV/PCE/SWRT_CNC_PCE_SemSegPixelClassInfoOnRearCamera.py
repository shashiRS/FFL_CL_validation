...
SIGNAL_DATA = "SEMSEG_REAR_SIGNALS"
...
class PCESEMSEGSignals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        SEMSEG_IMAGEDATA_RC = "SemSeg_imageData_rc"
        SEMSEG_IMAGEDATAOFFSET_RC = "SemSeg_imageDataOffset_rc"
    ...
    def get_properties(self):
        ...
        signal_dict = {
            ...
            self.Columns.SEMSEG_IMAGEDATA_RC: ".GRAPPA_RC_IMAGE.GrappaSemSegRcImage.imageData",
            self.Columns.SEMSEG_IMAGEDATAOFFSET_RC: ".GRAPPA_RC_IMAGE.GrappaSemSegRcImage.imageDataOffset",
        }
        return signal_dict
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_SEMSEG_PIXELCLASSINFO_ONREARCAM",
    description="This test step confirms that the component provides the semseg class information",
    expected_result=BooleanResult(TRUE),
)
class PCESemsegClassInfoOnRearCameraTestStep(TestStep):
    ...
    def process(self):
        ...
        signal_check = []
        signal_check.append("semseg_sig_status_rc")
        signal_check.append("Base_ctrl_opmode_rc")
        ...
        if signal_availability:
            ...
            evaluation_desc = "RC Semseg pixel classes"
            # Filters df for 'semseg_sig_status == AL_SIG_STATE_OK' and 'Base_ctrl_opmode is == GS_BASE_OM_RUN'
            df_filtered_check = df[
                (df["semseg_sig_status_rc"] == SigStatus.AL_SIG_STATE_OK)
                & (df["Base_ctrl_opmode_rc"] == BaseCtrl.GS_BASE_OM_RUN)
            ]
            df_filtered = df_filtered_check.drop_duplicates(subset=["semseg_timestamp_rc"], keep="last")
            if not df_filtered.empty:
                # if 'semseg_sig_status == AL_SIG_STATE_OK' and 'Base_ctrl_opmode is == GS_BASE_OM_RUN'
                for _, row in df_filtered.iterrows():
                    pce = PCESemsegClassInfoOnRearCameraTestStep()
                    # get the semseg image data offset for every row
                    self.idx = int(row["SemSeg_imageDataOffset_rc"])
                    ...
                    Semseg_Imagedata = list(
                        map(lambda x: pce.get_semseg_imagedata(row=row, camera="rc"), range(self.idx, maximum_index))
                    )
                ...
                if all(list_passed):
                    ...
                    evaluation_result = "valid class is provided for all pixels on RC semseg data output"
                    test_result = fc.PASS
                    ...
                    evaluation_result = (
                        "valid class information is not provided for any one pixel on RC semseg data output"
                    )
            ...
            df_filtered_check = df[["mts_ts", "semseg_timestamp_rc", "semseg_sig_status_rc", "Base_ctrl_opmode_rc"]]
            df_plot = df_filtered_check[
                (df_filtered_check["semseg_sig_status_rc"] == SigStatus.AL_SIG_STATE_OK)
                & (df_filtered_check["Base_ctrl_opmode_rc"] == BaseCtrl.GS_BASE_OM_RUN)
            ]
            df_plot = df_plot.drop_duplicates(subset=["semseg_timestamp_rc"], keep="last")
            fig = go.Figure()
            # Semseg status
            fig.add_trace(
                go.Scatter(
                    x=df_plot["semseg_timestamp_rc"],
                    y=df_plot["semseg_sig_status_rc"],
                    mode="lines",
                    name="semseg_sig_status_rc",
                )
            )
            # Base control opmode
            fig.add_trace(
                go.Scatter(
                    x=df_plot["semseg_timestamp_rc"],
                    y=df_plot["Base_ctrl_opmode_rc"],
                    mode="lines",
                    name="Base_ctrl_opmode_rc",
                )
            )
...
class PCESemsegPixelClassInfoOnRearCamera(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCESemsegClassInfoOnRearCameraTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        PCESemsegPixelClassInfoOnRearCamera,
        test_bsigs,
        temp_dir=temp_dir,
