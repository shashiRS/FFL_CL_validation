...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_IntrinsicImages",
    description="This test step confirms that the same data received from chips data is provided on OD data output.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PCESignals)
class PCEImageIntrinsicOnODDataOutputTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera in camera_list:
            ...
            dict_signal_check.append(f"Detection_Results_FOCALX_{camera}")
            dict_signal_check.append(f"Detection_Results_FOCALY_{camera}")
            dict_signal_check.append(f"Detection_Results_CenterX_{camera}")
            dict_signal_check.append(f"Detection_Results_CenterY_{camera}")
            dict_signal_check.append(f"Detection_Results_CameraType_{camera}")
        ...
        eval_0 = "Image intrinsic parameters of detection results and chips are same."
        ...
        if signal_availability:
            ...
            for camera in camera_list:
                ...
                # checks for each camera instance
                if not df_filtered.empty:
                    # compares detection result image intrinsics with CHIPS image intrinsics
                    if (
                        (
                            df_filtered[f"Detection_Results_FOCALX_{camera}"] == df_filtered[f"Chipsy_focalx_{camera}"]
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_FOCALX_{camera}"]
                            == df_filtered[f"Chipsuv_focalx_{camera}"] * 2
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_FOCALY_{camera}"] == df_filtered[f"Chipsy_focaly_{camera}"]
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_FOCALY_{camera}"]
                            == df_filtered[f"Chipsuv_focaly_{camera}"] * 2
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_CenterX_{camera}"]
                            == df_filtered[f"Chipsy_Centerx_{camera}"]
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_CenterX_{camera}"]
                            == df_filtered[f"Chipsuv_Centerx_{camera}"] * 2
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_CenterY_{camera}"]
                            == df_filtered[f"Chipsy_Centery_{camera}"]
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_CenterY_{camera}"]
                            == df_filtered[f"Chipsuv_Centery_{camera}"] * 2
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_CameraType_{camera}"]
                            == df_filtered[f"Chipsy_CameraType_{camera}"]
                        ).all()
                        and (
                            df_filtered[f"Detection_Results_CameraType_{camera}"]
                            == df_filtered[f"Chipsuv_CameraType_{camera}"]
                        ).all()
                    ):
                        # perform following when image intrinsics of detection results == Image intrinsic parameters
                        # of CHIPS
                        dict_camera_result[camera] = True
                        evaluation_result[camera] = (
                            f"Image intrinsic parameters of {camera.swapcase()} Detection "
                            f"Results == Image intrinsic parameters of {camera.swapcase()} CHIPS."
                        )
                        verdict_result[camera] = "passed"
                    else:
                        # perform following when image intrinsics of LSC detection results != Image intrinsic parameters
                        # of LSC CHIPS
                        dict_camera_result[camera] = False
                        evaluation_result[camera] = (
                            f"Image intrinsic parameters of {camera.swapcase()} "
                            f"Detection Results != Image intrinsic parameters of {camera.swapcase()} CHIPS."
                        )
            ...
            if all(ele is True for ele in dict_camera_result.values()):
                # sets test to Pass when image intrinsics of CHIPS and detection results from all camera instances
                # is same
                test_result = fc.PASS
                self.result.measured_result = TRUE
            elif any(ele is False for ele in dict_camera_result.values()):
                # sets test to Pass when image intrinsics of CHIPS and detection results from any of camera instances
                # is not same
                test_result = fc.FAIL
            ...
            for camera in camera_list:
                fig = go.Figure()
                pce_data = ft.signals_to_be_plotted(df, camera, "detection_results")
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1893842"],
            fc.TESTCASE_ID: ["38833"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: ["Ensures that the component sends the CHIPS intrinsic image data on OD data output."],
...
@verifies("L3_PCE_1893842")
@testcase_definition(
    name="SWRT_CNC_PCE_ObjectDetctionImageIntrinsicFwd",
    description="This test ensures that the component sends the CHIPS intrinsic image data on OD data output.",
)
@register_inputs("/parking")
# @register_inputs("/TSF_DEBUG/")
class PCEImageIntrinsicOnODDataOutput(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCEImageIntrinsicOnODDataOutputTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    """
    # test_bsigs = r"V:\Grappa_bsig's\Image_Intrinsics\2024.09.24_at_06.39.38_camera-mi_13.bsig"
    test_bsigs = r"D:\csv's\2024.09.26_at_11.14.23_camera-mi_019.bsig"
    debug(
        PCEImageIntrinsicOnODDataOutput,
        test_bsigs,
        temp_dir=temp_dir,
