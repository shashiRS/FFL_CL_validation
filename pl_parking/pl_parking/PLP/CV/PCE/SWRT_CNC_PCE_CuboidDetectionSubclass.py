#!/usr/bin/env python3
"""Defining  cuboid Subclass cases"""
import logging
import os
import sys
import tempfile
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_CuboidSubClass",
    description="This test step confirms that the component provides the most confident cuboid subclass",
...
class PCECuboidSubclassTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera in check_cameras:
            ...
            signal_check.append(f"num_cuboid_detections_{camera}")
            signal_check.append(f"detection_results_timestamp_{camera}")
            for det in range(0, 64):
                for conf in range(0, 3):
                    signal_check.append((f"cuboid_detections_subclass_confidence_of_{camera}_det_{det}", conf))
                for keypoint in range(0, 8):  # cuboid detections have 8 keypoints
                    signal_check.append(f"cuboid_detections_key_points_x_of_{camera}_det_{det}_keypoint_{keypoint}")
                    signal_check.append(f"cuboid_detections_key_points_y_of_{camera}_det_{det}_keypoint_{keypoint}")
                signal_check.append(f"cuboid_detections_subclass_id_of_{camera}_det_{det}")
        ...
        evaluation_desc = {}
        # Holds the availability of cuboid detections in each camera
        cuboid_detections = {}
        ...
        for camera in check_cameras:
            ...
            evaluation_desc[camera] = f"Subclass of Cuboid Detections in {camera.swapcase()}"
            # get labeled data for all the frames
            gf.get_data_from_label(data=data, camera=camera, class_type="Cuboid Detections")
            # get simulation/bsig data for all the frames
            sf.get_data_from_bsig(df=df, camera=camera, class_type="cuboid_detections")
            # Check if there are cuboid detections available in both simulation and labeled data
            if bool(gf.gt_detections_timestamps[camera]) and bool(sf.bsig_detections_timestamps[camera]):
                ...
                cuboid_detections[camera] = True
                # get the valid timestamps and detections where there are associated detections found with labeled data
                ef.get_valid_timestamp_and_detections(gf, sf, camera, "Cuboid Detections")
                # Check if there are any associated cuboid detections available in recording
                ...
                cuboid_detections[camera] = False
        eval_0 = "Ensures that the component provides most confident subclass for all detected cuboids in each frame."
        ...
        dict_obj_keypoints = {}
        # Holds the status of validity of each detection confidence
        ...
        if signal_availability:
            for cam in check_cameras:
                ...
                if cuboid_detections[cam] is True:
                    ...
                    if availability_of_associated_detections[cam] is True:
                        ...
                        df_filtered_specific_columns = df[
                            ...
                            & (df[f"num_cuboid_detections_{cam}"] != 0)
                        ]
                        ...
                        validity_of_keypoints, invalid_keypoints_timestamps, dict_obj_keypoints[cam] = (
                            ft.check_validity_of_cuboid_keypoints(df_filtered, cam, ef)
                        )
                        if validity_of_keypoints and not df_filtered.empty:
                            ...
                            # and 'num_cuboid_detections is !=0'
                            ...
                            ) = ft.check_cuboid_subclass_confidence(
                                df_filtered, cam, ef, dict_obj_keypoints[cam], "subclass"
                            )
                            if valid_subclass_confidence:  # if subclass confidence is valid
                                # check whether the most confident subclass is provided for all the cuboid detections.
                                valid_det_subclass, invalid_subclass_timestamps = ft.valid_cuboid_subclass(
                                    df_filtered, cam, ef, dict_obj_keypoints[cam], dict_det_subclass_confidence[cam]
                                )
                                if (ele is True for ele in valid_det_subclass.values()):
                                    # sets test to Pass when most confident subclass is provided for all the detections
                                    # in each frame.
                                    result_subclass_id[cam] = True
                                    evaluation_result[cam] = (
                                        "Most Confident subclass is provided for all detected cuboid detections in each"
                                        "frame"
                                    )
                                    verdict_result[cam] = "passed"
                                else:
                                    # sets test to Fail when most confident subclass is not provided for any of the
                                    # detections in each frame.
                                    result_subclass_id[cam] = False
                                    evaluation_result[cam] = (
                                        f"Most Confident subclass is not provided at timestamps: {invalid_subclass_timestamps}"
                                    )
                                    verdict_result[cam] = "failed"
                            else:
                                # when subclass id is not lies between 0 and 1
                                result_subclass_id[cam] = fc.NOT_ASSESSED
                                evaluation_result[cam] = (
                                    f"subclass confidence is not valid at timestamps: {invalid_subclass_conf_timestamps}"
                                )
                        ...
                        # when there is no associated/valid detections in bsig/rec.
                        result_subclass_id[cam] = fc.NOT_ASSESSED
                        evaluation_result[cam] = "No associated cuboid detections are available in SIM"
                        verdict_result[cam] = "Not assessed"
                else:
                    # when there is no cuboid detections available in recording or GT
                    result_subclass_id[cam] = fc.NOT_ASSESSED
                    evaluation_result[cam] = "No cuboid detections are available in GT and/or SIM"
                    verdict_result[cam] = "Not assessed"
            if all(ele is True for ele in result_subclass_id.values()):
                # perform following when most confident subclass is provided for all of the cuboid detections.
                test_result = fc.PASS
                self.result.measured_result = TRUE
                self.result.details["Step_result"] = test_result
            elif any(ele is False for ele in result_subclass_id.values()):
                # perform following when most confident subclass is not provided for any of the cuboid detections.
                test_result = fc.FAIL
        ...
        for camera in check_cameras:
            ...
            if (
                cuboid_detections[camera] is True
                ...
                if not df_filtered.empty:
                            ...
                            for det in range(0, num_cuboid_detection):
                                ...
                                if (ef.valid_detections_in_camera[camera][f"{timestamps}_{det}"]) is True:
                                    if (
                                        ...
                                        for conf in range(0, 8):
                                            list_keypoints_x.append(
                                                row[
                                                    f"cuboid_detections_key_points_x_of_{camera}_det_{det}_keypoint_{conf}"
                                                ]
                                            )
                                            list_keypoints_y.append(
                                                row[
                                                    f"cuboid_detections_key_points_y_of_{camera}_det_{det}_keypoint_{conf}"
                                                ]
                                            )
                                        for conf in range(0, 3):
                                            list_subclass_conf.append(
                                                row[
                                                    f"cuboid_detections_subclass_confidence_of_{camera}_det_{det}", conf
                                                ]
                                            )
                                        list_subclass_id[f"{timestamps}[{det}]"] = row[
                                            f"cuboid_detections_subclass_id_of_{camera}_det_{det}"
                                        ]
                                        # adding the subclass confidence of each detection to the figure
                                        fig_cuboid_subclass_conf.add_trace(
                                            go.Scatter(
                                                x=x_arr_sub,
                                                y=np.array(list_subclass_conf),
                                                mode="lines+markers",
                                                name=f"OD_timestamp_{timestamps}_Cuboid[{det}].subclassconfidence",
                                            )
                                        )
                                        ...
                                        fig_keypoints.add_trace(
                                            go.Scatter(
                                                ...
                                                mode="lines+markers",
                                                name=f"OD_timestamp_{timestamps}_Cuboid[{det}].keypointX",
                                            )
                                        )
                                        fig_keypoints.add_trace(
                                            go.Scatter(
                                                ...
                                                mode="lines+markers",
                                                name=f"OD_timestamp_{timestamps}_Cuboid[{det}].keypointy",
                                            )
                                        )
                                if valid_detection > 0:
                                    num_of_valid_det[timestamps] = valid_detection
                                    num_of_valid_det_cam[camera] = True
                    # adding the subclass id of each detection to the figure
                    fig_subclass_id.add_trace(
                        go.Scatter(
                            x=np.array(list(list_subclass_id.keys())),
                            y=np.array(list(list_subclass_id.values())),
                            mode="lines+markers",
                            name="Subclass_ID",
                        )
                    )
                    ...
                    # adding the number of valid cuboid detections in each frame to the figure
                    fig_num_of_detections_sig_status.add_trace(
                        go.Scatter(
                            x=df_graph["keys"],
                            y=df_graph["values"],
                            mode="lines+markers",
                            name="num_of_valid_cuboid_detections",
                        )
                    )

                fig_cuboid_subclass_conf.layout = go.Layout(
                    yaxis=dict(tickformat="14"),
                    xaxis=dict(tickformat="14"),
                    xaxis_title=f"{camera.swapcase()}_Sub Classes",
                    yaxis_title="SubclassConfidence",
                    title=f"{camera.swapcase()} Valid Cuboid detections Subclass Confidence",
                )
                fig_num_of_detections_sig_status.layout = go.Layout(
                    ...
                    title=f"{camera.swapcase()} Number of valid cuboid detections in each frame",
                )
                fig_keypoints.layout = go.Layout(
                    yaxis=dict(tickformat="14"),
                    xaxis=dict(tickformat="14"),
                    xaxis_title=f"{camera.swapcase()} Cuboid Keypoints",
                    yaxis_title="Keypoint values",
                    title=f"{camera.swapcase()} Valid cuboid detections Keypoints",
                )
                fig_subclass_id.layout = go.Layout(
                    yaxis=dict(tickformat="14"),
                    xaxis=dict(tickformat="14"),
                    xaxis_title=f"{camera.swapcase()}_timestamp[det](us)",
                    yaxis_title="Sub class ID",
                    title=f"{camera.swapcase()} Valid cuboid detections Subclass ID",
                )
                if num_of_valid_det_cam[camera] is True:
                    # adding the number of valid cuboid detections in each frame to the plot
                    plots.append(fig_num_of_detections_sig_status)
                    # adding the subclass confidence of each detection to the plot
                    plots.append(fig_cuboid_subclass_conf)
                    # adding the keypoints of each detection to the plot
                    plots.append(fig_keypoints)
                    # adding the subclass id of each detection to the plot
                    plots.append(fig_subclass_id)
                    fig_num_of_detections_sig_status.update_layout(PlotlyTemplate.lgt_tmplt, showlegend=True)
                    fig_cuboid_subclass_conf.update_layout(PlotlyTemplate.lgt_tmplt, showlegend=True)
                    fig_keypoints.update_layout(PlotlyTemplate.lgt_tmplt, showlegend=True)
                    fig_subclass_id.update_layout(PlotlyTemplate.lgt_tmplt, showlegend=True)

        result_df = {
            "Verdict": {"value": test_result.title(), "color": fh.get_color(test_result)},
            fc.REQ_ID: ["1568033", "2260078"],
            fc.TESTCASE_ID: ["61419"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: ["Ensures that the component provides most confident cuboid subclass."],
...
@verifies("L3_PCE_1568033, L3_PCE_2260078")
@testcase_definition(
    name="SWRT_CNC_PCE_CuboidSubClass",
    description="This test ensures that the component provides the most confident cuboid subclass.",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class PCECuboidSubclass(TestCase):
    """Cuboid Subclass test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    """
    test_bsigs = r"D:\csv's\OD_Bsigs\2024.12.12_at_12.23.53_camera-mi_9056.bsig"
    debug(
        PCECuboidSubclass,
        test_bsigs,
        temp_dir=temp_dir,
