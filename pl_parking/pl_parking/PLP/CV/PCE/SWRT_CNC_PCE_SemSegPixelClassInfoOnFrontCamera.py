...
SIGNAL_DATA = "SEMSEG_FRONT_SIGNALS"
...
class PCESEMSEGSignals(SignalDefinition):
    ...
    class Columns(SignalDefinition.Columns):
        ...
        SEMSEG_IMAGEDATA_FC = "SemSeg_imageData_fc"
        SEMSEG_IMAGEDATAOFFSET_FC = "SemSeg_imageDataOffset_fc"
    ...
    def get_properties(self):
        ...
        signal_dict = {
            ...
            self.Columns.SEMSEG_IMAGEDATA_FC: ".GRAPPA_FC_IMAGE.GrappaSemSegFcImage.imageData",
            self.Columns.SEMSEG_IMAGEDATAOFFSET_FC: ".GRAPPA_FC_IMAGE.GrappaSemSegFcImage.imageDataOffset",
        }
        return signal_dict
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_SEMSEG_PIXELCLASSINFO_ONFRONTCAM",
    description="This test step confirms that the component provides the semseg class information",
    expected_result=BooleanResult(TRUE),
)
class PCESemsegClassInfoOnFrontCameraTestStep(TestStep):
    ...
    def process(self):
        ...
        signal_check = []
        signal_check.append("semseg_sig_status_fc")
        signal_check.append("Base_ctrl_opmode_fc")
        ...
        if signal_availability:
            ...
            evaluation_desc = "FC Semseg pixel classes"
            # Filters df for 'semseg_sig_status == AL_SIG_STATE_OK' and 'Base_ctrl_opmode is == GS_BASE_OM_RUN'
            df_filtered_check = df[
                (df["semseg_sig_status_fc"] == SigStatus.AL_SIG_STATE_OK)
                & (df["Base_ctrl_opmode_fc"] == BaseCtrl.GS_BASE_OM_RUN)
            ]
            df_filtered = df_filtered_check.drop_duplicates(subset=["semseg_timestamp_fc"], keep="last")
            ...
            if not df_filtered.empty:
                ...
                for _, row in df_filtered.iterrows():
                    pce = PCESemsegClassInfoOnFrontCameraTestStep()
                    # get the semseg image data offset in every frame
                    self.idx = int(row["SemSeg_imageDataOffset_fc"])
                    ...
                    Semseg_Imagedata = list(
                        map(lambda x: pce.get_semseg_imagedata(row=row, camera="fc"), range(self.idx, maximum_index))
                    )
                ...
                if all(list_passed):
                    ...
                    evaluation_result = "valid class is provided for all pixels on FC semseg data output"
                    test_result = fc.PASS
                    ...
                    evaluation_result = (
                        "valid class information is not provided for any one pixel on FC semseg data output"
                    )
            ...
            df_filtered_check = df[["mts_ts", "semseg_timestamp_fc", "semseg_sig_status_fc", "Base_ctrl_opmode_fc"]]
            df_plot = df_filtered_check[
                (df_filtered_check["semseg_sig_status_fc"] == SigStatus.AL_SIG_STATE_OK)
                & (df_filtered_check["Base_ctrl_opmode_fc"] == BaseCtrl.GS_BASE_OM_RUN)
            ]
            df_plot = df_plot.drop_duplicates(subset=["semseg_timestamp_fc"], keep="last")
            fig = go.Figure()
            # Semseg status
            fig.add_trace(
                go.Scatter(
                    x=df_plot["semseg_timestamp_fc"],
                    y=df_plot["semseg_sig_status_fc"],
                    mode="lines",
                    name="semseg_sig_status_fc",
                )
            )
            # Base control opmode
            fig.add_trace(
                go.Scatter(
                    x=df_plot["semseg_timestamp_fc"],
                    y=df_plot["Base_ctrl_opmode_fc"],
                    mode="lines",
                    name="Base_ctrl_opmode_fc",
                )
            )
...
class PCESemsegPixelClassInfoOnFrontCamera(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCESemsegClassInfoOnFrontCameraTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        PCESemsegPixelClassInfoOnFrontCamera,
        test_bsigs,
        temp_dir=temp_dir,
