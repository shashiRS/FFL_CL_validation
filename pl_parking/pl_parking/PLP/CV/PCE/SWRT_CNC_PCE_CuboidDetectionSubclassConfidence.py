#!/usr/bin/env python3
"""Defining  cuboid Subclass cases confidence"""
import logging
import os
import sys
import tempfile
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_CuboidSubClassConfidence",
...
class PCECuboidSubclassConfidenceTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera in check_cameras:
            ...
            evaluation_desc[camera] = f"Subclass Confidence of Cuboid Detections in {camera.swapcase()}"
        ...
        eval_0 = "Ensures that the component provides subclass confidence for all cuboid detections in each frame"
        ...
        if signal_availability:
                    ...
                    if availability_of_associated_detections[cam] is True:
                        ...
                        if validity_of_keypoints and not df_filtered.empty:
                            ...
                            if valid_subclass_confidence:
                                # sets test to Pass when subclass confidence of each detected cuboid lies in between 0 and 1
                                result_subclass_id[cam] = True
                                evaluation_result[cam] = (
                                    f"subclass confidence of each {cam.swapcase()} detected cuboid lies in between 0 and 1"
                                )
                                verdict_result[cam] = "passed"
                            else:
                                # sets test to Fail when subclass confidence of each detected cuboid does not lies in between 0 and 1
                                result_subclass_id[cam] = False
                                evaluation_result[cam] = (
                                    f"subclass confidence of each {cam.swapcase()} detected cuboid does not lies in between 0 and 1"
                                )
            ...
            if all(ele is True for ele in result_subclass_id.values()):
                # perform following when subclass confidence provided for all cuboid detections lies in range of 0 and 1
                test_result = fc.PASS
                self.result.measured_result = TRUE
                self.result.details["Step_result"] = test_result
            elif any(ele is False for ele in result_subclass_id.values()):
                # perform following when subclass confidence provided for any cuboid detections does not lies in range
                # of 0 and 1
                test_result = fc.FAIL
...
@verifies("L3_PCE_1568147")
@testcase_definition(
    name="SWRT_CNC_PCE_CuboidSubClassConfidence",
    description="This test ensures that the cuboid subclass confidence lies in range of 0 to 1",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class PCECuboidSubclassConfidence(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCECuboidSubclassConfidenceTestStep,
        ]
