#!/usr/bin/env python3
"""Defining  Bounding Box Subclass cases"""
import logging
import os
import sys
import tempfile
...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_PCE_BoundingBoxSubClass",
    description="This test step confirms that the component provides the most confident bounding subclass",
...
class PCEBoundingBoxSubclassTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera in check_cameras:
            ...
            for det in range(0, 64):
                ...
                signal_check.append(f"bounding_box_detections_subclass_id_of_{camera}_det_{det}")
        ...
        eval_0 = (
            "Ensures that the component provides most confident subclass for all bounding box detections"
            " in each frame."
        )
        ...
        if signal_availability:
            for cam in check_cameras:
                ...
                if bounding_detections[cam] is True:
                    ...
                    if availability_of_associated_detections[cam] is True:
                        ...
                        if validity_of_keypoints and not df_filtered.empty:
                            ...
                            if valid_subclass_confidence:  # if subclass confidence is valid
                                ...
                                valid_det_subclass, invalid_subclass_timestamps = ft.valid_bounding_box_subclass(
                    ...
                    result_subclass_id[cam] = fc.NOT_ASSESSED
                    evaluation_result[cam] = "No bounding box detections are available in SIM/GT"
                    verdict_result[cam] = "Not assessed"
        ...
        for camera in check_cameras:
            ...
            if (
                                ...
                                if (ef.valid_detections_in_camera[camera][f"{timestamps}_{det}"]) is True:
                                    if (
                                        ...
                                        list_subclass_id[f"{timestamps}[{det}]"] = row[
                                            f"bounding_box_detections_subclass_id_of_{camera}_det_{det}"
                                        ]
                ...
                fig_subclass_id.layout = go.Layout(
                    ...
                    title=f"{camera.swapcase()} Valid Bounding Box detections Subclass ID",
                )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1568149", "2261304"],
            fc.TESTCASE_ID: ["61449"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: ["Ensures that the component provides most confident bounding box subclass."],
...
@verifies("L3_PCE_1568149, L3_PCE_2261304")
@testcase_definition(
    name="SWRT_CNC_PCE_BoundingBoxSubClass",
    description="This test ensures that the component provides the most confident bounding box subclass.",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class PCEBoundingBoxSubclass(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            PCEBoundingBoxSubclassTestStep,
        ]
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    # )
    test_bsigs = r"D:\repoting_ui_check_pce\data\PCE_JSON_GT\2024.12.04_at_14.23.36_camera-mi_9056.bsig"
    debug(
        PCEBoundingBoxSubclass,
        test_bsigs,
        temp_dir=temp_dir,
