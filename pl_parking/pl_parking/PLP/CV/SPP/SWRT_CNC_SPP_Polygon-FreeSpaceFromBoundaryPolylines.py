#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Polygon-FreeSpaceFromBoundaryPolylines."""
...
SIGNAL_DATA = "testcase_39996"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Polygon-FreeSpaceFromBoundaryPolylines",
    description="Check if the polygons are created from the generated polylines, the boundaries of the obstacles.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppPolygonFreeSpaceFromBoundaryPolylinesTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            ...
            eval_cond = False
            cam_evaluation = " ".join(
                "Free space polygons are created from the generated polylines, the boundaries of "
                "the obstacles".split()
            )
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if (
                        ...
                        boundaries_polylines = extract_polylines(
                            camera=camera, start=start_index, end=end_index, row=row
                        )

                        polygons = extract_polylines(camera=camera, start=0, end=number_of_polygons, row=row)

                        # Check if vertices_of_boundaries are included in vertices_of_polygons
                        frame_result, frame_failed_log = self.points_included(frame, boundaries_polylines, polygons)
        ...
        remark = " ".join(
            "Check that polygons are created from the generated polylines, the boundaries of " "the obstacles.".split()
        )
...
@verifies("L3_SPP_1565900")
@testcase_definition(
    name="SWRT_CNC_SPP_Polygon-FreeSpaceFromBoundaryPolylines",
    description="This test verifies if the polygons are created from the generated polylines, the boundaries of "
    "the obstacles.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_m-oBKl-pEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppPolygonFreeSpaceFromBoundaryPolylines(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppPolygonFreeSpaceFromBoundaryPolylinesTestStep,
        ]
