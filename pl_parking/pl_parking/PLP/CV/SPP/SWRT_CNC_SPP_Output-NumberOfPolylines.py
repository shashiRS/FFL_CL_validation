#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-NumberOfPolylines."""
...
SIGNAL_DATA = "testcase_39925"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-NumberOfPolylines",
    description="Check if SPP function provide as output a numberOfPolylines field.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputNumberOfPolylinesTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            ...
            eval_cond = False
            cam_evaluation = " ".join(
                "Signal pSppPolylines.numberOfPolylines is available and its value is positive".split()
            )
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if polylines_sig_status == SppSigStatus.AL_SIG_STATE_OK:
                        eval_cond = True
                        frame_result, frame_failed_log = check_number_of_polylines(
                            number_of_polylines=number_of_polylines,
                            frame=frame,
                        )
                ...
                if eval_cond:
                    if not all(frame_results):
                        ...
                        cam_evaluation = " ".join(
                            f"The evaluation is FAILED in {len(list_of_failed)} frames: {list_of_failed[0]}".split()
                        )
                    else:
                    ...
                    cam_evaluation = " ".join(
                        "Preconditions are not meet: pSppPolylines.sSigHeader.eSigStatus != AL_SIG_STATE_OK".split()
                    )
        ...
        remark = " ".join(
            "Check that SPP function provide as output a numberOfPolylines field that represents the number of "
            "polylines. This number should not be negative.".split()
        )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1498850"],
            fc.TESTCASE_ID: ["39925"],
            fc.TEST_DESCRIPTION: [
                "Check that SPP function provide as output a numberOfPolylines field that represents "
...
@verifies("L3_SPP_1498850")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-NumberOfPolylines",
    description=(
        "This test verifies if SPP function provide as output a numberOfPolylines field that represents "
        "the number of polylines."
    ),
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_bbGzPkZZEe68PtCWeWkWbA&oslc_config.context=https%3A%2F%2F"
...
class SppOutputNumberOfPolylines(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputNumberOfPolylinesTestStep,
        ]
