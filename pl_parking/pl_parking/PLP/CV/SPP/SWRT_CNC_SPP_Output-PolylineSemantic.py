#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-PolylineSemantic."""
...
from pl_parking.PLP.CV.SPP.constants import SensorSource, SppSemantics, SppSigStatus
from pl_parking.PLP.CV.SPP.ft_helper import (
    SPPPreprocessor,
    SPPSignals,
    check_polyline_semantic_value,
...
SIGNAL_DATA = "testcase_52470"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-PolylineSemantic",
    description="Check if SPP function provides a semantic field for each polyline.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputPolylineSemanticTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            ...
            eval_cond = False
            cam_evaluation = " ".join(
                f"Signal pSppPolylines.polylines[%].semantic is provided with one of the following values: "
                f"{SppSemantics.STATIC_OBSTACLE}, {SppSemantics.DYNAMIC_OBSTACLE}, "
                f"{SppSemantics.CURB}".split()
            )
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if (
                        ...
                        frame_result, frame_failed_log = check_polyline_semantic_value(
        ...
        remark = " ".join(
            f"Check that SPP function provides a semantic field for each polyline."
            f"It has one of the following values: {SppSemantics.STATIC_OBSTACLE}, {SppSemantics.DYNAMIC_OBSTACLE}, "
            f"{SppSemantics.CURB}".split()
        )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565882"],
            fc.TESTCASE_ID: ["52470"],
            fc.TEST_DESCRIPTION: ["Check if SPP function provide a semantic field for each polyline"],
        }
...
@verifies("L3_SPP_1565882")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-PolylineSemantic",
    description="This test verifies if the SPP function provides a semantic field for each polyline.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_4MHSwF-nEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppOutputPolylineSemantic(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputPolylineSemanticTestStep,
        ]
