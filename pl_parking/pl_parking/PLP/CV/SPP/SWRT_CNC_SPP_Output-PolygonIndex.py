#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-PolygonIndex."""
...
from pl_parking.PLP.CV.SPP.ft_helper import (
    SPPPreprocessor,
    SPPSignals,
    check_polygon_vertex_start_index,
...
SIGNAL_DATA = "testcase_40401"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-PolygonIndex",
    description="Check if the function provides the start index for the first vertex of the polygons.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputPolygonIndexTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            frame_results = []
            eval_cond = False
            cam_evaluation = " ".join("The start index for the first point of each polygon is provided.".split())
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if polylines_sig_status == SppSigStatus.AL_SIG_STATE_OK and number_of_polygons >= 1:
                        eval_cond = True
                        frame_result, frame_failed_log = check_polygon_vertex_start_index(
                            camera=camera, number_of_polygons=number_of_polygons, frame=frame, row=row
                        )
                ...
                if eval_cond:
                    if not all(frame_results):
                        ...
                        cameras[camera] = fc.PASS
                    cameras.update({camera: cameras[camera]})
        ...
        remark = " ".join("Check if the function provides the start index for the first vertex of the polygons".split())
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565890"],
            fc.TESTCASE_ID: ["40401"],
            fc.TEST_DESCRIPTION: ["Check if SPP function provide the start index for the first vertex of the polygons"],
        }
...
@verifies("L3_SPP_1565890")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-PolygonIndex",
    description="This test verifies if SPP function provides the start index for the first vertex of the polygons.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_uTB4V1-oEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppOutputPolygonIndex(TestCase):
