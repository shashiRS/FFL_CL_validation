#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-PolylineNumberOfVertices."""
...
SIGNAL_DATA = "testcase_52464"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-PolylineNumberOfVertices",
    description="Check if the SPP function provides a number of vertices field for each polyline.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputPolylineNumVerticesTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            ...
            eval_cond = False
            cam_evaluation = " ".join(
                f"All {SppPolylines.SPP_MAX_NUMBER_POLYLINES} polylines are available and "
                f"each polyline contains at least {SppPolylines.SPP_MIN_NUMBER_PTS_POLYLINE} points.".split()
            )
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if (
                        ...
                        eval_cond = True
                        # check that each polyline has at least {SPP_MIN_NUMBER_PTS_POLYLINE} points
                        ...
                        frame_result, frame_failed_log = check_num_vertices_range(
                            ...
                            min_number_of_points=SppPolylines.SPP_MIN_NUMBER_PTS_POLYLINE,
                            frame=frame,
                            row=row,
                        )
        ...
        remark = " ".join(
            f"Check that all {SppPolylines.SPP_MAX_NUMBER_POLYLINES} polylines are available and "
            f"each polyline contains at least {SppPolylines.SPP_MIN_NUMBER_PTS_POLYLINE} points.".split()
        )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565886"],
            fc.TESTCASE_ID: ["52464"],
            fc.TEST_DESCRIPTION: ["Check if SPP function provide a number of vertices field for each polyline"],
        }
...
@verifies("L3_SPP_1498843, L3_SPP_1565886, L3_SPP_1565915")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-PolylineNumberOfVertices",
    description="This test verifies if the SPP function provides a number of vertices field for each polyline.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_f2mrBF-oEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppOutputPolylineNumVertices(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputPolylineNumVerticesTestStep,
        ]
