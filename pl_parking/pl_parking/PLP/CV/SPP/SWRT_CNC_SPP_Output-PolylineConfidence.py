#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-PolylineConfidence."""
...
from pl_parking.PLP.CV.SPP.ft_helper import (
    SPPPreprocessor,
    SPPSignals,
    check_polylines_confidence_range,
...
SIGNAL_DATA = "testcase_52469"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-PolylineConfidence",
    description="Check if SPP function provides a confidence field for each polyline.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputPolylineConfidenceTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            ...
            eval_cond = False
            cam_evaluation = " ".join(
                "Signal pSppPolylines.polylines[%].confidence is available for each polyline and its value is "
                "positive".split()
            )
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if (
                        ...
                        frame_result, frame_failed_log = check_polylines_confidence_range(
        ...
        remark = " ".join(
            "Check that SPP function provides a confidence field for each polyline. "
            "The confidence value value should be in range [0..1]".split()
        )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565881"],
            fc.TESTCASE_ID: ["52469"],
            fc.TEST_DESCRIPTION: ["Check if SPP function provide a confidence field for each polyline"],
        }
...
@verifies("L3_SPP_1565881")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-PolylineConfidence",
    description="This test verifies if the SPP function provides a confidence field for each polyline.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_14jUsF-nEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppOutputPolylineConfidence(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputPolylineConfidenceTestStep,
        ]
