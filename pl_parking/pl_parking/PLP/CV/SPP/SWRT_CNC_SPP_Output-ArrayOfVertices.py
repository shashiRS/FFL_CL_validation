#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-ArrayOfVertices."""
...
SIGNAL_DATA = "testcase_39928"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-ArrayOfVertices",
    description="Check if SPP function provides as output an array of vertices.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputArrayOfVerticesTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            cam_evaluation = " ".join("An array of vertices is provided for this camera".split())
        ...
        remark = " ".join("Check that all vertices are available on each camera instance in the bsig file.".split())
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1498846"],
            fc.TESTCASE_ID: ["39928"],
            fc.TEST_DESCRIPTION: ["Check if SPP function provide an array of vertices"],
        }
...
@verifies("L3_SPP_1498846")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-ArrayOfVertices",
    description="This test verifies if SPP function provides as output an array of vertices.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_bbGzPUZZEe68PtCWeWkWbA&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppOutputArrayOfVertices(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputArrayOfVerticesTestStep,
        ]
