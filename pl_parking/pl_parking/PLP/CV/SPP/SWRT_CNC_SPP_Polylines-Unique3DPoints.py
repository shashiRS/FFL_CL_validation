#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Polylines-Unique3DPoints."""
...
SIGNAL_DATA = "testcase_39962"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Polylines-Unique3DPoints",
    description="Check if each detected polyline is described by a set of unique 3D points.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppPolylineUniquePointsTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            eval_cond = False
            cam_evaluation = " ".join("All detected polylines are described by a set of unique 3D points".split())
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if (
                        ...
                        for polyline_index, polyline in enumerate(polylines):
                            # Use numpy.unique with axis=0 to get unique 3D points and their counts
                            unique_points, counts = np.unique(polyline, axis=0, return_counts=True)

                            # Create a dictionary to store frequencies
                            frequency_dict = {tuple(point): count for point, count in zip(unique_points, counts)}

                            # Check the frequencies
                            for point, count in frequency_dict.items():
                                if count > 1:
                                    list_of_failed.append(
                                        (
                                            frame,
                                            f"Polyline[{polyline_index + number_of_polygons}]:{polyline}",
                                            f"{point}: {count} times",
                                        )
                                    )
                if eval_cond:
        ...
        remark = " ".join("Check that each detected polyline is described by a set of unique 3D points.".split())
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565910"],
            fc.TESTCASE_ID: ["39962"],
            fc.TEST_DESCRIPTION: ["Check if each detected polyline is described by a set of unique 3D points"],
        }
...
@verifies("L3_SPP_1565910")
@testcase_definition(
    name="SWRT_CNC_SPP_Polylines-Unique3DPoints",
    description="This test verifies if each detected polyline is described by a set of unique 3D points.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_wLwpoV-qEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppPolylineUniquePoints(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppPolylineUniquePointsTestStep,
        ]
