#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Polylines-OrderOfDetectingPointsOnFrontCamera."""
...
SIGNAL_DATA = "testcase_39970"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Polylines-OrderOfDetectingPointsOnFrontCamera",
    description="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppPolylinesOrderOfDetectingPointsOnFrontCameraTestStep(TestStep):
    ...
    def process(self):
        ...
        signal_summary = {}
        camera = SensorSource.SPP_FC_DATA
        ...
        eval_cond = False
        cam_evaluation = " ".join(
            "All vertices are order on the front camera, they start from positive values towards negative values on "
                    ...
                    for polyline_index, polyline in enumerate(polylines):
                        if not all(polyline[i][1] >= polyline[i + 1][1] for i in range(len(polyline) - 1)):
                            list_of_failed.append(
                                ...
                                f"are not in descending order on y-axis"
                            )
        ...
        remark = " ".join(
            "Check if the order of detecting the points on the front camera starts from positive values "
            "towards negative values on y-axis for each polyline.".split()
        )
        ...
        result_df = {
            ...
            fc.TEST_DESCRIPTION: [
                "Check if the order of detecting the points on the front camera starts from positive "
                "values towards negative values on y-axis for each polyline"
            ],
        }
...
@testcase_definition(
    name="SWRT_CNC_SPP_Polylines-OrderOfDetectingPointsOnFrontCamera",
    description="This test verifies if the order of detecting the points on the front camera starts from positive "
    "values towards negative values on y-axis for each polyline.",
...
class SppPolylinesOrderOfDetectingPointsOnFrontCamera(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppPolylinesOrderOfDetectingPointsOnFrontCameraTestStep,
        ]
