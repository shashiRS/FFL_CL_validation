#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-PolygonConfidence."""
...
SIGNAL_DATA = "testcase_52487"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-PolygonConfidence",
    description="Check if SPP function provides a confidence field for each polygon",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputPolygonConfidenceTestStep(TestStep):
    ...
    def process(self):
        ...
        remark = " ".join(
            "Check that SPP function provides a confidence field for each polygon. "
            "The confidence value should be in range [0..1]".split()
        )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565871"],
            fc.TESTCASE_ID: ["52487"],
            fc.TEST_DESCRIPTION: ["Check if SPP function provide a confidence field for each polygon"],
        }
...
@testcase_definition(
    name="SWRT_CNC_SPP_Output-PolygonConfidence",
    description="This test verifies if the SPP function provides a confidence field for each polygon.",
...
class SppOutputPolygonConfidence(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputPolygonConfidenceTestStep,
        ]
