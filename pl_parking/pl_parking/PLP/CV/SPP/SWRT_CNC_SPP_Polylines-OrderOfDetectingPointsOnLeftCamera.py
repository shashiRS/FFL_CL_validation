#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Polylines-OrderOfDetectingPointsOnLeftCamera."""
...
SIGNAL_DATA = "testcase_39966"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Polylines-OrderOfDetectingPointsOnLeftCamera",
    description="Check if the order of detecting the points on the left camera starts from negative values towards "
    "positive values on x-axis for each polyline.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppPolylinesOrderOfDetectingPointsOnLeftCameraTestStep(TestStep):
    ...
    def process(self):
        ...
        signal_summary = {}
        camera = SensorSource.SPP_LSC_DATA
        ...
        eval_cond = False
        cam_evaluation = " ".join(
            "All vertices are order on the left camera, they start from negative values towards positive values "
            "on x-axis for each polyline".split()
        )
                    ...
                    for polyline_index, polyline in enumerate(polylines):
                        if not all(polyline[i][0] <= polyline[i + 1][0] for i in range(len(polyline) - 1)):
                            list_of_failed.append(
                                ...
                                f"are not in ascending order on x-axis"
                            )
        ...
        remark = " ".join(
            "Check if the order of detecting the points on the left camera starts from negative values "
            "towards positive values on x-axis for each polyline.".split()
        )
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565911"],
            fc.TESTCASE_ID: ["39966"],
            fc.TEST_DESCRIPTION: [
                "Check if the order of detecting the points on the left camera starts from negative "
                "values towards positive values on x-axis for each polyline"
            ],
        }
...
@testcase_definition(
    name="SWRT_CNC_SPP_Polylines-OrderOfDetectingPointsOnLeftCamera",
    description="This test verifies if the order of detecting the points on the left camera starts from negative "
    "values towards positive values on x-axis for each polyline.",
...
class SppPolylinesOrderOfDetectingPointsOnLeftCamera(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppPolylinesOrderOfDetectingPointsOnLeftCameraTestStep,
        ]
