#!/usr/bin/env python3
"""Validation of Test Case SWRT_CNC_SPP_Output-FirstPolylines."""
...
SIGNAL_DATA = "testcase_40173"


@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SPP_Output-FirstPolylines",
    description="Check if the first polylines from the polylines list are polygons",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, SPPSignals)
@register_pre_processor(alias="check_signals", pre_processor=SPPPreprocessor)
class SppOutputFirstPolylinesTestStep(TestStep):
    ...
    def process(self):
        ...
        for camera, pre_processor_result in cameras_from_preprocessor.items():
            frame_results = []
            eval_cond = False
            cam_evaluation = " ".join("First polylines from the polylines list are polygons".split())
            ...
            if signals_available:
                ...
                for frame, row in reader_df.iterrows():
                    ...
                    if polylines_sig_status == SppSigStatus.AL_SIG_STATE_OK and number_of_polylines > 1:
                        eval_cond = True
                        frame_result, frame_failed_log = check_first_polylines(
                            camera=camera,
        ...
        remark = " ".join("Check that the first polylines from the polylines list are polygons.".split())
        ...
        result_df = {
            ...
            fc.REQ_ID: ["1565887"],
            fc.TESTCASE_ID: ["40173"],
            fc.TEST_DESCRIPTION: ["Check if the first polylines from the polylines list are polygons"],
        }
...
@verifies("L3_SPP_1565887")
@testcase_definition(
    name="SWRT_CNC_SPP_Output-FirstPolylines",
    description="This test verifies if the first polylines from the polylines list are polygons.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2F"
    "jazz.conti.de%2Frm4%2Fresources%2FBI_iBXSY1-oEe6hyY-R-i-gng&componentURI=https%3A%2F%2Fjazz.conti.de%2F"
...
class SppOutputFirstPolylines(TestCase):
    ...
    def test_steps(self):
        ...
        return [
            SppOutputFirstPolylinesTestStep,
        ]
