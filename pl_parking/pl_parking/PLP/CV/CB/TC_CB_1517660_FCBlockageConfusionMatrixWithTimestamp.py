#!/usr/bin/env python3
"""This is the test case to get check timestamp
values should be increasing order along with calculate ConfusionMatrixWithFrontCameraBlockage
"""
...
READER_NAME = "ConfusionMatrixWithFrontCameraBlockageReaderTimestamp"
...
@teststep_definition(
    step_number=1,
    name="Test step to Check Timestamp signals value is in increasing order and calculate ConfusionMatrix With FrontCamera Blockage",  # this would be shown as a test step name in html report
    description=(
        "Test step to Check Timestamp signals value is in increasing order and calculate ConfusionMatrix With FrontCamera Blockage"
...
class ConfusionMatrixWithFrontCameraBlockage(TestStep):
    ...
    and Check Timestamp signals value is in increasing order.
    ...
    def process(self, **kwargs):
        ...
        ############# validate timestamp are in increasing order

        unique_TimeStamp = reader_DF["TimeStamp"].unique().tolist()
        # print(f"unique_TimeStamp is :{unique_TimeStamp}")

        # initializing list
        test_list = unique_TimeStamp

        # printing original lists
        # print("Original list : " + str(test_list))

        # using zip() + all()
        # to check for strictly increasing list, res is True
...
@testcase_definition(
    name="Test case to Timestamp signals value in increasing order and for creation of confusion_matrix for FC_Blockage",
    description=(
        "Test case to Timestamp signals value in increasing order and for creation of confusion_matrix for FC_Blockage"
    ),
)
