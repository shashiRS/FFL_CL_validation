...
class Signals(SignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            self.Columns.lvmdStatusPort: [
                ".MfLvmd1.lvmdStatusPort.sSigHeader.eSigStatus",
            ],
        }
...
@teststep_definition(
    step_number=1,
    name="MF LVMD Regression test",  # this would be shown as a test step name in html report
    description=("Check that Sig Status for MF LVMD component is not error"),
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(SIGNAL_DATA, Signals)
class TestStepMF_LVMD(ft_helper.BaseStepRregressionTS):
...
@testcase_definition(
    name="MF LVMD Regression Test",
    description=("Check Sig Status for LVMD component"),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class LVMD_Regression(TestCase):
