...
class Signals(SignalDefinition):
    ...
    def __init__(self):
        ...
        self._properties = {
            self.Columns.whpProcDebugPort: [
                ".MFWhP1.whpProcDebugPort.sSigHeader.eSigStatus",
            ],
            self.Columns.whpProcOutputPort: [
                ".MFWhP1.whpProcOutputPort.sSigHeader.eSigStatus",
            ],
        }
...
@teststep_definition(
    step_number=1,
    name="MF MFWhP Regression test",  # this would be shown as a test step name in html report
    description=("Check that Sig Status for MF MFWhP component is not error"),
    expected_result=BooleanResult(
        TRUE
    ),  # this expected result would be compared with measured_result and give a verdict
)
@register_signals(SIGNAL_DATA, Signals)
class TestStepMF_MFWhP(ft_helper.BaseStepRregressionTS):
...
@testcase_definition(
    name="MF MFWhP Regression Test",
    description=("Check Sig Status for MFWhP component"),
)
# Define the class for test case which will inherit the TestCase class and will return the test steps.
class MFWhP_Regression(TestCase):
