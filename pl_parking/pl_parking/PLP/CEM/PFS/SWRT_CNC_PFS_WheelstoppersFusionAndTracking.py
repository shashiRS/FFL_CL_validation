...
class TestStepFtWSOutput(TestStep):
    ...
    def process(self, **kwargs):
        ...
        first_in_ts = min(
            [
                ws_data[ws_data["PMDWs_Front_numberOfLines"] > 0]["PMDWs_Front_timestamp"].min(),
                ws_data[ws_data["PMDWs_Rear_numberOfLines"] > 0]["PMDWs_Rear_timestamp"].min(),
                ws_data[ws_data["PMDWs_Left_numberOfLines"] > 0]["PMDWs_Left_timestamp"].min(),
                ws_data[ws_data["PMDWs_Right_numberOfLines"] > 0]["PMDWs_Right_timestamp"].min(),
            ]
        )
        ...
        last_in_ts = max(
            [
                ws_data[ws_data["PMDWs_Front_numberOfLines"] > 0]["PMDWs_Front_timestamp"].max(),
                ws_data[ws_data["PMDWs_Rear_numberOfLines"] > 0]["PMDWs_Rear_timestamp"].max(),
                ws_data[ws_data["PMDWs_Left_numberOfLines"] > 0]["PMDWs_Left_timestamp"].max(),
                ws_data[ws_data["PMDWs_Right_numberOfLines"] > 0]["PMDWs_Right_timestamp"].max(),
            ]
        )
        ...
        if not pcl_type.empty:
            ...
            ws_front = ws_data[["PMDWs_Front_numberOfLines", "PMDWs_Front_timestamp"]].drop_duplicates()
            ws_front = ws_front.loc[(ws_front["PMDWs_Front_timestamp"] != 0)]
            ws_rear = ws_data[["PMDWs_Rear_numberOfLines", "PMDWs_Rear_timestamp"]].drop_duplicates()
            ws_rear = ws_rear.loc[(ws_rear["PMDWs_Rear_timestamp"] != 0)]
            ws_left = ws_data[["PMDWs_Left_numberOfLines", "PMDWs_Left_timestamp"]].drop_duplicates()
            ws_left = ws_left.loc[(ws_left["PMDWs_Left_timestamp"] != 0)]
            ws_right = ws_data[["PMDWs_Right_numberOfLines", "PMDWs_Right_timestamp"]].drop_duplicates()
            ws_right = ws_right.loc[(ws_right["PMDWs_Right_timestamp"] != 0)]
            ...
            fig.add_trace(
                go.Scatter(
                    x=ws_front["PMDWs_Front_timestamp"], y=ws_front["PMDWs_Front_numberOfLines"], name="ws_front"
                )
            )
            fig.add_trace(
                go.Scatter(x=ws_rear["PMDWs_Rear_timestamp"], y=ws_rear["PMDWs_Rear_numberOfLines"], name="ws_rear")
            )
            fig.add_trace(
                go.Scatter(x=ws_left["PMDWs_Left_timestamp"], y=ws_left["PMDWs_Left_numberOfLines"], name="ws_left")
            )
            fig.add_trace(
                go.Scatter(
                    x=ws_right["PMDWs_Right_timestamp"], y=ws_right["PMDWs_Right_numberOfLines"], name="ws_right"
                )
            )
            plots.append(fig)
