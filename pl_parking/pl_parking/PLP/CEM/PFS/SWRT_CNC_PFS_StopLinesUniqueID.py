...
@teststep_definition(
    step_number=1,
    name="CEM PFS SL Unique ID",
    description="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, CemSignals)
class TestStepFtSLUniqueID(TestStep):
    """StopLines Unique ID Test."""
    ...
    def process(self, **kwargs):
        ...
        if not pcl_type.empty:
            ...
            for time_frame in delimiter_data:
                ids = []
                for sl in time_frame.stop_line_array:
                    if sl.delimiter_id in ids:
                        failed += 1
                        values = [[time_frame.timestamp], [sl.delimiter_id]]
                        rows.append(values)
                    else:
                        ids.append(sl.delimiter_id)
            ...
            if failed:
                ...
                test_result = fc.PASS
                evaluation = "CEM provides an identifier for each stop line"
        ...
        signal_summary = pd.DataFrame(
            {
                "Evaluation": {
                    "1": "CEM provides an identifier for each Stopline",
                },
                "Result": {
                    "1": evaluation,
                },
        ...
        sig_sum = fh.build_html_table(signal_summary, table_title="PFS Stoplines Unique ID")
        self.result.details["Plots"].append(sig_sum)

        result_df = {
            "Verdict": {"value": test_result.title(), "color": fh.get_color(test_result)},
            fc.REQ_ID: ["2260076"],
            fc.TESTCASE_ID: ["70643"],
            fc.TEST_SAFETY_RELEVANT: [fc.NOT_AVAILABLE],
            fc.TEST_DESCRIPTION: [
                "This test checks that CEM provides an identifier for each stopline,"
...
@verifies("2260076")
@testcase_definition(
    name="SWRT_CNC_PFS_StopLinesUniqueID",
    description="This test checks that CEM provides an identifier for each stopline,"
    "and the identifier is unique in each timeframe.",
)
class FtSLUniqueID(TestCase):
    """Example test case."""
