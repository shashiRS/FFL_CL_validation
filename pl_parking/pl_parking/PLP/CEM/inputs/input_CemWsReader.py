    ...
    def convert_to_class(self) -> typing.Dict[PMDCamera, typing.List[PMDTimeFrame]]:
        ...
        for camera in PMDCamera:
            ...
            for _, row in self.data.iterrows():
                timestamp = row[f"CemWs_{self.camera_strings[camera]}_timestamp"]
                number_of_lines = int(row[f"CemWs_{self.camera_strings[camera]}_numberOfLines"])
                pmd_lines = []

                for i in range(number_of_lines):
                    line = PMDLine(
                        row[(f"CemWs_{self.camera_strings[camera]}_parkingLines_lineId", i)],
                        PMDLinePoint(
                            row[(f"CemWs_{self.camera_strings[camera]}_parkingLines_lineStartX", i)] * -1,
                            row[(f"CemWs_{self.camera_strings[camera]}_parkingLines_lineStartY", i)] * -1,
                        ),
                        PMDLinePoint(
                            row[(f"CemWs_{self.camera_strings[camera]}_parkingLines_lineEndX", i)] * -1,
                            row[(f"CemWs_{self.camera_strings[camera]}_parkingLines_lineEndY", i)] * -1,
                        ),
                        row[(f"CemWs_{self.camera_strings[camera]}_parkingLines_lineConfidence", i)],
                    )
