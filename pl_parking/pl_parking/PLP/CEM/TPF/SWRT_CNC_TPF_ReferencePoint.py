"""TestCases checking the reference point of the detected object."""
...
FT_TPF_REFERENCE_POINT = "FT_TPF_REFERENCE_POINT"
...
@teststep_definition(
    step_number=1,
    name="ReferencePoint",
    description="This test will verify if the position provided by TPF is defined is are in [-250,250] m range.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(FT_TPF_REFERENCE_POINT, fh_tpf.TPFSignals)
class TestStepReferencePoint(TestStep):
    ...
    def process(self, **kwargs):
        ...
        signal_summary = {}
        reference_point_error_list = []
        self.result.details.update(
            {"Plots": [], "Plot_titles": [], "Remarks": [], "file_name": os.path.basename(self.artifacts[0].file_path)}
        )

        tpf_reader = self.readers[FT_TPF_REFERENCE_POINT]
        ...
        reference_point_description = "The evaluation of the signal is <b>PASSED</b>."
            ...
            for _, row in tpf_reader.iterrows():
                ...
                if signal_state == fc.AL_SIG_STATE_OK:
                    for object_index in range(number_of_objects):
                        object_reference_point_x = row[(sig.OBJECTS_SHAPE_REFERENCEPOINT_X, object_index)]
                        object_reference_point_y = row[(sig.OBJECTS_SHAPE_REFERENCEPOINT_Y, object_index)]

                        if (
                            object_reference_point_x < -250
                            or object_reference_point_x > 250
                            or object_reference_point_y < -250
                            or object_reference_point_y > 250
                        ):
                            error_dict = {
                                ...
                                "Reference Point X": object_reference_point_x,
                                "Reference Point Y": object_reference_point_y,
                                "Description": "out of range",
                                "Expected result": "[-250, 250]",
                            }
                            if test_result != fc.FAIL:
                                reference_point_description = " ".join(
                                    ...
                                    f"confidence {error_dict['Confidence']} with <b>Reference Point(X, Y)</b> "
                                    f"({error_dict['Reference Point X']}, {error_dict['Reference Point Y']}) which is "
                                    f"{error_dict['Description']} ({error_dict['Expected result']}).".split()
                                )
                            test_result = fc.FAIL
                            reference_point_error_list.append(error_dict)
        else:
            reference_point_description = " ".join("Signal <b>not evaluated</b> because it is not available.".split())
            test_result = fc.NOT_ASSESSED

        signal_summary[tpf_object.get_properties()[sig.OBJECTS_SHAPE_REFERENCEPOINT_X][0]] = reference_point_description
        remark = "This test will verify if the position provided by TPF is defined and in [-250,250] [m] range."
...
@verifies(
    requirement="2032268",
    doors_url=r"https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_uOifUOU-Ee6f_fKhM-zv_g&componentURI=https%3A%2F%2Fjazz.conti.de%2Frm4%2Frm-projects%2F_D9K28PvtEeqIqKySVwTVNQ%2Fcomponents%2F_tpTA4CuJEe6mrdm2_agUYg&oslc.configuration=https%3A%2F%2Fjazz.conti.de%2Fgc%2Fconfiguration%2F17099",
)
@testcase_definition(
    name="ReferencePoint",
    description="This test will verify if the position provided by TPF is defined is are in [-250,250] m range.",
