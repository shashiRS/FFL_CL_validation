"""TestCases checking class certainty is in (0, 1] range."""
...
FT_TPF_CLASS_PROBABILITY = "FT_TPF_CLASS_PROBABILITY"
...
@teststep_definition(
    step_number=1,
    name="ValidateClassProbability",
    description="This test will verify if the class probability provided by TPF is is in (0, 1] range.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(FT_TPF_CLASS_PROBABILITY, fh_tpf.TPFSignals)
class TestStepClassProbability(TestStep):
    ...
    def process(self, **kwargs):
        ...
        signal_summary = {}
        confidence_error_list = []
        self.result.details.update(
            {"Plots": [], "Plot_titles": [], "Remarks": [], "file_name": os.path.basename(self.artifacts[0].file_path)}
        )

        tpf_reader = self.readers[FT_TPF_CLASS_PROBABILITY]
        ...
        confidence_description = "The evaluation of the signal is <b>PASSED</b>."
        ...
        if number_of_unavailable_signals == 0:
            # Verify if the confidence(class probability) is in the defined range (0, 1]
                    ...
                    for object_index in range(number_of_objects):
                        ...
                        if object_confidence <= 0 or object_confidence > 1:
                            ...
                            if test_result != fc.FAIL:
                                confidence_description = " ".join(
                                    f"The evaluation of the signal is <b>FAILED</b> at timestamp {error_dict['Timestamp']} "
                                    f"for object with ID {error_dict['Object ID']} and class {error_dict['Class']} with "
                                    f"confidence <b>{error_dict['Confidence']}</b> which is {error_dict['Description']} "
                                    f"({error_dict['Expected result']}).".split()
                                )
                            test_result = fc.FAIL
                            confidence_error_list.append(error_dict)

        else:
            confidence_description = " ".join("Signal <b>not evaluated</b> because it is not available.".split())
            test_result = fc.NOT_ASSESSED

        signal_summary[tpf_object.get_properties()[sig.OBJECTS_CLASSPROBABILITY][0]] = confidence_description
        remark = "Class probability should be in (0, 1] range."
...
class ClassProbability(TestCase):
    """Timestamp test case."""
