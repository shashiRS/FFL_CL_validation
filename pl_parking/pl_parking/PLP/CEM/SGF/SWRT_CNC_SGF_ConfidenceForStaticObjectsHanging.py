...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SGF_ConfidenceForStaticObjectsHanging",  # this would be shown as a test step name in html report
    description=(
        "This test case checks that SGF provides the confidence for Static Objects being hanging and this confidence is between 0 and 1."
    ),  # this would be shown as a test step description in html report
    doors_url="https://jazz.conti.de/qm4/web/console/LIB_L3_SW_TM/_or7dMJpREe6OHr4fEH59Xg#action=com.ibm.rqm.planning.home.actionDispatcher&subAction=viewTestCase&id=38894",
...
class TestStepFtSGFOutputHanging(TestStep):
    ...
    Read the following data from the whole recording:
        ...
        3. ADC5xx_Device.CEM_EM_DATA.SGF_SgfOutput.staticObjectsOutput.objects[k].heightConfidences.hanging is an number between 0 and 1 in all cases
    """
    ...
    def process(self, **kwargs):
        ...
        if static_object_df.filter(regex="hangingConfidence_polygon").empty:
            ...
            max_holds = all(
                static_object_df["hangingConfidence_polygon", i].max(skipna=True) <= 1
...
@testcase_definition(
    name="SWRT_CNC_SGF_ConfidenceForStaticObjectsHanging",
    description="This test case checks that SGF provides the confidence for Static Objects being hanging and this confidence is between 0 and 1",
    doors_url="https://jazz.conti.de/qm4/web/console/LIB_L3_SW_TM/_or7dMJpREe6OHr4fEH59Xg#action=com.ibm.rqm.planning.home.actionDispatcher&subAction=viewTestCase&id=38894",
)
...
class FtSGFOutputHanging(TestCase):
    ...
    def test_steps(self):
        ...
        return [TestStepFtSGFOutputHanging]  # in this list all the needed test steps are included
