...
@teststep_definition(
    step_number=1,
    name="SWRT_CNC_SGF_WheelTraversableConfidence",
    description="This test checks that a confidence for each Static Object "
    "being wheel traversable is provided and is between 0 and 1.",
    doors_url="https://jazz.conti.de/qm4/web/console/LIB_L3_SW_TM/_or7dMJpREe6OHr4fEH59Xg#action=com.ibm.rqm.planning.home.actionDispatcher&subAction=viewTestCase&id=38891",
...
class TestStepFtSGFOutputWheelTraversable(TestStep):
    ...
    def process(self, **kwargs):
        ...
        if static_object_df.filter(regex="wheelTraversableConfidence_polygon").empty:
            ...
            max_holds = all(
                static_object_df["wheelTraversableConfidence_polygon", i].max(skipna=True) <= 1
...
class FtSGFOutputWheelTraversable(TestCase):
    ...
    def test_steps(self):
        ...
        return [TestStepFtSGFOutputWheelTraversable]  # in this list all the needed test steps are included
