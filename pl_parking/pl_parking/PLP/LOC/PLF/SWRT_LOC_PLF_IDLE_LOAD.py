...
SIGNAL_DATA = "IDLE_LOAD"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF IDLE Load state",
    description="This test step confirms that in idle state, after "
    "receiving Load event LOC shall move from idle state to waitmap state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputIDLELoad(TestStep):
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that state changed from idle state to waitmap state."
            if self.result.measured_result is TRUE
            else "A Load event occurred in an idle state, yet the transition from the idle state failed."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=93836",
)
@testcase_definition(
    name="LOC-PLF IDLE Load state",
    description="This test ensure that in idle state, after receiving Load event LOC shall move from idle to waitmap "
    "state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_nWjyZIf2Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputIDLELoad(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputIDLELoad]
