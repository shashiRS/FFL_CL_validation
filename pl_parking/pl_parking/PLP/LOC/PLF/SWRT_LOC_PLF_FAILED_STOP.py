...
SIGNAL_DATA = "FAILED_STOP"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF FAILED Stop state",
    description="This test step confirms that in Failed state, after "
    "receiving stop event LOC shall move from Error state to Idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputFAILEDStop(TestStep):
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that state changed from Error state to idle state."
            if self.result.measured_result is TRUE
            else "A Stop event occurred in an Failed state, yet the transition from the Failed state failed."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=107401",
)
@testcase_definition(
    name="LOC-PLF FAILED Stop state",
    description="This test ensure that in Failed state, after receiving stop event LOC shall move from "
    "Error state to Idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_DbQhUIf5Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputFAILEDStop(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputFAILEDStop]
