...
@teststep_definition(
    step_number=1,
    name="LOC-PLF IDLE Continue state",
    description="This test step confirms that in idle state, after receiving Continue "
    "event LOC shall remain in the idle state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputIDLEContinue(TestStep):
    """Test step for idle state transition."""
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=93838",
)
@testcase_definition(
    name="LOC-PLF IDLE Continue state",
    description="This test confirms that in idle state, after receiving Continue "
    "event LOC shall remain in the idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_zphY1If2Ee-ZB7Q23GLe5g&vvc.configuration"
