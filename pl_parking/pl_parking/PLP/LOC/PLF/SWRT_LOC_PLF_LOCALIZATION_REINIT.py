...
SIGNAL_DATA = "LOCALIZATION_REINIT"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF LOCALIZATION Reinit state",
    description="This test step confirms that in Localization state, after "
    "receiving Reinit event LOC shall remain in the Localization state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputLOCALIZATIONReinit(TestStep):
    """Transitions from the Localization state,
    when Reinit event received, LOC shall remains in same state.
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            # Check for reinit event conditions
            if checker.isReinitEvent(index, previous_index):
                event_result = fc.PASS
                # Check for failed state conditions
                if checker.isInLocalizationState(index):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an Localization state when a reinit event arrives."
            if self.result.measured_result is TRUE
            else "A reinit event occurred in an Localization state, yet it failed to stay in the Localization state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=101899",
)
@testcase_definition(
    name="LOC-PLF LOCALIZATION Reinit state",
    description="This test ensure that in Localization state, after receiving Reinit event LOC shall remain in the Localization state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_CmM-EIf4Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputLOCALIZATIONReinit(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputLOCALIZATIONReinit]
