...
SIGNAL_DATA = "FAILED_REINIT"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF FAILED Reinit state",
    description="This test step confirms that in Failed state, after "
    "receiving Reinit event LOC shall move from Failed state to Waitmap state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputFAILEDReinit(TestStep):
    """Transitions from the Failed state,
    when Reinit event received, LOC shall move from failed to waitmap.
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            if checker.isReinitEvent(index, previous_index):
                ...
                if checker.isInFailedState(index):
        ...
        # Build signal details and remarks
        ...
        remark_message = (
            "The transition state change indicates that state changed from Failed state to Waitmap state."
            if self.result.measured_result is TRUE
            else "A Reinit event occurred in an Failed state, yet the transition from the Failed state failed."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=107400",
)
@testcase_definition(
    name="LOC-PLF FAILED Reinit state",
    description="This test ensure that in Failed state, after receiving Reinit event LOC shall move from Failed state"
    "to Waitmap state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_wjDFMof4Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputFAILEDReinit(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputFAILEDReinit]
