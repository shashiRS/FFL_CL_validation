...
SIGNAL_DATA = "LOCALIZATION_LOAD"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF LOCALIZATION Load state",
    description="This test step confirms that in Localization state, after "
    "receiving Load event LOC shall move from Localization state to waitmap state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputLOCALIZATIONLoad(TestStep):
    """Transitions from the Localization state,
    when Load event received, LOC shall remains in same state.
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            # Check for load event conditions
            if checker.isLoadEvent(index, previous_index):
        ...
        remark_message = (
            "The transition state change indicates that state changed from Localization state to waitmap state."
            if self.result.measured_result is TRUE
            else "A Load event occurred in an Localization state, yet the transition from the Localization state failed."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=101895",
)
@testcase_definition(
    name="LOC-PLF LOCALIZATION Load state",
    description="This test ensure that in Localization state, after receiving Load event LOC shall move from "
    "Localization state to waitmap state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_uIMBwIf3Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputLOCALIZATIONLoad(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputLOCALIZATIONLoad]
