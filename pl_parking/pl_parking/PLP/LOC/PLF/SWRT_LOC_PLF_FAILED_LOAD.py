...
SIGNAL_DATA = "FAILED_LOAD"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF FAILED Load state",
    description="This test step confirms that in Failed state, after "
    "receiving Load event LOC shall move from Failed state to Waitmap state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputFAILEDLoad(TestStep):
    """Transitions from the Failed state,
    when Load event received, LOC shall move from failed state to waitmap.
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            ...
            else "A Load event occurred in an Failed state, yet the transition from the Failed state failed."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=107399",
)
@testcase_definition(
    name="LOC-PLF FAILED Load state",
    description="This test ensure that in Failed state, after receiving Load event LOC shall move from "
    "Failed state to Waitmap state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_h0-uQof4Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputFAILEDLoad(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputFAILEDLoad]
