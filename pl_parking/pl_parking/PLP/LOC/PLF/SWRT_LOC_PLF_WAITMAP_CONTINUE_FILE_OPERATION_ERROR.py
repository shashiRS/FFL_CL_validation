...
SIGNAL_DATA = "WAITMAP_CONTINUE_ERROR"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF WAITMAP Continue File Operation Error state",
    description="This test step confirms that in waitmap state, after receiving Continue event and a "
    "File_Operation_Error message from Parameter Handler LOC shall move from waitmap state to failed state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputWAITMAPContinueFileOperationError(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            if checker.isContinueEvent(index, previous_index) and checker.isLoadResultError(index):
                ...
                if checker.isInWaitmapState(index):
                    ...
                    next_conditions_wrong = not checker.isInFailedState(next_index)
                    # Check for continue event conditions
        ...
        remark_message = (
            "The transition state change indicates that the state changes from waitmap state to failed state when a "
            "continue and File_Operation_Error event arrives."
            if self.result.measured_result is TRUE
            else "A Continue and File_Operation_Error event occurred in an waitmap state, "
            "yet it failed to move from waitmap state to failed state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=97487",
)
@testcase_definition(
    name="LOC-PLF WAITMAP Continue File Operation Error state",
    description="This test confirms that in waitmap state, after receiving Continue event and a File_Operation_Error"
    "message from Parameter Handler LOC shall move from waitmap state to failed state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_S36HcIf3Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputWAITMAPContinueFileOperationError(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        """
        return [TestStepFtLOCOutputWAITMAPContinueFileOperationError]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    debug(
        FtLOCOutputWAITMAPContinueFileOperationError,
