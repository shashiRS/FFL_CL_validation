...
SIGNAL_DATA = "WAITMAP_LOAD"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF WAITMAP Load state",
    description="This test step confirms that in waitmap state, after "
    "receiving Load event LOC shall remain in the waitmap state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputWAITMAPLoad(TestStep):
    """Transitions from the WAITMAP state,
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an "
            "waitmap state when a load event arrives."
            if self.result.measured_result is TRUE
            else "A Load event occurred in an waitmap state, yet it failed to stay in the waitmap state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=97484",
)
@testcase_definition(
    name="LOC-PLF WAITMAP Load state",
    description="This test ensure that in waitmap state, after receiving Load event LOC "
    "shall remain in the waitmap state."
    "state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_-zUXwof2Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputWAITMAPLoad(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputWAITMAPLoad]
