...
SIGNAL_DATA = "IDLE_REINIT"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF IDLE Reinit state",
    description="This test step confirms that in idle state, after "
    "receiving Reinit event LOC shall remain in the idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputIDLEReinit(TestStep):
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an idle state when a reinit event arrives."
            if self.result.measured_result is TRUE
            else "A reinit event occurred in an idle state, yet it failed to stay in the idle state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=93839",
)
@testcase_definition(
    name="LOC-PLF IDLE Reinit state",
    description="This test ensure that in idle state, after receiving Reinit event LOC shall remain in the idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_10h30If2Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputIDLEReinit(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputIDLEReinit]
