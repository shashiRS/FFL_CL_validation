...
SIGNAL_DATA = "LOCALIZATION_CONTINUE"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF LOCALIZATION Continue state",
    description="This test step confirms that in Localization state, after receiving Continue "
    "event LOC shall remain in the Localization state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputLOCALIZATIONContinue(TestStep):
    """Test step for Localization state transition."""
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            if checker.isContinueEvent(index, previous_index):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an Localization state when a "
            "continue event arrives."
            if self.result.measured_result is TRUE
            else "A Continue event occurred in an Localization state, yet it failed to stay in the Localization state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=101897",
)
@testcase_definition(
    name="LOC-PLF LOCALIZATION Continue state",
    description="This test confirms that in Localization state, after receiving Continue "
    "event LOC shall remain in the Localization state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_1a4zUYf3Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputLOCALIZATIONContinue(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        """
        return [TestStepFtLOCOutputLOCALIZATIONContinue]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    debug(
        FtLOCOutputLOCALIZATIONContinue,
