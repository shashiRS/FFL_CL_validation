...
SIGNAL_DATA = "WAITMAP_CONTINUE_SUCCESS"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF WAITMAP Continue File Operation Success state",
    description="This test step confirms that in waitmap state, after receiving Continue event and a File_Operation"
    "_Success message from Parameter Handler LOC shall move from waitmap state to Localization state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputWAITMAPContinueFileOperationSuccess(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            if checker.isContinueEvent(index, previous_index) and checker.isLoadResultSuccess(index):
                event_result = fc.PASS

                # Check for waitmap state conditions
                if checker.isInWaitmapState(index):
                    current_state_result = fc.PASS
                    next_index = index + 1  # Continue event detected
                    next_conditions_wrong = not checker.isInLocalizationState(next_index)
        ...
        signal_details = plf_summary.build_signal_summary(
            event="continue_success",
            current_state="waitmap",
            next_state="localization",
        ...
        remark_message = (
            "The transition state change indicates that the state changes from waitmap state to success state when a "
            "continue and File_Operation_Success event arrives."
            if self.result.measured_result is TRUE
            else "A Continue and File_Operation_Error event occurred in an waitmap state, "
            "yet it failed to move from waitmap state to success state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=97488",
)
@testcase_definition(
    name="LOC-PLF WAITMAP Continue File Operation Success state",
    description="This test confirms that in waitmap state, after receiving Continue event and a File_Operation_Success"
    "message from Parameter Handler LOC shall move from waitmap state to Localization state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_IJk8gYf3Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputWAITMAPContinueFileOperationSuccess(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        """
        return [TestStepFtLOCOutputWAITMAPContinueFileOperationSuccess]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    debug(
        FtLOCOutputWAITMAPContinueFileOperationSuccess,
