...
SIGNAL_DATA = "WAITMAP_REINIT"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF WAITMAP Reinit state",
    description="This test step confirms that in waitmap state, after "
    "receiving Reinit event LOC shall remains in waitmap state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputWAITMAPReinit(TestStep):
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            ...
            "reinit event arrives."
            if self.result.measured_result is TRUE
            else "A reinit event occurred in an waitmap state, yet it failed to stay in the waitmap state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=97485",
)
@testcase_definition(
    name="LOC-PLF WAITMAP Reinit state",
    description="This test ensure that in waitmap state, after receiving Reinit event LOC shall remains in "
    "waitmap state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_Vh-hBIf3Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputWAITMAPReinit(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputWAITMAPReinit]
