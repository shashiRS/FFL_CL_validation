...
def build_signal_summary(result):
    ...
    return pd.DataFrame(
        {
            "Signal_Evaluation": {
                "1": f"{loc_signal.get_complete_signal_name(PLF_Signals.Columns.PLF_STD_POSITION_X)},  "
                f"{loc_signal.get_complete_signal_name(PLF_Signals.Columns.PLF_STD_POSITION_Y)}",
            },
            "Result": {
                "1": f"values of below signal should not be zero   "
                f"{PLF_Signals.Columns.PLF_STD_POSITION_X} and {PLF_Signals.Columns.PLF_STD_POSITION_Y} ",
            },
            "Verdict": {"1": f"{result}"},
        }
    )
...
class TestStepFtLOCOutputSDPOSITION_x_y(TestStep):
    ...
    def process(self, **kwargs):
        ...
        # to check sd is estimated check in each cycle there should be non-zero value
        plf_std_x_missing = (df["plf_std_x_m"] == 0).any()
        plf_std_y_missing = (df["plf_std_y_m"] == 0).any()
        test_result = fc.FAIL if plf_std_x_missing or plf_std_y_missing else fc.PASS
        ...
        if is_passed:
            remark = (
                "State confirms that the standard deviation of the ego vehicle "
                "position [sigmaX,sigmaY]m is estimated correctly."
            )
        else:
            remark = "Since test is failed, ego vehicle sd_position [sigmaX,sigmaY]m was not estimated correctly."
        ...
        config = create_config(Required_signals, annotation_text=" sd_position [sigmaX,sigmaY]m")
        plots = (
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=91006",
)
@testcase_definition(
    name="LOC-PLF SD POSITION_x_y",
    description="This test confirm that MappingAndLocalizationLowSpeed shall provide the estimate of the standard "
    "deviation of the ego vehicle position [sigmaX,sigmaY]m",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_aJQHIDLREe-Om-dBJsz4EQ&vvc.configuration"
...
class FtLOCOutputSDPOSITION_x_y(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSDPOSITION_x_y]
