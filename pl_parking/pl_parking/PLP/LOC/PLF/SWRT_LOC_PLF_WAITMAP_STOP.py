...
SIGNAL_DATA = "WAITMAP_STOP"
loc_signal = PLF_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-PLF WAITMAP Stop state",
    description="This test step confirms that in waitmap state, after "
    "receiving stop event LOC shall move from waitmap to idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputWAITMAPStop(TestStep):
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that state changed from waitmap state to idle state."
            if self.result.measured_result is TRUE
            else "A Stop event occurred in an waitmap state, yet the transition from the waitmap state failed."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=97486",
)
@testcase_definition(
    name="LOC-PLF WAITMAP Stop state",
    description="This test ensure that in waitmap state, after receiving stop event LOC shall "
    "move from waitmap to idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_C1soEIf3Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputWAITMAPStop(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputWAITMAPStop]
