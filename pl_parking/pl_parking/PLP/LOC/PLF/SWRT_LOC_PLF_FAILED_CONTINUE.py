...
@teststep_definition(
    step_number=1,
    name="LOC-PLF FAILED Continue state",
    description="This test step confirms that in Failed state, after receiving Continue "
    "event LOC shall remain in the Failed state.",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, PLF_Signals)
class TestStepFtLOCOutputFAILEDContinue(TestStep):
    """Test step for Failed state transition."""
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an Failed state when a "
            "continue event arrives."
            if self.result.measured_result is TRUE
            else "A Continue event occurred in an Failed state, yet it failed to stay in the Failed state."
        )
...
@verifies(
    ...
    "home.actionDispatcher&subAction=viewTestCase&id=107398",
)
@testcase_definition(
    name="LOC-PLF FAILED Continue state",
    description="This test confirms that in Failed state, after receiving Continue "
    "event LOC shall remain in the Failed state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_rTOKUIf4Ee-ZB7Q23GLe5g&vvc.configuration"
...
class FtLOCOutputFAILEDContinue(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        """
        return [TestStepFtLOCOutputFAILEDContinue]
