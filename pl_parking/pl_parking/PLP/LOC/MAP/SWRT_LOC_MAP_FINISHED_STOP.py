...
SIGNAL_DATA = "FINISHED_STOP"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP FINISHED Stop state",
    description="This test step confirms that in finished state, after "
    "receiving stop event map is moved from finished to idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputFINISHEDStop(TestStep):
    """Transitions from the FINISHED state,
    when stop event received, map shall move from finished state to idle state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state changed from finished state to idle state."
            if self.result.measured_result is TRUE
            else "A stop event occurred in an error state, yet the transition from the finished state failed."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=73585",
)
@testcase_definition(
    name="LOC-MAP FINISHED Stop state",
    description="This test ensure that in finished state, after "
    "receiving stop event map shall move from finished to idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_XnaJ4FYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputFINISHEDStop(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputFINISHEDStop]
