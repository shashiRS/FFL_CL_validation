...
@teststep_definition(
    step_number=1,
    name="LOC-MAP SAVING State",
    description="This test step will confirm that it will realize an saving state",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGState(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            if not checker.isInSavingState(index):
        ...
        remark_message = (
            "The transition state indicates that it realized an saving state."
            if self.result.measured_result is TRUE
            else "The transition state indicates that it did not realize an saving state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=75107",
)
@testcase_definition(
    name="LOC-MAP SAVING state",
    description="This test confirm that it will realize an saving state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
              "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_6qw101WUEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputSAVINGState(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGState]
