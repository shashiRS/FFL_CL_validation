...
@teststep_definition(
    step_number=1,
    name="LOC-MAP IDLE Stop state",
    description="This test step confirms that in idle state, after receiving stop event map shall remain in idle state.",
    doors_url="",
...
class TestStepFtLOCOutputIDLEStop(TestStep):
    ...
    when stop event received, map shall move from idle state to idle state
    """
...
@testcase_definition(
    name="LOC-MAP IDLE Stop state",
    description="This test ensure that in idle state, after receiving stop event map shall remain in idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_sXvasFYZEe-WGbhflTJCAg&vvc.configuration"
