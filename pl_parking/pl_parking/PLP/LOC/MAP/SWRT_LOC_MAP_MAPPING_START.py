...
SIGNAL_DATA = "MAPPING_START"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP MAPPING Start state",
    description="This test step confirms that in mapping state, "
    "after receiving start event map shall remain in mapping state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputMAPPINGStart(TestStep):
    """Transitions from the MAPPING state,
    when start event received, map shall move from mapping state to mapping state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an mapping state when a "
            "start event arrives."
            if self.result.measured_result is TRUE
            else "A Start event occurred in an idle state, yet it failed to stay in the mapping state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=71876",
)
@testcase_definition(
    name="LOC-MAP MAPPING Start state",
    description="This test ensure that in mapping state,"
    "after receiving start event map shall remain in mapping state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_0Q2sAVYZEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputMAPPINGStart(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputMAPPINGStart]
