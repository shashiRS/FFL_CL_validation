...
SIGNAL_DATA = "MAPPING_STOP"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP MAPPING Stop state",
    description="This test step confirms that in mapping state, after "
    "receiving stop event map is moved from mapping to idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputMAPPINGStop(TestStep):
    """Transitions from the MAPPING state,
    when stop event received, map shall move from mapping state to idle state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state changed from mapping state to idle state."
            if self.result.measured_result is TRUE
            else "A stop event occurred in an mapping state, yet the transition from the mapping state failed."
        )
...
@verifies(
    ...
    "planning.home.actionDispatcher&subAction=viewTestCase&id=71880",
)
@testcase_definition(
    name="LOC-MAP MAPPING Stop state",
    description="This test ensure that in mapping state, after "
    "receiving stop event map shall move from mapping to idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_Arb9YVYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputMAPPINGStop(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputMAPPINGStop]
