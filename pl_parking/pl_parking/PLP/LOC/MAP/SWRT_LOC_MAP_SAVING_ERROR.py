...
SIGNAL_DATA = "SAVING_ERROR"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP SAVING Error state",
    description="This test step confirms that in saving state, after "
    "receiving map is fully saved from ParameterHandler map shall move from saving to error state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGErrorState(TestStep):
    """Transitions from the SAVING Error state,
    when start event received, map shall move from saving state to error state
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            # Check for store_error event conditions
            if checker.isParameterHandlerFailed(index, previous_index):
                event_result = fc.PASS
                # Check for saving state conditions
                if checker.isInSavingState(index):
                    current_state_result = fc.PASS
                    next_index = index + 1
                    next_conditions_wrong = not checker.isInErrorState(next_index)
        ...
        remark_message = (
            "The transition state change indicates that the state changed from saving state to error state."
            if self.result.measured_result is TRUE
            else "A store_error event occurred in an saving state, yet the transition from the saving state failed."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=72280",
)
@testcase_definition(
    name="LOC-MAP SAVING Error state",
    ...
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_MttccVYaEe-WGbhflTJCAg&vvc.configuration="
...
class FtLOCOutputSAVINGErrorState(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGErrorState]
