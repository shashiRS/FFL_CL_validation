...
@teststep_definition(
    step_number=1,
    name="LOC-MAP IDLE State",
    description="This test step will confirm that it will realize an idle state",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputIDLEState(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            if not checker.isInIdleState(index):
        ...
        remark_message = (
            "The transition state indicates that it realized an idle state."
            if self.result.measured_result is TRUE
            else "The transition state indicates that it did not realize an idle state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=75105",
)
@testcase_definition(
    name="LOC-MAP IDLE state",
    description="This test confirm that it will realize an idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
              "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_dxbTAFWUEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputIDLEState(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputIDLEState]
