    ...
    return pd.DataFrame(
        {
            "Signal_Evaluation": {
                ...
                "2": f"{loc_signal.get_complete_signal_name(MAP_Signals.Columns.MAP_SAVE_DATA_MAP_ID)} ,"
            ...
            "Result": {
                ...
                "2": f"{MAP_Signals.Columns.MAP_SAVE_DATA_MAP_ID} == {MAP_Signals.Columns.MAP_DATA_MAP_ID}"
                f"   provided the same MapId that was received in its RecordingRequestPort input port.",
            },
...
@teststep_definition(
    step_number=1,
    name="LOC_MAP_SAVING_ResultMapId ",
    description="This test step confirms that component shall provide the same "
    "MapId that was received in its RecordingRequestPort input port.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGResultMapId(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in matching_indices:
            if index < len(data):
                if any(
                    data.at[index + i, "MAP_Data_mapID"] == data.at[index + i, "MAP_SAVE_Data_mapID"]
                    for i in range(1, 3)
                ):
        ...
        if test_result == fc.PASS:
            ...
            remark = "provided the same MapId that was received in its RecordingRequestPort input port."
        else:
            self.result.measured_result = FALSE
            final_df = data
            remark = "failed hence Not received MapId that was received in its RecordingRequestPort input port"
        ...
        plot_titles.append("")
        annotation_text = (
            "Provided same MapId" if self.result.measured_result is TRUE else "failed-not Provided same MapId"
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=83246",
)
@testcase_definition(
    name="LOC_MAP_SAVING_ResultMapId ",
    description="This test ensure component shall provide the same MapId that was received "
    "in its RecordingRequestPort input port.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_FjgHdipHEe-t3Zp5B0W8XQ&vvc.configuration"
...
class FtLOCOutputSAVINGResultMapId(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGResultMapId]
