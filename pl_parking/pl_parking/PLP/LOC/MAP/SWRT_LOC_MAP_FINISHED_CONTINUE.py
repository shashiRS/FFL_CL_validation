...
SIGNAL_DATA = "FINISHED_CONTINUE"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP FINISHED Continue state",
    description="This test step confirms that in finished state, "
    "after receiving Continue event map shall remain in finished state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputFINISHEDContinue(TestStep):
    """Transitions from the FINISHED state,
    when continue event received, map shall move from finished state to finished state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an finished state when a "
            "continue event arrives."
            if self.result.measured_result is TRUE
            else "A Continue event occurred in an finished state, yet it failed to stay in the finished state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=73586",
)
@testcase_definition(
    name="LOC-MAP FINISHED Continue state",
    description="This test ensure that in finished state, "
    "after receiving Continue event map shall remain in finished state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_aiD9IlYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputFINISHEDContinue(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputFINISHEDContinue]
