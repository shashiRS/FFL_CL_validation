...
SIGNAL_DATA = "ERROR_STORE"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP ERROR Store state",
    description="This test step confirms that in error state, "
    "after receiving Store event map shall remain in error state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputERRORStore(TestStep):
    """Transitions from the ERROR state,
    when error event received, map shall move from error state to error state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an error state when a "
            "continue event arrives."
            if self.result.measured_result is TRUE
            else "A Continue event occurred in an Failed state, yet it failed to stay in the error state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=73588",
)
@testcase_definition(
    name="LOC-MAP ERROR Store state",
    description="This test ensure that in error state, after receiving Store event map shall remain in error state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_qH_wMFYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputERRORStore(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputERRORStore]
