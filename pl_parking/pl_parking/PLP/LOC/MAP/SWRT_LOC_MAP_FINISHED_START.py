...
SIGNAL_DATA = "FINISHED_START"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP FINISHED Start state",
    description="This test step confirms that in finished state, after "
    "receiving start event map is moved from finished to mapping state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputFINISHEDStart(TestStep):
    """Transitions from the FINISHED state,
    when start event received, map shall move from finished state to mapping state
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            # Check for start event conditions
            if checker.isStartEvent(index, previous_index):
                event_result = fc.PASS
                # Check for finished state conditions
                if checker.isInFinishedState(index):
                    current_state_result = fc.PASS
                    next_index = index + 1
                    next_conditions_wrong = not checker.isInMappingState(next_index)
        ...
        remark_message = (
            "The transition state change indicates that the state changed from finished state to mapping state."
            if self.result.measured_result is TRUE
            else "A start event occurred in an finished state, yet the transition from the finished state failed."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=73584",
)
@testcase_definition(
    name="LOC-MAP FINISHED Start state",
    description="This test ensure that in finished state, after "
    "receiving start event map shall move from finished to mapping state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_SBDlYVYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputFINISHEDStart(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputFINISHEDStart]
