...
SIGNAL_DATA = "IDLE_STORE"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP IDLE Store state",
    description="This test step confirms that in idle state, after "
    "receiving Store event map is moved from idle to error state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputIDLEStore(TestStep):
    """Transitions from the IDLE state,
    when error event received, map shall move from idle state to error state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state changed from idle state to error state."
            if self.result.measured_result is TRUE
            else "A store event occurred in an idle state, yet the transition from the idle state failed."
        )
...
@verifies(
    ...
    "planning.home.actionDispatcher&subAction=viewTestCase&id=71728",
)
@testcase_definition(
    name="LOC-MAP IDLE Store state",
    description="This test ensure that in idle state, after "
    "receiving Store event map shall move from idle to error state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_wQOvMlYZEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputIDLEStore(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputIDLEStore]
