...
SIGNAL_DATA = "ERROR_CONTINUE"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP ERROR Continue state",
    description="This test step confirms that in error state, "
    "after receiving Continue event map shall remain in error state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputERRORContinue(TestStep):
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=73583",
)
@testcase_definition(
    name="LOC-MAP ERROR Continue state",
    description="This test ensure that in error state, after receiving Continue event map shall remain in error state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_pJ4iIVYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputERRORContinue(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputERRORContinue]
