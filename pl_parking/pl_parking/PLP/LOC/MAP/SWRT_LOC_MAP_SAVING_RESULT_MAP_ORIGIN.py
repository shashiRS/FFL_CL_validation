    ...
    return pd.DataFrame(
        {
            "Signal_Evaluation": {
                ...
                f"{loc_signal.get_complete_signal_name(MAP_Signals.Columns.MAP_ORIGIN_HEADING)},  "
                f"{loc_signal.get_complete_signal_name(MAP_Signals.Columns.MAP_ORIGIN_HEMISPHERE)},  "
            ...
            "Result": {
                ...
                "2": f"Any one of theses signals should be greater than 1 "
                f"{MAP_Signals.Columns.MAP_ORIGIN_HEADING} or  "
                f"{MAP_Signals.Columns.MAP_ORIGIN_HEMISPHERE} or  "
                f"{MAP_Signals.Columns.MAP_ORIGIN_X} or  "
                f"{MAP_Signals.Columns.MAP_ORIGIN_Y} or  "
                f"{MAP_Signals.Columns.MAP_ORIGIN_ZONE}  "
                f"provided a positional information about the origin of the map as a MapOrigin.",
            },
...
@teststep_definition(
    step_number=1,
    name="SWRT_LOC_MAP_SAVING_ResultMapOrigin ",
    description="This test step confirms that component shall provide a positional information about "
    "the origin of the map as a MapOrigin.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGResultMapOrigin(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in matching_indices:
            if index < len(data):
                if (
                    data.at[index + 1, "mapOrigin.heading"] >= 1
                    or data.at[index + 1, "mapOrigin.hemisphere"] >= 1
                    or data.at[index + 1, "mapOrigin.x"] >= 1
                    or data.at[index + 1, "mapOrigin.y"] >= 1
                    or data.at[index + 1, "mapOrigin.zone"] >= 1
                ):
                    result_data.extend(data.iloc[index + 1 : index + 2])
        ...
        if test_result == fc.PASS:
            ...
            remark = "provided a positional information about the origin of the map as a MapOrigin."
        else:
            self.result.measured_result = FALSE
            final_df = data
            remark = "failed hence positional information about the origin of the map as a MapOrigin is not updated."
        ...
        plot_titles.append("")
        annotation_text = "MapOrigin" if self.result.measured_result is TRUE else "failed-MapOrigin"
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=83249",
)
@testcase_definition(
    name="SWRT_LOC_MAP_SAVING_ResultMapOrigin ",
    description="This test ensure component shall provide a positional information about "
    "the origin of the map as a MapOrigin.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_FjlnACpHEe-t3Zp5B0W8XQ&vvc.configuration"
...
class FtLOCOutputSAVINGResultMapOrigin(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGResultMapOrigin]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    debug(
        FtLOCOutputSAVINGResultMapOrigin,
