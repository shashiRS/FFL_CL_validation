...
SIGNAL_DATA = "SAVING_START"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP SAVING Start state",
    description="This test step confirms that in saving state, after "
    "receiving working on map operations from ParameterHandler map shall remain in saving state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGStart(TestStep):
    """Transitions from the SAVING state,
    when start event received, map shall move from saving state to saving state
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            # Check for store_none event conditions
            if checker.isParameterHandlerWorking(index):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an saving state when a "
            "store event arrives."
            if self.result.measured_result is TRUE
            else "A store event occurred in an saving state state, yet it failed to stay in the error state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=72278",
)
@testcase_definition(
    name="LOC-MAP SAVING Start state",
    ...
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_IYyqo1YaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputSAVINGStart(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGStart]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    # test_bsigs = os.path.join(fh.DEBUG_ROOT_FOLDER, fh.BSIG_PATH)
    debug(
        FtLOCOutputSAVINGStart,
