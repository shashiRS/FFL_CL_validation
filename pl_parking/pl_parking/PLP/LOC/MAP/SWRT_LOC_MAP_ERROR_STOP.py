...
SIGNAL_DATA = "ERROR_STOP"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP ERROR Stop state",
    description="This test step confirms that in error state, after "
    "receiving stop event map is moved from error to idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputERRORStop(TestStep):
    """Transitions from the ERROR state,
    when stop event received, map shall move from error state to idle state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state changed from error state to idle state."
            if self.result.measured_result is TRUE
            else "A stop event occurred in an error state, yet the transition from the error state failed."
        )
...
@verifies(
    ...
    ""
    "actionDispatcher&subAction=viewTestCase&id=73591",
)
@testcase_definition(
    name="LOC-MAP ERROR Stop state",
    description="This test ensure that in error state, after "
    "receiving stop event map shall move from error to idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_m1-l0lYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputERRORStop(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputERRORStop]
