...
@teststep_definition(
    step_number=1,
    name="LOC-MAP ERROR State",
    description="This test step will confirm that it will realize an error state",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputERRORState(TestStep):
    """This test step confirms that it is in error state"""
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            if not checker.isInErrorState(index):
        ...
        remark_message = (
            "The transition state indicates that it realized an error state."
            if self.result.measured_result is TRUE
            else "The transition state indicates that it did not realize an error state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=75104",
)
@testcase_definition(
    name="LOC-MAP ERROR state",
    description="This test confirm that it will realize an error state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
              "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_NP3vxlWVEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputERRORState(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputERRORState]
