...
SIGNAL_DATA = "MAPPING_CONTINUE"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP MAPPING Continue state",
    description="This test step confirms that in mapping state, "
    "after receiving Continue event map shall remain in mapping state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputMAPPINGContinue(TestStep):
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            ...
            else "A Continue event occurred in an idle state, yet it failed to stay in the mapping state."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=71885",
)
@testcase_definition(
    name="LOC-MAP MAPPING Continue state",
    ...
    "https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_CViuwFYaEe-WGbhflTJCAg&vvc.configuration="
...
class FtLOCOutputMAPPINGContinue(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputMAPPINGContinue]
