...
SIGNAL_DATA = "IDLE_START"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP IDLE Start state",
    description="This test step confirms that in idle state, after "
    "receiving start event map is moved from idle to mapping state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputIDLEStart(TestStep):
    """Transitions from the IDLE state,
    when start event received, map shall move from idle state to mapping state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state changed from idle state to mapping state."
            if self.result.measured_result is TRUE
            else "A start event occurred in an idle state, yet the transition from the idle state failed."
        )
...
@verifies(
    "71446",
    "https://jazz.conti.de/qm4/web/console/LIB_L3_SW_TM/_ufIY0DRKEe-YwuveGcgWRQ#action=com.ibm.rqm."
    "planning.home.actionDispatcher&subAction=viewTestCase&id=71446",
)
@testcase_definition(
    name="LOC-MAP IDLE Start state",
    description="This test ensure that in idle state, after "
    "receiving start event map shall move from idle to mapping state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_d5JfolYYEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputIDLEStart(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputIDLEStart]
