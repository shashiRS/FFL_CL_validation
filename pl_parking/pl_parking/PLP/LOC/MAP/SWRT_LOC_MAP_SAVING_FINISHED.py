...
SIGNAL_DATA = "SAVING_FINISHED"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP SAVING Finished state",
    description="This test step confirms that in saving state, after "
    "receiving map is fully saved from ParameterHandler map shall move from saving to finished state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGFinishedState(TestStep):
    """Transitions from the SAVING Finished state,
    when start event received, map shall move from saving state to finished state
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            # Check for store_success event conditions
            if checker.isParameterHandlerSuccessful(index, previous_index):
                event_result = fc.PASS
                # Check for saving state conditions
                if checker.isInSavingState(index):
                    current_state_result = fc.PASS
                    next_index = index + 1
                    next_conditions_wrong = not checker.isInFinishedState(next_index)
        ...
        remark_message = (
            "The transition state change indicates that the state changed from saving state to finished state."
            if self.result.measured_result is TRUE
            else "A store_success event occurred in an saving state, yet the transition from the saving state failed."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=72279",
)
@testcase_definition(
    name="LOC-MAP SAVING Finished state",
    description="This test ensure that in saving state, after "
    "receiving map is fully saved from ParameterHandler map shall move from saving to finished state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_KBWj4lYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputSAVINGFinishedState(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGFinishedState]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    debug(
        FtLOCOutputSAVINGFinishedState,
