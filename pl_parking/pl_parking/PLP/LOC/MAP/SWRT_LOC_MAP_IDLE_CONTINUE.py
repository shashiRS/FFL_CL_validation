...
SIGNAL_DATA = "IDLE_CONTINUE"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP IDLE Continue state",
    description="This test step confirms that in idle state, "
    "after receiving Continue event map shall remain in idle state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputIDLEContinue(TestStep):
    """Transitions from the IDLE state,
    when Continue event received, map shall move from idle state to idle state
    """
    ...
    def process(self, **kwargs):
        ...
        remark_message = (
            "The transition state change indicates that the state remains in an idle state when a "
            "continue event arrives."
            if self.result.measured_result is TRUE
            else "A Continue event occurred in an idle state, yet it failed to stay in the idle state."
        )
...
@verifies(
    ...
    "planning.home.actionDispatcher&subAction=viewTestCase&id=71727",
)
@testcase_definition(
    name="LOC-MAP IDLE Continue state",
    description="This test ensure that in idle state, after receiving Continue event map shall remain in idle state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_vOBrVFYZEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputIDLEContinue(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputIDLEContinue]
