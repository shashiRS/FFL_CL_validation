    ...
    return pd.DataFrame(
        {
            ...
            "Result": {
                ...
                "2": f"{MAP_Signals.Columns.STORE_REQUEST} == {fh.TRUE}(true)"
                and f"{MAP_Signals.Columns.MAP_ID}(before saving) == {MAP_Signals.Columns.MAP_DATA_MAP_ID}(after saving)"
                f"   Saving the in memory map to the persistent storage after receiving a request.",
            },
...
@teststep_definition(
    step_number=1,
    name="LOC_MAP_SAVING_ResultPersistentStorage ",
    description="This test step confirms that component shall request to save the in memory map to the "
    "persistent storage after receiving a request in its RecordingRequestPort input port.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGResultPersistentStorage(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in matching_indices:
            if index < len(data):
                if data.at[index + 1, "MAP_Data_mapID"] == data.at[index, "MP_mapID"]:
        ...
        if test_result == fc.PASS:
            ...
            remark = "Saving the in memory map to the persistent storage after receiving a request."
        else:
            self.result.measured_result = FALSE
            final_df = data
            remark = "Failed hence Not saving the in memory map to the persistent storage"
        ...
        plot_titles.append("")
        annotation_text = "saved memory map" if self.result.measured_result is TRUE else "failed-to save memory map"
...
@testcase_definition(
    name="LOC_MAP_SAVING_ResultPersistentStorage ",
    description="This test ensure that component shall request to save the in memory map to the "
    "persistent storage after receiving a request in its RecordingRequestPort input port.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_FjdrMipHEe-t3Zp5B0W8XQ&vvc.configuration"
...
class FtLOCOutputSAVINGResultPersistentStorage(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGResultPersistentStorage]
...
def main(temp_dir: Path = None, open_explorer=True):
    ...
    """
    debug(
        FtLOCOutputSAVINGResultPersistentStorage,
