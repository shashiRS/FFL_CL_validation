    ...
    return pd.DataFrame(
        {
            "Signal_Evaluation": {
                ...
                f"{loc_signal.get_complete_signal_name(MAP_Signals.Columns.MAP_RESULT_ID)} "
                f"{loc_signal.get_complete_signal_name(MAP_Signals.Columns.MAP_RECORDING_STATUS)} ",
            },
            "Result": {
                ...
                "2": f"{MAP_Signals.Columns.MAP_ID}(before saving) == {MAP_Signals.Columns.MAP_RESULT_ID}(after saving)"
                f" and  {MAP_Signals.Columns.MAP_RECORDING_STATUS} == 3(Success)  "
                f"   informed its clients about the result of the saving on its RecordingResultPort output port.",
            },
...
@teststep_definition(
    step_number=1,
    name="LOC_MAP_SAVING_Update_Result ",
    description="This test step component shall inform its clients about the result of the saving on its "
    "RecordingResultPort output port.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputSAVINGUpdateResult(TestStep):
    ...
    def process(self, **kwargs):
        ...
        for index in matching_indices:
            if index < len(data):
                if any(
                    data.at[index, "MP_mapID"] == data.at[index + i, "MP_result_mapID"]
                    and data.at[index + i, "mapRecordingStatus"] == StateChange.SUCCESS
                    for i in range(1, 2)
                ):
        ...
        if test_result == fc.PASS:
            ...
            remark = "informed its clients about the result of the saving on its RecordingResultPort output port."
        else:
            self.result.measured_result = FALSE
            final_df = data
            remark = (
                "failed to inform its clients about the result of the saving on its " "RecordingResultPort output port."
            )
        ...
        plot_titles.append("")
        annotation_text = "Map Saved" if self.result.measured_result is TRUE else "failed-to save Map"
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=83245",
)
@testcase_definition(
    name="LOC_MAP_SAVING_Update_Result ",
    description="This test ensure component shall inform its clients about the result of the saving on its "
    "RecordingResultPort output port.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_FjoqUSpHEe-t3Zp5B0W8XQ&vvc.configuration"
...
class FtLOCOutputSAVINGUpdateResult(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputSAVINGUpdateResult]
