...
SIGNAL_DATA = "ERROR_START"
loc_signal = MAP_Signals()


@teststep_definition(
    step_number=1,
    name="LOC-MAP ERROR Start state",
    description="This test step confirms that in error state, after "
    "receiving start event map is moved from error to mapping state.",
    doors_url="",
    expected_result=BooleanResult(TRUE),
)
@register_signals(SIGNAL_DATA, MAP_Signals)
class TestStepFtLOCOutputERRORStart(TestStep):
    """Transitions from the ERROR state,
    when start event received, map shall move from error state to mapping state
    """
    ...
    def process(self, **kwargs):
        ...
        for index in range(1, len(data) - 1):
            ...
            if checker.isStartEvent(index, previous_index):
                ...
                # Check for error state conditions
                if checker.isInErrorState(index):
        ...
        remark_message = (
            "The transition state change indicates that the state changed from error state to mapping state."
            if self.result.measured_result is TRUE
            else "A start event occurred in an error state, yet the transition from the error state failed."
        )
...
@verifies(
    ...
    "actionDispatcher&subAction=viewTestCase&id=73589",
)
@testcase_definition(
    name="LOC-MAP ERROR Start state",
    description="This test ensure that in error state, after "
    "receiving start event map shall move from error to mapping state.",
    doors_url="https://jazz.conti.de/rm4/web#action=com.ibm.rdm.web.pages.showArtifactPage&artifactURI"
    "=https%3A%2F%2Fjazz.conti.de%2Frm4%2Fresources%2FBI_iOQz0FYaEe-WGbhflTJCAg&vvc.configuration"
...
class FtLOCOutputERRORStart(TestCase):
    """Example test case."""
    ...
    def test_steps(self):
        ...
        return [TestStepFtLOCOutputERRORStart]
