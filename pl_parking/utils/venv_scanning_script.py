...
def _check_testcase_teststep(module_name, import_errors):
    ...
    def handle_exception(exception):
        msg_lst = [f"ERROR: Failed to scan: {module_name}."]
    ...
    try:
        ...
        if "libGL.so.1" in str(ex):
            pass
            #return discovered_testcases_tsf, discovered_testcases_trc, discovered_statistics_plugin, discovered_custom_overview_plugin
        else:
            handle_exception(ex)
    ...
    for component in components:
        if os.path.isdir(os.path.join(workspace_path, component)) and component != 'Trcruntime':
            ...
            for mi in pkgutil.walk_packages(m.__path__, m.__name__ + "."):
                try:
                    tsf_tests, trc_tests, statistics_overview, custom_overview = _check_testcase_teststep(mi.name, import_errors)
...
def main():
    import_errors = scan_environment(sys.argv[1])
    if import_errors:
        with open(os.path.join( os.path.dirname(__file__),'..','imports-output.txt'), 'a') as f:
            print("Errors found in imports:")
            print("########################")
